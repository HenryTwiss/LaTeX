...
\begin{document}
  ...
  \section{The Ideal Norm}
    ...
    \end{proof}
  \section{Moments}
    Our first result only uses the Euler-Maclaurin summation formula summation and we use this summation formula to prove a useful lemma:

    \begin{lemma}\label{lem:weak_approximate_functional_equation_Riemann_zeta}
      For any integer $N \ge 1$ and $\s > 0$, we have
      \[
    ...
    \begin{proof}
      ...
      \]
      By \cref{cor:Euler_Maclaurin_summation_formula_integer_limit_version_specialization}, we can rewrite the sum on the right-hand side as
      \[
        \sum_{n > N}\frac{1}{n^{s}} = \lim_{M \to \infty}\frac{N^{-s}-M^{-s}}{2}+\int_{N}^{\infty}u^{-s}-s\left(u-\lfloor u \rfloor-\frac{1}{2}\right)u^{-(s+1)}\,du.
      \]
      As $\s > 1$, $M^{-\s} \to 0$ as $M \to \infty$ so that
      \[
    ...
    \end{theorem}
    \begin{proof}
      From \cref{lem:weak_approximate_functional_equation_Riemann_zeta}, we have
      \[
        \z(s) = \sum_{n \le N}\frac{1}{n^{s}}+\frac{N^{-s}}{2}-\frac{N^{1-s}}{1-s}-s\int_{N}^{\infty}\left(u-\lfloor u \rfloor-\frac{1}{2}\right)u^{-(s+1)}\,du,
      \]
      for $\s > 0$. As $\lfloor u \rfloor-u-\frac{1}{2} \ll 1$, we have 
      \[
        s\int_{N}^{\infty}\left(u-\lfloor u \rfloor-\frac{1}{2}\right)u^{-(s+1)}\,du = O\left(|s|\int_{N}^{\infty}u^{-(\s+1)}\,du\right) = O\left(\frac{|s|N^{-\s}}{\s}\right).
      \]
      Taking $s = \frac{1}{2}+it$ and $N = t$ for $t \ge 2$, we obtain the estimate
      \[
