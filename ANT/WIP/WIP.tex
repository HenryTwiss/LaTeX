...
\begin{document}
  ...
  \section{Misc.}
    ...
    \begin{proof}
      ...
      \]
      where the second equality holds because $\g'\g$ runs over all the elements of $\PSL_{2}(\Z)$ and the last equality holds because $\mc{F}$ is the standard fundamental domain. This shows that every point in $\H$ is $\G$-equivalent to a point in $\mc{F}_{\G}$. We will now show that no two distinct interior points of $\mc{F}_{\G}$ are $\G$-equivalent. We will prove this by contradiction. So suppose $z_{1},z_{2} \in \mc{F}_{\G}$ are distinct interior points such that there is a $\g \in \G$ with $\g z_{1} = z_{2}$. By the definition of $\mc{F}_{\G}$, there exists $g_{1},g_{2} \in \G\backslash\PSL_{2}(\Z)$ such that $z_{1} = \g_{1}z'_{1}$ and $z_{2} = \g_{2}z'_{2}$ with $z'_{1},z'_{2} \in \mc{F}$. But then $\g\g_{1}z'_{1} = \g_{2}z'_{2}$ whence $\g_{2}^{-1}\g\g_{1}z'_{1} = z'_{2}$. This shows that $z'_{1}$ and $z'_{2}$ are $\PSL_{2}(\Z)$-equivalent. As $\mc{F}$ is a fundamental domain, $z'_{1}$ and $z'_{2}$ must be on the boundary of $\mc{F}$ which is a contradiction.
    \end{proof}
\end{document}
