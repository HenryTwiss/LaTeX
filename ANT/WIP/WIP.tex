...
\begin{document}
  ...
  \section{Misc.}
    ...
    \end{proposition}
        ...
        \begin{proof}
      We first show that every point in $\H$ is $\G$-equivalent to a point in $\mc{F}_{\G}$. From the definition of $\mc{F}_{\G}$, we have
      \[
        \bigcup_{\g' \in \G}\g'\mc{F}_{\G} = \bigcup_{\g' \in \G}\bigcup_{\g \in \G\backslash\PSL_{2}(\Z)}\g'\g\mc{F} = \bigcup_{\g \in \PSL_{2}(\Z)}\g\mc{F} = \H,
      \]
      where the second equality holds because $\g'\g$ runs over all the elements of $\PSL_{2}(\Z)$ and the last equality holds because $\mc{F}$ is a fundamental domain for $\PSL_{2}(\Z)$ by \cref{prop:fundamental_domain_modular_group}. This shows that every point in $\H$ is $\G$-equivalent to a point in $\mc{F}_{\G}$. We will now show that if two points of $\mc{F}_{\G}$ are $\G$-equivalent via a non-identity element then they lie on the boundary of $\mc{F}_{\G}$. Equivalently, we must show that if two points $z_{1} \in \g_{1}\mc{F}$ and $z_{2} \in \g_{2}\mc{F}$, for $\g_{1},\g_{2} \in \G\backslash\PSL_{2}(\Z)$, are $\G$-equivalent via a non-identity element then they lie on the boundary of $\mc{F}_{\G}$. First suppose $z_{1}$ or $z_{2}$ belonged to the interior of $\g_{1}\mc{F}$ or $\g_{2}\mc{F}$. Then $\g_{1}^{-1}z_{1}$ and $\g_{2}^{-1}z_{2}$ would be two points of $\mc{F}$, where at least one of which is not on the boundary, that are $\PSL_{2}(\Z)$-equivalent. But this is impossible because $\mc{F}$ is a fundamental domain by \cref{prop:fundamental_domain_modular_group}. Therefore we may assume $z_{1}$ and $z_{2}$ are on the boundaries of $\g_{1}\mc{F}$ and $\g_{2}\mc{F}$ respectively. Now suppose at least one of them, say $z_{1}$, is in the interior of $\mc{F}_{\G}$. Then $z_{1} \in \g_{3}\mc{F}$ for some $\g_{3} \in \G\backslash\PSL_{2}(\Z)$ with $\g_{1} \neq \g_{3}$. But then $z_{1} \in \g_{1}\mc{F} \cap \g_{3}\mc{F}$ and so $\g_{1}^{-1}z$ and $\g_{3}^{-1}z$ are two distinct points on the boundary of $\mc{F}$.
    \end{proof}
\end{document}
