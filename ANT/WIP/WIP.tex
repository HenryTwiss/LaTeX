...
\begin{document}
  ...
  \section{The Ideal Norm}
    ...
    \end{proof}
  \section{Moments}
    Our last summation formula is reminiscent of Abel's summation formula and is known as the \textbf{Euler–Maclaurin summation formula}\index{Euler–Maclaurin summation formula}:

    \begin{theorem*}[Euler–Maclaurin summation formula]
      For every $X$ and $Y$ with $0 \le X < Y$ and continuously differentiable function $\phi:[X,Y] \to \C$, we have
      \begin{align*}
        \sum_{X < n \le Y}a_{n}\phi(n) = \left(X-\lfloor X \rfloor-\frac{1}{2}\right)\phi(X)-\left(Y-\lfloor Y \rfloor-\frac{1}{2}\right)\phi(Y)+\int_{X}^{Y}\phi(u)\,du+\int_{X}^{Y}\left(u-\lfloor u \rfloor-\frac{1}{2}\right)\phi'(u)\,du.
      \end{align*}
    \end{theorem*}

    Our first result only uses partial summation. We first prove a useful lemma:

    \begin{lemma}\label{lem:weak_approximate_functional_equation_Riemann_zeta}
      For $x \ge 1$ and $\s > 0$, we have
      \[
        \z(s) = \sum_{n < x}\frac{1}{n^{s}}-\frac{s}{(1-s)x^{s-1}}-s\int_{x}^{\infty}\frac{\{u\}}{u^{s+1}}\,du.
      \]
    \end{lemma}
    \begin{proof}
      First suppose $\s > 1$ and consider
      \[
        \z(s)-\sum_{n < x}\frac{1}{n^{s}} = \sum_{n \ge x}\frac{1}{n^{s}}.
      \]
      Applying Abel's summation formula (see \cref{cor:Abels_summation_formula_limit_version}) to the right-hand side gives
      \[
        \z(s)-\sum_{n < x}\frac{1}{n^{s}} = \lim_{Y \to \infty}A(Y)Y^{-s}+s\int_{x}^{\infty}A(u)u^{-(s+1)}\,du.
      \]
      But as $A(x) = \lfloor x \rfloor$ and $\s > 1$, we find that $A(Y)Y^{-s} \to 0$ as $Y \to \infty$. In particular,
      \[
        \z(s)-\sum_{n < x}\frac{1}{n^{s}} = s\int_{x}^{\infty}\frac{\lfloor u \rfloor}{u^{s+1}}\,du.
      \]
      Isolating $\z(s)$ and noting that $\lfloor u \rfloor = u-\{u\}$ gives
      \[
        \z(s) = \sum_{n < x}\frac{1}{n^{s}}+s\int_{x}^{\infty}\frac{u-\{u\}}{u^{s+1}}\,du.
      \]
      But since
      \[
        s\int_{x}^{\infty}\frac{u}{u^{s+1}}\,du = s\int_{x}^{\infty}\frac{1}{u^{s}}\,du = -\frac{s}{(1-s)x^{s-1}},
      \]
      we obtain
      \[
        \z(s) = \sum_{n < x}\frac{1}{n^{s}}-\frac{s}{(1-s)x^{s-1}}-s\int_{x}^{\infty}\frac{\{u\}}{u^{s+1}}\,du,
      \]
      for $\s > 1$. Since this is the desired formula, it remains to show that the right-hand side is meromorphic for $\s > 0$. Indeed, the first two terms are, with the second term having a simple pole at $s = 1$, so it suffices to show that the integral is. To do so we will show that the integral is locally absolutely uniformly convergent in this region. Indeed, let $K$ is a compact subset in this region and set $\a = \min_{s \in K}(\s)$ and $\b = \max_{s \in K}(|s|)$. Then we have to show that the integral is absolutely uniformly convergent on $K$. As $0 < \{u\} < 1$, we have
      \[
        s\int_{x}^{\infty}\frac{\{u\}}{u^{s+1}}\,du \ll \b\int_{x}^{\infty}\frac{1}{u^{\a+1}}\,du = \frac{\b}{x^{\a}} \ll_{\a,\b} 1,
      \]
      as desired. Therefore the formula holds for $\s > 0$.
    \end{proof}

    We can now prove our first result concerning the second moment of the Riemann zeta function:
