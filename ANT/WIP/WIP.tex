...
\begin{document}
  ...
  \section{Cyclotomic Dedekind Zeta Functions}
    ...
    \begin{proof}
      ...
      \]
      Now the subgroup of $\left(\Z/\frac{n}{p^{e_{p}}}\Z\right)^{\ast}$ generated by $p$ is of order $f_{p}$ by definition of $f_{p}$. Moreover, the characters of this must be of the form
      \[
        \chi(np) = \w_{p}^{nk},
      \]
      for $0 \le k \le f_{p}-1$, since there are $f_{p}$ many such characters. These are clearly primitive Dirichlet characters modulo $f_{p}$. Now the fundamental equality gives $\vphi(d) = r_{p}\vphi(p^{e_{p}})f_{p}$ from which it follows that $\vphi\left(\frac{n}{p^{e_{p}}}\right) = r_{p}f_{p}$. Therefore each $\chi$ has $r_{p}$ many lifts to a Dirichlet character modulo $\frac{n}{p^{e_{p}}}$. \todo{xxx}
    \end{proof}
