\chapter{Moment Results}
  ...
  \section{\todo{The Second Moment of the Riemann Zeta Function}}
    ...
    \begin{proof}
      ...
      It follows that
      \[
        M_{2}(T,\z) = \int_{0}^{T}\sum_{n,m \ll \frac{t}{\sqrt{\log(t)}}}\frac{1}{(nm)^{\frac{1}{2}+it}}\,dt+O\left(\int_{0}^{T}\sum_{n \ll \frac{t}{\sqrt{\log(t)}}}\frac{\log^{\frac{1}{4}}(t)}{\sqrt{n}}\,dt\right)+O\left(\int_{0}^{T}\log^{\frac{1}{2}}(t)\,dt\right).
      \]
      ...
      \[
        M_{2}(T,\z) = O\left(\int_{0}^{T}\sum_{n,m \ll \frac{t}{\sqrt{\log(t)}}}\frac{1}{(nm)^{\frac{1}{2}+it}}\,dt\right)+O(T\log(T)).
      \]
      Therefore, it suffices to show that the remaining integral is $O(T\log(T))$. By the Fubini–Tonelli theorem, we may interchange the sum and integral so that
      \[
        \int_{0}^{T}\sum_{n,m \ll \frac{t}{\sqrt{\log(t)}}}\frac{1}{(nm)^{\frac{1}{2}+it}}\,dt = \sum_{n,m \ll \frac{T}{\sqrt{\log(T)}}}\int_{\max(m,n)}^{T}\frac{1}{(nm)^{\frac{1}{2}+it}}\,dt.
      \]
    \end{proof}