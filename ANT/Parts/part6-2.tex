...
\section{The Moment Conjectures}
  ...
  \]
  we expect the terms for which $|\conj{I}| \neq |\conj{J}|$ to be near zero when integrating from $0$ to $T$. In the case $|\conj{I}| = |\conj{J}|$, these factors cancel (in the limit as $\a_{i} \to 0$ for all $i$) and therefore we do not expect these terms to be near zero. The two equalities in part (v) are known as the \textbf{moment conjectures}\index{moment conjectures}:
