\chapter{Analytic Preliminaries}
  ...
  \section{The Gamma Function}
    ...
    \]
    so that this integral is absolutely uniformly convergent on $K$ too. Thus $\G(s)$ is absolutely uniformly convergent on $K$. In particular, $\G(s)$ is holomorphic for $\s > 0$. Also note that $\G(s)$ is real for $s > 0$. The most basic properties of $\G(s)$ are collected in the following result:
    ...
    Putting \cref{prop:Factorial_properties_of_gamma_function} (i) and (ii) together we see that $\G(n) = (n-1)!$ for any integer $n \ge 1$. So $\G(s)$ can be thought of as a holomorphic extension of the factorial function. Another special value of the gamma function is
    \begin{equation}\label{equ:gamma_function_at_1/2}
      \G\left(\frac{1}{2}\right) = \int_{0}^{\infty}\frac{e^{-x}}{\sqrt{x}}\,dx = 2\sqrt{\pi}\int_{0}^{\infty}e^{-\pi x^{2}}\,dx = \pi\int_{-\infty}^{\infty}e^{-\pi x^{2}}\,dx = \sqrt{\pi}.
    \end{equation}
    where in the second equality we have made the change of variables $x \mapsto \pi x^{2}$ and recognized the last integral as the Gaussian integral (see \cref{append:Special_Integrals}). We can also use \cref{prop:Factorial_properties_of_gamma_function} (ii) to extended $\G(s)$ to a meromorphic function on all of $\C$:

    \begin{theorem}\label{thm:continuation_of_gamma_function}
      For any $n \ge 0$, $\G(s)$ admits meromorphic continuation to the region $\s > -n$ given by
      \[
        \G(s) = \frac{\G(s+n+1)}{s(s+1) \cdots (s+n)}.
      \]
      In particular, $\G(s)$ admits meromorphic continuation to $\C$ with poles at $s = -n$ for $n \ge 0$. All of these poles are simple and with residue $\frac{(-1)^{n}}{n!}$ at $s = -n$.
    \end{theorem}
    \begin{proof}
      Using \cref{prop:Factorial_properties_of_gamma_function} (ii) repeatedly, we have
      \[
        \G(s) = \frac{\G(s+n+1)}{s(s+1) \cdots (s+n)},
      \]
      for any integer $n \ge 0$. The right-hand side defines an meromorphic function in the region $\s > -n$ and away from $0,-1,\ldots,-n$. Letting $n$ be arbitrary, we see that $\G(s)$ has meromorphic continuation to $\C$ with poles at $s = -n$. We now compute the residue at $s = -n$. Around this point $\G(s)$ admits meromorphic continuation with representation
      \[
        \frac{\G(s+n+1)}{s(s+1) \cdots (s+n)},
      \]
      where all of the factors except for $s+n$ are holomorphic at $s = -n$. Thus the pole is simple and
      \[
        \Res_{s = -n}\G(s) = \lim_{z \to -n}\frac{\G(s+n+1)(s+n)}{s(s+1) \cdots (s+n)} = \frac{\G(1)}{(-n)(1-n) \cdots (-1)} = \frac{(-1)^{n}}{n!}.
      \]
    \end{proof}

    In particular, \cref{thm:continuation_of_gamma_function} implies $\Res_{s = 0}\G(s) = 1$ and $\Res_{s = 1}\G(s) = -1$. Now that we have deduced the meromorphic continuation of $\G(s)$, we need to introduce a related function. We define the \textbf{beta function}\index{beta function} $B(s,u)$ by
    \[
    ...
    \[
      \int_{0}^{1}x^{s-1}(1-x)^{u-1}\,dx \ll \int_{0}^{1}x^{\a-1}(1-x)^{\b-1}\,dx \ll_{\a,\b} 1,
    \]
    so that this integral is absolutely uniformly convergent on $K \x L$. Thus $B(s,u)$ is absolutely uniformly convergent on $K \x L$. This means $B(s,u)$ is holomorphic for $\s > 0$ and $\tau > 0$. We now turn to basic properties of the beta function. Making the change of variables $x \mapsto 1-x$ in the integral defining $B(s,u)$, we see that
    \[
        B(s,u) = \int_{0}^{1}x^{s-1}(1-x)^{u-1}\,dx = \int_{0}^{1}x^{u-1}(1-x)^{s-1}\,dx = B(u,s),
    \]
    which is to say that the beta function is symmetric in $s$ and $u$. If instead we make the change of variables $x \mapsto \frac{x}{x+1}$, we obtain
    \begin{align*}
      B(s,u) &= \int_{0}^{\infty}\left(\frac{x}{x+1}\right)^{s-1}\left(1-\frac{x}{x+1}\right)^{u-1}\frac{1}{(x+1)^{2}}\,dx \\
      &= \int_{0}^{\infty}\left(\frac{x}{x+1}\right)^{s-1}\left(\frac{1}{x+1}\right)^{u-1}\frac{1}{(x+1)^{2}}\,dx \\
      &= \int_{0}^{\infty}\frac{x^{s-1}}{(x+1)^{s+u}}\,dx.
    \end{align*}
    In short,
    \begin{equation}\label{equ:residue_form_beta_function}
      B(s,u) = \int_{0}^{\infty}\frac{x^{s-1}}{(x+1)^{s+u}}\,dx.
    \end{equation}
    \cref{equ:residue_form_beta_function} is immensely useful when $s+u = 1$ because we can evaluate $B(s,u)$ using the residue theorem. However, we first show that the beta function is deeply related to the gamma function:
    ...
    \end{proposition}
    \begin{proof}
      First use the Fubini–Tonelli theorem to write
      \begin{align*}
        \G(s)\G(u) = \left(\int_{0}^{\infty}e^{-x}x^{s-1}\,dx\right)\left(\int_{0}^{\infty}e^{-y}y^{u-1}\,dy\right) = \int_{0}^{\infty}\int_{0}^{\infty}e^{-(x+y)}x^{s-1}y^{u-1}\,dx\,dy.
      \end{align*}
      Performing the change of variables $(x,y) \mapsto (x,y-x)$, the last double integral becomes
      \[
        \int_{0}^{\infty}\int_{0}^{y}e^{-y}x^{s-1}(y-x)^{u-1}\,dx\,dy,
      \]
      because the absolute value of the determinant of the Jacobian matrix is $1$.
      The further change of variables $x \mapsto xy$ gives
      \[
        \int_{0}^{\infty}\int_{0}^{1}e^{-y}(xy)^{s-1}(y-xy)^{u-1}\,dx\,dy.
      \]
      Using the Fubini–Tonelli theorem again, we recognize this double integral as
      \begin{align*}
        \int_{0}^{\infty}\int_{0}^{1}e^{-y}(xy)^{s-1}(y-xy)^{u-1}\,dx\,dy = \left(\int_{0}^{1}x^{s-1}(1-x)^{u-1}\,dx\right)\left(\int_{0}^{\infty}e^{-y}y^{s+u-1}\,dy\right) = B(s,u)\G(s+u).
      \end{align*}
      Therefore
      \[
        \G(s)\G(u) = B(s,u)\G(s+u),
    ...
    We can use \cref{prop:integral_reprepsentation_for_beta_function} together with \cref{equ:residue_form_beta_function} to prove some infamous properties of the gamma function. The first is \textbf{Euler's reflection formula}\index{Euler's reflection formula}:
    ...
    \end{theorem*}
    \begin{proof}
      By the identity theorem it suffices to prove this for real $s$ with $0 < s < 1$. Then \cref{prop:integral_reprepsentation_for_beta_function} gives
      \[
        \G(s)\G(1-s) = B(s,1-s)\G(1).
      \]
      By \cref{prop:Factorial_properties_of_gamma_function} (i) together with \cref{equ:residue_form_beta_function}, it follows that
      \[
      ...
      \begin{figure}[ht]
        ...
        \begin{tikzpicture}[scale=3]
          ...
          \draw[->-] ({asin(0.1/2/0.2)}:0.2) -- node[above] {\tiny{$\eta_{1}$}} ({asin(0.1/2/1.5)}:1.5);
          \draw[->-] ({asin(0.1/2/1.5)}:1.5) arc ({asin(0.1/2/1.5)}:{360-asin(0.1/2/1.5)}:1.5);
          \node at (-1,0) [above left] {\tiny{$\eta_{2}$}};
          \draw[->-] ({-asin(0.1/2/1.5)}:1.5) -- node[below] {\tiny{$\eta_{3}$}} ({-asin(0.1/2/0.2)}:0.2);
          \draw[->-] ({-asin(0.1/2/0.2)}:0.2) arc ({-asin(0.1/2/0.2)}:{asin(0.1/2/0.2)-360}:0.2);
          \node at (-0.2,0) [below left] {\tiny{$\eta_{4}$}};

          \node at ({asin(0.1/2/0.2)}:0.2) [circle,fill,inner sep=1pt]{};
          \node at ({asin(0.1/2/0.2)}:0.2) [above right] {\tiny{$\e e^{i\d_{R,\e}}$}};

          \node at ({asin(0.1/2/1.5)}:1.5) [circle,fill,inner sep=1pt]{};
          \node at ({asin(0.1/2/1.5)}:1.5) [above right] {\tiny{$Re^{i\d_{R,\e}}$}};
          
          \node at ({-asin(0.1/2/0.2)}:0.2) [circle,fill,inner sep=1pt]{};
          \node at ({-asin(0.1/2/0.2)}:0.2) [below right] {\tiny{$\e e^{i\left(2\pi-\d_{R,\e}\right)}$}};

          \node at ({-asin(0.1/2/1.5)}:1.5) [circle,fill,inner sep=1pt]{};
          \node at ({-asin(0.1/2/1.5)}:1.5) [below right] {\tiny{$Re^{i(2\pi-\d_{R,\e})}$}};
        \end{tikzpicture}
        \caption{A keyhole contour.}
        \label{fig:reflection_formula_contour}
      \end{figure}

      So it suffices to show that the remaining integral is $\frac{\pi}{\sin(\pi s)}$. Let $\eta = \sum_{1 \le i \le 4}\eta_{i}$ be the contour in \cref{fig:reflection_formula_contour} for some $R > 1$ and $0 < \e < 1$ with corresponding angle $\d_{R,\e} > 0$. Define integrals
      \[
        I_{i}(R,\e) = \int_{\eta_{i}}\frac{z^{s-1}}{z+1}\,dz,
      \]
      for $1 \le i \le 4$, so that
      \begin{equation}\label{equ:Euler_reflection_formula_1}
        \int_{\eta}\frac{z^{s-1}}{z+1}\,dz = I_{1}(R,\e)+I_{2}(R,\e)+I_{3}(R,\e)+I_{4}(R,\e).
      \end{equation}
      We will now evaluate the integral in the left-hand side of \cref{equ:Euler_reflection_formula_1} using the residue theorem and take the limits of the integrals on the right-hand of \cref{equ:Euler_reflection_formula_1} as $\e \to 0$ and $R \to \infty$. On the one hand, the only poles of the integrand enclosed by the contour $\eta$ is a simple pole at $z = -1$. So the residue theorem implies
      \begin{equation}\label{equ:Euler_reflection_formula_2}
        \int_{\eta}\frac{z^{s-1}}{z+1}\,dz = \Res_{z = -1}\left(\frac{z^{s-1}}{z+1}\right) = 2\pi i(-1)^{s-1} = 2\pi ie^{(s-1)\log_{0}(-1)} = 2\pi ie^{\pi i(s-1)},
      \end{equation}
      where in the last equality we have used that $\log_{0}(-1) = i\pi$ (since this is the branch cut of the logarithm along the positive real axis). Now for $I_{1}(R,\e)$, we clearly have
      \begin{equation}\label{equ:Euler_reflection_formula_3}
        \lim_{\e \to 0}\lim_{R \to \infty}I_{1}(R,\e) = \int_{0}^{\infty}\frac{x^{s-1}}{x+1}\,dx.
      \end{equation}
      For $I_{3}(R,\e)$, the argument of $s$ along $\eta_{3}$ is $2\pi$ that of it along $\eta_{1}$. Hence
      \[
        I_{3}(R,\e) = \int_{\eta_{3}}\frac{z^{s-1}}{z+1}\,dz = \int_{-\eta_{1}}\frac{(e^{2\pi i}z)^{s-1}}{e^{2\pi i}z+1}\,dz = -e^{2\pi i(s-1)}\int_{\eta_{1}}\frac{z^{s-1}}{z+1}\,dz = -e^{2\pi i(s-1)}I_{1}(R,\e),
      \]
      and it follows from \cref{equ:Euler_reflection_formula_3} that
      \begin{equation}\label{equ:Euler_reflection_formula_4}
        \lim_{\e \to 0}\lim_{R \to \infty}I_{3}(R,\e) = e^{2\pi i(s-1)}\int_{0}^{\infty}\frac{x^{s-1}}{x+1}\,dx.
      \end{equation}
      For $I_{2}(R,\e)$, make the change of variables $z \mapsto Re^{i\t}$ to see that
      \[
        |I_{2}(R,\e)| \le \left|\int_{\d_{R,\e}}^{2\pi-\d_{R,\e}}\frac{(Re^{i\t})^{(s-1)}}{Re^{i\t}+1}iRe^{i\t}\,d\t\right| \le \int_{\d_{R,\e}}^{2\pi-\d_{R,\e}}\frac{R^{s}}{R-1}\,d\t \le 2\pi\frac{R^{s}}{R-1},
      \]
      where we have applied the reverse triangle inequality to get $|Re^{i\t}+1| \ge R-1$. This estimate implies
      \[
        \lim_{R \to \infty}|I_{2}(R,\e)| \le \lim_{R \to \infty}2\pi\frac{R^{s}}{R-1} = \lim_{R \to \infty}2\pi sR^{s-1} = 0,
      \]
      where in the first equality we have used L'H\^opital's rule and the second equality follows since $0 < s < 1$. Thus
      \begin{equation}\label{equ:Euler_reflection_formula_5}
        \lim_{\e \to 0}\lim_{R \to \infty}I_{2}(R,\e) = 0.
      \end{equation}
      For $I_{4}(R,\e)$, the change of variables $z \mapsto \e e^{i\t}$ implies
       \[
        |I_{4}(R,\e)| \le \left|\int_{\d_{R,\e}}^{2\pi-\d_{R,\e}}\frac{(\e e^{i\t})^{(s-1)}}{\e e^{i\t}+1}i\e e^{i\t}\,d\t\right| \le \int_{\d_{R,\e}}^{2\pi-\d_{R,\e}}\frac{\e^{s}}{1-\e}\,d\t \le 2\pi\frac{\e^{s}}{1-\e},
      \]
       where we have applied the reverse triangle inequality to get $|\e e^{i\t}+1| \ge 1-\e$. Note that this estimate is independent of $R$ and implies
       \[
        \lim_{\e \to 0}|I_{4}(R,\e)| \le \lim_{\e \to 0}2\pi\frac{\e^{s}}{1-\e} = 0.
       \]
       Hence
      \begin{equation}\label{equ:Euler_reflection_formula_6}
        \lim_{\e \to 0}\lim_{R \to \infty}I_{4}(R,\e) = 0.
      \end{equation}
      Substituting \cref{equ:Euler_reflection_formula_2,equ:Euler_reflection_formula_3,equ:Euler_reflection_formula_4,equ:Euler_reflection_formula_5,equ:Euler_reflection_formula_6} into \cref{equ:Euler_reflection_formula_1} yields
      \[
       (1-e^{2\pi i(s-1)})\int_{0}^{\infty}\frac{x^{s-1}}{x+1}\,dx = 2\pi ie^{\pi i(s-1)}.
      \]
      Using the formula $\sin(s) = \frac{e^{is}+e^{-is}}{2i}$, we find that
      \[
        \int_{0}^{\infty}\frac{x^{s-1}}{x+1}\,dx = \frac{2\pi ie^{\pi i(s-1)}}{1-e^{2\pi i(s-1)}} = \pi\frac{2i}{e^{i\pi s}-e^{-i\pi s}} = \frac{\pi}{\sin(\pi s)},
      \]
      for $0 < s < 1$ as claimed. The identity theorem finishes the proof.
    \end{proof}

    Euler's reflection formula can be used to show that the gamma function has no zeros. Indeed, Euler's reflection formula implies
    \[
      \G(s)\G(1-s)\frac{\sin(\pi s)}{\pi} = 1,
    \]
    provided $s$ is not a nonpositive integer. Since $\sin(\pi s)$ is nonzero provided $s$ is not an integer, the above identity implies that $\G(s)$ is also nonzero except possibly at positive integers. But for any $n \ge 1$, we have $\G(n) = (n-1)!$ which is clearly nonzero. In short, $\G(s)$ has no zeros and so $\frac{1}{\G(s)}$ is entire. This is an essential fact about the gama function. As a nice consequence, we can obtain the meromorphic continuation of the beta function:
    ...
    \end{theorem}
    \begin{proof}
      Since $\G(s)$ has no zeros, \cref{prop:integral_reprepsentation_for_beta_function} implies that
      \[
        B(s,u) = \frac{\G(s)\G(u)}{\G(s+u)},
      \]
      for all $\s > 0$ and $\tau > 0$. \cref{thm:continuation_of_gamma_function} completes the proof.
    \end{proof}
    
    The second infamous property of the gamma function is \textbf{Legendre's duplication formula}\index{Legendre's duplication formula} (see \cite{remmert1998classical} for a proof):

    \begin{theorem*}[Legendre's duplication formula]
      For any $s \in \C-\{0,-1,-2,\ldots\}$, we have
      \[
        \G(s)\G\left(s+\frac{1}{2}\right) = 2^{1-2s}\sqrt{\pi}\G(2s).
      \]
    \end{theorem*}
    \begin{proof}
      By the identity theorem it suffices to prove this for real $s$ with $s > 1$. Consider
      \[
        B(s,s) = \int_{0}^{1}x^{s-1}(1-x)^{s-1}\,dx.
      \]
      The change of variables $x \mapsto \frac{x+1}{2}$ transforms the integral into the form
      \[
        \frac{1}{2}\int_{-1}^{1}\left(\frac{x+1}{2}\right)^{s-1}\left(1-\frac{x+1}{2}\right)^{s-1}\,dx = \frac{1}{2}\int_{-1}^{1}\left(\frac{x+1}{2}\right)^{s-1}\left(\frac{1-x}{2}\right)^{s-1}\,dx =  2^{1-2s}\int_{-1}^{1}(1-x^{2})^{s-1}\,dx.
      \]
      Since the integrand of the last integral is even, we can express it as
      \[
      ...
      \]
      Upon invoking \cref{thm:continuation_of_beta_function}, this identity becomes
      \[
        \frac{\G(s)^{2}}{\G(2s)} = 2^{1-2s}\frac{\G\left(\frac{1}{2}\right)\G(s)}{\G\left(s+\frac{1}{2}\right)}.
      \]
      As $\G(s)$ is nonzero, cancelling the common $\G(s)$ factor and using \cref{equ:gamma_function_at_1/2} gives
      \[
        \frac{\G(s)}{\G(2s)} = \frac{2^{1-2s}\sqrt{\pi}}{\G\left(s+\frac{1}{2}\right)},
      \]
      which is equivalent to the desired formula. The identity theorem finishes the proof.
    \end{proof}

    Since $\frac{1}{\G(s)}$ is entire, we turn to finding its Hadamard factorization. Recall
    \begin{equation}\label{equ:limit_form_of_exponential}
      \lim_{n \to \infty}\left(\frac{x}{n}+1\right)^{n} = e^{x},
    \end{equation}
    for any $x > 0$ and that $\left(\frac{x}{n}+1\right)^{n}$ is monotonically increasing in $n$. We first prove the following lemma:
    ...
    \begin{proof}
      ...
      \]
      where we have used \cref{prop:Factorial_properties_of_gamma_function} (ii). As
      \[
        \lim_{n \to \infty}\left(\frac{s+n-1}{n}\right) = 1,
      \]
      the previous identity implies that the claim holds for $s$ if and only if it holds for $s-1$. Therefore we may assume $\s < 0$. Equivalently, it suffices to show
      \[
        \lim_{n \to \infty}\frac{n^{s}\G(n-s)}{\G(n)} = 1,
      \]
      for $\s > 0$. Appealing to \cref{equ:residue_form_beta_function,thm:continuation_of_beta_function}, we have
      \[
       \frac{n^{s}\G(n-s)}{\G(n)} = \frac{n^{s}B(s,n-s)}{\G(s)} = \frac{1}{\G(s)}n^{s}\int_{0}^{\infty}\frac{x^{s-1}}{(x+1)^{n}}\,dx = \frac{1}{\G(s)}\int_{0}^{\infty}\frac{x^{s-1}}{\left(\frac{x}{n}+1\right)^{n}}\,dx,
      \]
      where in the last equality we have made the change of variables $x \mapsto \frac{x}{n}$. By \cref{equ:limit_form_of_exponential} and that $\left(\frac{x}{n}+1\right)^{n}$ is monotonically increasing in $n$, we can employ the monotone convergence theorem to obtain 
      \[
        \lim_{n \to \infty}\frac{n^{s}\G(n-s)}{\G(n)} = \frac{1}{\G(s)}\int_{0}^{\infty}\lim_{n \to \infty}\frac{x^{s-1}}{\left(\frac{x}{n}+1\right)^{n}}\,dx = \frac{1}{\G(s)}\int_{0}^{\infty}e^{-x}x^{s-1}\,dx = 1.
      \]
    \end{proof}

    Obtaining the Hadamard factorization of $\frac{1}{\G(s)}$ is now a fairly simple matter. Recall that the Euler-Mascheroni constant $\g$ is defined as the limit
    ...
    \begin{theorem}\label{thm:Hadamard_factorization_for_reciprocial_of_gamma}
      For any $s \in \C-\{0,-1,-2,\ldots\}$, we have
      \[
    ...
    \end{theorem}
    \begin{proof}
      By applying \cref{prop:Factorial_properties_of_gamma_function} (ii) repeatedly, we have
      \[
        \frac{1}{\G(s)} = \frac{s(s+1) \cdots (s+k-1)}{\G(s+k)},
      \]
      for any $k \ge 1$. Now for every $1 \le n \le k-1$, write
      \[
       s+n = e^{\frac{s}{n}}e^{-\frac{s}{n}}n\left(1+\frac{s}{n}\right).
      \]
      Then our previous identity takes the form
      \[
        \frac{1}{\G(s)} = se^{\left(1+\frac{1}{2}+\cdots+\frac{1}{k}\right)s}\prod_{1 \le n \le k}\left(1+\frac{s}{n}\right)e^{-\frac{s}{n}}\frac{(k-1)!}{\G(s+k)},
      \]
      which, using the facts $k^{-s}= e^{-s\log(k)}$ and $\G(k) = (k-1)!$, we rewrite as
      \[
    ...
    From \cref{thm:Hadamard_factorization_for_reciprocial_of_gamma}, we immediately see that $\frac{1}\G(s)$ is of order $1$ (see \cref{append:Factorizations_and_Finite_Order}). In particular, this means $\G(s)$ is of order $1$ for $\s > 0$. We will also require a well-known asymptotic formula for the gamma function known as \textbf{Stirling's formula}\index{Stirling's formula}:

    \begin{theorem*}[Stirling's formula]
      Let $|\arg(s)| < \pi-\e$ and $|s| > \d$ for some $\e,\d > 0$. Then
      \[
        \G(s) = \sqrt{2\pi}s^{s-\frac{1}{2}}e^{-s}\left(1+O\left(\frac{1}{s}\right)\right).
      \]
      In particular,
      \[
    ...
    \end{theorem*}
    \begin{proof}
      We first additionally suppose $\s > 0$. Complexify the integral defining $\G(s+1)$ and observe that
      \[
        \G(s+1) = \int_{0}^{\infty}e^{-z}e^{s\log(z)}\,dz.
      \]
      Now change variables $z \to s(1+z)$ to see that
      \[
        \G(s+1) = s\int_{-1}^{\infty}e^{-s(1+z)}e^{s\log(s(1+z))}\,dz = s^{s+1}e^{-s}\int_{-1}^{\infty}e^{s(\log(1+z)-z)}\,dz,
      \]
      where we have homotoping the transformed contour to $(-1,\infty)$. We will now estimate the remaining integral. Being by defining integrals
      \[
        I(s) = \int_{-1}^{\infty}e^{s(\log(1+z)-z)}\,dz, \quad I_{1}(s) = \int_{-1}^{1}e^{s(\log(1+z)-z)}\,dz, \quad \text{and} \quad I_{2}(s) = \int_{1}^{\infty}e^{s(\log(1+z)-z)}\,dz,
      \]
      so that
      ...
      \]
      and using the fact $\log(3+z) = \log(1+z)+O(1)$ gives
      \[
        I_{2}(s) = e^{-2s}\int_{-1}^{\infty}e^{s(\log(1+z)+O(1)-z)}\,dz = e^{-2s}\int_{-1}^{\infty}e^{s(\log(1+z)-z)}(1+O(1))\,dz = I(s)(e^{-2s}+O(e^{-2s})).
      \]
      Whence
      \[
        I(s) = I_{1}(s)+I(s)(e^{-2s}+O(e^{-2s})).
      \]
      Now isolate $I(s)$ to see that
      \[
        I(s) = \frac{1}{(1-e^{-2s})}\left(I_{1}(s)+O(I(s)e^{-2s})\right) = I_{1}(s)+O(I(s)e^{-2s}) = I_{1}(s)+O(I_{1}(s)e^{-2s})+O(I_{2}(s)e^{-2s}).
      \]
      But we also have $I_{2}(s) = O(1)$ by the exponential decay of the integrand, so that the previous estimate gives
      \begin{equation}\label{equ:Stirlings_formula_2}
        I(s) = I_{1}(s)+O(I_{1}(s)e^{-2s})+O(e^{-2s}).
      \end{equation}
      ...
      \]
      At $z = 0$, the first derivative is zero and the second derivative is negative. Taking the Taylor series of $\log(1+z)-z$ and truncating after the third term gives
      \[
        \log(1+z)-z = -\frac{1}{2}z^{2}+\frac{1}{3}z^{3}+O(z^{4}).
      \]
      Using this estimate in the previous integral results in
      \[
        I_{1}(s) = \int_{-1}^{1}e^{s\left(-\frac{1}{2}z^{2}+\frac{1}{3}z^{3}+O(z^{4})\right)}\,dz = \int_{-1}^{1}e^{-\frac{s}{2}z^{2}}e^{\frac{1}{3}sz^{3}+O(sz^{4})}\,dz.
      \]
       Considering the Taylor series of $e^{z}$ and truncating after the second term, we find that
      \[
        e^{\frac{1}{3}sz^{3}+O(sz^{4})} = 1+\frac{1}{3}sz^{3}+O(sz^{4})+O(s^{2}z^{6}).
      \]
      Plugging this estimate into the previous integral yields
      \[
        I_{1}(s) = \int_{-1}^{1}e^{-\frac{s}{2}z^{2}}\left(1+\frac{1}{3}sz^{3}+O(sz^{4})+O(s^{2}z^{6})\right)\,dz.
      \]
      Since the first and second terms in the asymptotic are even and odd in $z$ respectively, symmetry gives
      \[
        I_{1}(s) = 2\int_{0}^{1}e^{-\frac{s}{2}z^{2}}\left(1+O(sz^{4})+O(s^{2}z^{6})\right)\,dz.
      \]
      ...
      \begin{equation}\label{equ:Stirlings_formula_3}
        I_{1}(s) = \sqrt{2}s^{-\frac{1}{2}}\int_{0}^{\frac{|s|}{2}}e^{-z}z^{-\frac{1}{2}}\left(1+O\left(\frac{1}{s}\right)\right)\,dz = \sqrt{2}s^{-\frac{1}{2}}\int_{0}^{\frac{|s|}{2}}e^{-z}z^{-\frac{1}{2}}\,dz\left(1+O\left(\frac{1}{s}\right)\right),
      \end{equation}
      where we have homotoped the transformed contour to $\left(0,\frac{|s|}{2}\right)$. We now estimate the remaining integral in \cref{equ:Stirlings_formula_3}. To do this, observe
      \[
        \int_{0}^{\frac{|s|}{2}}e^{-z}z^{-\frac{1}{2}}\,dz = \int_{0}^{\infty}e^{-z}z^{-\frac{1}{2}}\,dz-\int_{\frac{|s|}{2}}^{\infty}e^{-z}z^{-\frac{1}{2}}\,dz = \sqrt{\pi}-\int_{\frac{|s|}{2}}^{\infty}e^{-z}z^{-\frac{1}{2}}\,dz,
      \]
      because the first integral in the middle expression is $\G\left(\frac{1}{2}\right) = \sqrt{\pi}$. By the exponential decay of the integrand, we have say
      \[
        \int_{\frac{|s|}{2}}^{\infty}e^{-z}z^{-\frac{1}{2}}\,dz = O\left(\int_{\frac{|s|}{2}}^{\infty}\frac{1}{z^{3}}\,dz\right) = O\left(\frac{1}{s^{2}}\right),
      \]
      so that 
      \begin{equation}\label{equ:Stirlings_formula_4}
        \int_{0}^{\frac{|s|}{2}}e^{-z}z^{-\frac{1}{2}}\,dz = \sqrt{\pi}+O\left(\frac{1}{s^{2}}\right).
      \end{equation}
      ...
      \begin{equation}\label{equ:Stirlings_formula_5}
        I_{1}(s) = \sqrt{2\pi}s^{-\frac{1}{2}}\left(1+O\left(\frac{1}{s}\right)\right).
      \end{equation}
      Combining \cref{equ:Stirlings_formula_1,equ:Stirlings_formula_2,equ:Stirlings_formula_5} yields
      \[
        \G(s+1) = \sqrt{2\pi}s^{s+\frac{1}{2}}e^{-s}\left(1+O\left(\frac{1}{s}\right)\right).
      \]
      Now apply \cref{prop:Factorial_properties_of_gamma_function} (ii) to get
      \[
        \G(s) = \sqrt{2\pi}s^{s-\frac{1}{2}}e^{-s}\left(1+O\left(\frac{1}{s}\right)\right).
      \]
      This proves the asymptotic formula when $\s > 0$. In fact, applying \cref{prop:Factorial_properties_of_gamma_function} (ii) repeatedly shows
      \[
        \G(s-n) = \sqrt{2\pi}\frac{s^{s-\frac{1}{2}}}{(s-1)(s-2) \ldots (s-n)}e^{-s}\left(1+O\left(\frac{1}{s}\right)\right),
      \]
      for any $n \ge 0$. Since $\frac{1}{s-k} = \frac{1}{s}+O\left(\frac{1}{s^{2}}\right)$ for $1 \le k \le n$, it follows that
      \[
        \G(s-n) = \sqrt{2\pi}s^{s-n-\frac{1}{2}}e^{-s}\left(1+O\left(\frac{1}{s}\right)\right).
      \]
      As $n$ was arbitrary, this proves the asymptotic formula when $\s \le 0$ as well. The asymptotic equivalence follows immediately.
    \end{proof}

    If $\s$ is bounded, Stirling's formula gives a useful asymptotic equivalence showing that $\G(s)$ decays exponentially in $t$ as $s \to \infty$:
    
    \begin{corollary}\label{equ:weaker_Stirling_formula}
    Let $|\arg(s)| < \pi-\e$ and $|s| > \d$ for some $\e,\d > 0$. Then for bounded $\s$, we have
      \[
    ...
    \begin{proof}
      ...
      \]
      upon taking the absolute value of the right-hand side (which is permitted because we are considering asymptotic equivalence). Since $\s$ is bounded, we have $|s| \sim |t|$, $\arg(s) \sim \sgn(t)\frac{\pi}{2}$, and $t\sgn(t) = |t|$. In particular, $-(t\arg(s)+\s) \sim -|t|\frac{\pi}{2}$. Hence
      \[
    ...
    Having discussed the major properties of the gamma function, we now introduce a related function. We call $\frac{\G'}{\G}(s)$ the \textbf{digamma function}\index{digamma function}. Equivalently, the digamma function is the logarithmic derivative of the gamma function. Note that upon taking the logarithmic derivative of $\G(s+1) = s\G(s)$, we see that the digamma function satisfies the related formula
    \[
      \frac{\G'}{\G}(s+1) = \frac{\G'}{\G}(s)+\frac{1}{s}.
    \]
    Using \cref{thm:Hadamard_factorization_for_reciprocial_of_gamma} to take the logarithmic derivative of $\frac{1}{s\G(s)}$, we obtain a useful expression for the digamma function:

    \begin{corollary}\label{cor:logarithmic_derivative_of_gamma}
      For all $s \in \C-\{0,1,2,\ldots\}$, we have
      \[
    ...
    \end{corollary}
    \begin{proof}
      By \cref{prop:Factorial_properties_of_gamma_function} (ii), we have $\frac{1}{\G(s+1)} = \frac{1}{s\G(s)}$. Taking the logarithmic derivative using \cref{thm:Hadamard_factorization_for_reciprocial_of_gamma} gives
      \[
        -\frac{\G'}{\G}(s+1) = \g+\sum_{n \ge 1}\left(\frac{1}{s+n}-\frac{1}{n}\right).
      \]
      This is the desired formula and the statement regarding the poles follows immediately.
    \end{proof}
    
    As for estimates involving the digamma function, taking the logarithm of Stirling's formula gives
    \begin{equation}\label{equ:log_gamma_estimate}
      \log\G(s) = \frac{1}{2}\log(2\pi)+\left(s-\frac{1}{2}\right)\log(s)-s+O\left(\frac{1}{s}\right),
    \end{equation}
    provided $|\arg(s)| < \pi-\e$ and $|s| > \d$ for some $\e,\d > 0$. We can obtain a useful estimate for the digamma function by using \cref{equ:log_gamma_estimate}:

    \begin{proposition}\label{equ:approximtion_for_digamma}
    Let $|\arg(s)| < \pi-\e$ and $|s| > \d$ for some $\e,\d > 0$. Then
    \[
      \frac{\G'}{\G}(s) = \log(s)+O\left(\frac{1}{s}\right).
    \]
    \end{proposition}
    \begin{proof}
      Set $g(s) = \frac{1}{2}\log(2\pi)+\left(s-\frac{1}{2}\right)\log(s)-s$. Then \cref{equ:log_gamma_estimate} implies
      \[
        \log\G(s) = g(s)+O\left(\frac{1}{s}\right).
      \]
      By Cauchy's integral formula, we have
      \begin{align*}
        \frac{\G'}{\G}(s) &= \frac{d}{ds}(g(s)+\log\G(s)-g(s)) \\
        &= g'(s)+\frac{d}{ds}(\log\G(s)-g(s)) \\
        &= \log(s)+\frac{1}{2\pi i}\int_{C}\frac{\log\G(u)-g(u)}{(u-s)^{2}}\,du,
      \end{align*}
      where $C$ is a circle about $s$ of sufficiently small radius $\eta$ depending upon $\e$ and $\d$. Therefore
      \[
        \left|\frac{\G'}{\G}(s)-\log(s)\right| \le \frac{1}{2\pi}\int_{C}\frac{|\log\G(u)-g(u)|}{\eta^{2}}\,|du| \ll \frac{1}{s},
      \]
      where the last estimate follows because $\log\G(u)-g(u) = O\left(\frac{1}{u}\right)$ and $u \asymp s$.
    \end{proof}