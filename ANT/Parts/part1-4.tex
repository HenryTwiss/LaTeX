\chapter{Analytic Preliminaries}
  ...
  \section{The Gamma Function}
    ...
    We can use \cref{prop:integral_reprepsentation_for_beta_function} to prove some infamous properties of the gamma function. The first is the \textbf{Euler reflection formula}\index{Euler reflection formula}:
    ...
    \begin{proof}
      ...
      \]
      and by the definition of the beta function together with \cref{prop:Factorial_properties_of_gamma_function} (i), we further have
      \[
        \G(s)\G(1-s) = \int_{0}^{1}x^{s-1}(1-x)^{-s}\,dx.
      \]
      So it suffices to show that the remaining integral is $\frac{\pi}{\sin(\pi s)}$. First make the change of variables $x \mapsto \frac{x}{1+x}$ so that the integral becomes
      \begin{align*}
        \G(s)\G(1-s) &= \int_{0}^{\infty}\left(\frac{x}{x+1}\right)^{s-1}\left(1-\frac{x}{x+1}\right)^{-s}\frac{1}{(x+1)^{2}}\,dx \\
        &= \int_{0}^{\infty}\left(\frac{x}{x+1}\right)^{s-1}\left(\frac{1}{x+1}\right)^{-s}\frac{1}{(x+1)^{2}}\,dx \\
        &= \int_{0}^{\infty}\frac{x^{s-1}}{x+1}\,dx.
      \end{align*}
      We will evaluate this last integral by contour integration. Complexify the integral so that it takes the form
      \[
        \int_{\Im(z) = 0}\frac{z^{s-1}}{z+1}\,dz.
      \]
      ...
      Let $\eta = \sum_{1 \le i \le 4}\eta_{i}$ be the contour in \cref{fig:reflection_formula_contour} for some $R > 1$ and $0 < \e < 1$ with corresponding angle $\d_{R,\e} > 0$. Define integrals
      \[
      ...
      \begin{equation}\label{equ:Euler_reflection_formula_3}
        \lim_{\e \to 0}\lim_{R \to \infty}I_{1}(R,\e) = \int_{\Im(z) = 0}\frac{z^{s-1}}{z+1}\,dz.
      \end{equation}
      ...
      \begin{equation}\label{equ:Euler_reflection_formula_4}
        \lim_{\e \to 0}\lim_{R \to \infty}I_{3}(R,\e) = e^{2\pi i(s-1)}\int_{\Im(z) = 0}\frac{z^{s-1}}{z+1}\,dz.
      \end{equation}
      For $I_{2}(R,\e)$, make the change of variables $s \mapsto Re^{i\t}$ for $\t \in [\d_{R,\e},2\pi-\d_{R,\e}]$ to see that
      \[
      ...
      \end{equation}
      For $I_{4}(R,\e)$, the change of variables $s \mapsto \e e^{i\t}$ for $\t \in [\d_{R,\e},2\pi-\d_{R,\e}]$ implies
       \[
      ...
      \[
       (1-e^{2\pi i(s-1)})\int_{\Im(z) = 0}\frac{z^{s-1}}{z+1}\,dz = 2\pi ie^{\pi i(s-1)}.
      \]
      Using the formula $\sin(z) = \frac{e^{iz}+e^{-iz}}{2i}$, we find that
      \[
        \int_{\Im(z) = 0}\frac{z^{s-1}}{z+1}\,dz = \frac{2\pi ie^{\pi i(s-1)}}{1-e^{2\pi i(s-1)}} = \pi\frac{2i}{e^{i\pi s}-e^{-i\pi s}} = \frac{\pi}{\sin(\pi s)},
      \]
    ...
    There is also the important Hadamard factorization of the reciprocal of $\G(s)$ (see \cite{stein2003complex} for a proof):
    ...
    Note that \cref{equ:weaker_Stirling_formula} only tells us how $\G(s)$ grows for large $t$ when $\s$ is bounded. In any case, taking the logarithm of Stirling's formula gives
    ...
    \end{equation}
    provided $|\arg(s)| < \pi-\e$ and $|s| > \d$ for some $\e,\d > 0$, which will be useful. In fact, from \cref{equ:log_gamma_estimate} we can obtain a useful estimate for the digamma function:
