\chapter{Analytic Preliminaries}
  ...
  \section{The Gamma Function}
    ...
    We can use \cref{prop:integral_reprepsentation_for_beta_function} together with \cref{equ:residue_form_beta_function} to prove some infamous properties of the gamma function. The first is the \textbf{Euler reflection formula}\index{Euler reflection formula}:

    \begin{theorem*}[Euler reflection formula]
    ...
    The Euler reflection formula can be used to show that the gamma function has no zeros. Indeed, the Euler reflection formula implies
    \[
    ...
    The second infamous property of the gamma function is the \textbf{Legendre duplication formula}\index{Legendre duplication formula} (see \cite{remmert1998classical} for a proof):
    ...
    \begin{proof}
      ...
      \]
      where in the last equality we have made the change of variables $x \to \frac{x}{n}$. By \cref{equ:limit_form_of_exponential} and that $\left(\frac{x}{n}+1\right)^{n}$ is monotonically increasing in $n$, we can employ the monotone convergence theorem to obtain 
      \[
    ...
    \begin{proof}
      ...
      \[
        \left|\frac{\G'}{\G}(s)-\log(s)\right| \le \frac{1}{2\pi}\int_{C}\frac{|\log\G(u)-g(u)|}{\eta^{2}}\,|du| \ll_{\e,\d} \frac{1}{s},
      \]
      where the last estimate follows because $\log\G(u)-g(u) = O_{\e,\d}\left(\frac{1}{u}\right)$ and $u \asymp_{\e,\d} s$.
    \end{proof}