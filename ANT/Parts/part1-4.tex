\chapter{Analytic Tools}
  A good selection of analytic topics that need some discussion before studying number theory are the following:
  \begin{itemize}
    \item Summation Formulas,
    \item Complex Integration Techniques,
  ...
  \section{The Fourier Transform}
    ...
    \begin{theorem}\label{thm:existance_of_Fourier_series}
      Suppose $f(\mathbf{x})$ is smooth and $1$-periodic in each component. Then the Fourier series of $f(\mathbf{x})$ converges uniformly everywhere to $f(\mathbf{x})$.
    \end{theorem}
    ...
    \begin{proof}
      ...
      \]
      Now $F_{P}(\mathbf{x})$ is smooth and $1$-periodic in each component because $F(\mathbf{x})$ is smooth and invariant under translation by $\L$. Therefore $F_{P}(\mathbf{x})$ admits a Fourier series converging uniformly everywhere to $F_{P}(\mathbf{x})$ by \cref{thm:existance_of_Fourier_series}. We compute the $\mathbf{t}$-th Fourier coefficient of $F_{P}(\mathbf{x})$ as follows:
      \begin{align*}
