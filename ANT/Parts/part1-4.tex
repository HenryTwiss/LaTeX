\chapter{Analytic Preliminaries}
  ...
  \section{The Fourier Transform}
    ...
    In the case of $1$-periodic functions of a single variable, we can do better:

    \begin{theorem}\label{thm:existance_of_Fourier_series_Dirichlet}
      Suppose $f(x)$ is a function on $\R$ that is $1$-periodic and satisfies the following conditions:
      \begin{enumerate}[label*=(\roman*)]
        \item $f(x)$ is absolutely integrable on $[0,1]$.
        \item $f(x)$ has a finite number of local extrema on $[0,1]$.
        \item $f(x)$ is continuous on $[0,1]$ except for at most a finite number of jump discontinuities.
      \end{enumerate}
      Then the Fourier series of $f(x)$ converges locally uniformly to $f(x)$ on every set where $f(x)$ is continuous. Moreover, at any jump discontinuity, the Fourier series of $f(x)$ converges to the average of the left-hand and right-hand limits of $f(x)$.
    \end{theorem}

    The properties (i)-(iii) in \cref{thm:existance_of_Fourier_series_Dirichlet} are known as the \textbf{Dirichlet conditions}\index{Dirichlet conditions}.

    \begin{remark}
      More generally, the Dirichlet conditions in \cref{thm:existance_of_Fourier_series_Dirichlet} can be weakened to only supposing that $f(x)$ is of bounded variation.
    \end{remark}
    ...
    Using the Dirichlet conditions, we can prove a slightly stronger form of the Poisson summation formula in the single variable setting:
    ...
    \begin{theorem}
      ...
      \]
      satisfies the Dirichlet conditions, where the $\ast$ indicates that $f(x+n)$ is meant to represent the average of the left-hand and right-hand limits at jump discontinuities. Then
      \[
    ...
    \end{theorem}
    \begin{proof}
      Observe that $F(x)$ is $1$-periodic. As $F(x)$ satisfies the Dirichlet conditions by assumption, it admits a Fourier series converging locally uniformly to $F(x)$ wherever $F(x)$ is continuous by \cref{thm:existance_of_Fourier_series_Dirichlet}. In fact, by the construction of $F(x)$ and that the Fourier series of $F(x)$ converges to the average of the left-hand and right-hand limits at jump discontinuities, the Fourier series of $F(x)$ converges uniformly to $F(x)$ everywhere (not just locally since $F(x)$ is $1$-periodic and there are at most finitely many jump discontinuities). Moreover, $F(x)$ is absolutely integrable on $[0,1]$ since it is smooth. We compute the $t$-th Fourier coefficient of $F(x)$ as follows:
      \begin{align*}
