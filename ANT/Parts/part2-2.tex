\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Estimating the Central Value}
    ...
    \begin{proof}
      ...
      \[
         \sum_{k \ll 1}\left|\sum_{2^{k-1}\sqrt{q(f)} \le n \le 2^{k+1}\sqrt{q(f)}}\frac{a_{f}(n)}{\sqrt{n}}\psi_{k}\left(\frac{n}{\sqrt{q(f)}}\right)\right| \ll \max_{X \ll \sqrt{q(f)}}\left(\left|\sum_{\frac{X}{2} \le n \le 2X}\frac{a_{f}(n)}{q(f)^{\frac{1}{4}}}\psi\left(\frac{n}{X}\right)\right|\right).
      \]
      By Abel's summation formula (see \cref{{append:Summation_Formulas}}), the inner sum is
      \[
