\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Lindel\"of Hypothesis and Convexity Bounds}
    ...
    \]
  \section{Estimate on the Critical Line}
    The Lindel\"of hypothesis is concerned with the growth of the $L$-function $L(s,f)$ along the critical line. Here we provide a way to obtain estimates for $L(s,f)$ at $s = \frac{1}{2}+it$ in terms of finite sums. First, let $\psi(y)$ is a bump function with compact support in $\left[\frac{1}{2},2\right]$. For example, we may take
    \[
    ...
    \]
    and we will suppress the dependence upon $t$ if $t = 0$. Our result is the following:
    ...
    \begin{theorem}\label{thm:critical_line_estimate}
      ...
      \[
        L\left(\frac{1}{2}+it,f\right) \ll \max_{X \ll \sqrt{\mf{q}\left(\frac{1}{2}+it,f\right)}}\left(\left|\frac{A_{\psi}(X,t,f)}{\mf{q}\left(\frac{1}{2}+it,f\right)^{\frac{1}{4}}}\right|\right)+\left|\frac{R\left(\frac{1}{2}+it,f\right)}{q(f)^{\frac{1}{4}}\g\left(\frac{1}{2}+it,f\right)}\right|,
      \]
      where $R(s,f)$ is zero if $\L(s,f)$ is entire, and otherwise
      \[
        R(s,f) = \Res_{u = 1-s}\frac{\L\left(s+u,f\right)}{u}+\Res_{u = -s}\frac{\L\left(s+u,f\right)}{u}.
      \]
    \end{theorem}
    \begin{proof}
    ...
    In view of \cref{thm:central_value_estimate}, it suffices to bound $A_{\psi}(X,t,f)$ for $X \ll \sqrt{\mf{q}\left(\frac{1}{2}+it,f\right)}$ to estimate $L\left(\frac{1}{2}+it,f\right)$. Sometimes, however, we are only concerned with the size of $L(s,f)$ at the central point. The value of $L(s,f)$ at the central point is called the \textbf{central value}\index{central value} of $L(s,f)$. Many important properties about $L(s,f)$ can be connected to its central value. Any argument used to estimate the central value of an $L$-function is called a \textbf{central value estimate}\index{central value estimate}. Taking $t = 0$ in \cref{thm:critical_line_estimate} gives a way to obtain central value estimates:

    \begin{theorem}\label{thm:central_value_estimate}
      Let $L(s,f)$ be an $L$-function and let $\psi(y)$ be a bump function with compact support in $\left[\frac{1}{2},2\right]$. Then
      \[
        L\left(\frac{1}{2},f\right) \ll \max_{X \ll \sqrt{q(f)}}\left(\left|\frac{A_{\psi}(X,f)}{q(f)^{\frac{1}{4}}}\right|\right)+\left|\frac{R\left(\frac{1}{2},f\right)}{q(f)^{\frac{1}{4}}\g\left(\frac{1}{2},f\right)}\right|,
      \]
      where $R(s,f)$ is zero if $\L(s,f)$ is entire, and otherwise
      \[
        R(s,f) = \Res_{u = 1-s}\frac{\L\left(s+u,f\right)}{u}+\Res_{u = -s}\frac{\L\left(s+u,f\right)}{u}.
    ...
    Indeed, \cref{thm:central_value_estimate} showed that it suffices to bound $A_{\psi}(X,f)$ for $X \ll \sqrt{q(f)}$ to estimate the central value of $L(s,f)$.
