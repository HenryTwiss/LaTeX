\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Analytic Data of \texorpdfstring{$L$}{L}-functions}
    ...
    \]
    with coefficients $a_{f}(n) \in \C$ attached the arithmetic object $f$. We call $L(s,f)$ an \textbf{$L$-function}\index{$L$-function} if it satisfies the following properties:
    ...
    \end{equation}
    is valid in the vertical strip $a \le \s \le b$ provided $s$ is distance $\e$ away from the poles of $\G(s)$. Clearly \cref{equ:modified_digamma_estimate} implies \cref{equ:digamma_estimate} when $|t| > 1$. It follows from the definition of $\g(s,f)$ that the estimates
  ...
  \section{Zero Density}
    ...
    \]
    In other words, $N(T,f)$ is the number of nontrivial zeros of $L(s,f)$ with ordinate in $[-T,T]$. We will prove the following asymptotic formula for $N(T,f)$:
