\chapter{The Geometry of Number Fields}
  ...
  \section{Minkowski Space}
    ...
    \begin{proof}
      ...
      \]
      by \cref{prop:base_change_quotient_determinant} since $\Norm_{K}(\mf{a})$ is the absolute value of the determinant of the base change matrix from $\k_{1},\ldots,\k_{n}$ to $\d\k_{1},\ldots,\d\k_{n}$ (from the definition of the norm). Similarly,
      \[
        |\det(M(\d\k_{1},\ldots,\d\k_{n}))| = |\Norm_{K}(\d)||\det(M(\k_{1},\ldots,\k_{n}))|,
      \]
      by \cref{prop:base_change_quotient_determinant} because $|\Norm_{K}(\d)|$ is the absolute value of the determinant of the base change matrix from $\k_{1},\ldots,\k_{n}$ to $\d\k_{1},\ldots,\d\k_{n}$ (since $\Norm_{K}(\d)$ is the determinant of $T_{\d}$). As $\Norm_{K}(\mf{f}) = \frac{\Norm_{K}(\mf{a})}{|\Norm_{K}(\d)|}$ (because $\Norm_{K}(\mf{a}) = \Norm_{K}(\d\mf{f}) = |\Norm_{K}(\d)|\Norm_{K}(\mf{a})$ using complete multiplicativity of the ideal norm and \cref{prop:ideal_and_field_norms_are_identical}), these two identities for $|\det(M(\d\k_{1},\ldots,\d\k_{n}))|$ together imply the claim.
    \end{proof}
