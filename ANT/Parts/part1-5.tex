\chapter{Exponential Sums}
  Throughout number theory are various sums which naturally appear. A large class of which are exponential sums as they play the role of discrete counterparts to continuous objects (there is a rich underpinning here). Without a sufficient understanding of these sums, they would cause a discrete obstruction to an analytic problem that we wish to solve.
  ...
  \section{Quadratic Gauss Sums}
    ...
    \]
    \cref{thm:quadratic_Gauss_sum_modulus} says that this value lies on the unit circle when $m$ is odd. Thus the question of the evaluation of quadratic Gauss sums reduces to determining what the epsilon factor is. This was completely resolved and the original proof is due to Gauss in 1808 (see \cite{Gauss1808summatio}) while more modern proofs use analytic techniques. The essential idea is to rewrite $g(m)$ in a form where \cref{thm:Poisson_summation_formula_Dirichlet-Jordan_test} can be applied. The precise statement is the following:
    ...
    \begin{proof}
      ...
      \]
      Necessarily $f(x)$ is absolutely integrable on $\R$ as it is compactly supported. It is also continuously differentiable with a finite number of jump discontinuities (they are at $x = 0$ and $x = m$). Then so is
      \[
        \asum_{n \in \Z}f(x+n)
      \]
      ...
      \]
      To compute the integral, as $g(1) = 1$ and $e^{-\frac{2\pi i}{4}} = -i$, taking $m = 1$ yields
      \[
