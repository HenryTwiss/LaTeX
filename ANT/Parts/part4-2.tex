\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dedekind Zeta Functions}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      because $e^{\mathbf{N}^{\vee}\l(\e)} = \mathbf{1}$ if $\e \in \mu(K)$ and we have interchanged the sum and integrals by the Fubini–Tonelli theorem. As $\boldsymbol{\e}^{\mathbf{N}} = |\Norm(\e)| = 1$ for all $\e \in \mc{O}_{K}^{\ast}$, we can use \cref{prop:unit_if_and_only_if_absolute_value_norm_1} to write our expression as
      \[
