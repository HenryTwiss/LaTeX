\chapter{The Theory of Automorphic and Maass Forms}
  ...
  \section{Automorphic and Maass Forms}
    Define $\e(\g,z)$ by
    \[
    ...
    \[
      (f|_{k}\s_{\mf{a}})(z) = \sum_{n \in \Z}a_{\mf{a}}(y,n)e^{2\pi inx},
    \]
    with \textbf{Fourier coefficients}\index{Fourier coefficients} $a_{\mf{a}}(y,n)$. If $\mf{a} = \infty$, we will drop this dependence and in this case $(f|_{k}\s_{\mf{a}}) = f$. Unlike holomorphic forms, $(f|_{k}\s_{\mf{a}})$ need not be smooth and so this Fourier series need not converge uniformly or even pointwise anywhere nevertheless to $(f|_{k}\s_{\mf{a}})$. Next up are the automorphic forms. We say that a function $f:\H \to \C$ is an \textbf{automorphic form}\index{automorphic form} of \textbf{weight}\index{weight} $k$, \textbf{eigenvalue}\index{eigenvalue} $\l$, \textbf{level}\index{level} $N$, and \textbf{character}\index{character} $\chi$ if following properties are satisfied:
    \begin{enumerate}[label*=(\roman*)]
      \item $f$ is smooth on $\H$.
      \item $(f|_{\e,k}\g)(z) = \chi(\g)f(z)$ for all $\g \in \G$.
      \item $f$ is an eigenfunction for $\D_{k}$ with eigenvalue $\l$.
    \end{enumerate}
    In property (iii), we will often let $s \in \C$ be such that $\l = s(1-s)$ and write $\l = \l(s)$ so that the eigenvalue can be determined by $s$. It turns turns out that Property (i) is implied by (iii). This is because $\D_{k}$ is an elliptic operator and any eigenfunction of an elliptic operator is automatically analytic and hence smooth (see \cite{evans2022partial} for a proof in the weight zero case and \cite{duke2002subconvexity} for notes on the general case). Let $\s_{\mf{a}}$ be a scaling matrix for the $\mf{a}$ cusp. As automorphic forms are automorphic functions, it follows by \cref{prop:Laplace_is_invariant} that $f|_{k}\s_{\mf{a}}$ is an automorphic form on $\s_{\mf{a}}^{-1}\G\s_{\mf{a}}\backslash\H$ of the same weight, eigenvalue, and character as $f$. As $f|_{k}\s_{\mf{a}}$ is also $1$-periodic and smooth (because $f$ is), \cref{thm:existance_of_Fourier_series} shows that $f$ admits a \textbf{Fourier series}\index{Fourier series} at the $\mf{a}$ cusp given by
    \[
      (f|_{k}\s_{\mf{a}})(z) = \sum_{n \in \Z}a_{\mf{a}}(y,s,n)e^{2\pi inx},
    \]
    with \textbf{Fourier coefficients}\index{Fourier coefficients} $a_{\mf{a}}(y,s,n)$. If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly and in this case $f|_{k}\s_{\mf{a}} = f$. Moreover, this Fourier series converges uniformly everywhere. The Fourier coefficients $a_{\mf{a}}(y,s,n)$ are mostly determined by $\D_{k}$. To see this, since $f|_{k}\s_{\mf{a}}$ is smooth we may differentiate the Fourier series of $f|_{k}\s_{\mf{a}}$ termwise. The fact that $f|_{k}\s_{\mf{a}}$ is an eigenfunction for $\D_{k}$ with eigenvalue $\l(s)$ gives the ODE
    \[
      (4\pi^{2}n^{2}y^{2}-2\pi nky)a_{\mf{a}}(y,s,n)-y^{2}a_{\mf{a}}''(y,s,n) = \l(s)a_{\mf{a}}(y,s,n).
    \]
    If $n \neq 0$, this is a Whittaker equation (see \cref{append:Whittaker_Functions}). To see this, first put the ODE in homogeneous form
    \[
      y^{2}a_{\mf{a}}''(y,s,n)-(4\pi^{2}n^{2}y^{2}-2\pi nky-\l(s))a_{\mf{a}}(y,s,n) = 0.
    \]
    Now make the change of variables $y \mapsto \frac{y}{4\pi|n|}$ to get
    \[
      y^{2}a_{\mf{a}}''(n,4\pi|n|y,s)-\left(\frac{y^{2}}{4}-\sgn(n)\frac{k}{2}y-\l(s)\right)a_{\mf{a}}(n,4\pi|n|y,s) = 0,
    \]
    where $\sgn(n) = \pm1$ if $n$ is positive or negative respectively. Diving by $y^{2}$ results in
    \[
      a_{\mf{a}}''(n,4\pi|n|y,s)+\left(\frac{1}{4}-\frac{\sgn(n)\frac{k}{2}}{y}-\frac{\l(s)}{y^{2}}\right)a_{\mf{a}}(n,4\pi|n|y,s) = 0.
    \]
    As $\l(s) = s(1-s) = \frac{1}{4}-(s-\frac{1}{2})^{2}$, the above equation becomes
    \[
      a_{\mf{a}}''(n,4\pi|n|y,s)+\left(\frac{1}{4}-\frac{\sgn(n)\frac{k}{2}}{y}-\frac{\frac{1}{4}-\left(s-\frac{1}{2}\right)^{2}}{y^{2}}\right)a_{\mf{a}}(n,4\pi|n|y,s) = 0.
    \]
    ...
    \[
      a_{\mf{a}}(y,s,n) = a_{\mf{a}}(s,n)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y),
    \]
    for some coefficients $a_{\mf{a}}(s,n)$. If $n = 0$ then the differential equation is a second order linear ODE which is
    \[
      -y^{2}a_{\mf{a}}''(0,y,s) = \l(s)a_{\mf{a}}(0,y,s).
    \]
    ...
    \]
    The coefficients $a_{\mf{a}}(s,n)$ and $a_{\mf{a}}^{\pm}(s)$ are the only part of the Fourier series that actually depend on the implicit congruence subgroup $\G$. Using these coefficients, $f$ admits \textbf{Fourier-Whittaker series}\index{Fourier-Whittaker series} at the $\mf{a}$ cusp given by
    \[
      (f|_{k}\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(s)y^{s}+a_{\mf{a}}^{-}(s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(s,n)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)e^{2\pi inx},
    \]
    with \textbf{Fourier-Whittaker coefficients}\index{Fourier-Whittaker coefficients} $a_{\mf{a}}^{\pm}(s)$ and $a_{\mf{a}}(s,n)$. If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly and in this case $f|_{k}\s_{\mf{a}} = f$. Moreover, this Fourier-Whittaker series converges uniformly everywhere (since the Fourier series does). Last up are the Maass forms. We say that a function $f:\H \to \C$ is a \textbf{Maass form}\index{Maass form} on $\GH$ of \textbf{weight}\index{weight} $k$, \textbf{eigenvalue}\index{eigenvalue} $\l$, \textbf{level}\index{level} $N$, and \textbf{character}\index{character} $\chi$ if the following properties are satisfied:
    \begin{enumerate}[label*=(\roman*)]
      \item $f$ is smooth on $\H$.
      \item $(f|_{\e,k}\g)(z) = \chi(\g)f(z)$ for all $\g \in \G$.
      \item $f$ is an eigenfunction for $\D_{k}$ with eigenvalue $\l$.
    ...
    \end{enumerate}
    We say $f$ is a (Maass) \textbf{cusp form}\index{cusp form} if the additional property is satisfied:
    \begin{enumerate}[label*=(\roman*)]
      \setcounter{enumi}{4}
    ...
    As a consequence of \cref{prop:raisin_lowering_presrves_Maass_forms}, it suffices to study Maass forms of weights $k = 0,1$ although imposing this additional restriction is usually unnecessary. Let $\s_{\mf{a}}$ be a scaling matrix for the $\mf{a}$ cusp. As Maass forms are automorphic forms, it follows by \cref{prop:Laplace_is_invariant} that $f|_{k}\s_{\mf{a}}$ is an automorphic form on $\s_{\mf{a}}^{-1}\G\s_{\mf{a}}\backslash\H$ of the same weight, eigenvalue, and character as $f$. In particular, we only need to verify the growth condition as $y \to \infty$ because $f|_{k}\s_{\mf{a}}$ is $1$-periodic. Moreover, as $f$ is an automorphic form, $f$ admits a \textbf{Fourier-Whittaker series}\index{Fourier-Whittaker series} at the $\mf{a}$ cusp given by
    \[
      (f|_{k}\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(s)y^{s}+a_{\mf{a}}^{-}(s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(s,n)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)e^{2\pi inx},
    \]
    with \textbf{Fourier-Whittaker coefficients}\index{Fourier-Whittaker coefficients} $a_{\mf{a}}^{\pm}(s)$ and $a_{\mf{a}}(s,n)$. Property (v) implies that $f$ is a cusp form if and only if $a_{\mf{a}}^{\pm}(s) = 0$ for every cusp $\mf{a}$. It is useful to specify the Whittaker function in the case of weight zero Maass forms. When $k = 0$, \cref{thm:Whittaker_special_cases} implies that the Fourier-Whittaker series of $f$ at the $\mf{a}$ cusp takes the form
    \[
      (f|_{k}\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(s)y^{s}+a_{\mf{a}}^{-}(s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(s,n)\sqrt{4|n|y}K_{s-\frac{1}{2}}(2\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \]
    where $\H_{Y}$ is the horizontal strip defined by $Y \le \Im(z) \le 2Y$. This integral converges since $\G_{\infty}\backslash\H_{Y}$ is compact. Substituting in the Fourier-Whittaker series of $f$ at the $\mf{a}$ cusp, this integral can be expressed as
    \[
      \int_{Y}^{2Y}\int_{0}^{1}\sum_{n,m \neq 0}a_{\mf{a}}(s,n)\conj{a_{\mf{a}}(s,m)}W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\conj{W_{\sgn(m)\frac{k}{2},s-\frac{1}{2}}(4\pi|m|y)}e^{2\pi i(n-m)x}\,\frac{dy}{y^{2}}.
    \]
    ...
    \[
      \sum_{n \neq 0}\int_{Y}^{2Y}|a_{\mf{a}}(s,n)|^{2}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}}.
    \]
    In particular, we see that this is a sum of nonnegative terms. Retaining only a single term in the sum, we have
    \[
      |a_{\mf{a}}(s,n)|^{2}\int_{Y}^{2Y}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}} \ll \int_{\G_{\infty}\backslash\H_{Y}}|(f|_{k}\s_{\mf{a}})(z)|^{2}\,d\mu.
    \]
    ...
    \[
      |a_{\mf{a}}(s,n)|^{2}\int_{Y}^{2Y}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}} \ll \frac{1}{Y}.
    \]
  ...
  \section{Poincar\'e and Eisenstein Series}
    ...
    \end{align*}
    where in the second line we have used the cocycle condition and in the second to last line we have made the change of variables $\g' \mapsto \g'\g^{-1}$. We collect this work as a theorem:
    ...
    For $m = 0$, we write $E_{k,\chi,\mf{a}}(z,s) = P_{0,k,\chi,\mf{a}}(z,s)$ and call $E_{k,\chi}(z)$ the (Maass) \textbf{Eisenstein series}\index{Eisenstein series} of weight $k$ and character $\chi$ on $\GH$ at the $\mf{a}$ cusp. It is defined by
    \[
      E_{k,\chi,\mf{a}}(z,s) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\Im(\s_{\mf{a}}^{-1}\g z)^{s}.
    \]
    If $k = 0$, $\chi$ is the trivial character, or $\mf{a} = \infty$, we will drop these dependencies accordingly. It turns out that $E_{k,\chi,\mf{a}}(z,s)$ is actually a Maass form. The only things left to verify is that $E_{k,\chi,\mf{a}}(z,s)$ is an eigenfunction for $\D_{k}$ and satisfies the growth condition. For the former property, first observe that
    \[
      \D_{k}(y^{s}) = \left(-y^{2}\left(\frac{\del^{2}}{\del x^{2}}+\frac{\del^{2}}{\del y^{2}}\right)+iky\frac{\del}{\del x}\right)(y^{s}) = \l(s)y^{s}.
    \]
    Therefore $\Im(z)^{s} = y^{s}$ is an eigenfunction for $\D_{k}$ with eigenvalue $\l(s)$. Since $\D_{k}$ is invariant, we have
    \[
    ...
    \]
    which shows $E_{k,\chi,\mf{a}}(z,s)$ is also an eigenfunction for $\D_{k}$ with eigenvalue $\l(s)$. It remains to verify the growth condition. Let $\s_{\mf{b}}$ be a scaling matrix for the cusp $\mf{b}$. Then the Bruhat decomposition for $\s_{\mf{a}}^{-1}\G\s_{\mf{b}}$ gives
    \[
      \e(\s_{\mf{b}},z)^{-k}E_{k,\chi,\mf{a}}(\s_{\mf{b}}z,s) \ll \Im(z)^{\s}\sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{1}{|cz+d|^{2\s}}.
    \]
    ...
    \]
    Notice that the first sum is bounded provided $\s > 1$. Moreover, the exact same argument as for holomorphic Eisenstein series shows that the second sum is too provided $\Im(z) \ge 1$. So for all $\Im(z) \ge 1$ and $\s > 1$, we have
    \[
      \e(\s_{\mf{b}},z)^{-k}P_{m,k,\chi,\mf{a}}(\s_{\mf{b}}z,s) \ll \Im(z)^{\s}.
    \]
    This verifies the growth condition. We collect this work as a theorem:
    ...
    \begin{proposition}\label{prop:Fourier_series_Poincare_automorphic}
      Let $m \ge 1$, $k \ge 0$, $\chi$ be Dirichlet character with conductor dividing the level, and $\mf{a}$ and $\mf{b}$ be cusps of $\GH$. For $\s > 1$, the Fourier series of $P_{m,k,\chi,\mf{a}}(z,s)$ on $\GH$ at the $\mf{b}$ cusp is given by
      \[
        (P_{m,k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) = \sum_{t \in \Z}\left(\d_{\mf{a},\mf{b}}\d_{m,t}y^{s}e^{-2\pi my}+i^{-k}y^{1-s}\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}}}}\frac{S_{\chi,\mf{a},\mf{b}}(m,t,c)}{c^{2s}}A(y,s,m,t,c)\right)e^{2\pi itx},
      \]
      where $A(s,y,m,t,c)$ is the integral transform given by
      \[
        A(y,s,m,t,c) = \int_{-\infty}^{\infty}\frac{e^{-2\pi im\left(\frac{1}{c^{2}y(x+i)}\right)}}{(1-ix)^{s+\frac{k}{2}}(1+ix)^{s-\frac{k}{2}}}e^{-2\pi itxy}\,dx.
      \]
    \end{proposition}
    \begin{proof}
      ...
      \[
        (P_{m,k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) = \d_{\mf{a},\mf{{b}}}y^{s}e^{2\pi imz}+y^{s}\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}}, d \in \Z \\ d \tmod{c} \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(d)\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}z+cd}\right)}}{(cz+d)^{k}|cz+d|^{2s-k}},
      \]
      ...
      \begin{align*}
        \sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}}, d \in \Z \\ d \tmod{c} \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(d)\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}z+cd}\right)}}{(cz+d)^{k}|cz+d|^{2s-k}} &= \sum_{(c,\ell,r)}\cchi(c\ell+r)\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}z+c^{2}\ell+cr}\right)}}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}} \\
        &= \sum_{(c,\ell,r)}\cchi(r)\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}z+c^{2}\ell+cr}\right)}}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}} \\
        &= \sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\sum_{\ell \in \Z}\cchi(r)\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}z+c^{2}\ell+cr}\right)}}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}} \\
        &= \sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(r)\sum_{\ell \in \Z}\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}z+c^{2}\ell+cr}\right)}}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}},
      \end{align*}
     where on the right-hand side it is understood that we are summing over all triples $(c,\ell,r)$ with the prescribed properties and the second line holds since $\chi$ has conductor diving the level and $d \in \mc{D}_{\mf{a},\mf{b}}(c)$ is determined modulo $c$. Now let
      \[
        I_{c,r}(z,s) = \sum_{\ell \in \Z}\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}z+c^{2}\ell+cr}\right)}}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}}.
      \]
      We apply the Poisson summation formula to $I_{c,r}(z,s)$. By the identity theorem it suffices to apply the Poisson summation formula for $z = iy$ with $y > 0$. Accordingly, let $f(x)$ be given by
      \[
        f(x) = \frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}x+cr+ic^{2}y}\right)}}{(cx+r+icy)^{k}|cx+r+icy|^{2s-k}}.
      \]
      Then $f(x)$ is absolutely integrable on $\R$ because it exhibits polynomial decay of order $2\s > 1$ as $\s > 1$. We compute its Fourier transform
      \[
        (\mc{F}f)(t) = \int_{-\infty}^{\infty}f(x)e^{-2\pi itx}\,dx = \int_{-\infty}^{\infty}\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}x+cr+ic^{2}y}\right)}}{(cx+r+icy)^{k}|cx+r+icy|^{2s-k}}e^{-2\pi itx}\,dx.
      \]
      We can rewrite the latter integral as
      \[
        \int_{-\infty}^{\infty}\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}x+cr+ic^{2}y}\right)}}{(cx+r+icy)^{s+\frac{k}{2}}(cx+r-icy)^{s-\frac{k}{2}}}e^{-2\pi itx}\,dx.
      \]
      The change of variables $x \mapsto x-\frac{r}{c}$ yields
      \[
        e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}\int_{-\infty}^{\infty}\frac{e^{-2\pi im\left(\frac{1}{c^{2}x+ic^{2}y}\right)}}{(cx+icy)^{s+\frac{k}{2}}(cx-icy)^{s-\frac{k}{2}}}e^{-2\pi itx}\,dx.
      \]
      Making another change of variables $x \mapsto xy$ gives
      \[
        y^{1-2s}\frac{e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}}{c^{2s}}\int_{-\infty}^{\infty}\frac{e^{-2\pi im\left(\frac{1}{c^{2}y(x+i)}\right)}}{(x+i)^{s+\frac{k}{2}}(x-i)^{s-\frac{k}{2}}}e^{-2\pi itxy}\,dx,
      \]
      which we can further rewrite as
      \[
        y^{1-2s}i^{-k}\frac{e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}}{c^{2s}}\int_{-\infty}^{\infty}\frac{e^{-2\pi im\left(\frac{1}{c^{2}y(x+i)}\right)}}{(1-ix)^{s+\frac{k}{2}}(1+ix)^{s-\frac{k}{2}}}e^{-2\pi itxy}\,dx.
      \]
      So in total we obtain
      \[
        (\mc{F}f)(t) = y^{1-2s}i^{-k}\frac{e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}}{c^{2s}}A(y,s,m,t,c).
      \]
      By the Poisson summation formula and the identity theorem, we have
      \[
        I_{c,r}(z,s) = \sum_{t \in \Z}\left(y^{1-2s}i^{-k}\frac{e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}}{c^{2s}}A(y,s,m,t,c)\right)e^{2\pi itx}.
      \]
      Plugging this back into the Poincar\'e series gives a form of the Fourier series:
      \begin{align*}
        (P_{m,k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) &= \d_{\mf{a},\mf{b}}y^{s}e^{2\pi imz}+\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(r)y^{s}\sum_{t \in \Z}\left(y^{1-2s}i^{-k}\frac{e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}}{c^{2s}}A(y,s,m,t,c)\right)e^{2\pi itx} \\
        &= \sum_{t \in \Z}\left(\d_{\mf{a},\mf{b}}\d_{m,t}y^{s}e^{-2\pi my}+i^{-k}y^{1-s}\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(r)\frac{e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}}{c^{2s}}A(y,s,m,t,c)\right)e^{2\pi itx} \\
        &= \sum_{t \in \Z}\left(\d_{\mf{a},\mf{b}}\d_{m,t}y^{s}e^{-2\pi my}+i^{-k}y^{1-s}\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}}}}\frac{A(y,s,m,t,c)}{c^{2s}}\sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(r)e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}\right)e^{2\pi itx}.
      \end{align*}
      We will simplify the innermost sum. Since $a$ is the inverse for $r$ modulo $c$, the innermost sum above becomes
      \[
        \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(r)e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}} = \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(\conj{a})e^{2\pi im\frac{a}{c}+2\pi it\frac{\conj{a}}{c}} = \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\chi(a)e^{\frac{2\pi i(am+\conj{a}t)}{c}} = S_{\chi,\mf{a},\mf{b}}(m,t,c).
      \]
    ...
    In a similar manner, we can obtain the Fourier-Whittaker series of the Eisenstein series too:

    \begin{proposition}\label{prop:Fourier_Whittaker_series_Eisenstein_Maass}
      Let $k \ge 0$, $\chi$ be Dirichlet character with conductor dividing the level, and $\mf{a}$ and $\mf{b}$ be cusps of $\GH$. For $\s > 1$, the Fourier-Whittaker series of $E_{k,\chi,\mf{a}}(z,s)$ on $\GH$ at the $\mf{b}$ cusp is given by
      \[
        (E_{k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) = \d_{\mf{a},\mf{{b}}}y^{s}+\tau_{\mf{a},\mf{b}}(s)y^{1-s}+\sum_{t \neq 0}\tau_{\mf{a},\mf{b}}(s,t)W_{\sgn(t)\frac{k}{2},\frac{1}{2}-s}(4\pi|t|y)e^{2\pi itx},
      \]
      where
      \[
        \tau_{\mf{a},\mf{b}}(s) = \frac{4\pi i^{-k}\G(2s-1)}{2^{2s}\G\left(s+\frac{k}{2}\right)\G\left(s-\frac{k}{2}\right)}\sum_{c \in \mc{C}_{\mf{a},\mf{b}}}\frac{S_{\chi,\mf{a},\mf{b}}(0,0,c)}{c^{2s}},
      \]
      and
      \[
        \tau_{\mf{a},\mf{b}}(s,t) = \frac{\pi^{s}i^{-k}|t|^{s-1}}{\G\left(s+\sgn(t)\frac{k}{2}\right)}\sum_{c \in \mc{C}_{\mf{a},\mf{b}}}\frac{S_{\chi,\mf{a},\mf{b}}(0,t,c)}{c^{2s}}.
      \]
    \end{proposition}
    \begin{proof}
      From the cocycle condition, the Bruhat decomposition for $\s_{\mf{a}}^{-1}\G\s_{\mf{b}}$, and \cref{rem:Bruhat_modulo_infity_exact}, we have
      \[
        (E_{k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) = \d_{\mf{a},\mf{{b}}}y^{s}+y^{s}\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}}, d \in \Z \\ d \tmod{c} \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(d)\frac{1}{(cz+d)^{k}|cz+d|^{2s-k}},
      \]
      where $a$ and $b$ are chosen such that $\det\left(\begin{psmallmatrix} a & b \\ c & d \end{psmallmatrix}\right) = 1$ and we have used the fact that
      \[
        \frac{a}{c}-\frac{1}{c^{2}z+cd} = \frac{az+b}{cz+d}.
      ...
      Summing over all pairs $(c,d)$ with $c \in \mc{C}_{\mf{a},\mf{b}}$, $d \in \Z$, and $d \in \mc{D}_{\mf{a},\mf{b}}(c)$ is the same as summing over all triples $(c,\ell,r)$ with $c \in \mc{C}_{\mf{a},\mf{b}}$, $\ell \in \Z$, and $r$ taken modulo $c$ with $r \in \mc{D}_{\mf{a},\mf{b}}(c)$. Indeed, this is seen by writing $d = c\ell+r$. Moreover, since $ad-bc = 1$ we have $a(c\ell+r)-bc = 1$ which further implies that $ar \equiv 1 \tmod{c}$. So we may take $a$ to be the inverse for $r$ modulo $c$. Then
      \begin{align*}
        \sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}}, d \in \Z \\ d \tmod{c} \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(d)\frac{1}{(cz+d)^{k}|cz+d|^{2s-k}} &= \sum_{(c,\ell,r)}\cchi(c\ell+r)\frac{1}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}} \\
        &= \sum_{(c,\ell,r)}\cchi(r)\frac{1}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}} \\
        &= \sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\sum_{\ell \in \Z}\cchi(r)\frac{1}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}} \\
        &= \sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(r)\sum_{\ell \in \Z}\frac{1}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}},
      \end{align*}
     where on the right-hand side it is understood that we are summing over all triples $(c,\ell,r)$ with the prescribed properties and the second line holds since $\chi$ has conductor diving the level and $d \in \mc{D}_{\mf{a},\mf{b}}(c)$ is determined modulo $c$. Now let
      \[
        I_{c,r}(z,s) = \sum_{\ell \in \Z}\frac{1}{(cz+c\ell+r)^{k}|cz+c\ell+r|^{2s-k}}.
      \]
      We apply the Poisson summation formula to $I_{c,r}(z,s)$. By the identity theorem it suffices to apply the Poisson summation formula for $z = iy$ with $y > 0$. Accordingly, let $f(x)$ be given by
      \[
        f(x) = \frac{1}{(cx+r+icy)^{k}|cx+r+icy|^{2s-k}}.
      \]
      Then $f(x)$ is absolutely integrable on $\R$ because it exhibits polynomial decay of order $2\s > 1$ as $\s > 1$. We compute its Fourier transform
      \[
        (\mc{F}f)(t) = \int_{-\infty}^{\infty}f(x)e^{-2\pi itx}\,dx = \int_{-\infty}^{\infty}\frac{e^{-2\pi itx}}{(cx+r+icy)^{k}|cx+r+icy|^{2s-k}}\,dx.
      \]
      We can rewrite the latter integral as
      \[
        \int_{-\infty}^{\infty}\frac{e^{-2\pi itx}}{(cx+r+icy)^{s+\frac{k}{2}}(cx+r-icy)^{s-\frac{k}{2}}}\,dx.
      \]
      The change of variables $x \mapsto x-\frac{r}{c}$ gives
      \[
        e^{2\pi it\frac{r}{c}}\int_{-\infty}^{\infty}\frac{e^{-2\pi itx}}{(cx+icy)^{s+\frac{k}{2}}(cx-icy)^{s-\frac{k}{2}}}\,dx.
      \]
      Making another change of variables $x \mapsto xy$ results in
      \[
        y^{1-2s}\frac{e^{2\pi it\frac{r}{c}}}{c^{2s}}\int_{-\infty}^{\infty}\frac{e^{-2\pi itxy}}{(x+i)^{s+\frac{k}{2}}(x-i)^{s-\frac{k}{2}}}\,dx,
      \]
      which we can further rewrite as
      \[
        y^{1-2s}i^{-k}\frac{e^{2\pi it\frac{r}{c}}}{c^{2s}}\int_{-\infty}^{\infty}\frac{e^{-2\pi itxy}}{(1-ix)^{s+\frac{k}{2}}(1+ix)^{s-\frac{k}{2}}}\,dx.
      \]
      Appealing to \cref{append:Special_Integrals}, we recognize this last integral as
      \[
        \int_{-\infty}^{\infty}\frac{e^{-2\pi itxy}}{(1-ix)^{s+\frac{k}{2}}(1+ix)^{s-\frac{k}{2}}}\,dx = \begin{cases} \frac{4\pi\G(2s-1)}{2^{2s}\G\left(s+\frac{k}{2}\right)\G\left(s-\frac{k}{2}\right)} & \text{if $t = 0$,} \\ \frac{\pi(\pi|t|y)^{s-1}}{\G\left(s+\sgn(t)\frac{k}{2}\right)}W_{\sgn(t)\frac{k}{2},\frac{1}{2}-s}(4\pi|t|y) & \text{if $t \neq 0$.} \end{cases}.
      \]
      So in total we obtain
      \[
        (\mc{F}f)(t) = \begin{cases} y^{1-2s}\frac{4\pi i^{-k}\G(2s-1)}{2^{2s}\G\left(s+\frac{k}{2}\right)\G\left(s-\frac{k}{2}\right)}\frac{1}{c^{2s}} & \text{if $t = 0$,} \\ y^{-s}\frac{\pi^{s}i^{-k}|t|^{s-1}}{\G\left(s+\sgn(t)\frac{k}{2}\right)}\frac{e^{2\pi it\frac{r}{c}}}{c^{2s}}W_{\sgn(t)\frac{k}{2},\frac{1}{2}-s}(4\pi|t|y) & \text{if $t \neq 0$.} \end{cases}
      \]
      By the Poisson summation formula and the identity theorem, we have
      \begin{align*}
        I_{c,r}(z,s) &= y^{1-2s}\frac{4\pi i^{-k}\G(2s-1)}{2^{2s}\G\left(s+\frac{k}{2}\right)\G\left(s-\frac{k}{2}\right)}\frac{1}{c^{2s}} \\
        &+ \sum_{t \neq 0}\left(y^{-s}\frac{\pi^{s}i^{-k}|t|^{s-1}}{\G\left(s+\sgn(t)\frac{k}{2}\right)}\frac{e^{2\pi it\frac{r}{c}}}{c^{2s}}W_{\sgn(t)\frac{k}{2},\frac{1}{2}-s}(4\pi|t|y)\right)e^{2\pi itx}.
      \end{align*}
      Plugging this back into the Eisenstein series gives a form of the Fourier-Whittaker series:
    \begin{align*}
      (E_{k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) &= \d_{\mf{a},\mf{{b}}}y^{s}+y^{s}\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(r)y^{1-2s}\frac{4\pi i^{-k}\G(2s-1)}{2^{2s}\G\left(s+\frac{k}{2}\right)\G\left(s-\frac{k}{2}\right)}\frac{1}{c^{2s}} \\
      &+y^{s}\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\cchi(r)\sum_{t \neq 0}\left(y^{-s}\frac{\pi^{s}i^{-k}|t|^{s-1}}{\G\left(s+\sgn(t)\frac{k}{2}\right)}\frac{e^{2\pi it\frac{r}{c}}}{c^{2s}}W_{\sgn(t)\frac{k}{2},\frac{1}{2}-s}(4\pi|t|y)\right)e^{2\pi itx} \\
      &= \d_{\mf{a},\mf{{b}}}y^{s}+\frac{4\pi i^{-k}\G(2s-1)}{2^{2s}\G\left(s+\frac{k}{2}\right)\G\left(s-\frac{k}{2}\right)}\left(\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\frac{\cchi(r)}{c^{2s}}\right)y^{1-s} \\
      &+ \sum_{t \neq 0}\frac{\pi^{s}i^{-k}|t|^{s-1}}{\G\left(s+\sgn(t)\frac{k}{2}\right)}\left(\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}} \\ r \in \mc{D}_{\mf{a},\mf{b}}(c)}}\frac{\cchi(r)}{c^{2s}}e^{2\pi it\frac{r}{c}}\right)W_{\sgn(t)\frac{k}{2},\frac{1}{2}-s}(4\pi|t|y)e^{2\pi itx} \\
      &= \d_{\mf{a},\mf{{b}}}y^{s}+\frac{4\pi i^{-k}\G(2s-1)}{2^{2s}\G\left(s+\frac{k}{2}\right)\G\left(s-\frac{k}{2}\right)}\left(\sum_{c \in \mc{C}_{\mf{a},\mf{b}}}\frac{1}{c^{2s}}\sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(r)\right)y^{1-s} \\
      &+ \sum_{t \neq 0}\frac{\pi^{s}i^{-k}|t|^{s-1}}{\G\left(s+\sgn(t)\frac{k}{2}\right)}\left(\sum_{c \in \mc{C}_{\mf{a},\mf{b}}}\frac{1}{c^{2s}}\sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(r)e^{2\pi it\frac{r}{c}}\right)W_{\sgn(t)\frac{k}{2},\frac{1}{2}-s}(4\pi|t|y)e^{2\pi itx}.
    \end{align*}
    We will simplify the innermost sums. Since $a$ is the inverse for $r$ modulo $c$, the innermost sum in the third term above becomes
    \[
      \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(r)e^{2\pi it\frac{r}{c}} = \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(\conj{a})e^{2\pi it\frac{\conj{a}}{c}} = \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\chi(a)e^{\frac{2\pi i(am+\conj{a}t)}{c}} = S_{\chi,\mf{a},\mf{b}}(0,t,c).
    \]
    Similarly, we have
    \[
      \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(r) = \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\cchi(\conj{a}) = \sum_{r \in \mc{D}_{\mf{a},\mf{b}}(c)}\chi(a) = S_{\chi,\mf{a},\mf{b}}(0,0,c),
    \]
    for the second term. We then obtain our desired Fourier-Whittaker series:
    \[
      (E_{k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) = \d_{\mf{a},\mf{{b}}}y^{s}+\tau_{\mf{a},\mf{b}}(s)y^{1-s}+\sum_{t \neq 0}\tau_{\mf{a},\mf{b}}(s,t)W_{\sgn(t)\frac{k}{2},\frac{1}{2}-s}(4\pi|t|y)e^{2\pi itx}.
    \]
    \end{proof}
  \section{Inner Product Spaces of Automorphic Functions}
    Let $\mc{A}_{k}(\G,\chi)$ denote the complex vector space of all weight $k$ automorphic functions with character $\chi$ on $\GH$ and let $\mc{A}_{k,\l}(\G,\chi)$, $\mc{M}_{k,\l}(\G,\chi)$, and $\mc{C}_{k,\l}(\G,\chi)$ denote the associated subspaces of automorphic functions, Maass forms, and cusp forms of eigenvalue $\l$ respectively. If $k = 0$ or $\chi$ is the trivial character, we will suppress these dependencies. Note that if $\G_{1}$ and $\G_{2}$ are two congruence subgroups such that $\G_{1} \subseteq \G_{2}$ then we have the inclusion
    \[
    ...
    For $m = 0$, we write $E_{k,\chi,\mf{a}}(z,\psi) = P_{0,k,\chi,\mf{a}}(z,\psi)$ and call $E_{k,\chi,\mf{a}}(z,\psi)$ the (automorphic) \textbf{Eisenstein series}\index{Eisenstein series} of weight $k$ and character $\chi$ on $\GH$ at the $\mf{a}$ cusp and with respect to $\psi(y)$. It is defined by
    \[
    ...
    \[
       \frac{1}{V_{\G}}\int_{0}^{\infty}\int_{0}^{1}\left(\sum_{n \in \Z}a_{\mf{a}}(y,n)e^{2\pi inx}\right)\conj{\psi(y)}\,\frac{dx\,dy}{y^{2}}.
    \]
  ...
  \section{Spectral Theory of the Laplace Operator}
    ...
    \begin{theorem}\label{thm:meromorphic_continuation_of_Eisenstein_series}
      ...
      \[
        (E_{k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) = \d_{\mf{a},\mf{b}}y^{s}+\tau_{\mf{a},\mf{b}}(s)y^{1-s}+\sum_{n \neq 0}\tau_{\mf{a},\mf{b}}(s,n)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)e^{2\pi inx},
      \]
      where $\tau_{\mf{a},\mf{b}}(s)$ and $\tau_{\mf{a},\mf{b}}(s,n)$ are meromorphic functions.
    \end{theorem}

    Note that we have the Fourier-Whittaker series in \cref{thm:meromorphic_continuation_of_Eisenstein_series} already from \cref{prop:Fourier_Whittaker_series_Eisenstein_Maass}. Therefore \cref{thm:meromorphic_continuation_of_Eisenstein_series} really amounts to the fact that $\tau_{\mf{a},\mf{b}}(s)$ and $\tau_{\mf{a},\mf{b}}(s,n)$ admit meromorphic continuation to $\C$. The Eisenstein series $E_{k,\chi,\mf{a}}(z,s)$ also satisfy a functional equation. To state it we need some notation. Fix an ordering of the cusps $\mf{a}$ of $\GH$ and define
    \[
    ...
    As $\Phi(s)$ is symmetric by \cref{thm:functional_equation_of_Eisenstein_series}, if $\mf{a} = \infty$ or $\mf{b} = \infty$, we will suppress these dependencies for $\tau_{\mf{a},\mf{b}}(s)$ and $\tau_{\mf{a},\mf{b}}(s,n)$. Understanding the poles of $\tau_{\mf{a},\mf{b}}(s)$ are also important for understanding the poles of the Eisenstein series $E_{k,\chi,\mf{a}}(z,s)$ (see \cite{iwaniec2002spectral} for a proof in the weight zero case and \cite{duke2002subconvexity} for notes on the general case):
  ...
  \section{Double Coset Operators}
    ...
    \end{proof}
  \section{Diamond and Hecke Operators}
    ...
    \[
      (Xf)(z) = f(-\conj{z}).
    \]
    As $-\conj{z} = -x+iy$, $X$ acts by reflection with respect to $x$. We then define the parity \textbf{Hecke operator}\index{Hecke operator} $T_{-1}$ on $\mc{C}_{k,\nu}(\G_{1}(N))$ to be the linear operator given by
    \[
    ...
    Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$. As $T_{-1}$ is an involution, the only possible eigenvalues are $\pm 1$. Accordingly, we say that $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ is \textbf{even}\index{even} if $T_{-1}f = f$ and is \textbf{odd}\index{odd} if $T_{-1}f = -f$. Then by \cref{prop:parity_operator_properties}, we have
    \[
    ...
    \begin{proposition*}[Hecke relations, Maass]
  ...
  \section{Atkin-Lehner Theory}
    ...
    \]
    with $\det(W_{N}) = N$. We define the \textbf{Atkin-Lehner operator}\index{Atkin-Lehner operator} $\w_{N}$ on $\mc{C}_{k,\nu}(\G_{1}(N))$ to be the linear operator given by
    \[
  ...
  \section{The Ramanujan-Petersson Conjecture}
    ...
    \]
    We call this the $p$-th \textbf{Hecke polynomial}\index{Hecke polynomial} of $f$. We call the roots $\a_{1}(p)$ and $\a_{2}(p)$ the $p$-th \textbf{Hecke roots}\index{Hecke roots} of $f$. Then
    \[
    ...
    \begin{conjecture*}[Ramanujan-Petersson conjecture, Maass]
      Let $f \in \mc{C}_{k,\nu}(N,\chi)$ be a primitive Hecke-Maass eigenform with Hecke eigenvalues $\l_{f}(m)$ and let $\a_{1}(p)$ and $\a_{2}(p)$ be the $p$-th Hecke roots of $f$. Then for all primes $p$, we have
      \[
