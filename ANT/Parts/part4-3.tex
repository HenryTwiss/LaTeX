\chapter{The Theory of Automorphic and Maass Forms}
  ...
  \section{Poincar\'e and Eisenstein Series}
    ...
    \begin{proposition}\label{prop:Fourier_series_Poincare_automorphic}
      ...
      \[
        (P_{m,k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) = \sum_{t \in \Z}\left(\d_{\mf{a},\mf{b}}\d_{m,t}y^{s}e^{-2\pi my}+y^{1-s}\sum_{\substack{c \in \mc{C}_{\mf{a},\mf{b}}}}\frac{i^{-k}}{c^{2s}}A(y,s,m,t,c)S_{\chi,\mf{a},\mf{b}}(m,t,c)\right)e^{2\pi itz},
      \]
    ...
    \begin{proposition}\label{prop:Fourier_series_Eisenstein_Maass}
      ...
      \[
        (E_{k,\chi,\mf{a}}|\s_{\mf{b}})(z,s) = \todo{xxx}.
      \]
    \end{proposition}
    \begin{proof}
      From the cocycle condition, the Bruhat decomposition for $\s_{\mf{a}}^{-1}\G\s_{\mf{b}}$, and \cref{rem:Bruhat_modulo_infity_exact}, we have
     ...
     where on the right-hand side it is understood that we are summing over all triples $(c,\ell,r)$ with the prescribed properties and the second line holds since $\chi$ has conductor diving the level and $d \in \mc{D}_{\mf{a},\mf{b}}(c)$ is determined modulo $c$. Now let
      ...
      \[
        e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}\int_{-\infty}^{\infty}\frac{e^{-2\pi itx}}{(cx+icy)^{s+\frac{k}{2}}(cx-icy)^{s-\frac{k}{2}}}\,dx.
      \]
      Making another change of variables $x \to xy$ results in
      \[
        \frac{y^{1-2s}}{c^{2s}}e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}\int_{-\infty}^{\infty}\frac{e^{-2\pi itxy}}{(x+i)^{s+\frac{k}{2}}(x-i)^{s-\frac{k}{2}}}\,dx,
      \]
      which we can further rewrite as
      \[
        y^{1-2s}\frac{i^{-k}}{c^{2s}}e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}\int_{-\infty}^{\infty}\frac{e^{-2\pi itxy}}{(1-ix)^{s+\frac{k}{2}}(1+ix)^{s-\frac{k}{2}}}\,dx.
      \]
      Appealing to \cref{append:Special_Integrals}, we recognize this last integral as
      \[
        \int_{-\infty}^{\infty}\frac{e^{-2\pi itxy}}{(1-ix)^{s+\frac{k}{2}}(1+ix)^{s-\frac{k}{2}}}\,dx = \todo{xxx}.
      \]
      \todo{xxx}
    \end{proof}
