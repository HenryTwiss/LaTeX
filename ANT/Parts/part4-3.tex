\chapter{The Theory of Automorphic and Maass Forms}
  ...
  \section{Automorphic and Maass Forms}
    ...
    \[
      (f|_{k}\s_{\mf{a}})(z) = \sum_{n \in \Z}\rho_{\mf{a}}(n,y,s)e^{2\pi inx},
    \]
    with \textbf{Fourier coefficients}\index{Fourier coefficients} $\rho_{\mf{a}}(n,y,s)$. If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly and in this case $f|_{k}\s_{\mf{a}} = f$. As $f$ (and hence $f|_{k}\s_{\mf{a}}$ too) is smooth, it converges uniformly to its Fourier series everywhere. The Fourier coefficients $\rho_{\mf{a}}(n,y,s)$ are mostly determined by $\D_{k}$. To see this, since $f|_{k}\s_{\mf{a}}$ is smooth we may differentiate the Fourier series of $f|_{k}\s_{\mf{a}}$ termwise. The fact that $f|_{k}\s_{\mf{a}}$ is an eigenfunction for $\D_{k}$ with eigenvalue $\l(s)$ gives the ODE
    \[
      (4\pi^{2}n^{2}y^{2}-2\pi nky)\rho_{\mf{a}}(n,y,s)-y^{2}\rho_{\mf{a},yy}(n,y,s) = \l(s)\rho_{\mf{a}}(n,y,s).
    \]
    If $n \neq 0$, this is a Whittaker equation (see \cref{append:Whittaker_Functions}). To see this, first put the ODE in homogeneous form
    \[
      y^{2}\rho_{\mf{a},yy}(n,y,s)-(4\pi^{2}n^{2}y^{2}-2\pi nky-\l(s))\rho_{\mf{a}}(n,y,s) = 0.
    \]
    Now make the change of variables $y \mapsto \frac{y}{4\pi|n|}$ to get
    \[
      y^{2}\rho_{\mf{a},yy}(n,4\pi|n|y,s)-\left(\frac{y^{2}}{4}-\sgn(n)\frac{k}{2}y-\l(s)\right)\rho_{\mf{a}}(n,4\pi|n|y,s) = 0,
    \]
    where $\sgn(n) = \pm1$ if $n$ is positive or negative respectively. Diving by $y^{2}$ results in
    \[
      \rho_{\mf{a},yy}(n,4\pi|n|y,s)+\left(\frac{1}{4}-\frac{\sgn(n)\frac{k}{2}}{y}-\frac{\l(s)}{y^{2}}\right)\rho_{\mf{a}}(n,4\pi|n|y,s) = 0.
    \]
    As $\l(s) = s(1-s) = \frac{1}{4}-(s-\frac{1}{2})^{2}$, the above equation becomes
    \[
      \rho_{\mf{a},yy}(n,4\pi|n|y,s)+\left(\frac{1}{4}-\frac{\sgn(n)\frac{k}{2}}{y}-\frac{\frac{1}{4}-\left(s-\frac{1}{2}\right)^{2}}{y^{2}}\right)\rho_{\mf{a}}(n,4\pi|n|y,s) = 0.
    \]
    ...
    \[
      \rho_{\mf{a}}(n,y,s) = \rho_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y),
    \]
    for some coefficients $\rho_{\mf{a}}(n,s)$. If $n = 0$ then the differential equation is a second order linear ODE which is
    \[
      -y^{2}\rho_{\mf{a},yy}(0,y,s) = \l(s)\rho_{\mf{a}}(0,y,s).
    \]
    This is a Cauchy-Euler equation, and since $s$ and $1-s$ are the two roots of $z^{2}-z+\l$, the general solution is
    \[
      \rho_{\mf{a}}(0,y,s) = \rho_{\mf{a}}^{+}(s)y^{s}+\rho_{\mf{a}}^{-}(s)y^{1-s},
    \]
    The coefficients $\rho_{\mf{a}}(n,s)$ and $\rho_{\mf{a}}^{\pm}(s)$ are the only part of the Fourier series that actually depend on the implicit congruence subgroup $\G$. Using these coefficients, $f$ admits \textbf{Fourier-Whittaker series}\index{Fourier-Whittaker series} at the $\mf{a}$ cusp given by
    \[
      (f|_{k}\s_{\mf{a}})(z) = \rho_{\mf{a}}^{+}(s)y^{s}+\rho_{\mf{a}}^{-}(s)y^{1-s}+\sum_{n \neq 0}\rho_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)e^{2\pi inx},
    \]
    with \textbf{Fourier-Whittaker coefficients}\index{Fourier-Whittaker coefficients} $\rho_{\mf{a}}^{\pm}(s)$ and $\rho_{\mf{a}}(n,s)$. If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly and in this case $f|_{k}\s_{\mf{a}} = f$. As $f$ (and hence $f|_{k}\s_{\mf{a}}$ too) is smooth, it converges uniformly to its Fourier-Whittaker series everywhere. Last up are the Maass forms. We say that a function $f:\H \to \C$ is a \textbf{Maass form}\index{Maass form} on $\GH$ of \textbf{weight}\index{weight} $k$, \textbf{eigenvalue}\index{eigenvalue} $\l$, \textbf{level}\index{level} $N$, and \textbf{character}\index{character} $\chi$ if the following properties are satisfied:
    \begin{enumerate}[label*=(\roman*)]
      \item $f$ is smooth on $\H$.
      \item $(f|_{\e,k}\g)(z) = \chi(\g)f(z)$ for all $\g \in \G$.
      \item $f$ is an eigenfunction for $\D_{k}$ with eigenvalue $\l$.
    ...
    \[
      (f|_{k}\s_{\mf{a}})(z) = \rho_{\mf{a}}^{+}(s)y^{s}+\rho_{\mf{a}}^{-}(s)y^{1-s}+\sum_{n \neq 0}\rho_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)e^{2\pi inx},
    \]
    with \textbf{Fourier-Whittaker coefficients}\index{Fourier-Whittaker coefficients} $\rho_{\mf{a}}^{\pm}(s)$ and $\rho_{\mf{a}}(n,s)$. As $f$ (and hence $f|_{k}\s_{\mf{a}}$ too) is smooth, it converges uniformly to its Fourier-Whittaker series everywhere. Moreover, property (v) implies that $f$ is a cusp form if and only if $\rho_{\mf{a}}^{\pm}(s) = 0$ for every cusp $\mf{a}$. It is useful to specify the Whittaker function in the case of weight zero Maass forms. When $k = 0$, \cref{thm:Whittaker_special_cases} implies that the Fourier-Whittaker series of $f$ at the $\mf{a}$ cusp takes the form
    \[
      (f|_{k}\s_{\mf{a}})(z) = \rho_{\mf{a}}^{+}(s)y^{s}+\rho_{\mf{a}}^{-}(s)y^{1-s}+\sum_{n \neq 0}\rho_{\mf{a}}(n,s)\sqrt{4|n|y}K_{s-\frac{1}{2}}(2\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \[
      \int_{Y}^{2Y}\int_{0}^{1}\sum_{n,m \neq 0}\rho_{\mf{a}}(n,s)\conj{\rho_{\mf{a}}(m,s)}W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\conj{W_{\sgn(m)\frac{k}{2},s-\frac{1}{2}}(4\pi|m|y)}e^{2\pi i(n-m)x}\,\frac{dy}{y^{2}}.
    \]
    ...
    \[
      \sum_{n \neq 0}\int_{Y}^{2Y}|\rho_{\mf{a}}(n,s)|^{2}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}}.
    \]
    In particular, we see that this is a sum of nonnegative terms. Retaining only a single term in the sum, we have
    \[
      |\rho_{\mf{a}}(n,s)|^{2}\int_{Y}^{2Y}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}} \ll \int_{\G_{\infty}\backslash\H_{Y}}|(f|_{k}\s_{\mf{a}})(z)|^{2}\,d\mu.
    \]
    ...
    \[
      |\rho_{\mf{a}}(n,s)|^{2}\int_{Y}^{2Y}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}} \ll \frac{1}{Y}.
    \]
  ...
  \section{Inner Product Spaces of Automorphic Functions}
    ...
    \[
       \frac{1}{V_{\G}}\int_{0}^{\infty}\int_{0}^{1}\left(\sum_{n \in \Z}\rho_{\mf{a}}(n,y)e^{2\pi inx}\right)\conj{\psi(y)}\,\frac{dx\,dy}{y^{2}}.
    \]
  ...
  \section{Diamond and Hecke Operators}
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ have Fourier-Whittaker coefficients $\rho_{n}(f)$. Then for all primes $p$,
      \[
        (T_{p}f)(z) = \sum_{n \neq 0}\left(\rho_{np}(f)+\chi_{N,0}(p)p^{-1}\rho_{\frac{n}{p}}(\<p\>f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \]
      is the Fourier-Whittaker series of $T_{p}f$ where it is understood that $\rho_{\frac{n}{p}}(f) = 0$ if $p \nmid |n|$. Moreover, if $f \in \mc{C}_{k,\nu}(N,\chi)$ then $T_{p}f \in \mc{C}_{k,\nu}(N,\chi)$ and
      \[
        (T_{p}f)(z) = \sum_{n \neq 0}\left(\rho_{np}(f)+\chi(p)p^{-1}\rho_{\frac{n}{p}}(f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \]
      where it is understood that $\rho_{\frac{n}{p}}(f) = 0$ if $p \nmid |n|$.
    \end{proposition}
    \begin{proof}
    ...
    \begin{proposition}\label{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ have Fourier-Whittaker coefficients $\rho_{n}(f)$. Then for $m \ge 1$ with $(m,N) = 1$,
      \[
        (T_{m}f)(z) = \sum_{n \neq 0}\left(\sum_{d \mid (n,m)}d^{-1}\rho_{\frac{nm}{d^{2}}}(\<d\>f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \]
      is the Fourier-Whittaker series of $T_{m}f$. Moreover, if $f \in \mc{C}_{k,\nu}(N,\chi)$ then
      \[
        (T_{m}f)(z) = \sum_{n \neq 0}\left(\sum_{d \mid (n,m)}\chi(d)d^{-1}\rho_{\frac{nm}{d^{2}}}(f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx}.
      \]
    \end{proposition}
    \begin{proof}
    ...
    \[
      (Xf)(z) = f(-\conj{z}).
    \]
    As $-\conj{z} = -x+iy$, $X$ acts as reflection with respect to $x$. Then define the parity \textbf{Hecke operator}\index{Hecke operator} $T_{-1}$ on $\mc{C}_{k,\nu}(\G_{1}(N))$ to be the linear operator given by
    \[
  ...
  \section{Atkin-Lehner Theory}
    ...
    \begin{lemma}\label{lem:the_main_lemma_for_newforms_Maass}
      If $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ has Fourier-Whittaker coefficients $\rho_{n}(f)$ and is such that $\rho_{n}(f) = 0$ for all $n \ge 1$ whenever $(n,N) = 1$ then
      \[
    ...
    \]
    Note that if $f$ has Fourier-Whittaker coefficients $\rho_{n}(f)$ then $\conj{f}$ has Fourier-Whittaker coefficients $\conj{\rho_{n}(f)}$ by the conjugate symmetry of the Whittaker function (see \cref{append:Whittaker_Functions}). It turns out that $\conj{f}$ is indeed a cusp form:
  ...
  \section{Twists of Maass Forms}
    ...
    \[
      f(z) = \rho^{+}(f)y^{\frac{1}{2}+\nu}+\rho^{-}(f)y^{\frac{1}{2}-\nu}+\sum_{n \neq 0}\rho_{n}(f)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \[
      (f \ox \psi)(z) = \rho^{+}(f)\psi(0)y^{\frac{1}{2}+\nu}+\rho^{-}(f)\psi(0)y^{\frac{1}{2}-\nu}+\sum_{n \neq 0}\rho_{n}(f)\psi(n)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \[
      (U_{p}f)(z) = \sum_{n \neq 0}\rho_{np}(f)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
    \]
    and
    \[
