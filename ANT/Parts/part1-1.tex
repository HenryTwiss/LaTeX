\chapter{Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Summation Formulas}\label{append:Summation_Formulas}
    ...
    \end{theorem*}
    \begin{proof}
      Let $M = \lfloor X \rfloor$ and $N = \lfloor Y \rfloor$ so that $0 \le M \le N$. From partial sum summation by parts, we have
      \[
        \sum_{X < n \le Y}a_{n}\phi(n) = \sum_{M+1 \le n \le N}a_{n}\phi(n) = A(N)\phi(N)-A(M)\phi(M)-\sum_{M \le n \le N-1}A(n)(\phi(n+1)-\phi(n)).
      \]
      ...
      \end{align*}
      where in the second equality we have used that $A(n)$ is constant on the interval $[n,n+1)$ for every $n \ge 0$. Thus
      \[
        \sum_{X < n \le Y}a_{n}\phi(n) = A(N)\phi(N)-A(M)\phi(M)-\int_{M}^{N}A(u)\phi'(u)\,du.
      \]
      Using again the fact that $A(n)$ is constant on the interval $[n,n+1)$ for every $n \ge 0$, we can express the integral as
      \begin{align*}
