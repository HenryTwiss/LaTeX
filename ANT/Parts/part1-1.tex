\chapter{Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Summation Formulas}\label{append:Summation_Formulas}
    ...
    \end{theorem*}
    \begin{proof}
      Let $M = \lfloor X \rfloor$ and $N = \lfloor Y \rfloor$ so that $0 \le M \le B$. Then $X < n \le Y$ if and only if $M+1 \le n \le N$, so that partial sum summation by parts gives
      \[
