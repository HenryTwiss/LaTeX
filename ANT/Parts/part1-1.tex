\chapter{Conventional Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Asymptotics}
    ...
    \subsection*{\texorpdfstring{$o$}{o}-estimates and Symbols}
      ...
      \]
      as $M(\mathbf{x}) \neq 0$ for all $\mathbf{x}$ sufficiently close to $\mathbf{x}_{0}$ in norm because the same is assumed of $f(\mathbf{x})$. Therefore we only need to specify one of these formulas.
