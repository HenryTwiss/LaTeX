\chapter{Algebraic Integers}
  ...
  \section{Dedekind Domains}
    ...
    \]
    for all $i$. We will use the Chinese remainder theorem to prove a few useful lemmas about Dedekind domains. The first lemma is regarding the quotient of a Dedekind domain by a prime:
    ...
    Our second lemma shows that fractional ideals can be cancelled in quotients:
    ...
    \begin{lemma}\label{lem:cancellation_isomorphism}
      ...
      \[
        \mf{f}\mf{h}/\mf{g}\mf{h} \cong \mf{f}/\mf{g}.
      \]
    \end{lemma}
    \begin{proof}
      We have $\mf{f} = \frac{\mf{a}}{\a}$, $\mf{g} = \frac{\mf{b}}{\b}$, and $\mf{h} = \frac{\mf{c}}{\g}$ for nonzero $\a,\b,\g \in \smc{O}$ and integral ideals $\mf{a}$, $\mf{b}$, and $\mf{c}$. In view of the isomorphisms,
      \[
        \mf{f}\mf{h}/\mf{g}\mf{h} \cong \b\mf{a}\mf{c}/\a\mf{b}\mf{c} \quad \text{and} \quad \mf{f}/\mf{g} \cong \b\mf{a}/\a\mf{b},
      \]
      it suffices to show
      \[
        \mf{a}\mf{c}/\mf{b}\mf{c} \cong \mf{a}/\mf{b}.
      \]
      \todo{xxx}
    \end{proof}


    Our third lemma is that if a Dedekind domain contains only finitely many primes then it must be a principal ideal domain:
  ...
  \section{The Ideal Norm}
    ...
    \begin{proposition}\label{prop:norm_of_different_is_discriminant}
      Let $K$ be a number field of degree $n$. Then we have an isomorphism
      \[
    ...
    \end{proposition}
    \begin{proof}
      By \cref{prop:explicit_inverse_ideal}, $\mc{O}_{K} \subseteq \mf{D}_{K}^{-1}$. So using \cref{lem:cancellation_isomorphism}, we obtain the first isomorphism in the following chain:
      \[
        \mc{O}_{K}/\mf{D}_{K} \cong \mf{D}^{-1}_{K}/\mc{O}_{K} \cong \mc{O}_{K}^{\vee}/\mc{O}_{K}.
      \]
      This proves the first statement. For the second, this isomorphism shows that $\Norm_{K}(\mf{D}_{K}) = |\mc{O}_{K}^{\vee}/\mc{O}_{K}|$ by \cref{prop:norm_and_field_norms_are_identical}. Now let $\a_{1},\ldots,\a_{n}$ be an integral basis for $\mc{O}_{K}$. Then the dual basis $\a_{1}^{\vee},\ldots,\a_{n}^{\vee}$ is a basis for $\mc{O}_{K}^{\vee}$ and we have
      \[
