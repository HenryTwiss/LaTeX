\chapter{The Theory of Dirichlet Series}
  ...
  \section{Perron Formulas}
    ...
    There is also a truncated variant of Perron's formula which is often more useful for more subtle estimates. To state it, we will need to setup some notation and prove a lemma. For any $c > 0$ and $T > 0$, consider the integrals
    \[
    ...
    \begin{proof}
      ...
      \]
      Consider the case $y = 1$. We compute
      \begin{align*}
     ...
     This time
      ...
      \begin{figure}[h]
        ...
        \begin{tikzpicture}[scale=3]
          ...
          \node at (-0.5,0.75) [above] {\tiny{$\eta_{2}$}};
          \node at (-1.5,0) [above left] {\tiny{$\eta_{3}$}};
          \node at (-0.5,-0.75) [below] {\tiny{$\eta_{4}$}};

          \node at (0.5,-0.75) [circle,fill,inner sep=1pt]{};
          \node at (0.5,0.75) [circle,fill,inner sep=1pt]{};
          \node at (-1.5,0.75) [circle,fill,inner sep=1pt]{};
          \node at (-1.5,-0.75) [circle,fill,inner sep=1pt]{};

          \node at (0.5,-0.75) [below] {\tiny{$c-iT$}};
          \node at (0.5,0.75) [above] {\tiny{$c+iT$}};
          \node at (-1.5,0.75) [above] {\tiny{$-d+iT$}};
          \node at (-1.5,-0.75) [below] {\tiny{$-d-iT$}};
        \end{tikzpicture}
        \caption{A rectangular contour.}
      ...
      Since $y > 1$, taking the limit as $d \to \infty$ implies $I(y,T) = 1+O\left(\frac{y^{c}}{\log(y)T}\right)$ and thus $\d(y) = 1$. Now let $\g = \g_{1}+\g_{2}$ be the contour in \cref{fig:discontinuous_integral_contour_4}. This time
      \[
        I(y,T) = \frac{1}{2\pi i}\int_{\g_{1}}y^{s}\,\frac{ds}{s}.
      \]
      The integrand has a simple a pole at $s = 0$ and its residue is $1$. So the residue theorem implies
