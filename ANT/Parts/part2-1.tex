\chapter{Algebraic Integers}
  ...
  \section{\todo{Galois Extensions}}
    ...
    \]
    In other words, the decomposition field of $\mf{P}$ is the fixed field of $D_{\mf{p}}(\mf{P})$. In particular, $L^{D_{\mf{p}}(\mf{P})}$ is an intermediate field of $L/K$ by the fundamental theorem of Galois theory. We let $\mc{O}^{D}$ denote the integral closure of $\smc{O}$ in $L^{D_{\mf{p}}(\mf{P})}$ so that $\mc{O}/\mc{O}^{D}/\smc{O}$ is a tower of Dedekind extensions. Also, we will write $\mf{P}^{D}$ for the prime of $L^{D_{\mf{p}}(\mf{P})}$ below $\mf{P}$. Then we can illustrate this relationship via the following tower of extensions:
    ...
    \begin{proposition}\label{prop:decomposition_field_controls_decomposition}
    ...
    We can view \cref{prop:decomposition_field_controls_decomposition} (i) as a statement that $\mf{p}\mc{O}^{D}$ encodes all of the information about the distinct prime factors $\mf{p}\mc{O}$ splits into in $\mc{O}$. Moreover, \cref{prop:decomposition_field_controls_decomposition} (ii) can be interpreted as the fact that $\mc{O}^{D}/\smc{O}$ contains no information about the ramification index or inertia degree of $\mf{p}$ in $\mc{O}/\smc{O}$. All of that information is contained in $\mc{O}/\mc{O}^{D}$. We need to do more work to unpack this information. Continue to let $\mf{P}$ be a prime above $\mf{p}$. Since any $\tau \in D_{\mf{p}}(\mf{P})$ leaves $\mc{O}$ and $\mf{P}$ invariant, it induces an automorphism $\conj{\tau}$ of the residue class field $\F_{\mf{P}}$ defined by
    \[
      \conj{\tau}:\F_{\mf{P}}\to \F_{\mf{P}}\qquad \a \mapsto \tau(\a) \tmod{\mf{P}}.
    \]
    We then have a homomorphism
    \[
      T_{\mf{P}}:D_{\mf{p}}(\mf{P}) \to \Aut_{\F_{\mf{p}}}(\F_{\mf{P}}) \qquad \tau \mapsto \conj{\tau}.
    \]
    The following proposition shows that the residue class extensions $\F_{\mf{P}}/\F_{\mf{p}}$ are normal and that $T_{\mf{P}}$ is surjective:
    
    \begin{proposition}
      Let $\mc{O}/\smc{O}$ be a Dedekind extension of a finite Galois extension $L/K$, let $\mf{p}$ be a prime of $\smc{O}$, and let $\mf{P}$ be a prime of $\mc{O}$ above $\mf{p}$. Then the residue class extension $\F_{\mf{P}}/\F_{\mf{p}}$ is normal. Moreover, the homomorphism $T_{\mf{P}}$ is surjective.
    \end{proposition}
    \begin{proof}
      By \cref{prop:decomposition_field_controls_decomposition} (ii), $f_{\mf{p}}(\mf{P}^{D}) = 1$ so that we have an isomorphism $\F_{\mf{P}^{D}} \cong \F_{\mf{p}}$. Under this isomorphism we may assume $L^{D_{\mf{p}}(\mf{P})} = K$ which is to say $D_{\mf{p}}(\mf{P}) = \Gal(L/K)$. We will now prove that $\F_{\mf{P}}/\F_{\mf{p}}$ is normal. So let $\a \in \mc{O}$ and suppose $m_{\a}(x)$ and $m_{\conj{\a}}(x)$ are the minimal polynomials of $\a$ and $\conj{\a}$ over $K$ and $\F_{\mf{p}}$ respectively. As $\a \in \mc{O}$, \cref{prop:field_of_fractions_AKBL} implies $m_{\a}(x)$ has coefficients in $\smc{O}$. As $\a$ is a representative of $\conj{\a}$, we find that $\conj{\a}$ is a root of $\conj{m_{\a}}(x)$ and thus $m_{\conj{\a}}(x)$ divides $\conj{m_{\a}}(x)$. Since $L/K$ is normal (as it is Galois), $m_{\a}(x)$ splits into linear factors over $L$. These linear factors must also belong to $\mc{O}[x]$ since their roots are elements of $\mc{O}$ which itself is integrally closed. It follows that $\conj{m_{\a}}(x)$ splits into linear factors over $\F_{\mf{P}}$. As $m_{\conj{\a}}(x)$ divides $\conj{m_{\a}}(x)$, the same is true for $m_{\conj{\a}}(x)$. Thus $\F_{\mf{P}}/\F_{\mf{p}}$ is normal. We now prove the surjectivity of $T_{\mf{P}}$. Consider the maximal separable subextension of $\F_{\mf{P}}/\F_{\mf{p}}$. Since this extension is finite (because the residue class extension is), it is simple by the primitive element theorem. Letting $\t \in \mc{O}$ be such that $\conj{\t}$ is a primitive element, the maximal separable subextension is of the form $\F_{\mf{p}}(\conj{\t})/\F_{\mf{p}}$. Now let $\conj{\tau} \in \Aut_{\F_{\mf{p}}}(\F_{\mf{P}})$. Since $\conj{\tau}$ fixes $\F_{\mf{p}}$ pointwise, $\conj{\tau}(\conj{\t})$ is a root of $m_{\conj{\t}}(x)$. As $m_{\conj{\t}}(x)$ divides $\conj{m_{\t}}(x)$, we find that $\conj{\tau}(\conj{\t})$ is also a root of $\conj{m_{\t}(x)}$. Therefore there is a root $\t'$ of $m_{\t}(x)$ that is also a representative of $\conj{\tau}(\conj{\t})$. Because $L/K$ is Galois, we can find $\tau \in \Gal(L/K)$ such that $\tau(\t) = \t'$. Moreover, $\tau \in D_{\mf{p}}(\mf{P})$ since $D_{\mf{p}}(\mf{P}) = \Gal(L/K)$. Then $T_{\mf{P}}(\tau) = \conj{\tau}$ because they both take $\conj{\t}$ to the same element, hence act the same on $\F_{\mf{p}}(\conj{\t})/\F_{\mf{p}}$, and $\F_{\mf{P}}/\F_{\mf{p}}(\conj{\t})$ is purely inseparable so that it has trivial automorphism group. This proves $T_{\mf{P}}$ is surjective completing the proof.
    \end{proof}

    Again suppose $\mf{P}$ be a prime above $\mf{p}$. We define the \textbf{inertia group}\index{inertia group} $I_{\mf{p}}(\mf{P})$ of $\mf{P}$ by
    \[
      I_{\mf{p}}(\mf{P}) = \ker{T_{\mf{P}}}.
    \]
    Then $I_{\mf{p}}(\mf{P})$ is a subgroup of $D_{\mf{p}}(\mf{P})$. The associated \textbf{inertia field}\index{inertia field} $L^{I_{\mf{p}}(\mf{P})}$ of $\mf{P}$ is defined to be
    \[
      L^{I_{\mf{p}}(\mf{P})} = \{\l \in L:\text{$\tau(\l) = \l$ for all $\tau \in I_{\mf{p}}(\mf{P})$}\}.
    \]
    In other words, the inertia field of $\mf{P}$ is the fixed field of $I_{\mf{p}}(\mf{P})$. Since $I_{\mf{p}}(\mf{P})$ is a subgroup of $D_{\mf{p}}(\mf{P})$, the fundamental theorem of Galois theory implies that $L^{I_{\mf{p}}(\mf{P})}$ is an intermediate field of $L/L^{D_{\mf{p}}(\mf{P})}$. We let $\mc{O}^{I}$ denote the integral closure of $\smc{O}$ in $L^{I_{\mf{p}}(\mf{P})}$ so that $\mc{O}/\mc{O}^{I}/\smc{O}$ is a tower of Dedekind extensions. Also, we will write $\mf{P}^{I}$ for the prime of $L^{I_{\mf{p}}(\mf{P})}$ below $\mf{P}$. Then we can illustrate this relationship via the following tower of extensions:
