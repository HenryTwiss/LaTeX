\chapter{Algebraic Integers}
  ...
  \section{\todo{Galois Extensions}}
    ...
    We can view \cref{prop:decomposition_field_controls_decomposition} (i) as a statement that $\mf{p}\mc{O}^{D}$ encodes all of the information about the distinct prime factors $\mf{p}\mc{O}$ splits into in $\mc{O}$. Moreover, \cref{prop:decomposition_field_controls_decomposition} (ii) can be interpreted as the fact that $\mc{O}^{D}/\smc{O}$ contains no information about the ramification index or inertia degree of $\mf{p}$ in $\mc{O}/\smc{O}$. All of that information is contained in $\mc{O}/\mc{O}^{D}$. We need to do more work to unpack this information. Continue to let $\mf{P}$ be a prime above $\mf{p}$. Since any $\tau \in D_{\mf{p}}(\mf{P})$ leaves $\mc{O}$ and $\mf{P}$ invariant, it induces an automorphism $\conj{\tau}$ of the residue class field $\mc{O}/\mf{P}$ defined by
    \[
      \conj{\tau}:\F_{\mf{P}}\to \F_{\mf{P}}\qquad \a \mapsto \tau(\a) \tmod{\mf{P}}.
    \]
    Then we have a surjective homomorphism as the following proposition shows:

    \begin{proposition}
      Let $\mc{O}/\smc{O}$ be a Dedekind extension of a finite Galois extension $L/K$, let $\mf{p}$ be a prime of $\smc{O}$, and let $\mf{P}$ be a prime of $\mc{O}$ above $\mf{p}$. Then the residue class extension $\F_{\mf{P}}/\F_{\mf{p}}$ is normal. Moreover, we have a surjective homomorphism
      \[
        T:D_{\mf{p}}(\mf{P}) \to \Aut_{\F_{\mf{p}}}(\F_{\mf{P}}) \qquad \tau \mapsto \conj{\tau}.
      \]
    \end{proposition}
    \begin{proof}
      By \cref{prop:decomposition_field_controls_decomposition} (ii), $f_{\mf{p}}(\mf{P}^{D}) = 1$ so that we have an isomorphism $\F_{\mf{P}^{D}} \cong \F_{\mf{p}}$. Under this isomorphism we may assume $L^{D_{\mf{p}}(\mf{P})} = K$ which is to say $D_{\mf{p}}(\mf{P}) = \Gal(L/K)$. We will now prove that $\F_{\mf{P}}/\F_{\mf{p}}$ is normal. So let $\a \in \mc{O}$ and suppose $m_{\a}(x)$ and $m_{\conj{\a}}(x)$ are the minimal polynomials of $\a$ and $\conj{\a}$ over $K$ and $\F_{\mf{p}}$ respectively. As $\a \in \mc{O}$, \cref{prop:field_of_fractions_AKBL} implies $m_{\a}(x)$ has coefficients in $\smc{O}$. As $\a \in \conj{\a}$, we find that $\conj{\a}$ is a root of $\conj{m_{\a}}(x)$ and thus $m_{\conj{\a}}(x)$ divides $\conj{m_{\a}}(x)$. Since $L/K$ is normal (as it is Galois), $m_{\a}(x)$ splits into linear factors over $L$. These linear factors must also belong to $\mc{O}[x]$ since their roots are elements of $\mc{O}$ which itself is integrally closed. It follows that $\conj{m_{\a}}(x)$ splits into linear factors over $\mc{O}/\mf{P}$. As $m_{\conj{\a}}(x)$ divides $\conj{m_{\a}}(x)$, the same is true for $m_{\conj{\a}}(x)$. Thus $\F_{\mf{P}}/\F_{\mf{p}}$ is normal. We now prove the surjectivity of $T$. Consider the maximal separable subextension of $\F_{\mf{P}}/\F_{\mf{p}}$. Since this extension is finite (because the reside class extension is), it is simple by the primitive element theorem. Letting $\t \in \mc{O}$ be such that $\conj{\t}$ is a primitive element, the maximal separable subextension is of the form $\F_{\mf{p}}(\conj{\t})/\F_{\mf{p}}$. We claim
      \[
        \Aut_{\F_{\mf{p}}}(\F_{\mf{p}}(\conj{\t})) = \Aut_{\F_{\mf{p}}}(\F_{\mf{P}}).
      \]
      \todo{xxx}
    \end{proof}
