\chapter{Algebraic Integers}
  ...
  \section{Traces \& Norms}
    ...
    Even if an integral basis for $B$ over $A$ exists, where say $B$ is a free $A$-module of rank $n$, it is very difficult to explicitly write one down. However, there is one instance in which this is possible. We say that $B$ is \textbf{monogenic} over $A$ if $B = A[\b]$ for some $\b \in B$. It follows immediately that $1,\b,\ldots,\b^{n-1}$ is an integral basis for $B$ over $A$ since $B$ is of rank $n$. In addition, the traces $\Trace_{L/K}$ and $\Trace_{B/A}$ and norms $\Norm_{L/K}$ and $\Norm_{B/A}$ agree:
    ...
    \end{proposition}
    \begin{proof}
      Let $\b_{1},\ldots,\b_{n}$ be an integral basis for $B$ over $A$. Then $\b_{1},\ldots,\b_{n}$ is also a basis for $L/K$. It follows that the multiplication by $\b$ map in $L$ has the same matrix representation as it does in $B$. Therefore
      \[
        \Trace_{L/K}(\b) = \Trace{B/A}(\b) \quad \text{and} \quad \Norm_{L/K}(\b) = \Norm_{B/A}(\b),
      \]
      as desired.
  ...
  \section{Dedekind Extensions}
    ...
    \begin{proof}
      ...
      \]
      By \cref{prop:field_of_fractions_AKBL} and that $d \neq 0$, multiplication by $K$ shows $L = \l_{1}K+\cdots+\l_{m}K$ (as the reverse containment is trivial). Hence $\l_{1},\ldots,\l_{m}$ spans $L/K$ so that $m = n$ and $\mc{O}/\mf{p}\mc{O}$ is an $n$ dimensional $(\smc{O}/\mf{p})$-vector space. For $\mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}$, consider the descending chain
      \[
  ...
  \section{\todo{The Different \& Discriminant}}
    ...
    We now want to show that a prime $\mf{P}$ of $L$ ramifies if and only if it divides the different $\mf{D}_{\mc{O}/\smc{O}}$ provided the residue class extensions are separable. We first require a lemma:
    ...
    \begin{proof}
      ...
      \]
      for all $\a \in \mf{A}$. To prove this, observe that $\conj{\a}$ belongs to the subring $\mf{A}/\mf{p}\mc{O}$ of $\mc{O}/\mf{p}\mc{O}$ because $\a \in \mf{A}$. By construction, $\mf{A}$ is divisible by every prime factor of $\mf{p}\mc{O}$ and so a power, say $n$, of $\mf{A}$ is divisible by $\mf{p}\mc{O}$. But this means $T_{\conj{\a}}^{n} = 0$ so that $T_{\conj{\a}}$ is nilpotent. As nilpotent maps have trace zero, $\Trace_{(\mc{O}/\mf{p}\mc{O})/(\smc{O}/\mf{p})}(\conj{\a}) = 0$. But then $\Trace_{\mc{O}/\smc{O}}(\a) \equiv 0 \tmod{p}$ and, as $\a$ was arbitrary, the claim is justified. We now prove (i) and (ii):
      \begin{enumerate}[label=(\roman*)]
    ...
    \]
    for any integral basis $\a_{1},\ldots,\a_{n}$ for $K$. Then $\D_{K}$ is well-defined. Moreover, $\D_{K}$ is a non-zero integer by \cref{prop:norm_and_trace_AKBL,lem:trace_is_nondegenerate} and may very well be negative. In light of \cref{disc_as_square_of_embedding_matrix}, we also have
    \[
      |\det(M(\a_{1},\ldots,\a_{n}))| = \sqrt{|\D_{K}|}.
    \]
    In any case, $\D_{K}$ is related to $\mf{d}_{K}$ by the identity
    \[
  ...
  \section{\todo{The Ideal Norm}}
    ...
    \end{proposition}
    \begin{proof}
      In light of \cref{prop:ideal_norm_respects_localizations}, it suffices to assume $\mc{O}/\smc{O}$ is a local Dedekind extension. Then $\smc{O}$ is a discrete valuation ring, $\mc{O}$ is a principal ideal domain, and $\mc{O}/\smc{O}$ admits an integral basis $\a_{1},\ldots,\a_{n}$. Let $\mf{p}$ be the unique prime of $\smc{O}$ and $\pi$ be a uniformizer so that $\mf{p} = \pi\smc{O}$. Since $\mc{O}$ is also a unique factorization domain, let $\l = \mu\pi_{1}^{e_{1}} \cdots \pi_{r}^{e_{r}}$ be the prime factorization of $\l$ in $\mc{O}$ and let $\mf{P}_{i}$ be the prime $\mc{O}$ generated by the prime $\pi_{i}$ for $1 \le i \le r$. Then
      \[
        \l\mc{O} = \mf{P}_{1}^{e_{1}} \cdots \mf{P}_{r}^{e_{r}}.
      \]
      So on the one hand, we have
      \[
        \Norm_{\mc{O}/\smc{O}}(\l\mc{O}) = \mf{p}^{e_{1}f_{\mf{p}}(\mf{P}_{1})+\cdots+e_{r}f_{\mf{p}}(\mf{P}_{r})}.
      \]
      Complete multiplicativity of the field norm implies $\Norm_{L/K}(\l) = \Norm_{L/K}(\pi_{1})^{e_{1}} \cdots \Norm_{L/K}(\pi_{r})^{e_{r}}$. As $\smc{O}$ is a discrete valuation ring, we have prime factorizations $\Norm_{L/K}(\pi_{i}) = \eta\pi^{f_{i}}$ for all $i$. So on the other hand,
      \[
        \Norm_{L/K}(\l)\smc{O} = \mf{p}^{e_{1}f_{1}+\cdots+e_{r}f_{r}}
      \]
      It now suffices to show that $f_{i} = f_{\mf{p}}(\mf{P}_{i})$ for all $i$. \todo{xxx}
    \end{proof}

    We can now show the different and discriminant are related in the sense that the ideal norm of the different is the discriminant:
    ...
    \]
    called the \textbf{norm}\index{norm} of $K$. It follows from \cref{prop:norm_of_different_is_discriminant} that
    \[
    ...
    We can also compute the norm of a dual ideal:
