\chapter{Algebraic Integers}
  ...
  \section{Integrality}
    ...
    \[
      \overline{A} = \{\b \in B:\text{$\b$ is integral over $A$}\}.
    \]
  ...
  \section{Traces \& Norms}
    ...
    \end{proposition}
    \begin{proof}
      Clearly $\b_{1,1},\ldots,\b_{n_{1},1},\b_{1,2},\ldots,\b_{n_{2},2}$ is a basis for $B$. We also have $\b_{i,1}\b_{j,2} = 0$ for all $1 \le i \le n_{1}$ and $1 \le j \le n_{2}$. Letting $1 \le k \le n_{1}$ and $1 \le \ell \le n_{2}$, it follows that $d_{B/A}(\b_{1,1},\ldots,\b_{n_{1},1},\b_{1,2},\ldots,\b_{n_{2},2},)$ is the determinant of the block diagonal matrix
      \[
        \begin{pmatrix} (\Trace_{B/A}(\b_{i,1}\b_{k,1}))_{i,k} & \\ & (\Trace_{B/A}(\b_{j,2}\b_{\ell,2}))_{j,\ell} \end{pmatrix}.
      \]
      ...
      \[
        \begin{pmatrix} (\Trace_{B_{1}/A}(\b_{i,1}\b_{k,1}))_{i,k} & \\ & (\Trace_{B_{2}/A}(\b_{j,2}\b_{\ell,2}))_{j,\ell} \end{pmatrix}.
      \]
    ...
    \begin{proof}
      ...
      \[
        \a_{i,1} = \sum_{1 \le j \le n_{2}}\k_{i,j}\b_{j,2} \quad \text{and} \quad \a_{j,2} = \sum_{1 \le i \le n_{1}}\k_{i,j}\b_{i,1},
      \]
      for all $i$ and $j$ so that
      \[
        \l = \sum_{1 \le i \le n_{1}}\a_{i,1}\b_{i,1} \quad \text{and} \quad \l = \sum_{1 \le j \le n_{2}}\a_{j,2}\b_{j,2}.
      \]
      By linear disjointness, we have
      \[
      ...
      \[
        \sum_{1 \le k \le n_{1}}\s_{i,1}(\b_{k,1})\a_{k,1} = \s_{i,1}(\l) \quad \text{and} \quad \sum_{1 \le k \le n_{2}}\s_{j,2}(\b_{k,2})\a_{k,2} = \s_{j,2}(\l),
      \]
      respectively are equivalent to the identities
      \[
        \begin{pmatrix} \s_{1,1}(\b_{1,1}) & \cdots & \s_{1,1}(\b_{n_{1},1}) \\ \vdots & & \vdots \\ \s_{n_{1},1}(\b_{1,1}) & \cdots & \s_{n_{1},1}(\b_{n_{1},1}) \end{pmatrix}\begin{pmatrix} \a_{1,1} \\ \vdots \\ \a_{n_{1},1} \end{pmatrix} = \begin{pmatrix} \s_{1,1}(\l) \\ \vdots \\ \s_{n_{1},1}(\l) \end{pmatrix},
      \]
      and
      \[
        \begin{pmatrix} \s_{1,2}(\b_{1,2}) & \cdots & \s_{1,2}(\b_{n_{2},2}) \\ \vdots & & \vdots \\ \s_{n_{2},2}(\b_{1,2}) & \cdots & \s_{n_{2},2}(\b_{n_{2},2}) \end{pmatrix}\begin{pmatrix} \a_{1,2} \\ \vdots \\ \a_{n_{2},2} \end{pmatrix} = \begin{pmatrix} \s_{1,2}(\l) \\ \vdots \\ \s_{n_{2},2}(\l) \end{pmatrix},
      \]
      respectively. \todo{xxx}
    \end{proof}
