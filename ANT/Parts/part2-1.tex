\chapter{Algebraic Integers}
  ...
  \section{Dedekind Domains}
    ...
    \]
    for all $i$. We will use the Chinese remainder theorem to prove a few useful lemmas about Dedekind domains. The first of which is regarding the quotient of a Dedekind domain by a prime:
    ...
    Our second is that if a Dedekind domain contains only finitely many primes then it must be a principal ideal domain:

    \begin{lemma}\label{lem:Dedekind_with_finite_primes_is_PID}
    ...
    Our last lemma say that any fractional ideal is generated by at most two elements:

    \begin{lemma}\label{lem:fractional_ideal_generated_by_two_elements}
  ...
  \section{Dedekind Extensions}
    ...
    The inertia degrees and ramification indices satisfy a simple relationship to the degree of $L/K$. We describe this relationship in the following theorem known as the \textbf{fundamental equality}\index{fundamental equality}:
    ...
    \begin{proof}
      ...
      \]
      of $(\smc{O}/\mf{p})$-vector spaces. The dimension of $\mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}$ as a $(\smc{O}/\mf{p})$-vector space is the sum of the dimensions of the successive quotients in this chain. By the third isomorphism theorem, these quotients are of the form $\mf{P}_{i}^{e}/\mf{P}_{i}^{e+1}$ for $0 \le e \le e_{\mf{p}}(\mf{P}_{i})-1$ and are all isomorphic to $\mc{O}/\mf{P}_{i}$ by \cref{lem:isomorphism_of_quotient_by_prime_integral_ideals}. As the dimension of $\mc{O}/\mf{P}_{i}$ over $\smc{O}/\mf{p}$ is $f_{\mf{p}}(\mf{P}_{i})$ by definition, it follows that $\mc{O}/\mf{P}_{i}^{e_{\mf{p}}(\mf{P}_{i})}$ is a $(\smc{O}/\mf{p})$-vector space of dimension $e_{\mf{p}}(\mf{P}_{i})f_{\mf{p}}(\mf{P}_{i})$. Together with the Chinese remainder theorem, our dimension computations combine to give
      \[
        n = \sum_{1 \le i \le r}e_{\mf{p}}(\mf{P}_{i})f_{\mf{p}}(\mf{P}_{i}).
      \]
    \end{proof}

    We also classify primes according to extremal cases of the fundamental equality. We say $\mf{p}$ is \textbf{inert}\index{inert} if $r = 1$ so that there is a single prime $\mf{P}$ above $\mf{p}$ and we have $e_{\mf{p}}(\mf{P}) = 1$ and $f_{\mf{p}}(\mf{P}) = n$ by the fundamental equality. Then
    \[
      \mf{p}\mc{O} = \mf{P},
    \]
    which means $\mf{p}\mc{O}$ is prime. We say $\mf{p}$ is \textbf{totally split}\index{totally split} if $r = n$ so that there are primes $\mf{P}_{i}$ above $\mf{p}$ with $e_{\mf{p}}(\mf{P}_{i}) = f_{\mf{p}}(\mf{P}_{i}) = 1$ for $1 \le i \le n$ by the fundamental equality. Hence
    \[
    ...
    \begin{proof}
      ...
      \[
        \pi:\mc{O} \to \mc{O}/\mf{p}\mc{O} \qquad \t \mapsto \t \pmod{\mf{p}\mc{O}}.
      \]
      ...
      \]
      for some ramification indices $e_{p}(\mf{P}_{i})$ for all $i$. We will be done if we can show $e_{p}(\mf{P}_{i}) = e_{i}$ for all $i$. To accomplish this, observe that we have an isomorphism
      \[
      ...
      \]
      Since the $\mf{P}_{i}$ are prime, we have $e_{p}(\mf{P}_{i}) \le e_{i}$ for all $i$. But the fundamental equality then gives
      \[
      ...
      \]
      where the last equality follows by the prime factorization of $\conj{m_{\t}}(x)$ and that $\deg(\conj{m_{\t}}(x)) = \deg(m_{\t}(x))$ because $m_{\t}(x)$ is monic. This shows that $e_{p}(\mf{P}_{i}) = e_{i}$ for all $i$ which completes the proof.
    \end{proof}
  ...
  \section{The Different \& Discriminant}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \]
        of $(\smc{O}/\mf{p})$-vector spaces. As $\mf{P}^{e}/\mf{P}^{e_{\mf{p}}(\mf{P})}$ is an ideal of $\mc{O}/\mf{P}^{e_{\mf{p}}(\mf{P})}$ for $0 \le e \le e_{\mf{p}}(\mf{P})-1$, the multiplication by $\conj{\b}$ map is a linear operator on each $\mf{P}^{e}/\mf{P}^{e_{\mf{p}}(\mf{P})}$. By the third isomorphism theorem, the successive quotients are of the form $\mf{P}^{e}/\mf{P}^{e+1}$ for $0 \le e \le e_{\mf{p}}(\mf{P})-1$ and are all isomorphic to $\mc{O}/\mf{P}$ by \cref{lem:isomorphism_of_quotient_by_prime_integral_ideals}. Therefore we have a decomposition
        \[
