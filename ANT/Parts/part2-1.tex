\chapter{Algebraic Integers}
  ...
  \section{Dedekind Domains}
    ...
    Our second lemma constructs an integral ideal relatively prime to a given integral ideal:

    \begin{lemma}
      Let $\smc{O}$ be a Dedekind domain and $\mf{a}$ and $\mf{b}$ be integral ideals. Then there exists an integral ideal $\mf{c}$ such that $\mf{a}\mf{c}$ is principal and relatively prime to $\mf{b}$.
    \end{lemma}
    \begin{proof}
      \todo{xxx}
    \end{proof}

    Our last lemma shows that fractional ideals can be cancelled in quotients:
