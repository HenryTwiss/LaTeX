\chapter{Algebraic Integers}
  ...
  \section{\todo{Galois Extensions}}
    ...
    \[
      D_{\mf{p}}(\mf{P}) = \{\s \in \Gal(L/K):\s(\mf{P}) = \mf{P}\}.
    \]
    The associated \textbf{decomposition field}\index{decomposition field} $L^{D_{\mf{p}}(\mf{P})}$ of $\mf{P}$ is defined to be
    \[
      L^{D_{\mf{p}}(\mf{P})} = \{\l \in L:\text{$\s(\l) = \l$ for all $\s \in D_{\mf{p}}(\mf{P})$}\}.
    \]
    In other words, the decomposition field of $\mf{P}$ is the fixed field of the decomposition group of $\mf{P}$. In particular, $L^{D_{\mf{p}}(\mf{P})}$ is an intermediate field of $L/K$. We illustrate this with the following extension:
    
    \begin{center}
      \begin{tikzcd}[row sep=large, column sep=large]
        \mf{P} \subset L \arrow[dash]{d} \\
        L^{D_{\mf{p}}(\mf{P})} \arrow[dash]{d} \\
        \mf{p} \subset K.
      \end{tikzcd}
    \end{center}

    Any of the decomposition groups $D_{\mf{p}}(\mf{P})$ encode how $\mf{p}\mc{O}$ splits into distinct prime factors. Indeed, if $\s$ runs over a complete set of representatives for the coset space $\Gal(L/K)/D_{\mf{p}}(\mf{P})$, then $\s(\mf{P})$ runs over the distinct primes above $\mf{p}$ by \cref{prop:Galois_action_on_primes_is_transitive}. It follows that the number of prime factors of $\mf{p}\mc{O}$ is equal to the index $[\Gal(L/K):D_{\mf{p}}(\mf{P})]$. In particular, $\mf{p}$ is inert if and only if $D_{\mf{p}}(\mf{P}) = \Gal(L/K)/D_{\mf{p}}(\mf{P})$ which is further equivalent to $L^{D_{\mf{p}}(\mf{P})} = K$. Antithetically, $\mf{p}$ is totally split if and only if $D_{\mf{p}}(\mf{P}) = \{\id\}$ which is further equivalent to $L^{D_{\mf{p}}(\mf{P})} = L$.
    \todo{xxx}
