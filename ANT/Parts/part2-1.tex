\chapter{Algebraic Integers}
  ...
  \section{Dedekind Extensions}
    ...
    Unfortunately, there is no general way to see how $\mf{p}\mc{O}$ factors for an arbitrary Dedekind extension $\mc{O}/\smc{O}$ of a finite separable extension $L/K$. However, we can make some progress in this respect. Let $\t$ be a primitive element for $L$ so that $L = K[\t]$. By \cref{prop:field_of_fractions_AKBL} we can multiply by a nonzero element of $\smc{O}$, if necessary, to ensure that $\t \in \mc{O}$ and hence its minimal polynomial $m_{\t}(x)$ has coefficients in $\smc{O}$. We then define the \textbf{conductor}\index{conductor} $\mf{Q}_{\mc{O}/\smc{O}}$ of $\smc{O}[\t]$ by
    \[
    ...
    \begin{theorem*}[\todo{Dedekind-Kummer theorem}]
