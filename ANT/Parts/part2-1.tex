\chapter{Algebraic Integers}
  ...
  \section{Traces \& Norms}
    ...
    Even if an integral basis for $B$ over $A$ exists, where say $B$ is a free $A$-module of rank $n$, it is very difficult to explicitly write one down. However, there is one instance in which this is possible. We say that $B$ is \textbf{monogenic} over $A$ if $B = A[\b]$ for some $\b \in B$. It follows immediately that $1,\b,\ldots,\b^{n-1}$ is an integral basis for $B$ over $A$ since $B$ is of rank $n$.
    
    \begin{proposition}
      If $B$ is monogenic over $A$, then the integral basis $1,\b,\ldots,\b^{n-1}$ is sometimes called a \textbf{power basis}\index{power basis} for $B$ over $A$.
    \end{proposition}
  ...
  \section{\todo{The Ideal Norm}}
    ...
    \end{corollary}
    \begin{proof}
      This follows immediately from $\mf{f}^{\vee} = \mf{f}^{-1}\mf{D}_{K}^{-1}$, complete multiplicativity of the norm, and \cref{prop:norm_of_different_is_discriminant}.
    \end{proof}
