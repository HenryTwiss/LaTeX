\chapter{Algebraic Integers}
  ...
  \section{Dedekind Domains}
    ...
    \end{proposition}
    \begin{proof}
      By maximality, there exists $\b \in \mf{p}^{n}-\mf{p}^{n+1}$. Now consider the homomorphism
      \[
    ...
    \begin{proof}
      ...
      \[
        \mf{a} = \mf{p}_{1}^{e_{1}} \cdots \mf{p}_{r}^{e_{r}},
      \]
      with $e_{i} \ge 0$ for $1 \le i \le r$. By maximality, there exists $\a_{i} \in \mf{p}_{i}^{e_{i}}-\mf{p}_{i}^{e_{i}+1}$ for all $i$. Since $\mf{p}_{1}^{e_{1}+1},\ldots,\mf{p}_{r}^{e_{r}+1}$ are pairwise relatively prime, the Chinese remainder theorem implies that there exists $\a \in \smc{O}$ with 
      \[
    ...
    \begin{proof}
      ...
      \[
        \mf{a} = \mf{p}_{1}^{e_{1}} \cdots \mf{p}_{r}^{e_{r}},
      \]
      with $e_{i} \ge 0$ for $1 \le i \le r$. By maximality, there exists $\b_{i} \in \mf{p}_{i}^{e_{i}}-\mf{p}_{i}^{e_{i}+1}$ for all $i$. Since $\mf{p}_{1}^{e_{1}+1},\ldots,\mf{p}_{r}^{e_{r}+1}$ are pairwise relatively prime, the Chinese remainder theorem implies that there exists $\b \in \smc{O}$ with 
      \[
  ...
  \section{Localization}
    ...
    In view of \cref{prop:localization_prime_bijection}, the primes $\mf{p}_{D}$ of $\smc{O}D^{-1}$ are of the form $\mf{p} = \mf{p}D^{-1}$ for some prime $\mf{p}$ of $\smc{O}$ disjoint from $D$. With this notation, $\mf{p}_{D}$ corresponds to $\mf{p}$ under the bijections in \cref{prop:localization_prime_bijection}.
    ...
    Second, intersections of localizations behave with respect to fractional ideals and units:
    ...
    \begin{proof}
      ...
      \]
      Then $\mf{a}$ is clearly an integral ideal of $\smc{O}$ with $\b \in \mf{a}$. It also cannot be contained in any prime $\mf{p}$. Indeed, since $\frac{\a}{\b} \in \smc{O}_{\mf{p}}$ we have $\b \notin \mf{p}$ and so $\b \in \mf{a}-\mf{p}$. As $\smc{O}$ is noetherian (which we recall is equivalent to every nonempty collection of ideals having a maximal element), there exists a maximal integral ideal $\mf{m}$ which is necessarily a prime. As $\mf{a}$ is not contained in any prime, $\mf{a}$ is not contained in $\mf{m}$ and thus $\mf{a} = \smc{O}$. But then $1 \in \mf{a}$ and so $\a \in \b\smc{O}$ implying $\frac{\a}{\b} \in \smc{O}$ which proves the reverse containment and the second identity follows. For the third identity, the forward containment is clear. For the reverse containment, suppose $\frac{\a}{\b} \in \bigcap_{\text{$\mf{p}$ prime}}\smc{O}_{\mf{p}}^{\ast}$. We have already showed $\frac{\a}{\b} \in \smc{O}$ so it suffices to show $\frac{\a}{\b}$ is a unit. As $\b \notin \mf{p}$ for any prime $\mf{p}$, it is necessarily not in any maximal integral ideal. Therefore $\b \in \smc{O}^{\ast}$ because the complement of the union of all maximal integral ideals is exactly the set of units. Interchanging the roles of $\a$ and $\b$ shows that $\a \in \smc{O}^{\ast}$ as well. Hence $\frac{\a}{\b} \in \smc{O}^{\ast}$ which proves the reverse containment and the third equality follows.
    \end{proof}

    A noetherian integral domain is said to be \textbf{local}\index{local} if it has a unique maximal integral ideal. As we might expect, localizing at a prime will produce a local noetherian integral domain and hence justifies the term localization. For the localization $\smc{O}_{\mf{p}}$ of $\smc{O}$ at $\mf{p}$, set $\mf{m}_{\mf{p}} = \mf{p}_{\mf{p}}$. Then $\smc{O}_{\mf{p}}$ is a local noetherian integral domain with maximal integral ideal $\mf{m}_{\mf{p}}$. Indeed, $\smc{O}_{\mf{p}}$ is already a noetherian integral domain and it is clear that $\mf{m}_{\mf{p}}$ is an ideal of $\smc{O}_{\mf{p}}$. By \cref{prop:localization_prime_bijection}, the map
    \[
      \mf{q} \to \mf{q}_{\mf{p}},
    \]
    is a bijection between the primes of $\smc{O}$ contained in $\mf{p}$ the primes of $\smc{O}_{\mf{p}}$. As maximal integral ideals are necessarily prime, $\mf{m}_{\mf{p}}$ is the unique maximal integral ideal of $\smc{O}_{\mf{p}}$. This implies
    \[
      \smc{O}_{\mf{p}}^{\ast} = \smc{O}_{\mf{p}}-\mf{m}_{\mf{p}},
    \]
    because the complement of the union of all maximal integral ideals is exactly the set of units and $\mf{m}_{\mf{p}}$ is the only one. It follows that $\smc{O}_{\mf{p}}^{\ast}+\mf{m}_{\mf{p}} = \smc{O}_{\mf{p}}^{\ast}$. Indeed, the the reverse containment is trivial and the forward containment follows since otherwise $\smc{O}_{\mf{p}}^{\ast}+\mf{m}_{\mf{p}} \subseteq \mf{m}_{\mf{p}}$ which would imply $\smc{O}_{\mf{p}}^{\ast} \subseteq \mf{m}_{\mf{p}}$ and this is impossible because $\mf{m}_{\mf{p}}$ is maximal (hence proper). In other words, the sum of a unit of $\smc{O}_{\mf{p}}$ and an element of $\mf{m}_{\mf{p}}$ is a unit of $\smc{O}_{\mf{p}}$. Also, if $\mf{p}$ itself is maximal we can say more:
    ...
    \end{theorem}
    \begin{proof}
      Recall that $\smc{O}_{\mf{p}}$ is local for any prime $\mf{p}$ of $\smc{O}$ and so $\mf{m}_{\mf{p}}$ is the unique maximal integral ideal of $\smc{O}_{\mf{p}}$. For the forward implication, suppose $\smc{O}$ is a Dedekind domain. Then $\smc{O}_{\mf{p}}$ is as well by \cref{prop:localization_of_Dedekind_is_Dedekind}. Since primes are maximal in Dedekind domains, $\mf{m}_{\mf{p}}$ is also the only prime of $\smc{O}_{\mf{p}}$. By maximality, there exists $\pi \in \mf{m}_{\mf{p}}-\mf{m}_{\mf{p}}^{2}$ and hence $\mf{m}_{\mf{p}} = \pi\smc{O}_{\mf{p}}$ which further implies $\mf{m}_{\mf{p}}^{n} = \pi^{n}\smc{O}_{\mf{p}}$ for all $n \ge 1$. As these are the only integral ideals of $\smc{O}_{\mf{p}}$, we see that $\smc{O}_{\mf{p}}$ is a principal ideal domain and hence a discrete valuation ring. This proves the forward implication. For the reverse implication, suppose all of the localizations $\smc{O}_{\mf{p}}$ are discrete valuation rings. Then they are principal ideal domains and hence integrally closed by \cref{lemma:unique_factorization_domains_are_integrally_closed}. As the intersection of integrally closed rings is clearly integrally closed, it follows from \cref{prop:ring_is_intersection_of_all_localizations} that $\smc{O}$ is integrally closed and therefore an integrally closed integral domain by our assumptions. As $\smc{O}$ is noetherian by assumption, it remains to show that every prime of $\smc{O}$ is maximal. Since $\smc{O}$ is noetherian (which we recall is equivalent to every nonempty collection of ideals having a maximal element), $\mf{p} \subseteq \mf{q}$ for some maximal integral ideal $\mf{q}$ which is necessarily prime. Under the inclusion-preserving bijections in \cref{prop:localization_prime_bijection}, we have
      \[
        \mf{p} = \mf{p}_{\mf{q}} \cap \smc{O} = \mf{m}_{\mf{q}} \cap \smc{O} = \mf{q},
      \]
      where the middle equality holds because $\mf{p}_{\mf{q}} = \mf{m}_{\mf{q}}$ as $\smc{O}_{\mf{q}}$ has a unique maximal integral ideal which is also the unique prime (as we have seen). Hence $\mf{p}$ itself must be maximal.
    \end{proof}
  ...
  \section{Ramification}
    ...
    \begin{proposition}\label{prop:ramification_setup_respects_localization}
      Let $\mc{O}/\smc{O}$ be a Dedekind extension of a finite separable extension $L/K$ and let $D \subseteq \smc{O}-\{0\}$ be a multiplicative subset. Then for the Dedekind extension $\mc{O}D^{-1}/\smc{O}D^{-1}$, $\mf{P}_{D}$ is above $\mf{p}_{D}$ if and only if $\mf{P}$ is above $\mf{p}$. Moreover, there are isomorphisms
      \[
        \mc{O}D^{-1}/\mf{P}_{D} \cong \mc{O}/\mf{P} \quad \text{and} \quad \smc{O}D^{-1}/\mf{p}_{D} \cong \smc{O}/\mf{p}.
      \]
      Lastly, we have equalities
      \[
        f_{\mf{p}_{D}}(\mf{P}_{D}) = f_{\mf{p}}(\mf{P}) \quad \text{and} \quad e_{\mf{p}_{D}}(\mf{P}_{D}) = e_{\mf{p}}(\mf{P}).
      \]
    \end{proposition}
    \begin{proof}
      ...
      \[
        \mf{P}_{D} \cap \smc{O}D^{-1} = \mf{p}D^{-1}.
      \]
      Hence $\mf{P}_{D}$ is above $\mf{p}_{D}$. On the other hand, suppose $\mf{P}_{D}$ is above $\mf{p}_{D}$. Then $\mf{P}_{D} \cap \smc{O}D^{-1} = \mf{p}D^{-1}$ and applying the second bijection in \cref{prop:localization_prime_bijection} gives
      \[
        \mf{P} \cap \smc{O} = \mf{p}.
      \]
      ...
      \[
        \Phi:\mc{O} \to \mc{O}D^{-1}/\mf{P}_{D} \qquad \a \mapsto \a \tmod{\mf{P}_{D}},
      \]
      and
      \[
        \phi:\smc{O} \to \smc{O}D^{-1}/\mf{p}_{D} \qquad \a \mapsto \a \tmod{\mf{p}_{D}}.
      \]
      They are surjections since $\mc{O}\mf{P}_{D} = \mc{O}D^{-1}\mf{P}_{D}$ and $\smc{O}\mf{p}_{D} = \smc{O}D^{-1}\mf{p}_{D}$. Moreover, $\ker\Phi = \mf{P}$ and $\ker\phi = \mf{p}$. Therefore the isomorphisms follow from the first isomorphism theorem. This proves the second statement. In fact, these two isomorphisms together give
      \[
        f_{\mf{p}_{D}}(\mf{P}_{D}) = f_{\mf{p}}(\mf{P}),
      \]
      by the definition of the inertia degrees. Finally, we have
      \[
        e_{\mf{p}_{D}}(\mf{P}_{D}) = e_{\mf{p}}(\mf{P}),
      \]
      because the first statement implies that the powers of the prime factors of $\mf{p}_{D}\mc{O}D^{-1}$ are the same as those of $\mf{p}\mc{O}$.
    \end{proof}

    The inertia degrees and ramification indices satisfy a simple relationship to the degree of $L/K$. We describe this relationship in the following theorem:
  ...
  \section{\todo{The Ideal Norm}}
    ...
    \begin{proposition}\label{prop:ideal_norm_respects_localizations}
      ...
      \[
        \Norm_{\mc{O}D^{-1}/\smc{O}D^{-1}}(\mf{F}_{D}) = \Norm_{\mc{O}/\smc{O}}(\mf{F})_{D}.
      \]
    \end{proposition}
    \begin{proof}
      ...
      \[
        \Norm_{\mc{O}D^{-1}/\smc{O}D^{-1}}(\mf{P}_{D}) = \Norm_{\mc{O}/\smc{O}}(\mf{P})_{D}.
      \]
      This is immediate from \cref{prop:ramification_setup_respects_localization} and the definition of the ideal norm.
    \end{proof}

    We can now show that the ideal norm is compatible with the field trace:
  ...
  \section{\todo{The Different \& Discriminant}}
    ...
    \begin{proposition}\label{prop:localization_of_dual_is_dual_of_localization}
      ...
      \[
        (\mf{F}_{D})^{\vee} = (\mf{F}^{\vee})_{D}.
      \]
    \end{proposition}
    \begin{proof}
      For the forward containment, suppose $\l \in (\mf{F}_{D})^{\vee}$. Then $\Trace_{L/K}(\l\mf{F}_{D}) \subseteq \smc{O}D^{-1}$. As $D \subset K$, linearity of the trace implies $\l = \frac{\a}{\d}$ with $\Trace_{L/K}(\a\mf{F})\d \subseteq \smc{O}D^{-1}$ so that $\a \in \mf{F}^{\vee}$ and $\d \in D$. Hence $\l \in (\mf{F}^{\vee})_{D}$ and the forward containment follows. For the reverse containment, suppose $\frac{\a}{\d} \in (\mf{F}^{\vee})_{D}$. Then $\a \in \mf{F}^{\vee}$ so that $\Trace_{L/K}(\a\mf{F}) \subseteq \smc{O}$ and $\d \in D$. As $D \subset K$, linearity of the trace implies $\Trace_{L/K}(\l\mf{F}) \subseteq \smc{O}D^{-1}$ with $\l = \frac{\a}{\d}$. Hence $\frac{\a}{\d} \in (\mf{F}_{D})^{\vee}$ and the reverse containment holds.
    \end{proof}
    ...
    \end{proposition}
    \begin{proof}
      In view of \cref{prop:different_and_discriminant_respect_localization}, we may assume $\mc{O}/\smc{O}$ is a local Dedekind extension. Therefore $\smc{O}$ is a discrete valuation ring, $\mc{O}$ is a principal ideal domain, and $\mc{O}/\smc{O}$ admits an integral basis say $\a_{1},\ldots,\a_{n}$. Then $\mf{d}_{\mc{O}/\smc{O}}$ is a principal integral ideal where
      \[
        \mf{d}_{\mc{O}/\smc{O}} = d_{L/K}(\a_{1},\ldots,\a_{n})\smc{O}.
      \]
