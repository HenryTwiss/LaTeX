\chapter{\texorpdfstring{$L$}{L}-functions of Holomorphic, Automorphic, and Maass Forms}
  ...
  \section{Hecke-Maass \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition and Euler Product}
      ...
      \[
        f(z) = \sum_{n \ge 1}\rho_{f}(n)\left(\sqrt{y}K_{\nu}(2\pi ny)e^{2\pi inx}+\frac{\rho_{f}(-n)}{\rho_{f}(n)}\sqrt{y}K_{\nu}(2\pi ny)e^{-2\pi inx}\right),
      \]
      with $\rho_{f}(1) = 1$. Thus if $f$ is a Hecke eigenform, $f$ is even or odd by \cref{prop:Hecke_Maass_eigenform_even_or_odd}, and the Fourier-Whittaker series takes the form
      \[
        f(z) = \sum_{n \ge 1}\rho_{f}(n)\sqrt{y}K_{\nu}(2\pi ny)\SC(2\pi nx),
      \]
      and the $\rho_{f}(n)$ are the Hecke eigenvalues of $f$ normalized so that they are constant on average. The \textbf{Hecke-Maass $L$-series}\index{Hecke-Maass $L$-series} (respectively \textbf{Hecke-Maass $L$-function}\index{Hecke-Maass $L$-function} if it is an $L$-function) $L(s,f)$ of $f$ is defined by the following Dirichlet series:
      \[
        L(s,f) = \sum_{n \ge 1}\frac{\rho_{f}(n)}{n^{s}}.
      \]
      We will see that $L(s,f)$ is a Selberg class $L$-function if $f$ is a primitive Hecke-Maass eigenform. From now on, we make this assumption about $f$. The Ramanujan-Petersson conjecture for Maass forms is not know so $L(s,f)$ has not been proven to be a Selberg class $L$-function. As it is conjectured to be, throughout we will assume that the Ramanujan-Petersson conjecture for Maass forms holds. The Hecke relations and the Ramanujan-Petersson conjecture for Maass forms together imply that the coefficients $\rho_{f}(n)$ are multiplicative and satisfy $\rho_{f}(n) \ll_{\e} n^{\e}$. By \cref{prop:Dirichlet_series_Euler_product}, $L(s,f)$ is locally absolutely uniformly convergent for $\s > 1$ and admits the following infinite product expression:
      \[
        L(s,f) = \prod_{p}\left(\sum_{n \ge 0}\frac{\rho_{f}(p^{n})}{p^{ns}}\right).
      \]
      To determine the Euler product, the Hecke relations imply that the coefficients $\rho_{f}(n)$ satisfy
      \begin{equation}\label{equ:primitive_Hecke_eigenform_recurrence_for_coefficients_of_Maass_L-function}
        \rho_{f}(p^{n}) = \begin{cases} \rho_{f}(p^{n-1})\rho_{f}(p)-\chi(p)\rho_{f}(p^{n-2}) & \text{if $p \nmid N$}, \\ (\rho_{f}(p))^{n} & \text{if $p \mid N$}, \end{cases}
      \end{equation}
      ...
      \begin{align*}
        \sum_{n \ge 0}\frac{\rho_{f}(p^{n})}{p^{ns}} &= 1+\frac{\rho_{f}(p)}{p^{s}}+\sum_{n \ge 2}\frac{\rho_{f}(p^{n})}{p^{ns}} \\
        &= 1+\frac{\rho_{f}(p)}{p^{s}}+\sum_{n \ge 2}\frac{\rho_{f}(p^{n-1})\rho_{f}(p)-\chi(p)\rho_{f}(p^{n-2})}{p^{ns}} \\
        &= 1+\frac{\rho_{f}(p)}{p^{s}}+\frac{\rho_{f}(p)}{p^{s}}\sum_{n \ge 1}\frac{\rho_{f}(p^{n})}{p^{ns}}-\frac{\chi(p)}{p^{2s}}\sum_{n \ge 0}\frac{\rho_{f}(p^{n})}{p^{ns}} \\
        &= 1+\left(\frac{\rho_{f}(p)}{p^{s}}-\frac{\chi(p)}{p^{2s}}\right)\sum_{n \ge 0}\frac{\rho_{f}(p^{n})}{p^{ns}}.
      \end{align*}
      By isolating the sum we find
      \[
        \sum_{n \ge 0}\frac{\rho_{f}(p^{n})}{p^{ns}} = \left(1-\frac{\rho_{f}(p)}{p^{s}}+\frac{\chi(p)}{p^{2s}}\right)^{-1}.
      \]
      On the other hand, if $p \mid N$ we have
      \[
        \sum_{n \ge 0}\frac{\rho_{f}(p^{n})}{p^{ns}} = \sum_{n \ge 0}\frac{(\rho_{f}(p))^{n}}{p^{ns}} = \left(1-\rho_{f}(p)p^{-s}\right)^{-1}.
      \]
      Therefore
      \[
        L(s,f) = \prod_{p \nmid N}(1-\rho_{f}(p)p^{-s}+\chi(p)p^{-2s})^{-1}\prod_{p \mid N}(1-\rho_{f}(p)p^{-s})^{-1}.
      \]
      Letting $\a_{1}(p)$ and $\a_{2}(p)$ be the $p$-th Hecke roots of $f$, we can express $L(s,f)$ as the following degree $2$ Euler product:
      \[
        L(s,f) = \prod_{p}(1-\a_{1}(p)p^{-s})^{-1}(1-\a_{2}(p)p^{-s})^{-1}.
      ...
      \begin{equation}\label{equ:Hecke_Maass_L_function_coefficient_formula}
        \rho_{f}(p^{n}) = \frac{\a_{1}(p)^{n+1}-\a_{2}(p)^{n+1}}{\a_{1}(p)-\a_{2}(p)}.
      \end{equation}
    \subsection*{The Integral Representation}
      ...
      \[
        \left(\frac{\del}{\del x}^{\d_{f}}f\right)(z) = \sum_{n \ge 1}\rho_{f}(n)(2\pi in)^{\d_{f}}\sqrt{y}K_{\nu}(2\pi ny)\cos(2\pi nx).
      \]
      ...
      \begin{align*}
        \int_{0}^{\infty}\left(\frac{\del}{\del x}^{\d_{f}}f\right)(iy)y^{s-\frac{1}{2}+\d_{f}}\,\frac{dy}{y} &= \int_{0}^{\infty}\sum_{n \ge 1}\rho_{f}(n)(2\pi in)^{\d_{f}}K_{\nu}(2\pi ny)y^{s+\d_{f}}\,\frac{dy}{y} \\
        &= \sum_{n \ge 1}\rho_{f}(n)(2\pi in)^{\d_{f}}\int_{0}^{\infty}K_{\nu}(2\pi ny)y^{s+\d_{f}}\,\frac{dy}{y} &&\text{FTT} \\
        &= \sum_{n \ge 1}\frac{\rho_{f}(n)}{(2\pi)^{s}n^{s}}i^{\d_{f}}\int_{0}^{\infty}K_{\nu}(y)y^{s+\d_{f}}\,\frac{dy}{y} &&\text{$y \mapsto \frac{y}{2\pi n}$} \\
        &= \frac{\G\left(\frac{s+\d_{f}+\nu}{2}\right)\G\left(\frac{s+\d_{f}-\nu}{2}\right)}{2^{2-\d_{f}}\pi^{s}(-i)^{\d_{f}}}\sum_{n \ge 1}\frac{\rho_{f}(n)}{n^{s}} && \text{\cref{append:Special_Integrals}} \\
      ...
      \begin{align*}
        \left(\frac{\del}{\del x}^{\d_{f}}\w_{N}f\right)(z) &= \left(\frac{\del}{\del x}^{\d_{f}}\right)f\left(-\frac{1}{Nz}\right) \\
         &= \left(\frac{\del}{\del x}^{\d_{f}}\right)\sum_{n \ge 1}\rho_{f}(n)\sqrt{\frac{y}{N|z|^{2}}}K_{\nu}(2\pi ny)\SC\left(-2\pi n\frac{x}{N|z|^{2}}\right) \\
        &= (-N|z|^{2})^{-\d_{f}}\sum_{n \ge 1}\rho_{f}(n)(2\pi in)^{\d_{f}}\sqrt{\frac{y}{N|z|^{2}}}K_{\nu}\left(2\pi n\frac{y}{N|z|^{2}}\right)\cos\left(-2\pi n\frac{x}{N|z|^{2}}\right) \\
  ...
  \section{The Rankin-Selberg Method}
    ...
    \subsection*{Beyond Level \texorpdfstring{$1$}{1}}
      The Rankin-Selberg method is much more complicated for arbitrary primitive Hecke or Hecke-Maass eigenforms, but the argument is essentially the same. Let $f$ and $g$ both be primitive Hecke or Heck-Maass eigenforms with Fourier or Fourier-Whittaker coefficients $a_{f}(n)$ and $a_{g}(n)$ or $\rho_{f}(n)$ and $\rho_{g}(n)$ respectively. We suppose $f$ has weight $k$/type $\nu$, level $N$, and character $\chi$, and $g$ has weight $\ell$/type $\eta$, level $M$, and character $\psi$. 
      The $L$-series $L(s,f \x g)$ of $f$ and $g$ is defined by
      \[
        L(s,f \x g) = \sum_{n \ge 1}\frac{a_{f \x g}(n)}{n^{s}} = \sum_{n \ge 1}\frac{a_{f}(n)\conj{a_{g}(n)}}{n^{s}} \quad \text{or} \quad L(s,f \x g) = \sum_{n \ge 1}\frac{\rho_{f \x g}(n)}{n^{s}} = \sum_{n \ge 1}\frac{\rho_{f}(n)\conj{\rho_{g}(n)}}{n^{s}},
      \]
      according to if $f$ and $g$ are Hecke or Hecke-Maass eigenforms respectively. The \textbf{Rankin-Selberg convolution}\index{Rankin-Selberg convolution} $L(s,f \ox g)$ of $f$ and $g$ is defined by
      \[
        L(s,f \ox g) = \sum_{n \ge 1}\frac{a_{f \ox g}(n)}{n^{s}} = L(2s,\chi\conj{\psi})L(s,f \x g) \quad \text{or} \quad L(s,f \ox g) = \sum_{n \ge 1}\frac{\rho_{f \ox g}(n)}{n^{s}} = L(2s,\chi\conj{\psi})L(s,f \x g),
      \]
      where $a_{f \ox g}(n) = \sum_{n = m\ell^{2}}\chi\conj{\psi}(\ell^{2})a_{f}(m)\conj{a_{g}(m)}$ and $\rho_{f \ox g}(n) = \sum_{n = m\ell^{2}}\chi\conj{\psi}(\ell^{2})\rho_{f}(m)\conj{\rho_{g}(m)}$, according to if $f$ and $g$ are Hecke or Hecke-Maass eigenforms respectively. The following generalization was obtained by Jacquet in the 1970's as a consequence of a more general framework (see \cite{jacquet1970automorphic,jacquet1972automorphic}):
  ...
  \section{The Ramanujan-Petersson Conjecture on Average}
    ...
    \begin{proposition}\label{prop:Ramanujan_Petersson_average}
      ...
      \[
      \sum_{n \le X}|a_{f}(n)| \ll_{\e} X^{1+\e} \quad \text{or} \quad \sum_{n \le X}|\rho_{f}(n)| \ll_{\e} X^{1+\e},
      \]
      according to if $f$ is a Hecke or Heck-Maass eigenform respectively.
    \end{proposition}
    \begin{proof}
      By the Cauchy-Schwarz inequality,
      \begin{equation}\label{equ:Ramanujan_conjecture_on_average_1}
        \left(\sum_{n \le X}|a_{f}(n)|\right)^{2} \le X\sum_{n \le X}|a_{f}(n)|^{2} \quad \text{or} \quad \left(\sum_{n \le X}|\rho_{f}(n)|\right)^{2} \le X\sum_{n \le X}|\rho_{f}(n)|^{2},
      \end{equation}
      respectively. The Rankin-Selberg square $L(s,f \ox f)$ is locally absolutely uniformly convergent for $\s > \frac{3}{2}$. Therefore it still admits meromorphic continuation to $\C$ with a simple pole at $s = 1$. By Landau's theorem, the abscissa of absolute convergence of $L(s,f \ox f)$, and hence $L(s,f \x f)$ too, is $1$. By \cref{prop:Dirichlet_series_coefficient_size_on_average}, we have
      \[
        \sum_{n \le X}|a_{f}(n)|^{2} \ll_{\e} X^{1+\e} \quad \text{or} \quad \sum_{n \le X}|\rho_{f}(n)|^{2} \ll_{\e} X^{1+\e},
      \]
      respectively. Substituting this bound into \cref{equ:Ramanujan_conjecture_on_average_1}, we obtain
      \[
        \left(\sum_{n \le X}|a_{f}(n)|\right)^{2} \ll_{\e} X^{2+\e} \quad \text{or} \quad \left(\sum_{n \le X}|\rho_{f}(n)|\right)^{2} \ll_{\e} X^{2+\e},
      \]
      respectively. Taking the square root yields
      \[
        \sum_{n \le X}|a_{f}(n)| \ll_{\e} X^{1+\e} \quad \text{or} \quad \sum_{n \le X}|\rho_{f}(n)| \ll_{\e} X^{1+\e},
      \]
      respectively, proving both cases.
    \end{proof}

    The bound in \cref{prop:Ramanujan_Petersson_average} should be compared with the implication $a_{f}(n) \ll_{\e} n^{\e}$ or $\rho_{f}(n) \ll_{\e} n^{\e}$ that follows from the corresponding Ramanujan-Petersson conjecture. While \cref{prop:Ramanujan_Petersson_average} is not useful in the holomorphic form case, it is in the Maass form case. Indeed, recall that if $f$ is a primitive Hecke-Maass eigenform we needed to assume the Ramanujan-Petersson conjecture for Maass forms to ensure $\rho_{f}(n) \ll_{\e} n^{\e}$ so that $L(s,f)$ was locally absolutely uniformly convergent for $\s > 1$. However, \cref{prop:Dirichlet_series_convergence_polynomial_bound_average,prop:Ramanujan_Petersson_average} now together imply $L(s,f)$ is locally absolutely uniformly convergent for $\s > 1$ without this assumption. Often \cref{prop:Ramanujan_Petersson_average} is all that is needed for additional applications instead of outright assuming the Ramanujan-Petersson conjecture for Maass forms.