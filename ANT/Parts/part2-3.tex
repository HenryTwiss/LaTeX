\chapter{Types of Number Fields}
  ...
  \section{\todo{Cyclotomic Number Fields}}
    ...
    \begin{lemma}\label{lem:prime_power_cyclotomic_lemma}
      ...
      \[
        |d_{\Q(\w)/\Q}| = p^{p^{(e-1)(ep-e-1)}}.
      \]
    \end{lemma}
    \begin{proof}
      In view of the definition of $\Phi_{n}(x)$ and \cref{equ:cyclotomic_for_prime_power}, we have
      \[
        x^{p^{k-1}(p-1)}+x^{p^{k-1}(p-2)}+\cdots+1 = \prod_{k \in (\Z/n\Z)^{\ast}}(x-\w^{k}).
      \]
      Setting $x = 1$ gives
      \[
        p = \prod_{k \in (\Z/n\Z)^{\ast}}(1-\w^{k})
      \]
      The factors $1-\w^{k}$ are clearly algebraic integers because $\w$ is (as a consequence of \cref{prop:cyclotomic_is_irreducible}). \todo{xxx}
    \end{proof}
