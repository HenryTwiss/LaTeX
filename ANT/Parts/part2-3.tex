  ...
  \section{\todo{Cyclotomic Number Fields}}
    ...
    \begin{proof}
      ...
      \[
        p\mc{O}_{\w} = \mf{p}_{\w}^{\vphi(n)}.
      \]
      Since the degree of $\Q(\w)$ is $\vphi(n)$, the fundamental equality implies that $\mf{p}_{\w}$ is prime (otherwise any prime factor has ramification index at least $\vphi(n)$) and that $f_{p}(\mf{p}_{\w}) = 1$. This proves the first two statements. \todo{xxx}
    \end{proof}
