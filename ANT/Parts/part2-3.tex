\chapter{Types of Number Fields}
  ...
  \section{Quadratic Number Fields}
    We will classify and discuss the structure of quadratic number fields. We first show that quadratic number fields are exactly those where we adjoint the square-root of a square-free integer other than $0$ or $1$:
    ...
    From now on, we will write fundamental discriminants other than $1$ as $\D_{d}$ instead of $D$ to clarify the connection to quadratic number fields. We will now discuss the factorization of $p\mc{O}_{d}$ in the quadratic number field $\Q(\sqrt{d})$ for a prime $p$. Since $\Q(\sqrt{d})$ is a degree $2$ extension, the fundamental equality implies that $p$ is either inert, totally ramified, or totally split. In other words, there are three possible cases for how $p\mc{O}_{d}$ factors:
    \[
      p\mc{O}_{d} = \mf{p}, \quad p\mc{O}_{d} = \mf{p}^{2}, \quad \text{and} \quad p\mc{O}_{d} = \mf{p}\mf{q},
    \]
    according to if $p$ is inert, totally ramified, or totally split. Since $\Q(\sqrt{d})$ is monogenic by \cref{prop:ring_of_integers_quadratic}, the conductor is $\mc{O}_{K}$ and so we can describe the factorization using the Dedekind-Kummer theorem for every prime $p$. In particular, we will connect the prime factorization to the quadratic character $\chi_{\D_{d}}$ associated to the fundamental discriminant $\D_{d}$:
  ...
  \section{\todo{Cyclotomic Number Fields}}
    Let $\w$ be a primitive $n$-th root of unity, and recall the $n$-th cyclotomic field $\Q(\w)$. Also recall that $\Q(\w)$ is independent of the choice of primitive root $\w$ since $\Q(\w)$ contains all $n$-th roots of unity. We say that a number field $K$ is \textbf{cyclotomic}\index{cyclotomic} if $K$ is the $n$-th cyclotomic field for some $n \ge 1$. That is, $K = \Q(\w)$ for some primitive $n$-th root of unity $\w$. In any case, our aim is to study the structure of cyclotomic number fields $\Q(\w)$. Recall that $\Q(\w)$ is of degree $\vphi(n)$ because the minimal polynomial of $\w$ over $\Q$ is the $n$-th cyclotomic polynomial
    \[
      \Phi_{n}(x) = \prod_{\substack{1 \le k \le n \\ (k,n) = 1}}\left(x-e^{\frac{2\pi ik}{n}}\right),
    \]
    which is of degree $\vphi(n)$. Moreover, recall that the cyclotomic polynomials also satisfy the useful identities
    \[
      \Phi_{p^{k}}(x) = \frac{x^{p^{k}}-1}{x^{p^{k-1}}-1} \quad \text{and} \quad x^{n}-1 = \prod_{d \mid n}\Phi_{d}(x).
    \]
    For convenience, write $\mc{O}_{\w} = \mc{O}_{\Q(\w)}$ and set
    \[
      \mf{p}_{\w} = (1-\w)\mc{O}_{\w}.
    \]
    We will prove a useful lemma which shows that $\mf{p}_{\w}$ is a prime of $\Q(\w)$ and more in the case $n$ is a prime power:

    \begin{lemma}\label{lem:prime_power_cyclotomic_lemma}
      Let $\Q(\w)$ be the cyclotomic number field generated by a primitive $n$-th root of unity $\w$ with $n = p^{k}$ for some prime $p$ and $k \ge 1$. Then
      \[
        p\mc{O}_{\w} = \mf{p}_{\w}^{\vphi(n)}.
      \]
      In particular, $\mf{p}_{\w}$ is a prime above $p$ with $f_{p}(\mf{p}_{\w}) = 1$. Moreover, $1,\w,\ldots,\w^{\vphi(n)-1}$ is a basis for $\Q(\w)/\Q$ with
      \[
        |d_{L/K}| = p^{p^{(k-1)(kp-k-1)}}.
      \]
    \end{lemma}
    \begin{proof}
      \todo{xxx}
    ...
    With \cref{lem:prime_power_cyclotomic_lemma}, we can now prove that $\mc{O}_{\w}$ is monogenic:
