\chapter{Types of Number Fields}
  ...
  \section{\todo{Cyclotomic Number Fields}}
    ...
    \begin{lemma}\label{lem:prime_power_cyclotomic_lemma}
      Let $\Q(\w)$ be the cyclotomic number field generated by a primitive $n$-th root of unity $\w$ with $n = p^{e}$ for some prime $p$ and $e \ge 1$. Then
      \[
      ...
      \[
        \pm d_{\Q(\w)/\Q}(1,\w,\ldots,\w^{\vphi(n)-1}) = p^{p^{(e-1)(ep-e-1)}}.
      \]
    \end{lemma}
    \begin{proof}
      In view of the definition of $\Phi_{n}(x)$ and \cref{equ:cyclotomic_for_prime_power}, we have
      \[
        x^{\frac{n}{p}(p-1)}+x^{\frac{n}{p}(p-2)}+\cdots+1 = \prod_{k \in (\Z/n\Z)^{\ast}}(x-\w^{k}).
      \]
      Setting $x = 1$ gives
      \[
        p = \prod_{k \in (\Z/n\Z)^{\ast}}(1-\w^{k})
      \]
      The factors $1-\w^{k}$ are clearly algebraic integers because $\w$ is (as a consequence of \cref{prop:cyclotomic_is_irreducible}). Then
      \[
      ...
      \]
      is also an algebraic integer. This means $\e_{k}$ is a unit in $\mc{O}_{\w}$. So upon setting $\e = \prod_{k \in (\Z/n\Z)^{\ast}}\e_{k}$, we conclude that
      \[
        p = \e(1-\z)^{\vphi(n)},
      \]
      and therefore
      \[
        p\mc{O}_{\w} = \mf{p}_{\w}^{\vphi(n)}.
      \]
      Since the degree of $\Q(\w)$ is $\vphi(n)$, the fundamental equality implies that $\mf{p}_{\w}$ is prime (otherwise any prime factor has ramification index at least $\vphi(n)$) and that $f_{p}(\mf{p}_{\w}) = 1$. This proves the first two statements. For the last two statements, $1,\w,\ldots,\w^{\vphi(n)-1}$ is a basis for $\Q(\w)/\Q$ since $\w$ is a primitive element for $\Q(\w)/\Q$. Now let $\w_{1},\ldots,\w_{\vphi(n)}$ be the conjugates of $\w$ with $\w_{1} = \w$. Then
      \[
        \Phi_{n}(x) = \prod_{1 \le i \le \vphi(n)}(x-\w_{i}).
      \]
      Since $\Q(\w)/\Q$ is Galois, $\Gal(\Q(\w)/\Q)$ is exactly the set of $\Q$-embeddings of $\Q(\w)$ into $\conj{\Q}$. Then \cref{equ:Vandermonde_determinant_for_discriminant} and \cref{prop:formulas_for_trace_and_norm} give the first and last equalities in the following chain respectively:
      \[
        d(1,\l,\ldots,\l^{\vphi(n)}) = \pm\prod_{\substack{1 \le i,j \le \vphi(n) \\ i \neq j}}(\w_{i}-\w_{j})^{2} = \pm\prod_{1 \le i \le \vphi(n)}\Phi_{n}'(\w_{i}) = \pm\Norm_{\Q(\w)/\Q}(\Phi_{n}'(\w)).
      \]
      It remains to show $\Norm_{\Q(\w)/\Q}(\Phi_{n}'(\w)) = \pm p^{p^{(e-1)(ep-e-1)}}$. To this end, \cref{equ:cyclotomic_for_prime_power} implies
      \[
        (x^{\frac{n}{p}}-1)\Phi_{n}(x) = x^{n}-1.
      \]
      Differentiating gives
      \[
        \left(\frac{n}{p}-1\right)x^{\frac{n}{p}-2}\Phi_{n}(x)+\left(x^{\frac{n}{p}}-1\right)\Phi_{n}'(x) = nx^{n-1}-1.
      \]
      Now set $x = \w$ to obtain
      \[
        \left(\w^{\frac{n}{p}}-1\right)\Phi_{n}'(\w) = n\w^{n-1}-1,
      \]
      where $\w^{\frac{n}{p}}$ is a primitive $p$-th root of unity. \todo{xxx}
    \end{proof}
