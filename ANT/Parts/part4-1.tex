\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Analytic Data of \texorpdfstring{$L$}{L}-functions}
    ...
    Let $L(s,f)$ be an $L$-function with local roots $\a_{j}(p)$ at $p$ and let $k \ge 1$. We say that an $L$-function $L(s,\sym^{k}f)$ of degree $d_{f}$ is the $k$-th \textbf{symmetric power}\index{symmetric power} of $L(s,f)$ if it satisfies the following adjustment:
    \begin{itemize}
      ...
      \]
    \end{itemize}
    When $k$ is $2$, $3$, etc. we simply refer to $L(s,\sym^{k}f)$ as the \textbf{symmetric square}\index{symmetric square}, \textbf{symmetric cube}\index{symmetric cube}, etc. When $k \le d_{f}$, we say that an $L$-function $L(s,\ext^{k}f)$ of degree $d_{f}$ is the $k$-th \textbf{exterior power}\index{exterior power} of $L(s,f)$ if it satisfies the following adjustment:
    \begin{itemize}
    ...
    \begin{remark}
      It is not an easy matter to determine if the symmetric and exterior power $L$-functions exist even if $L(s,f)$ does.
    \end{remark}
    ...
    \begin{itemize}
      ...
      \[
        L(s,f \ox g) = \prod_{p}\prod_{\substack{1 \le j \le d_{f} \\ 1 \le \ell \le d_{g}}}\left(1-\a_{j}(p)\conj{\b_{\ell}(p)}p^{-s}\right)^{-1}.
      \]
    \end{itemize}
