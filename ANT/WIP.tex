...
\section*{Extra}
This will be accomplished by bounding the norm of every integral ideal. To get a bound that is not too large, we first require a lemma:
    ...
    \begin{proof}
      ...
      \]
      To compute the remaining integral, for nonnegative integers $n$ and $m$ and $t \ge 0$, we let
      \[
        X''_{n,m}(t) = \left\{(x_{1},\ldots,x_{n},u_{1},\ldots,u_{m}):\sum_{1 \le i \le n}x_{i}+\sum_{1 \le j \le m}u_{j} \le t\right\},
      \]
      and set 
      \[
        I_{n,m}(t) = \int_{X''_{n,m}(t)}u_{1} \cdots u_{m}\,dx_{1} \cdots dx_{n}\,du_{1} \cdots du_{m}.
      \]
      ...
      \[
        I_{n,m}(t) = t^{n+2m}I_{n,m}(1).
      \]
      Using this identity and the dominated convergence theorem, we have
      \[
        I_{n,m}(1) = \int_{0}^{1}I_{n-1,m}(1-x_{n})\,dx_{n} = \int_{0}^{1}(1-x_{n})^{n+2m}I_{n-1,m}(1)\,dx_{n} = \frac{1}{n+2m}I_{n-1,m}(1).
      \]
      Upon repeating this procedure $n-1$ times, we get
      \[
        I_{n,m}(1) = \frac{1}{(n+2m) \cdots (2m+1)}I_{0,m}(1).
      \]
      Similarly, we have
      \[
        I_{0,m}(1) = \int_{0}^{1}I_{0,m-1}(1-u_{m})\,du_{m} = \int_{0}^{1}(1-u_{m})^{2m}I_{0,m-1}(1)\,du_{m}
      \]
