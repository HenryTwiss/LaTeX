  \chapter{Preliminaries}\label{ch:Preliminaries}
    ...
    \section{Dirichlet Characters}
      ...
      \begin{proposition}\label{prop:Dirichlet_orthogonality_relations}
        ...
        \begin{enumerate}[label=(\roman*)]
          ...
          \item
          \[
            \frac{1}{\phi(m)}\sum_{\chi \in \what{(\Z/m\Z)^{\ast}}}\chi(a)\cchi(b) = \d_{a,b}.
          \]
        \end{enumerate}
      \end{proposition}
      \begin{proof}
      ...
      \begin{corollary}\label{cor:Dirichlet_orthogonality_relations}
        ...
        \begin{enumerate}[label=(\roman*)]
          ...
          \item
          \[
            \frac{1}{\phi(m)}\sum_{\chi \in \what{(\Z/m\Z)^{\ast}}}\chi(a) = \d_{a,1}.
          \]
        \end{enumerate}
      \end{corollary}
      \begin{proof}
    ...
    \section{Special Sums}
      ...
      \subsection*{Ramanujan Sums}
        ...
        \begin{proposition}\label{prop:Ramanujan_sum_evaluation}
          For any positive integer $m$ and any non-zero $b \in \Z$,
          \[
      ...
      \subsection*{Quadratic Gauss Sums}
        ...
        \begin{proof}
          ...
          \begin{enumerate}[label=(\roman*)]
            ...
            \item If $m = p$, then \cref{prop:Gauss_sum_equivalence_for_primes}, \cref{prop:Gauss_sum_reduction} (ii), and that quadratic characters are their own conjugate altogether imply the claim. Now let $r$ be a positive integer and assume by strong induction that the claim holds when $m = p^{r'}$ for all positive integers $r'$ such that $r' < r$. Then by (i), we have
            ...
            \end{equation}
            It now suffices to prove the claim when $m = p^{r}q^{s}$ where $q$ is another odd prime and $s$ is a positive integer. Then by (ii) and \cref{equ:quadratic_Gauss_sum_reduction_1}, we compute
            \begin{align*}
      ...
      \subsection*{Kloosterman \& Sali\'e Sums}
        ...
        \]
        where the $'$ in the second sum indicates that the sum is over all $a \pmod{c}$ such that $(a,c) = 1$. If either $n = 0$ or $m = 0$ then the Sali\'e sum reduces to a Gauss sum.
    \section{Decay \& Integral Transforms}
      ...
      \subsection*{The Fourier Transform}
        If there were two topics from analysis that every analytic number theorist needs, one is the Fourier transform and the other is the Poisson summation formula. They are quite closely related. Suppose $f:\R \to \C$ is absolutely integrable. The \textbf{Fourier transform}\index{Fourier transform} $\hat{f}:\R \to \C$ of $f$ is defined by
        \[
    ...
    \section{The Gamma Function}
      ...
      \end{equation}
      The second integral in \cref{equ:Gamma_integral_bounded_1} is absolutely uniformly bounded on compacta in this region by \cref{met:decay_compacta_integral}. As for the first integral, let $K$ be a compact set in the region $\Re(z) > 0$. Set $\b = \inf_{z \in K}\{\Re(z)\}$. Then
      ...
      \end{equation}
      \cref{equ:Gamma_integral_bounded_2} implies that the first integral in \cref{equ:Gamma_integral_bounded_1} is absolutely uniformly bounded on compacta too. Altogether, this means $\G(z)$ is as well. Also note that for real $z > 0$, $\G(z)$ is real. The most basic properties of $\G(z)$ are the following:
      ...
      We will also require a well-known approximation for the gamma function known as \textbf{Stirling's formula}\index{Stirling's formula} (see \cite{remmert2013classical} for a proof):
      ...
      If the real part of $z$ lies inside some comapct set then Stirling's formula gives a useful estimate showing that $\G(z)$ exhibits exponenetial decay as $z \to \infty$ (see \cite{remmert2013classical} for a proof):
