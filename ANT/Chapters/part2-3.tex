\chapter{The Theory of Automorphic \& Maass Forms}
  ...
  \section{Automorphic \& Maass Forms}
    ...
    \]
    If $\e$ and $k$ are clear from content we will suppress this dependencies accordingly. The cocycle condition implies that the slash operator is multiplicative. Indeed, if $\g,\g' \in \GL_{2}^{+}(\Q)$, then
    \begin{align*}
      ((f|_{\e,k}\g')|_{\e,k}\g)(z) &= \e(\g,z)^{-k}(f|_{\e,k}\g')(\g z) \\
      &= \e(\g',\g z)^{-k}\e(\g,z)^{-k}f(\g'\g z) \\
      &= \e(\g'\g,z)^{-k}f(\g'\g z) && \text{cocycle condition} \\
      &= (f|_{\e,k}\g'\g)(z).
    \end{align*}
  ...
  \section{Double Coset Operators}
    ...
    \[
      (f[\a]_{k})(z) = \det(\a)^{-1}(f|_{\e,k}\a)(z) = \det(\a)^{-1}\e(\a,z)^{-k}f(\a z),
    \]
    Moreover, $[\a]_{k}$ is multiplicative because the determinant and slash operator both are. Also, if $\g \in \G$ and we choose the representative with positive determinant, then the chain of equalities
    \[
  ...
  \section{Diamond \& Hecke Operators}
    ...
    We need one last operator since cusp forms have Fourier coefficients for all $n \neq 0$. Let $X:\mc{C}_{k,\nu}(\G_{1}(N)) \to \mc{C}_{k,\nu}(\G_{1}(N))$ be the linear operator defined by
    \[
      (Xf)(z) = f(-\conj{z}).
    \]
    ...
    \begin{proposition}\label{prop:parity_operator_properties}
      $T_{-1}$ is an involution on $\mc{C}_{k,\nu}(\G_{1}(N))$. In particular, $T_{-1}$ is an involution on $\mc{C}_{k,\nu}(N,\chi)$ as well. If $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ has Fourier coefficients $a_{n}(f)$, then
      \[
        (T_{-1}f)(z) = \sum_{n \ge 1}a_{-n}(f)\d(\nu,k)W_{\frac{k}{2},\nu}(4\pi |n|y)e^{2\pi inx}+a_{n}(f)\d(\nu,k)^{-1}W_{-\frac{k}{2},\nu}(4\pi |n|y)e^{-2\pi inx},
      \]
      is the Fourier series of $T_{-1}f$. Moreover, $T_{-1}$ commutes with the diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$, is normal, and its adjoint is given by
      \[
        T_{-1}^{\ast} = -T_{-1}.
      \]
    \end{proposition}
    ...
    \[
      a_{m}(f) = \l_{f}(m),
    \]
    for all $m \ge 1$. This normalization is called the \textbf{Hecke normalization}\index{Hecke normalization} of $f$. The \textbf{Petersson normalization}\index{Petersson normalization} of $f$ is where we normalize so that $\<f,f\> = 1$. From the spectral theorem we have an analogous corollary as for holomorphic forms:
  ...
  \section{Atkin-Lehner Theory}
    ...
    \]
    The elements of such subspaces are called \textbf{oldforms}\index{oldforms} and \textbf{newforms}\index{newforms} respectively. Just as with holomorphic forms, we need a useful operator. Recall the matrix
    \[
    ...
    \[
      (\w_{N}f)(z) = N(f[W_{N}]_{k})(z) = \e(W_{N},z)^{-k}f\left(W_{N}z\right) = \left(\frac{z}{|z|}\right)^{-k}f\left(-\frac{1}{Nz}\right).
    \]
    ...
    \begin{proposition}\label{prop:Atkin_Lehner_adjoint_diamond_Hecke_Maass}
      For all $m \ge 1$ with $(m,N) = 1$, the diamond operators $\<m\>$ and Hecke operators $T_{m}$ satisfy the following relations:
      \[
        \w_{N}\<m\>\w_{N}^{-1} = \<m\>^{\ast} \quad \text{and} \quad \w_{N}T_{m}\w_{N}^{-1} = T_{m}^{\ast}.
      \]
    \end{proposition}
    \begin{proof}
    ...
    It turns out that the spaces of oldforms and newforms are invariant under the diamond and Hecke operators (argue as in the proof of \cref{prop:old/new_subspaces_are_invariant_holomorphic} given in \cite{diamond2005first}):

    \begin{proposition}\label{prop:old/new_subspaces_are_invariant_Maass}
      The spaces $\mc{C}_{k,\nu}^{\mathrm{old}}(\G_{1}(N))$ and $\mc{C}_{k,\nu}^{\mathrm{new}}(\G_{1}(N))$ are invariant under the diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$ with $(m,N) = 1$.
    \end{proposition}
    ...
    \begin{lemma}\label{lem:the_main_lemma_for_newforms_Maass}
      If $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ with Fourier coefficients $a_{n}(f)$ and such that $a_{n}(f) = 0$ for all $n \ge 1$ whenever $(n,N) = 1$, then
      \[
    ...
    \begin{theorem}\label{thm:newforms_characterization_Maass}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \item If $g \in \mc{C}_{k,\nu}^{\mathrm{new}}(\G_{1}(N))$ is of the same weight, eigenvalue, level, and character as $f$, and has the same Hecke eigenvalues, then $g = cf$ for some nonzero $c \in \C$.
      \end{enumerate}
    ...
    Statement (i) in \cref{thm:newforms_characterization_Maass} implies that primitive Hecke-Maass eigenforms satisfy the Hecke relations for all $n,m \ge 1$. Statement (ii) is referred to as \textbf{multiplicity one}\index{multiplicity one} for Maass forms. As is the case for holomorphic forms, $\mc{C}_{k,\nu}^{\mathrm{new}}(\G_{1}(N))$ contains one element per ``eigenvalue'' where we mean a set of eigenvalues one for each Hecke operator $T_{m}$. As a consequence of multiplicity one, all primitive Hecke-Maass eigenforms are either even or odd:

    \begin{proposition}
      If $f \in \mc{C}_{k,\nu}(N,\chi)$ is a primitive Hecke-Maass eigenform, then $f$ is either even or odd.
    \end{proposition}
    \begin{proof}
      By \cref{prop:parity_operator_properties}, the parity Hecke operator $T_{-1}$ commutes with all Hecke operators and is an involution on $\mc{C}_{k,\nu}(N,\chi)$. Therefore if $f \in \mc{C}_{k,\nu}(N,\chi)$ is a primitive Hecke-Maass eigenform then $T_{-1}f$ is too by multiplicity one. In particular,
      \[
        T_{-1}f = cf,
      \]
      for some nonzero $c \in \C$. But as $T_{-1}$ is an involution we must have $c = \pm 1$.
    \end{proof}
    ...
    \]
    Note that if $f$ has Fourier coefficients $a_{n}(f)$, then $\conj{f}$ has Fourier coefficients $\conj{a_{n}(f)}$ by the conjugate symmetry of the Whittaker function (see \cref{append:Whittaker_Functions}). It turns out that $\conj{f}$ is indeed a cusp form:
