\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    This Fourier series is independent of the scaling matrix because $f(\s_{\mf{a}}z)$ is. Moreover, the sum is over all $n \in \Z$ since $f$ may be unbounded as $\Im(z) \to \infty$. The Fourier coefficients $a_{\mf{a}}(n,y)$ are mostly determined by $\D$. To see this, since $f$ is smooth we may differentiate the Fourier series of $f(\s_{\mf{a}}z)$ termwise. The fact that $f$ is an eigenfunction for $\D$ with eigenvalue $\l$ gives the ODE
    \[
    ...
    \]
    for some coefficients $a_{\mf{a}}(n)$ and $b_{\mf{a}}(n)$ possibly depending upon $s$ (see \cref{append:Bessel_Functions}). But $I_{s-\frac{1}{2}}(2\pi|n|y)$ has exponential growth (see \cref{lem:exponential_growth_I-Bessel_function}) and since $f$ has moderate growth at the cusps we must have $b_{\mf{a}}(n) = 0$ for all $n \neq 0$. If $n = 0$, then the differential equation is a second order linear ODE which in homogeneous form is
    \[
    ...
    \[
      f(\s_{\mf{a}}z) = \sum_{n \neq 0}a_{\mf{a}}(n)\sqrt{y}K_{s-\frac{1}{2}}(2\pi|n|y)e^{\frac{2\pi inx}{t}}.
    \]
    Now $K_{s-\frac{1}{2}}(2\pi|n|y) = o_{s}(e^{-2\pi|n|y})$ (see \cref{lem:moderate_decay_K-Bessel_function}), so that
    \[
    ...
    \]
    Accordingly, we say that $f$ exhibits \textbf{moderate decay at the cusps}\index{moderate decay at the cusps}. In particular, $f$ is bounded on $\H$. From now on, we will always assume that our congruence subgroups are reduced at infinity so that $t = 1$.
  ...
  \section{\texorpdfstring{$L^{2}$}{L{2}}-integrable Automorphic Functions}
    ...
    \]
    If the congruence subgroup is clear from context we will supress the dependence upon $\G$. The integral is absolutely bounded by the Cauchy–Schwarz inequality and that $f,g \in \mc{L}(\G)$. As $f$ and $g$ are automorphic, the integral is independent of the choice of fundamental domain. These two facts imply that the Petersson inner product is well-defined. We will continue to use this notion even if $f$ and $g$ do not belong to $\mc{L}(\G)$ provided the integral is absolutely bounded. Moreover, we have the following proposition:
