\chapter{The Theory of Automorphic \& Maass Forms}
  ...
  \section{Automorphic \& Maass Forms}
    ...
    \]
    The coefficients $a_{\mf{a}}(n,s)$ and $a_{\mf{a}}^{\pm}(s)$ are the only part of the Fourier series that actually depend on the implicit congruence subgroup $\G$. Using these coefficients, $f$ admits \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(s)y^{s}+a_{\mf{a}}^{-}(s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \[
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(s)y^{s}+a_{\mf{a}}^{-}(s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)e^{2\pi inx}.
    \]
    Moreover, property (v) implies that $f$ is a cusp form if and only if $a_{\mf{a}}^{+}(s) = 0$ and $a_{\mf{a}}^{-}(s) = 0$ for every cusp $\mf{a}$. We can also easily derive a bound for the size of the Fourier coefficients of cusp forms. Fix some $Y > 0$ and consider
    \[
  ...
  \section{Diamond \& Hecke Operators}
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ Fourier coefficients $a_{n}(f)$ and $a^{\pm}(f)$. Then for all primes $p$,
      \[
    ...
    \begin{proposition}\label{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ with Fourier coefficients $a_{n}(f)$ and $a^{\pm}(f)$. Then for $m \ge 1$ with $(m,N) = 1$,
      \[
    ...
    \]
    for all $n \ge 1$ and with $\pm$ according to if $f$ is even or odd. Moreover, $a^{\pm}(f) = 0$ if $f$ is odd. Thus the fourier series of $F$ takes the form
    \begin{align*}
      f(z) = a^{+}y^{\frac{1}{2}+\nu}&+a^{-}y^{\frac{1}{2}-\nu} \\
    ...
    \end{align*}
    with $\pm$ according to if $f$ is even or odd and for some potentially different Fourier coefficients $a^{\pm}(f)$ (they are zero if $f$ is odd). If the weight is zero, the Fourier series drastically simplifies via, \cref{thm:Whittaker_special_cases}, that $\d(\nu,0) = 1$, and the exponential identities for sine and cosine, so that we obtain
    \[
