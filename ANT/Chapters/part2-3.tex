\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \begin{enumerate}[label=(\roman*)]
      ...
      \item $(f|_{\e,k}\a)(z) = o(e^{2\pi y})$ for all $\a \in \PSL_{2}(\Z)$.
    \end{enumerate}
    ...
    \begin{enumerate}[label=(\roman*)]
      ...
      \]
    \end{enumerate}
    Property (ii) is called the \textbf{automorphy condition}\index{automorphy condition} and we say that $f$ is \textbf{automorphic}\index{automorphic}. In particular, $f$ admits a Fourier series and is a function on $\mc{F}_{\G}$. The automorphy condition can equivalently be expressed as
    \[
      f(\g z) = \chi(\g)\e(\g,z)^{k}f(z).
    \]
    In property (iii), we will often let $s \in \C$ be such that $\l = s(1-s)$ and write $\l = \l(s)$ so that the eigenvalue can be determined by $s$. Property (iv) is called the \textbf{growth condition}\index{growth condition} for $f$ and we say $f$ is of \textbf{moderate growth at the cusps}\index{moderate growth at the cusps}. Clearly we only need to verify the growth condition on a set of scaling matrices for the cusps. As $\GL_{2}^{+}(\Q) \subset \PSL_{2}(\Z)\GL_{2}^{+}(\Q)$, it follows that $(f|_{\e,k}\a)(z) = o(e^{2\pi y})$ for all $\a \in \GL_{2}^{+}(\Q)$ and this condition is equivalent to the growth condition too. It turns turns out that Property (i) is implied by (iii). This is because $\D_{k}$ is an elliptic operator and any eigenfunction of an elliptic operator is automatically real-analytic and hence smooth (see \cite{evans2022partial} for a proof in the weight zero case and \cite{cohenmodular2017} for notes on the general case). 
    ...
    \]
    for some coefficients $a_{\mf{a}}^{+}(0)$ and $a_{\mf{a}}^{-}(0)$ possibly depending upon $s$. The coefficients $a_{\mf{a}}(n)$, $a_{\mf{a}}^{+}(0)$, and $a_{\mf{a}}^{-}(0)$ are the only part of the Fourier series that actually depend on the implicit congruence subgroup $\G$. Using these coefficients, $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp}:
    \[
    ...
    \]
    It follows that $f(\s_{\mf{a}}z)$ exhibits moderate decay for every cusp $\mf{a}$. Accordingly, we say that $f$ exhibits \textbf{moderate decay at the cusps}\index{moderate decay at the cusps}. Note that $f$ is then bounded on $\H$. For weight zero Maass forms, we can say more about the Fourier series. Indeed, \cref{thm:Whittaker_special_cases} implies that the Fourier series at the $\mf{a}$ cusp takes the form
    \[
  ...
  \section{Eisenstein \& Poincar\'e Series}
    ...
    \]
    If $k = 0$, $\chi$ is the trivial character, or $\mf{a} = \infty$, we will drop these dependences accordingly. We first show that $E_{k,\chi,\mf{a}}(z,s)$ is well-defined. To see that $\cchi(\g)$ is independent of $\g$, recall $\G_{\mf{a}} = \s_{\mf{a}}\G_{\infty}\s_{\mf{a}}^{-1}$ and let $\g' = \s_{\mf{a}}\eta_{\infty}\s_{\mf{a}}^{-1}\g$ with $\eta_{\infty} = \begin{psmallmatrix} 1 & n \\ 0 & 1 \end{psmallmatrix} \in \G_{\infty}$. Then
    \[
    ...
    \]
    where the last equality follows because $\e(\eta_{\infty},\s_{\mf{a}}^{-1}\g z) = 1$ (as $j(\eta_{\infty},\s_{\mf{a}}^{-1}\g z) = 1$). Thus $\e(\s_{\mf{a}}^{-1}\g,z)$ is also independent of the scaling matrix used. Altogether, this shows $E_{k,\chi,\mf{a}}(z,s)$ is independent of the scaling matrix used and thus is well-defined. We now show $E_{k,\chi,\mf{a}}(z,s)$ is locally absolutely uniformly convergent for $z \in \H$ and $\Re(s) > 1$. To see this, applying the Bruhat decomposition to $\s_{\mf{a}}^{-1}\G_{\mf{a}}\backslash\G = \G_{\infty}\backslash\s_{\mf{a}}^{-1}\G$ yields
    \[
      E_{k,\chi,\mf{a}}(z,s) \ll \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{\Im(z)^{\s}}{|cz+d|^{2\s}}.
    \]
    The latter series is locally absolutely uniformly convergent for $z \in \H$ and $\s > 1$ by \cref{prop:general_lattice_sum_convergence_for_two_variables}. Therefore $E_{k,\chi,\mf{a}}(z,s)$ is too. To see that it is automorphic on $\GH$, let $\g \in \G$. Then we make a short computation:
    \begin{align*}
    ...
    \end{align*}
    Now we show that $E_{k,\chi,\mf{a}}(z,s)$ is an eigenfunction for $\D_{k}$. Observe that
    \[
    ...
    \]
    which shows $E_{k,\chi,\mf{a}}(z,s)$ is also an eigenfunction for $\D_{k}$ with eigenvalue $\l(s)$. We now verify the growth condition for $E_{k,\chi,\mf{a}}(z,s)$. Let $\s_{\mf{b}}$ be a scaling matrix for the cusp $\mf{b}$. The cocycle condition and the Bruhat decomposition applied to $\s_{\mf{a}}^{-1}\G_{\mf{a}}\backslash\G\s_{\mf{b}} = \G_{\infty}\backslash\s_{\mf{a}}^{-1}\G\s_{\mf{b}}$ together give
    \[
    ...
    \]
    Notice that the first sum converges provided $\s > 1$. Moreover, the exact same argument as for holomorphic Eisenstein series shows that the second sum converges in this region as well. So for all $\Im(z) \ge 1$ and $\s > 1$, we have
    \[
      \e(\s_{\mf{b}},z)^{-k}E_{k,\chi,\mf{a}}(\s_{\mf{b}}z,s) \ll \Im(z)^{\s} = o(e^{2\pi\Im(z)}),
    \]
    provided $\Im(z) \ge 1$ and $\s > 1$. This verifies $E_{k,\chi,\mf{a}}(z,s)$ is of moderate growth at the cusps. We collect this work as a theorem:

    \begin{theorem}
      Let $k \ge 0$, $\chi$ be a Dirichlet character with conductor $q \mid N$, and $\mf{a}$ be a cusp of $\GH$. For $\Re(s) > 1$, the Eisenstein series
      \[
        E_{k,\chi,\mf{a}}(z,s) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\Im(\s_{\mf{a}}^{-1}\g z)^{s},
      \]
    ...
    \]
    and hence $e^{2\pi im\s_{\mf{a}}^{-1}\g z}$ is independent of the representative $\g$ used. It follows that $P_{m,k,\chi,\mf{a}}(z,s)$ is well-defined. We claim $P_{m,k,\chi,\mf{a}}(z,s)$ is also locally absolutely uniformly convergent for $z \in \H$ and $\Re(s) > 1$. To see this, first recall that $|e^{2\pi im\s_{\mf{a}}^{-1}\g z}| = e^{-2\pi m\Im(\s_{\mf{a}}^{-1}\g z)} < 1$. Then the Bruhat decomposition applied to $\s_{\mf{a}}^{-1}\G_{\mf{a}}\backslash\G = \G_{\infty}\backslash\s_{\mf{a}}^{-1}\G$ yields
    \[
      P_{k,\chi,\mf{a}}(z,s) \ll \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{\Im(z)^{\s}}{|cz+d|^{2\s}},
    \]
    and this latter series is locally absolutely uniformly convergent for $z \in \H$ and $\s > 1$ by \cref{prop:general_lattice_sum_convergence_for_two_variables}. Hence the same holds for $P_{m,k,\chi,\mf{a}}(z,s)$. Moreover, automorphy for $P_{m,k,\chi,\mf{a}}(z,s)$ is verified in exactly the same way as for $E_{k,\chi,\mf{a}}(z,s)$. As for the growth condition, let $\s_{\mf{b}}$ be a scaling matrix for the cusp $\mf{a}$. Then the bound $|e^{2\pi im\s_{\mf{a}}^{-1}\g z}| = e^{-2\pi m\Im(\s_{\mf{a}}^{-1}\g z)} < 1$, cocycle condition, and the Bruhat decomposition applied to $\s_{\mf{a}}^{-1}\G_{\mf{a}}\backslash\G = \G_{\infty}\backslash\s_{\mf{a}}^{-1}\G$ together give
    \[
      \e(\s_{\mf{b}},z)^{-k}P_{k,\chi,\mf{a}}(\s_{\mf{b}}z,s) \ll \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{\Im(z)^{\s}}{|cz+d|^{2\s}},
    \]
    and we may proceed in exactly the same way as for $E_{k,\chi,\mf{a}}(z,s)$. We collect this work as a theorem:

    \begin{theorem}
      Let $m \ge 1$, $k \ge 0$, $\chi$ be a Dirichlet character with conductor $q \mid N$, and $\mf{a}$ be a cusp of $\GH$. For $\Re(s) > 1$, the Poincar\'e series
      \[
        P_{m,k,\chi,\mf{a}}(z,s) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\Im(\s_{\mf{a}}^{-1}\g z)^{s}e^{2\pi im\s_{\mf{a}}^{-1}\g z},
      \]
  ...
  \section{\texorpdfstring{$L^{2}$}{L{2}}-integrable Automorphic Functions}
    ...
    \]
    Moreover, if $\chi$ is the trivial character of if $k = 0$, we will suppress the dependences accordingly. We will also write $\mc{A}_{k}(N,\chi)$ and $\mc{L}_{k}(N,\chi)$ respectively when $\G = \G_{0}(N)$. Since $f$ is automorphic, the integral $\G$-invariant and hence is independent of the choice of fundamental domain. Since this is an $L^{2}$-space, $\mc{L}_{k}(\G,\chi)$ is an induced inner product space (because the parallelogram law is satisfied). In particular, for any $f,g \in \mc{L}_{k}(\G,\chi)$ we define their \textbf{Petersson inner product}\index{Petersson inner product} to be
    \[
    ...
    \[
      \l = s(1-s) = \frac{1}{4}-\nu^{2} = \frac{1}{4}+r^{2}.
    \]
    We now introduce variations of the Eisenstein and Poincar\'e series. The Eisenstein series $E_{k,\chi,\mf{a}}(z,s)$ are not $L^{2}$-integrable over $\mc{F}_{\G}$ and so do not belong to $\mc{L}_{k}(\G,\chi)$. To obtain integrable automorphic functions we will argument these Eisenstein series so that their growth is subdued. Let $\psi(y)$ be a smooth function such that $\psi(y) \ll_{\e} y^{1+\e}$ as $y \to 0$ and let $\s_{\mf{a}}$ be a scaling matrix for the $\mf{a}$ cusp. We define the \textbf{(automorphic) Eisenstein series}\index{(automorphic) Eisenstein series} $E_{k,\chi,\mf{a}}(z,\psi)$ of weight $k$ and character $\chi$ on $\GH$ at the $\mf{a}$ cusp and with respect to $\psi(y)$ by
    \[
      E_{k,\chi,\mf{a}}(z,\psi) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\psi(\Im(\s_{\mf{a}}^{-1}\g z)).
    \]
    If $k = 0$, $\chi$ is the trivial character, or $\mf{a} = \infty$, we will drop these dependences accordingly. Moreover, if $\psi(y)$ is a bump function, we say that $E_{k,\chi,\mf{a}}(z,\psi)$ is \textbf{incomplete}\index{incomplete}. We first show $E_{k,\chi,\mf{a}}(z,\psi)$ is well-defined. This is easy as we have already showed that $\cchi(\g)$ is independent of the representative $\g$ and $\e(\s_{\mf{a}}^{-1}\g,z)^{-k}$ and $\Im(\s_{\mf{a}}^{-1}\g z)$ are independent of the scaling matrix used when discussing $E_{k,\chi,\mf{a}}(z,s)$. Thus $E_{k,\chi,\mf{a}}(z,\psi)$ is well-defined. We claim $E_{k,\chi,\mf{a}}(z,\psi)$ is also locally absolutely uniformly convergent. To see this, we require a technical lemma:
    ...
    Now we are ready to show that $E_{k,\chi,\mf{a}}(z,\psi)$ is locally absolutely uniformly convergent. Let $K$ be a compact subset of $\H$. Then it suffices to show $E_{k,\chi,\mf{a}}(z,\psi)$ is uniformly convergent on $K$. Applying the Bruhat decomposition to $\s_{\mf{a}}^{-1}\G_{\mf{a}}\backslash\G = \G_{\infty}\backslash\s_{\mf{a}}^{-1}\G$ gives
    \[
      E_{k,\chi,\mf{a}}(z,\psi) \ll \psi(\Im(z))+\sum_{\substack{(c,d) \in \Z^{2}-\{\mathbf{0}\}}}\psi\left(\frac{\Im(z)}{|cz+d|^{2}}\right).
    \]
    ...
    \]
    and this last series is locally absolutely uniformly convergent for $z \in \H$ by \cref{prop:general_lattice_sum_convergence_for_two_variables}. It follows that $E_{k,\chi,\mf{a}}(z,\psi)$ is too. Actually, we can do better if $\psi(y)$ is a bump function since finitely many terms will be nonzero. Indeed, $\s_{\mf{a}}^{-1}\G$ is a Fuchsian group because it is a subset of the modular group. So using $\s_{\mf{a}}^{-1}\G_{\mf{a}}\backslash\G = \G_{\infty}\backslash\s_{\mf{a}}^{-1}\G$ we see that $\{\s_{\mf{a}}^{-1}\g z:\g \in \G_{\mf{a}}\backslash\G\}$ is discrete. Since $\Im(z)$ is an open map it takes discrete sets to discrete sets so that $\{\Im(\s_{\mf{a}}^{-1}\g z):\g \in \G_{\mf{a}}\backslash\G\}$ is also discrete. Now $\psi(\Im(\s_{\mf{a}}^{-1}\g z))$ is nonzero if and only if $\Im(\s_{\mf{a}}^{-1}\g z) \in \mathrm{Supp}(\psi)$ and $\{\Im(\s_{\mf{a}}^{-1}\g z):\g \in \G_{\mf{a}}\backslash\G\} \cap \mathrm{Supp}(\psi)$ is finite as it is a discrete subset of a compact set (since $\psi(y)$ has compact support). Hence finitely many of the terms are nonzero. Moreover, the compact support of $\psi(y)$ then implies that $E_{k,\chi,\mf{a}}(z,\psi)$ is also compactly supported (since the function $\psi(\Im(\s_{\mf{a}}^{-1}\g z))$ is continuous and $\C$ is Hausdorff) and hence bounded on $\H$. As a consequence, $E_{k,\chi,\mf{a}}(z,\psi)$ is $L^{2}$-integrable. Also, $E_{k,\chi,\mf{a}}(z,\psi)$ is automorphic on $\GH$ and the automorphy is proved in exactly the same way as for the Eisenstein series $E_{k,\chi,\mf{a}}(z,s)$. We collect this work as a theorem:

    \begin{theorem}
      Let $k \ge 0$, $\chi$ be a Dirichlet character with conductor $q \mid N$, $\mf{a}$ be a cusp of $\GH$, and $\psi(y)$ be a smooth function such that $\psi(y) \ll_{\e} y^{1+\e}$ as $y \to 0$. The Eisenstein series
      \[
        E_{k,\chi,\mf{a}}(z,\psi) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\psi(\Im(\s_{\mf{a}}^{-1}\g z)),
      \]
    ...
    \]
    If $k = 0$, $\chi$ is the trivial character, or $\mf{a} = \infty$, we will drop these dependences accordingly. Moreover, if $\psi(y)$ is a bump function, we say that $P_{m,k,\chi,\mf{a}}(z,\psi)$ is \textbf{incomplete}\index{incomplete}. We claim that $P_{m,k,\chi,\mf{a}}(z,\psi)$ is well-defined. This is easy to see as we have already showed $\cchi(\g)$ is independent of the representative $\g$ and the functions $\e(\s_{\mf{a}}^{-1}\g,z)^{-k}$, $\Im(\s_{\mf{a}}^{-1}\g z)$, and $e^{2\pi im\s_{\mf{a}}^{-1}\g z}$ are all independent of the scaling matrix used. Just as for $E_{k,\chi,\mf{a}}(z,\psi)$, $P_{m,k,\chi,\mf{a}}(z,\psi)$ is locally absolutely uniformly convergent and finitely many term terms are nonzero if $\psi(y)$ is a bump function. In this latter case, $P_{m,k,\chi,\mf{a}}(z,\psi)$ is $L^{2}$-integrable. Lastly, automorphy for $P_{m,k,\chi,\mf{a}}(z,\psi)$ is verified in exactly the same way as for $E_{k,\chi,\mf{a}}(z,s)$. We collect this work as a theorem:

    \begin{theorem}
      Let $m \ge 1$, $k \ge 0$, $\chi$ be a Dirichlet character with conductor $q \mid N$, $\mf{a}$ be a cusp of $\GH$, and $\psi(y)$ be a smooth function such that $\psi(y) \ll_{\e} y^{1+\e}$ as $y \to 0$. The Poincar\'e series
      \[
        P_{m,k,\chi,\mf{a}}(z,\psi) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\psi(\Im(\s_{\mf{a}}^{-1}\g z))e^{2\pi im\s_{\mf{a}}^{-1}\g z},
      \]
    ...
    Returning to the Eisenstein series, we will compute their inner product against an arbitrary element of $\mc{L}_{k}(\G,\chi)$. Let $f \in \mc{L}_{k}(\G,\chi)$ and consider $E_{k,\chi,\mf{a}}(\cdot,\psi)$. We compute their inner product as follows:
    \begin{align*}
      ...
      &= \frac{1}{V_{\G}}\int_{\s_{\mf{a}}^{-1}\g\mc{F}_{\G}}\sum_{\g \in \GG}\chi(\g)\conj{\e(\s_{\mf{a}}^{-1}\g,\g^{-1}\s_{\mf{a}}z)^{-k}}f(\g^{-1}\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu && \text{$z \to \g^{-1}\s_{\mf{a}}z$} \\
      &= \frac{1}{V_{\G}}\int_{\g\mc{F}_{\G}}\sum_{\g \in \GG}\chi(\g)\conj{\e(\s_{\mf{a}}^{-1}\g,\g^{-1}\s_{\mf{a}}z)^{-k}}f(\g^{-1}\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu && \text{$\s_{\mf{a}}$ automorphism} \\
      &= \frac{1}{V_{\G}}\int_{\g\mc{F}_{\G}}\sum_{\g \in \GG}\chi(\g)\conj{\left(\frac{\e(I,z)}{\e(\g^{-1},\s_{\mf{a}}z)}\right)^{-k}}f(\g^{-1}\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu && \text{cocycle condition} \\
      &= \frac{1}{V_{\G}}\int_{\g\mc{F}_{\G}}\sum_{\g \in \GG}\chi(\g)\e(\g^{-1},\s_{\mf{a}}z)^{-k}f(\g^{-1}\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu && \text{$\conj{\e(\g,z)} = \e(\g,z)^{-1}$} \\
      &= \frac{1}{V_{\G}}\int_{\g\mc{F}_{\G}}\sum_{\g \in \GG}\chi(\g^{-1})^{-1}\e(\g^{-1},\s_{\mf{a}}z)^{-k}f(\g^{-1}\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu && \text{$\chi(\g) = \chi(\g^{-1})^{-1}$} \\
      &= \frac{1}{V_{\G}}\int_{\g\mc{F}_{\G}}\sum_{\g \in \GG}f(\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu  && \text{automorphy} \\
      &= \frac{1}{V_{\G}}\int_{\mc{F}_{\G}}\sum_{\g \in \GG}f(\s_{\mf{a}}\g z)\conj{\psi(\Im(\g z))}\,d\mu \\
      &= \frac{1}{V_{\G}}\int_{\G_{\infty}\backslash\H}f(\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu && \text{unfolding} \\
      &= \frac{1}{V_{\G}}\int_{0}^{\infty}\left(\int_{0}^{1}f(\s_{\mf{a}}(x+iy))\,dx\right)\conj{\psi(y)}\,\frac{dy}{y^{2}}.
    \end{align*}
    The inner integral is precisely the constant term in the Fourier series of $f$ at the $\mf{a}$ cusp. It follows that $f$ is orthogonal to $\mc{E}_{k}(\G,\chi)$ if and only if $f$ is a cusp form. This illustrates an important interaction between incomplete Eisenstein series and Maass cusp forms. To state it in another way, let $\mc{E}_{k}(\G,\chi)$ and $\mc{C}_{k}(\G,\chi)$ denote the subspaces of $\mc{A}_{k}(\G,\chi)$ generated by such forms respectively. If $\chi$ is the trivial character or if the weight $k$ is zero, we will suppress these dependences. Note that they are also subspaces of $\mc{B}_{k}(\G,\chi)$. Moreover, let $\mc{C}_{k,\nu}(\G,\chi)$ and $\mc{A}_{k,\nu}(\G,\chi)$ denote the corresponding subspaces of $\mc{C}_{k}(\G,\chi)$ and $\mc{A}_{k}(\G,\chi)$ whose type is $\nu$. For all of these spaces, if $\chi$ is the trivial character or if the weight $k$ is zero, we will suppress these dependences as well. So if $f \in \mc{B}_{k}(\G,\chi)$, then
    \[
  ...
  \section{Spectral Theory of the Laplace Operator}
    ...
    \begin{theorem}\label{thm:functional_equation_of_Eisenstein_series}
      ...
      \[
        \Phi(s)\Phi(1-s) = I.
      \]
      Moreover, it is unitary on the line $\Re(s) = \frac{1}{2}$ and hermitian if $s$ is real.
    \end{theorem}

    As $\Phi(s)$ is symmetric by \cref{thm:functional_equation_of_Eisenstein_series}, if $\mf{a} = \infty$ or $\mf{b} = \infty$, we will suppress these dependences for $\tau_{\mf{a},\mf{b}}$. Understanding the poles of $\tau_{\mf{a},\mf{b}}$ are also important for understanding the poles of the Eisenstein series $E_{k,\chi,\mf{a}}(z,s)$ (see \cite{iwaniec2002spectral} for a proof in the weight zero case and \cite{cohenmodular2017} for notes on the general case):
  ...
  \section{Atkin–Lehner Theory}
    ...
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{equ:lifting_operator_holomorphic} with $k = 0$, smoothness replacing holomorphy, automorphy replacing modularity, and the analogous growth condition for Maass forms, the only piece left to verify is that $f[\a_{d}]$ is an eigenfunction for $\D$ with eigenvalue $\l$ if $f$ is. This is easy since
      \[
    ...
    \]
    with $\det(W_{N}) = N$. We define the \textbf{Atkin–Lehner involution}\index{Atkin–Lehner involution} $\w_{N}$ to be the linear operator on $\mc{C}_{\nu}(\G_{1}(N))$ given by
    \[
    ...
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{prop:Atkin_Lehner_holomorphic} with $k = 0$, smoothness replacing holomorphy, automorphy replacing modularity, and the analogous growth condition for Maass forms, the only piece left to verify is that $\w_{N}f$ is an eigenfunction for $\D$ with eigenvalue $\l$ if $f$ is. This is easy since
      \[
  ...
  \section{The Ramanujan-Petersson Conjecture}
    ...
    \]
    Since $f$ is a cusp form, this integral is locally absolutely uniformly bounded by \cref{prop:decay_compacta_integral}. Moreover, this integral can be expressed as
    \[
      \int_{Y}^{\infty}\int_{0}^{1}\sum_{n,m \ge 1}\l_{f}(n)\conj{\l_{f}(m)}K_{\nu}(2\pi ny)\conj{K_{\nu}(2\pi my)}\SC(2\pi nx)\conj{\SC(2\pi mx)}\,\frac{dx\,dy}{y}.
    \]
    Appealing to the dominated convergence theorem, we can interchange the sum and the two integrals. Upon making this interchange, the identity
    \[
      \int_{0}^{1}\SC(2\pi nx)\conj{\SC(2\pi mx)}\,dx = \d_{n-m,0},
    \]
    implies that the inner integral cuts off all of the terms except the diagonal $n = m$, resulting in
    \[
      \sum_{n \ge 1}\int_{Y}^{\infty}|\l_{f}(n)|^{2}|K_{\nu}(2\pi ny)|^{2}|\SC(2\pi ny)|^{2}\,\frac{dx\,dy}{y}.
    \]
    In particular, we see that this is a sum of nonnegative terms and each integral is bounded. Retaining only a single term in the sum, we have
    \[
      |\l_{f}(n)|^{2}\int_{Y}^{\infty}|K_{\nu}(2\pi ny)|^{2}|\SC(2\pi ny)|^{2}\,\frac{dx\,dy}{y} \ll \int_{Y}^{\infty}\int_{0}^{1}|f(x+iy)|^{2}\,\frac{dx\,dy}{y^{2}}.
    \]
    Now $f$ is bounded on $\H$ so that
    \[
    ...
    \[
      |\l_{f}(n)|^{2}\int_{Y}^{\infty}K_{\nu}(2\pi|n|y)|^{2}|\SC(2\pi ny)|^{2}\,\frac{dx\,dy}{y} \ll \frac{1}{Y}.
    \]
    Since the integral is bounded, taking $Y = \frac{1}{n}$ results in
    \[
      \l_{f}(n) \ll n^{\frac{1}{2}}.
    \]
    This bound is known as the \textbf{Hecke bound}\index{Hecke bound} for Maass forms. However, the Ramanujan-Petersson conjecture and the Hecke relations together give the bound $\l_{f}(n) \ll \s_{0}(n)n^{\frac{1}{2}} \ll_{\e} n^{\frac{1}{2}+\e}$ (recall \cref{prop:sum_of_divisors_growth_rate}). It turns out that the Ramanujan-Petersson conjecture is tightly connected to another conjecture of Selberg about the smallest possible eigenvalue of weight zero Maass form on $\GH$. Note that the possible eigenvalues are discrete by \cref{thm:the_full_spectral_resolution} and so there exists a smallest eigenvalue. To state it, recall that if $f$ is a Maass form with eigenvalue $\l$ and spectral parameter $r$ on $\GH$, then $\l = \frac{1}{4}+r^{2}$ with either $r \in \R$ or $ir \in [0,\frac{1}{2})$. The \textbf{Selberg conjecture}\index{Selberg conjecture} claims that the second case never occurs in weight zero:
