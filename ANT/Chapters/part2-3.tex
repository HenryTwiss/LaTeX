\chapter{The Theory of Maass Forms}
  ...
  \section{Atkin–Lehner Theory}
    ...
    \]
    This verifies automorphy for $\w_{N}f$. Also, it is clear that $\w_{N}f$ is a cusp form because $f$ is. Hence $\w_{N}f$ is well-defined. Moreover, it is an involution because $\w_{N}(\w_{N}f) = f$ and so its only possible eigenvalues are $\pm 1$. The crucial fact we need is how $\w_{N}$ acts on $\mc{C}_{t}(N,\chi)$ (mimic the proof for holomorphic forms given in \cite{miyake1989modular}):

    \begin{proposition}\label{prop:Atkin_Lehner_conjugation_Maass}
      The Atkin-Lehner involution $w_{N}$ takes $\mc{C}_{t}(N,\chi)$ to $\mc{C}_{t}(N,\cchi)$ isomorphically. It also preserves the subspace of newforms. In particular, if $f \in \mc{C}_{t}(N,\chi)$ is a primitive Hecke-Maass eigenform then $\w_{N}f \in \mc{C}_{t}(N,\cchi)$ is also a primitive Hecke-Maass eigenform.
    \end{proposition}

    In accordance with \cref{prop:Atkin_Lehner_conjugation_Maass}, for a primitive Hecke-Maass eigenform $f \in \mc{C}_{t}(N,\chi)$ we set $\conj{f} = \w_{N}f$.
