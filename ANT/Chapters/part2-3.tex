\chapter{The Theory of Maass Forms}
  ...
  \section{Spectral Theory of the Laplace Operator}
    ...
    \[
      \<f,g\> = \frac{1}{2\pi}\int_{0}^{\infty}f(t)\conj{g(t)}\,dt,
    \]
    ...
    \[
      (E_{k,\chi,\mf{a}}f)(z) = \frac{1}{4\pi}\int_{0}^{\infty}f(t)E_{k,\chi,\mf{a}}\left(z,\frac{1}{2}+it\right)\,dt.
    \]
    ...
    \begin{proposition}\label{prop:Eisenstein_transform_property}
      If $f \in C_{0}^{\infty}(\R_{> 0})$, then $E_{\mf{a}}f$ is $L^{2}$-integrable over $\mc{F}_{\G}$. That is, $E_{k,\chi,\mf{a}}:C_{0}^{\infty}(\R_{> 0}) \to \mc{L}_{k}(\G,\chi)$. Moreover,
      \[
    ...
    \begin{theorem}\label{thm:incomplete_Eisenstein_series_spectrum}
      ...
      \[
        f(z) = \sum_{j}\<f,u_{j}\>u_{j}(z)+\sum_{\mf{a}}\frac{1}{4\pi}\int_{-\infty}^{\infty}\left\<f,E_{k,\chi,\mf{a}}\left(\cdot,\frac{1}{2}+it\right)\right\>E_{k,\chi,\mf{a}}\left(z,\frac{1}{2}+it\right)\,dt.
      \]
  ...
  \section{The Ramanujan-Petersson Conjecture}
    ...
    \]
    Since $f$ is a cusp form, this integral is locally absolutely uniformly bounded by \cref{met:decay_compacta_integral}.
