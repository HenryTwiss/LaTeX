\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n)$. We can simplify the Fourier series further. We say $f$ is \textbf{even}\index{even} if $f(-\conj{z}) = f(z)$ and is \textbf{odd}\index{odd} if $f(-\conj{z}) = -f(z)$. Since $-\conj{z} = -x+iy$, this means that $f$ is even or odd with respect to $\Re(z) = x$. Note that if $f$ is odd it must be a cusp form. As $e^{2\pi inx} = \cos(nx)+i\sin(nx)$, if $f$ has eigenvalue $\l = s(1-s)$, then the Fourier series of $f$ at the $\mf{a}$ cusp takes the form 
    \[
  ...
  \section{Eisenstein \& Poincar\'e Series}
    ...
    \]
    Therefore $\Im(z)^{s} = y^{s}$ is an eigenfunction for $\D_{k}$ with eigenvalue $\l(s)$. Since $\D_{k}$ is invariant,
    \[
    ...
    \[
      \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\left|\psi\left(\frac{\Im(z)}{|cz+d|^{2}}\right)\right| \ll \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\left(\frac{\Im(z)}{|cz+d|^{2}}\right)^{1+\e},
    \]
    ...
    \begin{theorem}
      Let $\psi:(0,\infty) \to \C$ be a smooth function such that $\psi(y) \ll y^{1+\e}$, for some $\e > 0$, as $y \to 0$. The Poincar\'e series
      \[
  ...
  \section{\texorpdfstring{$L^{2}$}{L{2}}-integrable Automorphic Functions}
    ...
    \]
    Unfortunately the Eisenstein series $E_{k,\chi,\mf{a}}(z,s)$ are not $L^{2}$-integrable over $\mc{F}_{\G}$ and so do not belong to $\mc{L}_{k}(\G,\chi)$. To obtain integrable automorphic functions we will argument these Eisenstein series so that they have compact support. Let $\psi:(0,\infty) \to \C$ be a smooth compactly supported function and let $\s_{\mf{a}}$ be a scaling matrix for the $\mf{a}$ cusp. We define the \textbf{(incomplete) Eisenstein series}\index{(incomplete) Eisenstein series} $E_{k,\chi,\mf{a}}(z,\psi)$ of weight $k$ and character $\chi$ on $\GH$ at the $\mf{a}$ cusp and with respect to $\psi$ by
    \[
    ...
    \begin{theorem}
      For any smooth compactly supported function $\psi:(0,\infty) \to \C$, the Eisenstein series
      \[
        E_{k,\chi,\mf{a}}(z,\psi) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\g,z)^{-k}\psi(\Im(\s_{\mf{a}}^{-1}\g z)),
      \]
  ...
  \section{Spectral Theory of the Laplace Operator}
    ...
    To begin decomposing the space $\mc{E}_{k}(\G,\chi)$, consider the subspace $C_{0}^{\infty}((0,\infty))$ of $\mc{L}^{2}((0,\infty))$ with the normalized standard inner product
    \[
      \<f,g\> = \frac{1}{2\pi}\int_{0}^{\infty}f(r)\conj{g(r)}\,dr,
    \]
    for any $f,g \in C_{0}^{\infty}((0,\infty))$. For each cups $\mf{a}$ of $\GH$ we associate the \textbf{Eisenstein transform}\index{Eisenstein transform} $E_{k,\chi,\mf{a}}:C_{0}^{\infty}((0,\infty)) \to \mc{A}_{k}(\G,\chi)$ defined by
    \[
    ...
    \begin{proposition}\label{prop:Eisenstein_transform_property}
      If $f \in C_{0}^{\infty}((0,\infty))$, then $E_{\mf{a}}f$ is $L^{2}$-integrable over $\mc{F}_{\G}$. That is, $E_{k,\chi,\mf{a}}:C_{0}^{\infty}((0,\infty)) \to \mc{L}_{k}(\G,\chi)$. Moreover,
      \[
        \<E_{k,\chi,\mf{a}}f,E_{k,\chi,\mf{b}}g\> = \d_{\mf{a},\mf{b}}\<f,g\>,
      \]
      for any $f,g \in C_{0}^{\infty}((0,\infty))$ and any two cusps $\mf{a}$ and $\mf{b}$.
    \end{proposition}
  ...
  \section{Diamond \& Hecke Operators}
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{A}_{t}(\G_{1}(N))$ Fourier coefficients $a_{n}(f)$, $a_{0}(f)$, and $b_{0}(f)$. Then for primes $p$ with $(p,N) = 1$,
      \begin{align*}
        (T_{p}f)(z) = (a_{0}(f)+\chi_{N,0}(p)a_{0}(\<p\>f))y^{\frac{1}{2}+it}&+(b_{0}(f)+\chi_{N,0}(p)b_{0}(\<p\>f))y^{\frac{1}{2}-it} \\
        &+ \sum_{n \neq 0}\left(a_{np}(f)+\chi_{N,0}(p)a_{\frac{n}{p}}(\<p\>f)\right)\sqrt{y}K_{it}(2\pi|n|y)e^{2\pi inx},
      \end{align*}
      ...
      \begin{align*}
        (T_{p}f)(z) = (a_{0}(f)+\chi(p)a_{0}(f))y^{\frac{1}{2}+it}&+(b_{0}(f)+\chi(p)b_{0}(f))y^{\frac{1}{2}-it} \\
        &+ \sum_{n \neq 0}\left(a_{np}(f)+\chi(p)a_{\frac{n}{p}}(f)\right)\sqrt{y}K_{it}(2\pi|n|y)e^{2\pi inx},
      \end{align*}
    ...
    We use \cref{prop:Hecke_operators_commute_Maass} to construct diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$ exactly as for holomorphic forms. Explicitly, the \textbf{diamond operator}\index{diamond operator} $\<m\>:\mc{A}_{t}(\G_{1}(N)) \to \mc{A}_{t}(\G_{1}(N))$ is defined to be the linear operator given by
    \[
      \<m\> = \begin{cases} \<m\> \text{ with $m$ taken modulo $N$} & \text{if $(m,N) = 1$,} \\ 0 & \text{if $(m,N) > 1$.} \end{cases}
    \]
    For the Hecke operators, if $m = p_{1}^{r_{1}}p_{2}^{r_{2}} \cdots p_{k}^{r_{k}}$ is the prime decomposition of $m$, then the $m$-th \textbf{Hecke operator}\index{Hecke operator} $T_{m}:\mc{A}_{k,t}(\G,\chi) \to \mc{A}_{k,t}(\G,\chi)$ is the linear operator given by
    \[
    ...
    \begin{proposition}\label{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{A}_{t}(\G_{1}(N))$ with Fourier coefficients $a_{n}(f)$, $a_{0}(f)$, and $b_{0}(f)$. Then for $m \ge 1$ with $(m,N) = 1$,
      \begin{align*}
        (T_{m}f)(z) = \left(\sum_{d \mid m}a_{0}(\<d\>f)\right)y^{\frac{1}{2}+it}&+\left(\sum_{d \mid m}b_{0}(\<d\>f)\right)y^{\frac{1}{2}-it} \\
        &+ \sum_{n \neq 0}\left(\sum_{d \mid (n,m)}a_{\frac{nm}{d^{2}}}(\<d\>f)\right)\sqrt{y}K_{it}(2\pi|n|y)e^{2\pi inx},
      \end{align*}
      is the Fourier series of $T_{m}f$. Moreover, if $f \in \mc{A}_{t}(N,\chi)$, then
      \begin{align*}
        (T_{m}f)(z) = \left(\sum_{d \mid m}\chi(d)a_{0}(f)\right)y^{\frac{1}{2}+it}&+\left(\sum_{d \mid m}\chi(d)b_{0}(f)\right)y^{\frac{1}{2}-it} \\
        &+ \sum_{n \neq 0}\left(\sum_{d \mid (n,m)}\chi(d)a_{\frac{nm}{d^{2}}}(f)\right)\sqrt{y}K_{it}(2\pi|n|y)e^{2\pi inx}.
      \end{align*}
    \end{proposition}
    \begin{proof}
  ...
  \section{Atkin–Lehner Theory}
    ...
    \[
      \mc{C}_{t}(\G_{1}(N))^{\mathrm{old}} = \bigop_{p \mid N}\Im(i_{p}),
    \]
    ...
    Interestingly, unlike holomorphic forms it is unknown if the Fourier coefficients of Maass forms are real or even algebraic in general. We require one last piece of machinery. As for holomorphic forms, we have an involution on the space $\mc{C}_{t}(N,\chi)$. Recall the matrix
    \[
    ...
    \]
    This verifies automorphy for $\w_{N}f$. Also, it is clear that $\w_{N}f$ is a cusp form because $f$ is. Hence $\w_{N}f$ is well-defined. Moreover, it is an involution because $\w_{N}^{2}f = f$ and so its only possible eigenvalues are $\pm 1$. The crucial fact we need is how $\w_{N}$ acts on $\mc{C}_{t}(N,\chi)$. To state the result, for $f \in \mc{C}_{t}(N,\chi)$ define
    \[
      \conj{f}(z) = \conj{f(-z)}.
    \]
  ...
  \section{The Ramanujan-Petersson Conjecture}
    ...
    \]
    This bound is known as the \textbf{Hecke bound}\index{Hecke bound} for Maass forms. However, the Ramanujan-Petersson conjecture gives the bound $\l_{f}(n) \ll \s_{0}(n)n^{\frac{1}{2}} \ll n^{\frac{1}{2}+\e}$. It turns out that the Ramanujan-Petersson conjecture is tightly connected to another conjecture of Selberg about the smallest possible eigenvalue of weight zero Maass form on $\GH$. Note that the possible eigenvalues are discrete by \cref{thm:the_full_spectral_resolution} and so there exists a smallest eigenvalue. To state it, recall that if $f$ is a Maass form with eigenvalue $\l$ on $\GH$, then $\l = \frac{1}{4}+t^{2}$ with either $t \in \R$ or $it \in [0,\frac{1}{2})$. \textbf{Selberg's eigenvalue conjecture}\index{Selberg's eigenvalue conjecture} claims that the second case never occurs in weight zero:

    \begin{conjecture}[Selberg's eigenvalue conjecture]
  ...
  \section{Twists of Maass Forms}
    We can also twist of Maass forms by Dirichlet characters. Let $f \in \mc{A}_{t}(N,\chi)$ with Fourier series
    \[
      f(z) = a_{0}(f)y^{\frac{1}{2}+it}+b_{0}(f)y^{\frac{1}{2}-it}+\sum_{n \neq 0}a_{n}(f)\sqrt{y}K_{it}(2\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \[
      (f \ox \psi)(z) = a_{0}(f)\psi(0)y^{\frac{1}{2}+it}+b_{0}(f)\psi(0)y^{\frac{1}{2}-it}+\sum_{n \neq 0}a_{n}(f)\psi(n)\sqrt{y}K_{it}(2\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \begin{proposition}\label{prop:twisted_Maass_forms_primitive}
      Suppose $f \in \mc{A}_{t}(N,\chi)$ and $\psi$ is a primitive Dirichlet character of conductor $q$. Then $f \ox \psi \in \mc{A}_{t}(Nq^{2},\chi\psi^{2})$. Moreover, if $f$ is a cusp form then so is $f \ox \psi$.
    \end{proposition}
    \begin{proof}
    ...
    The generalization of \cref{prop:twisted_Maass_forms_primitive} to all characters is slightly more involved. Define operators $U_{p}$ and $V_{p}$ on $\mc{A}_{t}(\G_{1}(N))$ to be the linear operators given by
    \[
      (U_{p}f)(z) = a_{0}(f)y^{\frac{1}{2}+it}+b_{0}(f)y^{\frac{1}{2}-it}+\sum_{n \neq 0}a_{np}(f)\sqrt{y}K_{it}(2\pi|n|y)e^{2\pi inx},
    \]
    and
    \[
      (V_{p}f)(z) = a_{0}(f)(py)^{\frac{1}{2}+it}+b_{0}(f)(py)^{\frac{1}{2}-it}+\sum_{n \neq 0}a_{n}(f)\sqrt{py}K_{it}(2\pi|n|py)e^{2\pi inpx},
    \]
    if $f$ has Fourier series
    \[
      f(z) = a_{0}(f)y^{\frac{1}{2}+it}+b_{0}(f)y^{\frac{1}{2}-it}+\sum_{n \neq 0}a_{n}(f)\sqrt{y}K_{it}(2\pi|n|y)e^{2\pi inx}.
    \]
    We will show that both $U_{p}$ and $V_{p}$ map $\mc{A}_{t}(\G_{1}(N))$ into $\mc{A}_{t}(\G_{1}(Np))$ and more:

    \begin{lemma}\label{lem:twisted_Maass_lemma}
      For any prime $p$, $U_{p}$ and $V_{p}$ map $\mc{A}_{t}(\G_{1}(N))$ into $\mc{A}_{t}(\G_{1}(Np))$. In particular, $U_{p}$ and $V_{p}$ map $\mc{A}_{t}(N,\chi)$ into $\mc{A}_{t}(Np,\chi\chi_{p,0})$. Moreover, $U_{p}$ and $V_{p}$ preserve the subspace of cusp forms.
    \end{lemma}
    \begin{proof}
      Mimic the proof of \cref{thm:diamond_operator_decomposition_holomorphic}.
    \end{proof}
    ...
    \begin{proposition}\label{prop:twisted_Maass_forms}
      Suppose $f \in \mc{A}_{t}(N,\chi)$ and $\psi$ is a Dirichlet character modulo $M$. Then $f \ox \psi \in \mc{A}_{t}(NM^{2},\chi\psi^{2})$. Moreover, if $f$ is a cusp form then so is $f \ox \psi$.
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{prop:twisted_holomorphic_forms}, it remains to show that $f \ox \psi_{p,0}$ is an eigenfunction with eigenvalue $\l$ if $f$ is. As $U_{p} = T_{p}$ for the $p$-th Hecke operator on $\mc{A}_{t}(\G_{1}(Np))$, $U_{p}$ commutes with $\D$. It is also clear that $V_{p}$ commutes with $\D$. These facts together with
      \[
