\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    Thus $f(\s_{\mf{a}}z)$ is $1$-periodic. Note that this means we only need to verify the growth condition as $y \to \infty$. As $f$ is smooth, it admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      f(\s_{\mf{a}}z) = \sum_{n \in \Z}a_{\mf{a}}(n,y,s)e^{2\pi inx},
    \]
    for some Fourier coefficients $a_{\mf{a}}(n,y,s)$ (the sum is over all $n \in \Z$ since $f$ may be unbounded as $z \to \infty$). Note that any Fourier series for $f$ is independent of the scaling matrix because $f(\s_{\mf{a}}z)$ is. The Fourier coefficients $a_{\mf{a}}(n,y,s)$ are mostly determined by $\D_{k}$. To see this, since $f$ is smooth we may differentiate the Fourier series of $f(\s_{\mf{a}}z)$ termwise. The fact that $f$ is an eigenfunction for $\D_{k}$ with eigenvalue $\l(s)$ gives the ODE
    \[
    ...
    \]
    The coefficients $a_{\mf{a}}(n,s)$, $a_{\mf{a}}^{+}(0,s)$, and $a_{\mf{a}}^{-}(0,s)$ are the only part of the Fourier series that actually depend on the implicit congruence subgroup $\G$. Using these coefficients, $f$ admits another \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
    ...
    \]
    where $\G_{\infty}\backslash\H_{Y}$ is the vertical strip $\G_{\infty}\backslash\H$ subject to the condition $y \ge Y$. Since $f$ is a cusp form, this integral is locally absolutely uniformly bounded by \cref{prop:decay_unbounded_inteval_integral}. Substituting in the Fourier series of $f$, this integral can be expressed as
    \[
      \int_{Y}^{\infty}\int_{0}^{1}\sum_{n,m \ge 1}a_{\mf{a}}(n,s)\conj{a_{\mf{a}}(m,s)}W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\conj{W_{\sgn(m)\frac{k}{2},s-\frac{1}{2}}(4\pi|m|y)}e^{2\pi i(n-m)x}\,\frac{dy}{y^{2}}.
    \]
    ...
    \[
      \sum_{n \ge 1}\int_{Y}^{\infty}|a_{\mf{a}}(n,s)|^{2}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}}.
    \]
    ...
    \[
      |a_{\mf{a}}(n,s)|^{2}\int_{Y}^{\infty}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}} \ll \int_{\G_{\infty}\backslash\H_{Y}}|f(\s_{\mf{a}}z)|^{2}\,d\mu.
    \]
    Now $f$ is bounded on $\H$ as it is a cusp form so that
    \[
      \int_{\G_{\infty}\backslash\H_{Y}}|f(\s_{\mf{a}}z)|^{2}\,d\mu \ll \int_{Y}^{\infty}\int_{0}^{1}\frac{dx\,dy}{y^{2}} \ll \frac{1}{Y}.
    \]
    Putting these two estimates together gives
    \[
      |a_{\mf{a}}(n,s)|^{2}\int_{Y}^{\infty}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}} \ll \frac{1}{Y}.
    \]
    Taking $Y = \frac{1}{|n|}$, the Whittaker function is absolutely bounded by \cref{lem:Whittaker_function_asymptotic}. Then $\frac{1}{y^{2}} \ll n^{2}$ and we obtain
    \[
    ...
    \]
    Observe that $f(\s_{\mf{a}}z)$ is then bounded on $\H$ and, in particular, $f$ is bounded on $\H$. While this Fourier series holds for all weights, it is somewhat deceiving since the Whittaker function has polynomial growth in $n$ by \cref{thm:Whittaker_special_cases}. Therefore it will be more useful to work with slightly modified versions depending on the weight $k$. If $k \ge 1$, then \cref{thm:Whittaker_special_cases} implies that $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      f(\s_{\mf{a}}z) = a_{\mf{a}}^{+}(0,s)y^{s}+a_{\mf{a}}^{-}(0,s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(n,s)y^{\sgn(n)\frac{k}{2}}e^{-2\pi|n|y}e^{2\pi inx},
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n,s)$. Similarly, if $k = 0$, then \cref{thm:Whittaker_special_cases} implies that $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      f(\s_{\mf{a}}z) = a_{\mf{a}}^{+}(0,s)y^{s}+a_{\mf{a}}^{-}(0,s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(n,s)\sqrt{y}K_{s-\frac{1}{2}}(2\pi|n|y)e^{2\pi inx},
    \]
    for again some different Fourier coefficients $a_{\mf{a}}(n,s)$. If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. From now on, we will always use this latter Fourier series for weight zero Maass forms unless specified otherwise. Moreover, we can simplify the Fourier series further in the weight zero setting. We say $f$ is \textbf{even}\index{even} if $f(-\conj{z}) = f(z)$ and is \textbf{odd}\index{odd} if $f(-\conj{z}) = -f(z)$. Since $-\conj{z} = -x+iy$, this means that $f$ is even or odd with respect to $x$. Note that if $f$ is odd it must be a cusp form. As $e^{2\pi inx} = \cos(nx)+i\sin(nx)$, if $f$ has eigenvalue $\l = s(1-s)$, then the Fourier series of $f$ at the $\mf{a}$ cusp takes the form
    \[
