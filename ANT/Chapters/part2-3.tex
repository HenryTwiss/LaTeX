\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    Accordingly, we say that $f$ exhibits \textbf{moderate decay at the cusps}\index{moderate decay at the cusps}. If $f(\s_{\mf{a}}z)$ exhibits exponential decay for every cusp $\mf{a}$ instead, we say that $f$ exhibits \textbf{exponential decay at the cusps}\index{exponential decay at the cusps}. Note that weight zero cusp forms necessarily exhibit exponential decay at the cusps. In either case, $f$ is bounded on $\H$. For weight zero Maass forms, we can say more about the Fourier series. Indeed, \cref{thm:Whittaker_special_cases} implies that the Fourier series at the $\mf{a}$ cusp takes the form
    \[
  ...
  \section{Eisenstein \& Poincar\'e Series}
    ...
    \]
    The latter series is locally absolutely uniformly convergent for $\s > 1$ and $z \in \H$ by \cref{prop:general_lattice_sum_convergence_for_two_variables}. Therefore $E_{k,\chi,\mf{a}}(z,s)$ is too. To see that it is automorphic on $\GH$, let $\g \in \G$. Then we make a short computation:
    \begin{align*}
    ...
    So altogether,
    \[
      E_{k,\chi,\mf{a}}(\s_{\mf{b}}z,s) \ll \Im(z)^{\s} = o(e^{2\pi\Im(z)}),
    \]
    ...
    \]
    and hence $e^{2\pi im\s_{\mf{a}}^{-1}\g z}$ is independent of the representative $\g$ used. It follows that $P_{m,k,\chi,\mf{a}}(z,s)$ is well-defined. We claim $P_{m,k,\chi,\mf{a}}(z,s)$ is also locally absolutely uniformly convergent for $z \in \H$ and $\Re(s) > 1$. To see this, first observe that $|e^{2\pi im\s_{\mf{a}}^{-1}\g z}| = e^{-2\pi m\Im(\s_{\mf{a}}^{-1}\g z)} < 1$ for all $\g \in \G$. Then applying the Bruhat decomposition to $\s_{\mf{a}}^{-1}\G_{\mf{a}}\backslash\G = \G_{\infty}\backslash\s_{\mf{a}}^{-1}\G$ yields
    \[
      P_{k,\chi,\mf{a}}(z,s) \ll \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{\Im(z)^{\s}}{|cz+d|^{2\s}},
    \]
    and this latter series is locally absolutely uniformly convergent for $\s > 1$ and $z \in \H$ by \cref{prop:general_lattice_sum_convergence_for_two_variables}. Hence the same holds for $P_{m,k,\chi,\mf{a}}(z,s)$. Moreover, automorphy for $P_{m,k,\chi,\mf{a}}(z,s)$ is verified in exactly the same way as for $E_{k,\chi,\mf{a}}(z,s)$. We collect this work as a theorem:
  ...
  \section{\texorpdfstring{$L^{2}$}{L{2}}-integrable Automorphic Functions}
    ...
    \begin{theorem}
      ...
      \[
        E_{k,\chi,\mf{a}}(z,\psi) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\psi(\Im(\s_{\mf{a}}^{-1}\g z)),
      \]
      is a smooth automorphic function on $\GH$. If $\psi(y)$ is a bump function, $E_{k,\chi,\mf{a}}(z,\psi)$ belongs to $\mc{L}_{k}(\G,\chi)$.
    \end{theorem}
    ...
    \begin{theorem}
      ...
      \[
        P_{m,k,\chi,\mf{a}}(z,\psi) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\psi(\Im(\s_{\mf{a}}^{-1}\g z))e^{2\pi im\s_{\mf{a}}^{-1}\g z},
      \]
      is a smooth automorphic function on $\GH$ and belongs to $\mc{L}_{k}(\G,\chi)$.
    \end{theorem}
