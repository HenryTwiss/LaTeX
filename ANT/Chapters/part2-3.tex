\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    One might expect that the Maass raising and lowering operators $R_{k}$ and $L_{k}$ act by changing the weight of a Maass form by $\pm 2$ respectively. This is indeed the case (see \cite{bumpautomorphic1997} for a proof):
    ...
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n,s)$ (they are $\sqrt{4|n|}$ multiples). If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. We also have a notion of parity for weight zero Maass forms. We say that $f$ is \textbf{even}\index{even} with respect to the $\mf{a}$ cusp if $(f|\s_{\mf{a}})(-\conj{z}) = (f|\s_{\mf{a}})(z)$ and is \textbf{odd}\index{odd} with respect to the $\mf{a}$ cusp if $(f|\s_{\mf{a}})(-\conj{z}) = -(f|\s_{\mf{a}})(z)$. If $\mf{a} = \infty$, we suppress this dependence. Since $-\conj{z} = -x+iy$, $f$ is even or odd with respect to the $\mf{a}$ cusp if and only if $(f|\s_{\mf{a}})$ is even or odd with respect to $x$. Moreover, As $e^{ix} = \cos(x)+i\sin(x)$, $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
