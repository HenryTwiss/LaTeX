\chapter{The Theory of Maass Forms}
  ...
  \section{Atkin–Lehner Theory}
    ...
    \]
    This verifies automorphy for $\w_{N}f$. Also, it is clear that $\w_{N}f$ is a cusp form because $f$ is. Hence $\w_{N}f$ is well-defined. Moreover, it is an involution because $\w_{N}(\w_{N}f) = f$ and so its only possible eigenvalues are $\pm 1$. The crucial fact we need is that primitive Hecke-Maass eigenforms are eigenfunctions for the Atkin–Lehner involution (mimic the proof for holomorphic forms given in \cite{miyake1989modular} or \cite{diamond2005first}):

    \begin{proposition}\label{prop:primitive_Hecke_Maass_eigenform_eigenfunction_Atkin_Lehner}
      If $f \in \mc{C}_{t}(N,\chi)$ is a primitive Hecke-Maass eigenform, then $f$ is an eigenfunction for the Atkin–Lehner involution. In particular,
      \[
        \w_{N}f = \w_{N}(f)f,
      \]
      with $\w_{N}(f) = \pm 1$.
