\chapter{The Theory of Maass Forms}
	Maass forms are the non-holomorphic analog to holomorphic forms. They are real-analytic, eigenfunctions for a differential operator, invariant with respect to a subgroup of $\PSL_{2}(\Z)$, and satisfy a growth condition. We introduce both Maass forms and their general theory. Although we define Maass forms with arbitrary weight, much of the theory is discussed only for weight zero Maass forms (where it is much more tractable).
  \section{Maass Forms}
    Define $\e(\g,z)$ by
    \[
    ...
    \]
    If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. Moreover, property (v) implies that $f$ is a cusp form if and only if $a_{\mf{a}}^{+}(0,s) = 0$ and $a_{\mf{a}}^{-}(0,s) = 0$ for every cusp $\mf{a}$. We also have a notion of parity for Maass forms. We say that $f$ is \textbf{even}\index{even} with respect to the $\mf{a}$ cusp if $(f|\s_{\mf{a}})(-\conj{z}) = (f|\s_{\mf{a}})(z)$ and is \textbf{odd}\index{odd} with respect to the $\mf{a}$ cusp if $(f|\s_{\mf{a}})(-\conj{z}) = -(f|\s_{\mf{a}})(z)$. If $\mf{a} = \infty$, we suppress this dependence. Since $-\conj{z} = -x+iy$, $f$ is even or odd with respect to the $\mf{a}$ cusp if and only if $(f|\s_{\mf{a}})$ is even or odd with respect to $x$. Moreover, As $e^{ix} = \cos(x)+i\sin(x)$, $f$ admits \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \begin{align*}
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(0,s)y^{s}&+a_{\mf{a}}^{-}(0,s)y^{1-s} \\
      &+ \sum_{n \ge 1}\left(a_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)+a_{\mf{a}}(-n,s)W_{\sgn(-n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\right)\SC(2\pi nx),
    \end{align*}
    for some potentially different Fourier coefficients $a_{\mf{a}}(n,s)$ and $a_{\mf{a}}^{\pm}(0,s)$ (the $a_{\mf{a}}(n,s)$ are $i$ multiples and the $a_{\mf{a}}^{\pm}(0,s)$ are zero if $f$ is odd) and where $\SC(x) = \cos(x)$ if $f$ is even and $\SC(x) = \sin(x)$ if $f$ is odd. From now on, if we are talking about even or odd Maass forms we will always mean this latter Fourier series. We can also easily derive a bound for the size of the Fourier coefficients of cusp forms. Fix some $Y > 0$ and consider
    \[
    ...
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n,s)$ (they are $\sqrt{4|n|}$ multiples). If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. Moreover, if $f$ is even or odd at the $\mf{a}$ cusp, then $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(0,s)y^{s}+a_{\mf{a}}^{-}(0,s)y^{1-s}+\sum_{n \ge 1}a_{\mf{a}}(n,s)\sqrt{y}K_{s-\frac{1}{2}}(2\pi|n|y)\SC(2\pi nx),
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n,s)$ and $a_{\mf{a}}^{\pm}(0,s)$ (the $a_{\mf{a}}(n,s)$ are $\sqrt{16|n|}$ or $\sqrt{16|n|}i$ multiples if $f$ is even or odd and the $a_{\mf{a}}^{\pm}(0,s)$ are zero if $f$ is odd). From now on, if we are talking about even or odd weight zero Maass forms we will always mean this latter Fourier series. Note that the Hecke bound is now
    \[
  ...
  \section{Inner Product Spaces of Automorphic Functions}
    ...
    \end{proposition}
    \begin{proof}
      Mimicking the computation in \cref{prop:Petersson_inner_product_hermitian}, we see that the Petersson inner product is a Hermitian inner product on $\mc{L}_{k}(\G,\chi)$. We now show that $\mc{L}_{k}(\G,\chi)$ is complete. Let $(f_{n})_{n \ge 1}$ be a Cauchy sequence in $\mc{L}_{k}(\G,\chi)$. Then $||f_{n}-f_{m}|| \to 0$ as $n,m \to \infty$. But
      \[
  ...
  \section{Double Coset Operators}
    We can extend the theory of double coset operators to weight zero Maass forms just as we did for holomorphic forms. Indeed, for any $\a \in \GL_{2}^{+}(\Q)$, we define the operator $[\a]$ on $\mc{A}_{\nu}(\G)$ to be the linear operator given by
    \[
      (f[\a])(z) = \det(\a)^{-1}f(\a z),
    \]
    Moreover, $[\a]$ is multiplicative by mimicking the same argument as for holomorphic forms. Also, if $\g \in \G$ and we choose the representative with positive determinant, then the chain of equalities
    \[
      (f[\g])(z) = f(\g z) = \chi(\g)f(z),
    \]
    is equivalent to the automorphy of $f$ on $\GH$ with character $\chi$. For any two congruence subgroups $\G_{1}$ and $\G_{2}$ (not necessarily of the same level) and any $\a \in \GL_{2}^{+}(\Q)$, we define the \textbf{double coset operator}\index{double coset operator} $[\G_{1}\a\G_{2}]$ to be the linear operator on $\mc{A}_{\nu}(\G_{1})$ given by
    \[
      (f[\G_{1}\a\G_{2}])(z) = \sum_{j}(f[\b_{j}])(z) = \sum_{j}\det(\b_{j})^{-1}f(\b_{j}z).
    \]
    As was the case for holomorphic forms, \cref{prop:double_congruence_subgroup_coset_decomposition_is_finite} implies that this sum is finite. Mimicking the same argument for $[\G_{1}\a\G_{2}]$, we see that $[\G_{1}\a\G_{2}]$ is also well-defined. There is also an analogous statement about the double coset operators for Maass forms:
    ...
    \begin{proof}
      ...
      \[
        \D(f[\G_{1}\a\G_{2}])(z) = \sum_{j}\D(f[\b_{j}])(z) = \l\sum_{j}\det(\b_{j})^{-1}f(\b_{j}z) = \l(f[\G_{1}\a\G_{2}])(z). 
      \]
      Thus $f[\G_{1}\a\G_{2}]$ is an eigenfunction for $\D$ with eigenvalue $\l$. This completes the proof.
    \end{proof}
  \section{Diamond \& Hecke Operators}
    Extending the theory of diamond operators and Hecke operators to weight zero Maass forms is also fairly straightforward. To see this, we have already shown that $\G_{1}(N)$ is normal in $\G_{0}(N)$ so
    \[
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{A}_{\nu}(\G_{1}(N))$ with Fourier coefficients $a_{n}(f)$ and $a_{0}^{\pm}(f)$. Then for primes $p$ with $(p,N) = 1$,
      \begin{align*}
    ...
    \]
    For the Hecke operators, if $m = p_{1}^{r_{1}}p_{2}^{r_{2}} \cdots p_{k}^{r_{k}}$ is the prime decomposition of $m$, then the $m$-th \textbf{Hecke operator}\index{Hecke operator} $T_{m}:\mc{A}_{\nu}(\G,\chi) \to \mc{A}_{\nu}(\G,\chi)$ is the linear operator given by
    \[
    ...
    \begin{proposition}\label{prop:Petersson_adjoint_Maass}
      ...
      \begin{enumerate}[label=(\roman*)]
        \item If $\a^{-1}\G\a \subseteq \PSL_{2}(\Z)$, then for all $f \in \mc{C}_{\nu}(\G,\chi)$ and $g \in \mc{C}_{\nu}(\a^{-1}\G\a)$, we have
        \[
          \<f[\a],g\>_{\a^{-1}\G\a} = \<f,g[\a^{-1}]\>_{\G}.
        \]
        \item For all $f,g \in \mc{C}_{\nu}(\G,\chi)$, we have
        \[
    ...
    When we are considering Maass forms on the full modular group, \cref{prop:Hecke_operators_normal_Maass} says that all of the diamond and Hecke operators are normal. Before discussing the spectral theory, we need one last operator. We define the \textbf{parity operator}\index{parity operator} $T_{-1}:\mc{A}_{\nu}(\G_{1}(N)) \to \mc{A}_{\nu}(\G_{1}(N))$ be the linear operator given by
    \[
      (T_{-1}f)(z) = f(-\conj{z}).
    \]
    Clearly $T_{-1}$ preserves the subspace of cusp forms. Moreover, we also have the following proposition encompassing important properties of the parity operator:
  ...
  \section{Atkin–Lehner Theory}
    ...
    \[
      (f[\a_{d}])(z) = d^{-1}f(\a_{d} z) = d^{-1}f(dz).
    \]
    ...
    \begin{proof}
      ...
      \[
        \D(f[\a_{d}])(z) = \l d^{-1}f(dz) = \l(f[\a_{d}])(z).
      \]
  ...
  \section{The Ramanujan-Petersson Conjecture}
    As for the size of the weight zero Fourier coefficients of Maass forms, much is currently unknown. But there is an analogous conjecture to the one for holomorphic forms. To state it, suppose $f \in \mc{C}_{\nu}(N,\chi)$ is a primitive Hecke-Maass eigenform with Fourier coefficients $\l_{f}(n)$. For each prime $p$, consider the polynomial
    \[
    ...
    The Ramanujan-Petersson conjecture has not yet been proved, but there has been partial progress toward the conjecture. The current best bound is $|\l_{f}(p)| \le 2p^{\frac{7}{64}}$ due to Kim and Sarnak (see \cite{kim2003functoriality} for the proof). Under the Ramanujan-Petersson conjecture, the Hecke relations give the improved bound $\l_{f}(n) \ll \s_{0}(n) \ll_{\e} n^{\e}$ (recall \cref{prop:sum_of_divisors_growth_rate}). It turns out that the Ramanujan-Petersson conjecture is tightly connected to another conjecture of Selberg about the smallest possible eigenvalue of Maass form on $\GH$. Note that the possible eigenvalues are discrete by \cref{thm:the_full_spectral_resolution} and so there exists a smallest eigenvalue. To state it, recall that if $f$ is a Maass form with eigenvalue $\l$ and spectral parameter $r$ on $\GH$, then $\l = \frac{1}{4}+r^{2}$ with either $r \in \R$ or $ir \in [0,\frac{1}{2})$. The \textbf{Selberg conjecture}\index{Selberg conjecture} claims that the second case never occurs:
  ...
  \section{Twists of Maass Forms}
    ...
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{prop:twisted_holomorphic_forms}, it remains to show that $f \ox \psi_{p,0}$ is an eigenfunction with eigenvalue $\l$ if $f$ is. As $U_{p} = T_{p}$ is the $p$-th Hecke operator on $\mc{A}_{\nu}(\G_{1}(Np))$, $U_{p}$ commutes with $\D$. It is also clear that $V_{p}$ commutes with $\D$. These facts together with
      \[
