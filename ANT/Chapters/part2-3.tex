\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    From now on, if we are talking about weight zero Maass forms we will always mean this latter Fourier series unless specified otherwise.
  ...
  \section{Double Coset Operators}
    We can extend the theory of double coset operators to Maass form just as we did for holomorphic forms. Indeed, for any $\a \in \GL_{2}^{+}(\Q)$, we define the operator $[\a]_{k}$ on $\mc{A}_{k,\nu}(\G)$ to be the linear operator given by
    \[
      (f[\a]_{k})(z) = \det(\a)^{-1}\e(\a,z)^{-k}f(\a z),
    \]
    Moreover, $[\a]_{k}$ is multiplicative by mimicking the same argument as for holomorphic forms. Also, if $\g \in \G$ and we choose the representative with positive determinant, then the chain of equalities
    \[
      (f[\g]_{k})(z) = \e(\g,z)^{-k}f(\g z) = \chi(\g)f(z),
    \]
    is equivalent to the automorphy of $f$ on $\GH$ with character $\chi$. For any two congruence subgroups $\G_{1}$ and $\G_{2}$ (not necessarily of the same level) and any $\a \in \GL_{2}^{+}(\Q)$, we define the \textbf{double coset operator}\index{double coset operator} $[\G_{1}\a\G_{2}]_{k}$ to be the linear operator on $\mc{A}_{k,\nu}(\G_{1})$ given by
    \[
    ...
    \begin{proposition}\label{prop:double_coset_operator_preserves_subspaces_Maass}
      For any two congruence subgroups $\G_{1}$ and $\G_{2}$, $[\G_{1}\a\G_{2}]_{k}$ maps $\mc{A}_{k,\nu}(\G_{1})$ into $\mc{A}_{k,\nu}(\G_{2})$. Moreover, $[\G_{1}\a\G_{2}]_{k}$ preserves the subspace of cusp forms.
    \end{proposition}
    \begin{proof}
  ...
  \section{Diamond \& Hecke Operators}
    ...
    \]
    for any $\a = \begin{psmallmatrix} \ast & \ast \\ \ast & d \end{psmallmatrix} \in \G_{0}(N)$. Therefore, for any $d$ taken modulo $N$, we define the \textbf{diamond operator} $\<d\>:\mc{A}_{k,\nu}(\G_{1}(N)) \to \mc{A}_{k,\nu}(\G_{1}(N))$ to be the linear operative given by
    \[
      (\<d\>f)(z) = (f[\a]_{k})(z),
    \]
    for any $\a = \begin{psmallmatrix} \ast & \ast \\ \ast & d \end{psmallmatrix} \in \G_{0}(N)$. As for holomorphic forms, the diamond operators are multiplicative and invertible. They also decompose $\mc{A}_{k,\nu}(\G_{1}(N))$ into eigenspaces. For any Dirichlet character modulo $N$, let
    \[
      \mc{A}_{k,\nu}(N,\chi) = \{f \in \mc{A}_{k,\nu}(\G_{1}(N)):\text{$\<d\>f = \chi(d)f$ for all $d \in (\Z/N\Z)^{\ast}$}\},
    \]
    be the $\chi$-eigenspace. Also let $\mc{C}_{k,\nu}(N,\chi)$ be the corresponding subspace of cusp forms. Then $\mc{A}_{k,\nu}(\G_{1}(N))$ admits a decomposition into these eigenspaces:

    \begin{proposition}\label{thm:diamond_operator_decomposition_Maass}
      We have a direct sum decomposition
      \[
        \mc{A}_{k,\nu}(\G_{1}(N)) = \bigop_{\chi \tmod{N}}\mc{A}_{k,\nu}(N,\chi).
      \]
    ...
    \begin{proposition}\label{prop:diamond_Hecke_operators_commute_Maass}
      For every $d \in (\Z/N\Z)^{\ast}$ and prime $p$, the diamond operator $\<d\>$ and the Hecke operator $T_{p}$ on $\mc{A}_{k,\nu}(\G_{1}(N))$ commute:
      \[
        \<d\>T_{p} = T_{p}\<d\>
      \]
    \end{proposition}
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{A}_{k,\nu}(\G_{1}(N))$ Fourier coefficients $a_{n}(f)$ and $a_{0}^{\pm}(f)$. Then for primes $p$ with $(p,N) = 1$,
      \begin{align*}
        (T_{p}f)(z) = (a_{0}^{+}(f)+\chi_{N,0}(p)p^{-1}a_{0}^{+}(\<p\>f))y^{\frac{1}{2}+\nu}&+(a_{0}^{-}(f)+\chi_{N,0}(p)p^{-1}a_{0}^{-}(\<p\>f))y^{\frac{1}{2}-\nu} \\
        &+ \sum_{n \neq 0}\left(a_{np}(f)+\chi_{N,0}(p)p^{-1}a_{\frac{n}{p}}(\<p\>f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \end{align*}
      is the Fourier series of $T_{p}f$ where it is understood that $a_{\frac{n}{p}}(f) = 0$ if $p \nmid |n|$. Moreover, if $f \in \mc{A}_{k,\nu}(N,\chi)$, then $T_{p}f \in \mc{A}_{k,\nu}(N,\chi)$ and
      \begin{align*}
        (T_{p}f)(z) = (a_{0}^{+}(f)+\chi(p)p^{-1}a_{0}^{+}(f))y^{\frac{1}{2}+\nu}&+(a_{0}^{-}(f)+\chi(p)p^{-1}a_{0}^{-}(f))y^{\frac{1}{2}-\nu} \\
    ...
    \begin{proposition}\label{prop:Hecke_operators_commute_Maass}
      Let $p$ and $q$ be primes and $d,e \in (\Z/N\Z)^{\ast}$. Then the Hecke operators $T_{p}$ and $T_{q}$ and diamond operators $\<d\>$ and $\<e\>$ on $\mc{A}_{k,\nu}(\G_{1}(N))$ form a simultaneously commuting family:
      \[
    ...
    We use \cref{prop:Hecke_operators_commute_Maass} to construct diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$ exactly as for holomorphic forms. Explicitly, the \textbf{diamond operator}\index{diamond operator} $\<m\>:\mc{A}_{k,\nu}(\G_{1}(N)) \to \mc{A}_{k,\nu}(\G_{1}(N))$ is defined to be the linear operator given by
    \[
    ...
    \begin{proposition}\label{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      ...
      \begin{align*}
        (T_{m}f)(z) = \left(\sum_{d \mid m}d^{-1}a_{0}^{+}(\<d\>f)\right)y^{\frac{1}{2}+\nu}&+\left(\sum_{d \mid m}d^{-1}a_{0}^{-}(\<d\>f)\right)y^{\frac{1}{2}-\nu} \\
        &+ \sum_{n \neq 0}\left(\sum_{d \mid (|n|,m)}d^{-1}a_{\frac{nm}{d^{2}}}(\<d\>f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \end{align*}
      is the Fourier series of $T_{m}f$. Moreover, if $f \in \mc{A}_{k,\nu}(N,\chi)$, then
      \begin{align*}
        (T_{m}f)(z) = \left(\sum_{d \mid m}\chi(d)d^{-1}a_{0}^{+}(f)\right)y^{\frac{1}{2}+\nu}&+\left(\sum_{d \mid m}\chi(d)d^{-1}a_{0}^{-}(f)\right)y^{\frac{1}{2}-\nu} \\
    ...
    When we are considering Maass forms on the full modular group, \cref{prop:Hecke_operators_normal_Maass} says that all of the diamond and Hecke operators are normal. Before discussing the spectral theory, we need one last operator. Let $X:\mc{A}_{k,\nu}(\G_{1}(N)) \to \mc{A}_{k,\nu}(\G_{1}(N))$ be the linear operator defined by
    \[
      (Xf)(z) = f(-\conj{z}).
    \]
    As $-\conj{z} = -x+iy$, $X$ acts as reflection with respect to $x$. Then define the \textbf{parity operator}\index{parity operator} $T_{-1}$ to be the linear operator on $\mc{A}_{k,\nu}(\G_{1}(N))$ given by
    \[
    ...
    \end{align*}
    with $\pm$ according to if $f$ is even or odd and for some potentially different Fourier coefficients $a_{0}^{\pm}(f)$ (they are zero if $f$ is odd). From now on, if we are talking about even or odd Maass forms we will always mean this latter Fourier series unless specified otherwise. If the weight is zero, the Fourier series drastically simplifies via, \cref{thm:Whittaker_special_cases}, that $\d(\nu,0) = 1$, and the exponential identities for sine and cosine, so that we obtain
    \[
      (T_{-1}f)(z) = a_{0}^{+}y^{\frac{1}{2}+\nu}+a_{0}^{-}y^{\frac{1}{2}-\nu}+\sum_{n \ge 1}a_{n}(f)\sqrt{4|n|y}K_{\nu}(2\pi |n|y)\SC(2\pi nx),
    \]
    for some different Fourier coefficient $a_{n}(f)$ and $a_{0}^{\pm}(f)$ (the $a_{n}(f)$ are either $2$ or $2i$ multiples according to if $f$ is even or odd and the $a_{0}^{\pm}(f)$ are zero if $f$ is odd) and where $\SC(x) = \cos(x)$ if $f$ is even and $\SC(x) = \sin(x)$ if $f$ is odd. From now on, if we are talking about even or odd weight zero Maass forms we will always mean this latter Fourier series unless specified otherwise. The benefit of working with even and odd Maass forms is that it suffices to determine the Fourier coefficients for $n \ge 0$ instead of $n \in \Z$. Now suppose $f$ is a non-constant Maass cusp form. Let the eigenvalue of $T_{m}$ for $f$ be $\l_{f}(m)$. We say that the $\l_{f}(m)$ are the \textbf{Hecke eigenvalues}\index{Hecke eigenvalues} of $f$. Similar to the case for holomorphic forms, if $f$ is a Maass form on $\G_{1}(N)\backslash\H$ that is a simultaneous eigenfunction for all diamond operators $\<m\>$ and Hecke operators $T_{m}$ with $(m,N) = 1$ and the parity operator $T_{-1}$, we call $f$ an \textbf{eigenform}\index{eigenform}. Necessarily, $f$ is even or odd because $T_{-1}^{2}f = f$ so that the only possible eigenvalues are $\pm 1$. If the condition $(m,N) = 1$ can be dropped, so that $f$ is a simultaneous eigenfunction for all diamond and Hecke operators, then we say $f$ is a \textbf{Hecke-Maass eigenform}\index{Hecke-Maass eigenform}. In particular, on $\G_{1}(1)\backslash\H$ all eigenforms are Hecke-Maass eigenforms. Now let $f$ have Fourier coefficient $a_{n}(f)$. As for holomorphic forms, if $f$ an eigenform \cref{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass} immediately implies that the first Fourier coefficient of $T_{m}f$ is $a_{m}(f)$ and so
    \[
    ...
    In particular, \cref{thm:eigenforms_forms_spectral_theory_Maass} implies that the orthonormal basis in \cref{thm:cusp_form_spectrum} can be taken to be an orthonormal basis of eigenforms. Also, just as in the holomorphic setting, we have \textbf{Hecke relations}\index{Hecke relations} for Maass forms:
  ...
  \section{Atkin–Lehner Theory}
    There is also an Atkin–Lehner theory for Maass form. As with holomorphic forms, we will only deal with congruence subgroups of the form $\G_{1}(N)$ and cusp forms on $\G_{1}(N)\backslash\H$. The trivial way to lift Maass forms from a smaller level to a larger level is via the natural inclusion $\mc{C}_{k,\nu}(\G_{1}(M)) \subseteq \mc{C}_{k,\nu}(\G_{1}(N))$ provided $M \mid N$ which follows from $\G_{1}(N) \le \G_{1}(M)$. Alternatively, for any $d \mid \frac{N}{M}$, let $\a_{d} = \begin{psmallmatrix} d & 0 \\ 0 & 1 \end{psmallmatrix}$. If $f \in \mc{C}_{k,\nu}(\G_{1}(M))$, then
    \[
    ...
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{equ:lifting_operator_holomorphic} with smoothness replacing holomorphy, automorphy replacing modularity, and the analogous growth condition for Maass forms, the only piece left to verify is that $f[\a_{d}]$ is an eigenfunction for $\D$ with eigenvalue $\l$ if $f$ is. This is easy since the invariance of $\D$ implies
      \[
    ...
    Interestingly, unlike holomorphic forms it is unknown if the Fourier coefficients of Maass forms are real or even algebraic in general. We require one last piece of machinery. As for holomorphic forms, we have an involution on the space $\mc{C}_{k,\nu}(N,\chi)$. Recall the matrix
    \[
      W_{N} = \begin{pmatrix} 0 & -1 \\ N & 0 \end{pmatrix},
    \]
    with $\det(W_{N}) = N$. We define the \textbf{Atkin–Lehner involution}\index{Atkin–Lehner involution} $\w_{N}$ to be the linear operator on $\mc{C}_{k,\nu}(\G_{1}(N))$ given by
    \[
      (\w_{N}f)(z) = N^{\frac{k}{2}}\e(W_{N},z)^{-k}f\left(W_{N}z\right) = \left(\frac{\sqrt{N}z}{|z|}\right)^{-k}f\left(-\frac{1}{Nz}\right).
    \]
    It is not too difficult to see that $\w_{N}$ is an involution on $\mc{C}_{k,\nu}(\G_{1}(N))$:

    \begin{proposition}\label{prop:Atkin_Lehner_Maass}
      $\w_{N}$ is an involution on $\mc{C}_{k,\nu}(\G_{1}(N))$.
    \end{proposition}
    \begin{proof}
    ...
    As $\w_{N}f$ is an involution its only possible eigenvalues are $\pm 1$. The crucial fact we need is how $\w_{N}$ acts on $\mc{C}_{k,\nu}(N,\chi)$. To state the result, for $f \in \mc{C}_{k,\nu}(N,\chi)$ define
    \[
      \conj{f}(z) = \conj{f(-z)}.
    \]
  ...
  \section{Twists of Maass Forms}
    We can also twist of Maass forms by Dirichlet characters. Let $f \in \mc{A}_{k,\nu}(N,\chi)$ with Fourier series
    \[
    ...
    \begin{proposition}\label{prop:twisted_Maass_forms_primitive}
      Suppose $f \in \mc{A}_{k,\nu}(N,\chi)$ and $\psi$ is a primitive Dirichlet character of conductor $q$. Then $f \ox \psi \in \mc{A}_{k,\nu}(Nq^{2},\chi\psi^{2})$. Moreover, if $f$ is a cusp form then so is $f \ox \psi$.
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{prop:twisted_holomorphic_forms} with smoothness replacing holomorphy, automorphy replacing modularity, and the analogous growth condition for Maass forms, the only piece left to verify is that $f \ox \psi$ is an eigenfunction for $\D$ with eigenvalue $\l$ if $f$ is in the case $\psi$ is primitive. This is easy since the invariance of $\D$ implies
      \[
    ...
    The generalization of \cref{prop:twisted_Maass_forms_primitive} to all characters is slightly more involved. Define operators $U_{p}$ and $V_{p}$ on $\mc{A}_{k,\nu}(\G_{1}(N))$ to be the linear operators given by
    \[
    ...
    \[
      f(z) = a_{0}^{+}(f)y^{\frac{1}{2}+\nu}+a_{0}^{-}(f)y^{\frac{1}{2}-\nu}+\sum_{n \neq 0}a_{n}(f)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx}.
    \]
    We will show that both $U_{p}$ and $V_{p}$ map $\mc{A}_{k,\nu}(\G_{1}(N))$ into $\mc{A}_{k,\nu}(\G_{1}(Np))$ and more:

    \begin{lemma}\label{lem:twisted_Maass_lemma}
      For any prime $p$, $U_{p}$ and $V_{p}$ map $\mc{A}_{k,\nu}(\G_{1}(N))$ into $\mc{A}_{k,\nu}(\G_{1}(Np))$. In particular, $U_{p}$ and $V_{p}$ map $\mc{A}_{k,\nu}(N,\chi)$ into $\mc{A}_{k,\nu}(Np,\chi\chi_{p,0})$. Moreover, $U_{p}$ and $V_{p}$ preserve the subspace of cusp forms.
    \end{lemma}
    \begin{proof}
      Mimic the proof of \cref{thm:diamond_operator_decomposition_holomorphic}.
    \end{proof}
    ...
    \begin{proposition}\label{prop:twisted_Maass_forms}
      Suppose $f \in \mc{A}_{k,\nu}(N,\chi)$ and $\psi$ is a Dirichlet character modulo $M$. Then $f \ox \psi \in \mc{A}_{k,\nu}(NM^{2},\chi\psi^{2})$. Moreover, if $f$ is a cusp form then so is $f \ox \psi$.
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{prop:twisted_holomorphic_forms}, it remains to show that $f \ox \psi_{p,0}$ is an eigenfunction with eigenvalue $\l$ if $f$ is. As $U_{p} = T_{p}$ is the $p$-th Hecke operator on $\mc{A}_{k,\nu}(\G_{1}(Np))$, $U_{p}$ commutes with $\D$. It is also clear that $V_{p}$ commutes with $\D$. These facts together with
      \[
