\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    where $\H_{Y}$ is the half-plane defined by $Y \le \Im(z) \le 2Y$. Since $\G_{\infty}\backslash\H_{Y}$ is compact, this integral is absolutely bounded. Substituting in the Fourier series of $f$ at the $\mf{a}$ cusp, this integral can be expressed as
    \[
    ...
    \]
    if $f$ is odd. Therefore, in even case $a_{\mf{a}}(-n,s) = a_{\mf{a}}(n,s)$ and in the odd case $-a_{\mf{a}}(-n,s) = a_{\mf{a}}(n,s)$ for all $n \ge 1$. Either way, $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
  ...
  \section{Twists of Maass Forms}
    ...
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{prop:twisted_holomorphic_forms}, it remains to show that $f \ox \psi_{p,0}$ is an eigenfunction with eigenvalue $\l$ if $f$ is. As $U_{p} = T_{p}$ for the $p$-th Hecke operator on $\mc{A}_{k,\nu}(\G_{1}(Np))$, $U_{p}$ commutes with $\D$. It is also clear that $V_{p}$ commutes with $\D$. These facts together with
      \[
