\chapter{The Theory of Maass Forms}
  ...
  \section{Diamond \& Hecke Operators}
    ...
    \]
    is equivalent to the automorphy of $f$ with character $\chi$ on $\G_{0}(N)\backslash\H$. So $f$ is a Maass form with character $\chi$ on $\G_{0}(N)\backslash\H$ if and only if $f[\g] = \chi(\g)f$ for all $\g \in \G_{0}(N)$ where $\g$ is chosen to be the representative with positive determinant. This means that if $\mc{A}_{r}(\G,\chi)$ denotes the space of Maass forms on $\GH$ with spectral parameter $r$ and character $\chi$, and $\mc{C}_{r}(\G,\chi)$ denotes the corresponding subspace of cusp forms, then $\mc{A}_{r}(N,\chi) = \mc{A}_{r}(\G_{0}(N),\chi)$ and $\mc{C}_{r}(N,\chi) = \mc{C}_{r}(\G_{0}(N),\chi)$. So by \cref{thm:diamond_operator_decomposition_Maass}, we have
    \[
