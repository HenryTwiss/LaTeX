\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    Accordingly, we say that $f$ exhibits \textbf{moderate decay at the cusps}\index{moderate decay at the cusps}. In particular, $f$ is bounded on $\H$. For weight zero Maass forms, we can say more about the Fourier series. Indeed, \cref{thm:Whittaker_special_cases} implies that the Fourier series at the $\mf{a}$ cusp takes the form
    \[
  ...
  \section{\texorpdfstring{$L^{2}$}{L{2}}-integrable Automorphic Functions}
    ...
    \]
    If $k = 0$, $\chi$ is the trivial character, or $\mf{a} = \infty$, we will drop these dependences accordingly. We claim that $P_{m,k,\chi,\mf{a}}(z,\psi)$ is well-defined. This is easy to see as we have already showed $\cchi(\g)$ is independent of the representative $\g$ and the functions $\e(\s_{\mf{a}}^{-1}\g,z)^{-k}$, $\Im(\s_{\mf{a}}^{-1}\g z)$, and $e^{2\pi im\s_{\mf{a}}^{-1}\g z}$ are all independent of the scaling matrix used. Now $P_{m,k,\chi,\mf{a}}(z,\psi)$ is locally absolutely uniformly convergent since only finitely many of the terms are nonzero. This is shown in exactly the same way as for $E_{k,\chi,\mf{a}}(z,\psi)$. Moreover, automorphy for $P_{m,k,\chi,\mf{a}}(z,\psi)$ is verified in exactly the same way as for $E_{k,\chi,\mf{a}}(z,s)$. Lastly, the compact support of $\psi(y)$ implies that $P_{m,k,\chi,\mf{a}}(z,\psi)$ is also compactly supported (since the function $\psi(\Im(\s_{\mf{a}}^{-1}\g z))$ is continuous and $\C$ is Hausdorff) and hence bounded on $\H$. Thus $P_{m,k,\chi,\mf{a}}(z,\psi)$ is $L^{2}$-integrable and therefore belongs to $\mc{L}_{k}(\G,\chi)$. We collect this work as a theorem:

    \begin{theorem}
      Let $m \ge 1$, $k \ge 0$, $\chi$ be a Dirichlet character with conductor $q \mid N$, $\mf{a}$ be a cusp of $\GH$, and $\psi(y)$ be a bump function. The Poincar\'e series
      \[
        P_{m,k,\chi,\mf{a}}(z,\psi) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\psi(\Im(\s_{\mf{a}}^{-1}\g z))e^{2\pi im\s_{\mf{a}}^{-1}\g z},
      \]
      is a smooth automorphic function on $\GH$ and belongs to $\mc{L}_{k}(\G,\chi)$.
    ...
    \end{align*}
    Unfolding the remaining integral, we see that
    \begin{align*}
      \frac{1}{V_{\G}}\int_{\g\mc{F}_{\G}}\sum_{\g \in \GG}f(\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu &= \frac{1}{V_{\G}}\int_{\mc{F}_{\G}}\sum_{\g \in \GG}f(\s_{\mf{a}}\g z)\conj{\psi(\Im(\g z))}\,d\mu \\
      &= \frac{1}{V_{\G}}\int_{\G_{\infty}\backslash\H}f(\s_{\mf{a}}z)\conj{\psi(\Im(z))}\,d\mu && \text{unfolding} \\
      &= \frac{1}{V_{\G}}\int_{0}^{\infty}\int_{0}^{1}f(\s_{\mf{a}}(x+iy))\conj{\psi(y)}\,\frac{dx\,dy}{y^{2}} \\
      &= \frac{1}{V_{\G}}\int_{0}^{\infty}\left(\int_{0}^{1}f(\s_{\mf{a}}(x+iy))\,dx\right)\conj{\psi(y)}\,\frac{dy}{y^{2}}.
    \end{align*}
    The inner integral is precisely the constant term in the Fourier series of $f$ at the $\mf{a}$ cusp. It follows that $f$ is orthogonal to $\mc{E}_{k}(\G,\chi)$ if and only if $f$ is a cusp form. So if $f \in \mc{B}_{k}(\G,\chi)$, then
    \[
  ...
  \section{The Ramanujan-Petersson Conjecture}
    ...
    \]
    implies that the inner integral cuts off all of the terms except the diagonal resulting in
    \[
