\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    Observe that $f|\s_{\mf{a}}$ is then bounded on $\H$ and, in particular, $f$ is bounded on $\H$. It is useful to have an alternative Fourier series in the case of weight zero Maass forms that specify the Whittaker function explicitly. When $k = 0$, \cref{thm:Whittaker_special_cases} implies that $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
  ...
  \section{Eisenstein Series}
    ...
    \[
      \e(\s_{\mf{b}},z)^{-k}E_{k,\chi,\mf{a}}(\s_{\mf{b}}z,s) \ll \Im(z)^{\s} = o(e^{2\pi\Im(z)}),
    \]
  ...
  \section{The Petersson Inner Product}
    Let $\G$ be a congruence subgroup of level $N$. Let $\mc{M}_{k}(\G,\chi)$ denote the space of all weight $k$ Maass forms with character $\chi$ on $\GH$. Let $\mc{C}_{k}(\G,\chi)$ denote the associated subspace of cusp forms. Moreover, let $\mc{M}_{k,\l}(\G,\chi)$ and $\mc{C}_{k,\l}(\G,\chi)$ denote the associated subspaces whose eigenvalue is $\l$. In any of these spaces, if $k = 0$ or $\chi$ is the trivial character, we will suppress these dependencies. If $\G_{1}$ and $\G_{2}$ are two congruence subgroups such that $\G_{1} \le \G_{2}$, then we have the inclusion
    \[
      \mc{M}_{k,\l}(\G_{2},\chi) \subseteq \mc{M}_{k,\l}(\G_{1},\chi),
    \]
    and this respects the subspace of cusp forms. So in general, the smaller the congruence subgroup the more Maass forms there are. We will need a dimensionality result regarding the space of Maass forms of a fixed weight and type. The result is that $\mc{M}_{k}(\G,\chi)$ is finite dimensional (see \cite{iwaniec2002spectral} for a proof in the case $\chi$ is trivial):

    \begin{theorem}\label{thm:Maass_forms_finite_dimensional}
      $\mc{M}_{k,\l}(\G,\chi)$ is finite dimensional.
    \end{theorem}

    Since $\mc{C}_{k,\l}(\G,\chi)$ is a subspace of $\mc{M}_{k,\l}(\G,\chi)$, \cref{thm:Maass_forms_finite_dimensional} implies that $\mc{C}_{k,\l}(\G,\chi)$ is finite dimensional as well. Just as for holomorphic forms, we can turn $\mc{C}_{k,\l}(\G,\chi)$ into an inner product space. For $f,g \in \mc{C}_{k,\l}(\G,\chi)$, define their \textbf{Petersson inner product}\index{Petersson inner product} by
    \[
      \<f,g\>_{\G} = \frac{1}{V_{\G}}\int_{\mc{F}_{\G}}f(z)\conj{g(z)}\,d\mu.
    \]
    If the congruence subgroup is clear from context we will suppress the dependence upon $\G$. Since $f$ and $g$ are bounded on $\H$ as they are cusp forms, the integral is locally absolutely uniformly convergent by \cref{prop:decay_finite_volume_integral}. The integrand is also $\G$-invariant so that the integral independent of the choice of fundamental domain. These two facts together imply that the Petersson inner product is well-defined. We will continue to use this notation even if $f$ and $g$ do not belong to $\mc{C}_{k,\l}(\G,\chi)$ provided the integral is locally absolutely uniformly convergent. A basic property of the Petersson inner product is that it is invariant with respect to the slash operator:
    ...
    More importantly, the Petersson inner product turns $\mc{C}_{k,\l}(\G,\chi)$ into a Hilbert space:
    ...
    If we suppose $f \in \mc{C}_{k,\l}(\G,\chi)$ has eigenvalue $\l$, then \cref{thm:Laplace_semi-definite_self-adjoint} implies $\l$ is real and positive. Since $\l = s(1-s)$ and $\l$ is real, $s$ and $1-s$ are either conjugates or real. In the former case, $s = 1-\conj{s}$ and we find that
    \[
    ...
    \[
      \l = s(1-s) = \frac{1}{4}-\nu^{2} = \frac{1}{4}+r^{2}.
    \]
    Therefore to specify $\l$ it suffices to specify either the spectral parameter $r$ or type $\nu$. Accordingly, we write $\mc{M}_{k,r}(\G,\chi)$ and $\mc{M}_{k,\nu}(\G,\chi)$ for the space $\mc{M}_{k,\l}(\G,\chi)$ and write $\mc{C}_{k,r}(\G,\chi)$ and $\mc{C}_{k,\nu}(\G,\chi)$ for the space $\mc{C}_{k,\l}(\G,\chi)$.
