\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. Moreover, property (v) implies that $f$ is a cusp form if and only if $a_{\mf{a}}^{+}(0,s) = 0$ and $a_{\mf{a}}^{-}(0,s) = 0$ for every cusp $\mf{a}$. Now observe that $-\conj{z} = -x+iy$. We say $f$ is \textbf{even}\index{even} with respect to $\mf{a}$ if $(f|\s_{\mf{a}})(-\conj{z}) = (f|\s_{\mf{a}})(z)$ and is \textbf{odd}\index{odd} with respect to $\mf{a}$ if $(f|\s_{\mf{a}})(-\conj{z}) = -(f|\s_{\mf{a}})(z)$. Equivalently, $f$ is even or odd with respect to $\mf{a}$ if and only if $(f|\s_{\mf{a}})$ is even or odd with respect to $x$. If $\mf{a} = \infty$, we will drop this dependence. Note that if $f$ is odd with respect to $\mf{a}$ then $a_{\mf{a}}^{\pm}(0,s) = 0$. As $e^{2\pi inx} = \cos(nx)+i\sin(nx)$, the Fourier series of $f$ at the $\mf{a}$ cusp takes the form
    \[
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(0,s)y^{s}+a_{\mf{a}}^{-}(0,s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\cos(2\pi nx),
    \]
    if $f$ is even, and
    \[
      (f|\s_{\mf{a}})(z) = \sum_{n \neq 0}a_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)i\sin(2\pi nx),
    \]
    if $f$ is odd. In particular, $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(0,s)y^{s}+a_{\mf{a}}^{-}(0,s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\SC(2\pi nx),
    \]
    for some potentially different Fourier coefficients $a_{\mf{a}}(n,s)$ and $a_{\mf{a}}^{\pm}(0,s)$ (the $a_{\mf{a}}(n,s)$ are $i$ multiples and $a_{\mf{a}}^{\pm}(0,s)$ is zero $f$ is odd) and where $\SC(x) = \cos(x)$ if $f$ is even and $\SC(x) = \sin(x)$ if $f$ is odd. If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. From now on, if we are discussing an even or odd Maass form we will always mean this last Fourier series. We can also easily derive a bound for the size of the Fourier coefficients of cusp forms. Fix some $Y > 0$ and consider
    \[
    ...
    \[
      \int_{Y}^{2Y}\int_{0}^{1}\sum_{n,m \ge 1}a_{\mf{a}}(n,s)\conj{a_{\mf{a}}(m,s)}W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\conj{W_{\sgn(m)\frac{k}{2},s-\frac{1}{2}}(4\pi|m|y)}e^{2\pi i(n-m)x}\,\frac{dy}{y^{2}}.
    \]
    ...
    \[
      \sum_{n \ge 1}\int_{Y}^{2Y}|a_{\mf{a}}(n,s)|^{2}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}}.
    \]
    ...
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n,s)$ (they are $\sqrt{4|n|}$ multiples). If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. Moreover, if $f$ is even or odd then $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(0,s)y^{s}+a_{\mf{a}}^{-}(0,s)y^{1-s}+\sum_{n \ge 1}a_{\mf{a}}(n,s)\sqrt{y}K_{s-\frac{1}{2}}(2\pi ny)\SC(2\pi nx),
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n,s)$ (they are either $\sqrt{16|n|}$ or $\sqrt{16|n|}i$ multiples depending on if $f$ is even or odd respectively). If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. From now on, if we are discussing an even or odd weight zero Maass form we will always mean this last Fourier series.
  ...
  \section{Double Coset Operators}
    ...
    \[
      (f[\a]_{k})(z) = \det(\a)^{k-1}\e(\a,z)^{-k}f(\a z),
    \]
    ...
    \[
      (f[\G_{1}\a\G_{2}]_{k})(z) = \sum_{j}(f[\b_{j}]_{k})(z) = \sum_{j}\det(\b_{j})^{k-1}\e(\b_{j},z)^{-k}f(\b_{j}z).
    \]
    ...
    \begin{proof}
      ...
      \[
        \D(f[\G_{1}\a\G_{2}]_{k})(z) = \sum_{j}\D(f[\b_{j}]_{k})(z) = \l\sum_{j}\det(\b_{j})^{k-1}\e(\b_{j},z)^{-k}f(\b_{j}z) = \l(f[\G_{1}\a\G_{2}]_{k})(z). 
      \]
  ...
  \section{Diamond \& Hecke Operators}
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      ...
      \begin{align*}
        ...
        &+ \sum_{n \neq 0}\left(a_{np}(f)+\chi_{N,0}(p)a_{\frac{n}{p}}(\<p\>f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \end{align*}
      ...
      \begin{align*}
        ...
        &+ \sum_{n \neq 0}\left(a_{np}(f)+\chi(p)a_{\frac{n}{p}}(f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \end{align*}
    ...
    \begin{proposition}\label{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      ...
      \begin{align*}
        ...
        &+ \sum_{n \neq 0}\left(\sum_{d \mid (|n|,m)}a_{\frac{nm}{d^{2}}}(\<d\>f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \end{align*}
      ...
      \begin{align*}
        ...
        &+ \sum_{n \neq 0}\left(\sum_{d \mid (|n|,m)}\chi(d)a_{\frac{nm}{d^{2}}}(f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx}.
      \end{align*}
    \end{proposition}
    \begin{proof}
    ...
    When we are considering Maass forms on the full modular group, \cref{prop:Hecke_operators_normal_Maass} says that all of the diamond and Hecke operators are normal. Before discussing the spectral theory, we need one last operator. Define the \textbf{parity operator}\index{parity operator} $T_{-1}:\mc{A}_{k,\nu}(\G_{1}(N)) \to \mc{A}_{k,\nu}(\G_{1}(N))$ to be the linear operator given by
    \[
      (T_{-1}f)(z) = f(-\conj{z}).
    \]
    Clearly the $T_{-1}$ preserves the subspace of cusp forms. If $f$ has Fourier coefficients $a_{n}(f)$, then
    \[
      (T_{-1}f)(z) = a_{0}^{+}(f)y^{\frac{1}{2}+\nu}+a_{0}^{-}(f)y^{\frac{1}{2}-\nu}+\sum_{n \neq 0}a_{n}(f)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{-2\pi inx}.
    \]
    In other words, $T_{-1}$ interchanges the positive and negative Fourier coefficients of $f$. Moreover, we also have the following proposition encompassing important properties of the parity operator:
    ...
    \begin{proposition}\label{prop:sign_operator_commutes_with_Hecke_and_diamond_and_is_normal}
      ...
      \begin{enumerate}
          ...
          \item On $\mc{C}_{k,\nu}(\G_{1}(N))$ the parity operator $T_{-1}$ is normal with respect to the Petersson inner product and the adjoint is given by
          \[
            T_{-1}^{\ast} = -T_{-1}.
          \]
      \end{enumerate}
    \end{proposition}
    \begin{proof}
      To prove (i), by linearity it suffices to assume $f \in \mc{A}_{k,\nu}(N,\chi)$. Then commutativity with the diamond operators $\<m\>$ is immediate from the definition. As for the Hecke operators $T_{m}$, since $T_{-1}$ interchanges the positive and negative Fourier coefficients of $f$, commutativity follows from \cref{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}. For (ii), $z \to -\conj{z}$ is an automorphism of $\H$ with $d\mu \to -d\mu$. So for any $f,g \in \mc{C}_{k,\nu}(\G_{1}(N))$, we have
      \begin{align*}
    ...
    In particular, \cref{thm:eigenforms_forms_spectral_theory_Maass} implies that the orthonormal basis in \cref{thm:cusp_form_spectrum} can be taken to be an orthonormal basis of eigenforms since it is clear that the parity operator $T_{-1}$, diamond operators $\<m\>$, and Hecke operators $T_{m}$ all commute with $\D$. Also, just as in the holomorphic setting, we have \textbf{Hecke relations}\index{Hecke relations} for Maass forms:
  ...
  \section{Atkin–Lehner Theory}
    ...
    \[
      (f[\a_{d}]_{k})(z) = d^{k-1}\e(\a_{d},z)^{-k}f(\a_{d} z) = d^{k-1}f(dz).
    \]
    ...
    \begin{proof}
      ...
      \[
        \D(f[\a_{d}]_{k})(z) = \l d^{k-1}f(dz) = \l(f[\a_{d}]_{k})(z).
      \]
    ...
    \begin{proposition}\label{prop:old/new_subspaces_are_invariant_Maass}
      The spaces $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{old}}$ and $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{new}}$ are invariant under the parity operator $T_{-1}$ and the diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$.
    \end{proposition}
