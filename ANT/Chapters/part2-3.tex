\chapter{The Theory of Automorphic \& Maass Forms}
  ...
  \section{Diamond \& Hecke Operators}
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ Fourier coefficients $a_{n}(f)$. Then for all primes $p$,
      \[
    ...
    \begin{proposition}\label{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ with Fourier coefficients $a_{n}(f)$. Then for $m \ge 1$ with $(m,N) = 1$,
      \[
    ...
    Just as for holomorphic forms, all of the diamond operators on $\mc{C}_{k,\nu}(\G_{1}(1))$ are the identity and therefore $T_{p}^{\ast} = T_{p}$ for all primes $p$. So the Hecke operators are self-adjoint (as are the diamond operators since they are the identity). We need one last operator since cusp forms have Fourier coefficients for all $n \neq 0$. Let $X:\mc{C}_{k,\nu}(\G_{1}(N)) \to \mc{C}_{k,\nu}(\G_{1}(N))$ be the linear operator defined by
    \[
      (Xf)(z) = f(-\conj{z}).
    \]
    ...
    \begin{proposition}\label{prop:parity_operator_properties}
      ...
      \]
      is the Fourier series of $T_{-1}f$. Moreover, $T_{-1}$ commutes with the diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$ with $(m,N) = 1$, is normal, and its adjoint is given by
      \[
        T_{-1}^{\ast} = -T_{-1}.
      \]
    \end{proposition}
    ...
    \]
    for all $n \ge 1$ and with $\pm$ according to if $f$ is even or odd. Thus the fourier series of $f$ takes the form
    \[
      f(z) = \sum_{n \ge 1}a_{n}(f)\left(\d(\nu,k)^{-1}W_{-\frac{k}{2},\nu}(4\pi |n|y)e^{-2\pi inx}\pm W_{\frac{k}{2},\nu}(4\pi |n|y)e^{2\pi inx}\right),
    \]
    with $\pm$ according to if $f$ is even or odd. If the weight is zero, the Fourier series drastically simplifies via, \cref{thm:Whittaker_special_cases}, the identity $\d(\nu,0) = 1$, and the exponential identities for sine and cosine, so that we obtain
    \[
  ...
  \section{Atkin-Lehner Theory}
    ...
    \begin{proposition}\label{prop:old/new_subspaces_are_invariant_Maass}
      The spaces $\mc{C}_{k,\nu}^{\mathrm{old}}(\G_{1}(N))$ and $\mc{C}_{k,\nu}^{\mathrm{new}}(\G_{1}(N))$ are invariant under the diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$ and parity Hecke operator $T_{-1}$.
    \end{proposition}
    ...
    \end{proposition}
    \begin{proof}
      By \cref{prop:parity_operator_properties}, the parity Hecke operator $T_{-1}$ commutes with all the Hecke operators $T_{m}$ for $m \ge 1$ with $(m,N) = 1$. Therefore if $f \in \mc{C}_{k,\nu}(N,\chi)$ is a primitive Hecke-Maass eigenform with Hecke eigenvalues $\l_{f}(m)$, then $T_{-1}f$ is an eigenform with the same Hecke eigenvalues. By \cref{prop:old/new_subspaces_are_invariant_Maass}, $T_{1}f$ is a newform and so multiplicity one gives
      \[
        T_{-1}f = cf,
      \]
