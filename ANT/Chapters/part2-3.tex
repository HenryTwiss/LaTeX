\chapter{The Theory of Maass Forms}
  ...
  \section{Maass Forms}
    ...
    \]
    The coefficients $a_{\mf{a}}(n,s)$, $a_{\mf{a}}^{+}(0,s)$, and $a_{\mf{a}}^{-}(0,s)$ are the only part of the Fourier series that actually depend on the implicit congruence subgroup $\G$. Using these coefficients, $f$ admits \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
    ...
    if $f$ is even, and
    \[
      f(z) = \sum_{n \neq 0}a_{\mf{a}}(n,s)iW_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\sin(2\pi nx),
    \]
    if $f$ is odd. Then
    \[
      a_{\mf{a}}(-n,s)W_{\sgn(-n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y) = \pm a_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y),
    \]
    for all $n \ge 1$ and with $\pm$ according to if $f$ is even or odd with respect to $\mf{a}$ respectively. All in all, $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(0,s)y^{s}+a_{\mf{a}}^{-}(0,s)y^{1-s}+\sum_{n \ge 1}a_{\mf{a}}(n,s)W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)\SC(2\pi nx),
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n,s)$ (they are either $2$ or $2i$ multiples depending on if $f$ is even or odd) and where $\SC(x) = \cos(x)$ if $f$ is even and $\SC(x) = \sin(x)$ if $f$ is odd. If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. From now on, if we are discussing an even or odd Maass form we will always mean this last Fourier series. We can also easily derive a bound for the size of the Fourier coefficients of cusp forms. Fix some $Y > 0$ and consider
    \[
    ...
    \]
    for some different Fourier coefficients $a_{\mf{a}}(n,s)$ (they are either $\sqrt{16|n|}$ or $\sqrt{16|n|}i$ multiples depending on if $f$ is even or odd respectively) and where $\SC(x) = \cos(x)$ if $f$ is even and $\SC(x) = \sin(x)$ if $f$ is odd. If $\mf{a} = \infty$ or $s$ is fixed, we will drop these dependencies accordingly. From now on, if we are discussing an even or odd weight zero Maass form we will always mean this last Fourier series.
  ...
  \section{Diamond \& Hecke Operators}
    ...
    \[
      (T_{-1}f)(z) = f(-\conj{z}).
    \]
    Clearly the $T_{-1}$ preserves the subspace of cusp forms. Moreover, we also have the following proposition encompassing important properties of the parity operator:

    \begin{proposition}\label{prop:sign_operator_commutes_with_Hecke_and_diamond_and_is_normal}
      \phantom{ }
      \begin{enumerate}
          \item On $\mc{A}_{k,\nu}(\G_{1}(N))$, the parity operator $T_{-1}$ commutes with the diamond operators $\<m\>$ and Hecke operators $T_{n}$ for all $m,n \ge 1$.
