\chapter{The Theory of Maass Forms}
  ...
  \section{Atkin–Lehner Theory}
    ...
    \begin{theorem}\label{thm:newforms_characterization_Maass}
      ...
      \end{enumerate}
      Moreover, the primitive Hecke-Maass eigenforms in $\mc{C}_{t}(\G_{1}(N))^{\mathrm{new}}$ form an orthogonal basis with respect to the Petersson inner product. Each such primitive Hecke-Maass eigenform $f$ lies in an eigenspace $\mc{C}_{t}(N,\chi)$ for a Dirichlet character $\chi$ modulo $N$ and $T_{m}f = \l_{f}(m)f$ for all $m \ge 1$.
    \end{theorem}
    \begin{proof}
