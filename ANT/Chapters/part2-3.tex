\chapter{The Theory of Maass Forms}
  ...
  \section{\texorpdfstring{$L^{2}$}{L{2}}-integrable Automorphic Functions}
    ...
    \]
    Therefore $s = \frac{1}{2}+it$. In the later case, since $s$ is real and $\l$ is positive we must have $s \in (0,1)$. It follows that in either case, we may write $\l = \frac{1}{4}+r^{2}$ with either $r \in \R$ or $ir \in [0,\frac{1}{2})$. We refer to $r$ as the \textbf{spectral parameter} of $f$. Note that we can then write
    \[
      \l = s(1-s) = \frac{1}{4}+r^{2}.
    \]
    ...
    \[
      \mc{B}_{k}(\G,\chi) = \mc{E}_{k}(\G,\chi) \op \mc{C}_{k}(\G,\chi) \quad \text{and} \quad \mc{B}_{k,r}(\G,\chi) = \mc{E}_{k,r}(\G,\chi) \op \mc{C}_{k,r}(\G,\chi),
    \]
  ...
  \section{Spectral Theory of the Laplace Operator}
    ...
    \begin{theorem}\label{thm:cusp_form_spectrum}
      The Laplace operator $\D_{k}$ has pure point spectrum on $\mc{C}_{k}(\G,\chi)$. The corresponding subspaces $\mc{C}_{k,r}(\G,\chi)$ have finite dimension and are mutually orthogonal. Letting $\{u_{j}\}$ be any orthonormal basis of cusp forms for $\mc{C}_{k}(\G,\chi)$, every $f \in \mc{C}_{k}(\G,\chi)$ admits a series of the form
      \[
  ...
  \section{Double Coset Operators}
    We can extend the theory of double coset operators to weight zero Maass form just as we did for holomorphic forms. Indeed, for any $\a \in \GL_{2}^{+}(\Q)$, we define the operator $[\a]$ on $\mc{A}_{r}(\G)$ to be the linear operator given by
    \[
    ...
    \[
      (f[\g])(z) = f(\g z) = f(z),
    \]
    is equivalent to the automorphy of $f$ on $\GH$. For any two congruence subgroups $\G_{1}$ and $\G_{2}$ (not necessarily of the same level) and any $\a \in \GL_{2}^{+}(\Q)$, we define the \textbf{double coset operator}\index{double coset operator} $[\G_{1}\a\G_{2}]$ to be the linear operator on $\mc{A}_{r}(\G_{1})$ given by
    \[
    ...
    \begin{proposition}\label{prop:double_coset_operator_preserves_subspaces_Maass}
      For any two congruence subgroups $\G_{1}$ and $\G_{2}$, $[\G_{1}\a\G_{2}]$ maps $\mc{A}_{r}(\G_{1})$ into $\mc{A}_{r}(\G_{2})$. Moreover, $[\G_{1}\a\G_{2}]$ preserves the subspace of cusp forms.
    \end{proposition}
    \begin{proof}
  ...
  \section{Diamond \& Hecke Operators}
    ...
    \]
    for any $\a = \begin{psmallmatrix} \ast & \ast \\ \ast & d \end{psmallmatrix} \in \G_{0}(N)$. Therefore, for any $d$ taken modulo $N$, we define the \textbf{diamond operator} $\<d\>:\mc{A}_{r}(\G_{1}(N)) \to \mc{A}_{r}(\G_{1}(N))$ to be the linear operative given by
    \[
      (\<d\>f)(z) = (f[\a])(z),
    \]
    for any $\a = \begin{psmallmatrix} \ast & \ast \\ \ast & d \end{psmallmatrix} \in \G_{0}(N)$. As for holomorphic forms, the diamond operators are multiplicative and invertible. They also decompose $\mc{A}_{r}(\G_{1}(N))$ into eigenspaces. For any Dirichlet character modulo $N$, let
    \[
      \mc{A}_{r}(N,\chi) = \{f \in \mc{A}_{r}(\G_{1}(N)):\<d\>f = \chi(d)f \text{ for all } d \in (\Z/N\Z)^{\ast}\},
    \]
    be the $\chi$-eigenspace. Also let $\mc{C}_{r}(N,\chi)$ be the corresponding subspace of cusp forms. Then $\mc{A}_{r}(\G_{1}(N))$ admits a decomposition into these eigenspaces:

    \begin{proposition}\label{thm:diamond_operator_decomposition_Maass}
      We have a direct sum decomposition
      \[
        \mc{A}_{r}(\G_{1}(N)) = \bigop_{\chi \tmod{N}}\mc{A}_{r}(N,\chi).
      \]
    ...
    \begin{proposition}\label{prop:diamond_Hecke_operators_commute_Maass}
      For every $d \in (\Z/N\Z)^{\ast}$ and prime $p$, the diamond operator $\<d\>$ and the Hecke operator $T_{p}$ on $\mc{A}_{r}(\G_{1}(N))$ commute:
      \[
        \<d\>T_{p} = T_{p}\<d\>
      \]
    \end{proposition}
    ...
    \begin{proposition}\label{prop:explicit_description_of_Hecke_operators_Maass}
      Let $f \in \mc{A}_{r}(\G_{1}(N))$. Then the Hecke operator $T_{p}$ acts on $f$ as follows:
      \[
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{A}_{r}(\G_{1}(N))$ Fourier coefficients $a_{n}(f)$, $a_{0}(f)$, and $b_{0}(f)$. Then for primes $p$ with $(p,N) = 1$,
      \begin{align*}
        (T_{p}f)(z) = (a_{0}(f)+\chi_{N,0}(p)a_{0}(\<p\>f))y^{\frac{1}{2}+ir}&+(b_{0}(f)+\chi_{N,0}(p)b_{0}(\<p\>f))y^{\frac{1}{2}-ir} \\
        &+ \sum_{n \neq 0}\left(a_{np}(f)+\chi_{N,0}(p)a_{\frac{n}{p}}(\<p\>f)\right)\sqrt{y}K_{ir}(2\pi|n|y)e^{2\pi inx},
      \end{align*}
      is the Fourier series of $T_{p}f$ where it is understood that $a_{\frac{n}{p}}(f) = 0$ if $p \nmid n$. Moreover, if $f \in \mc{A}_{r}(N,\chi)$, then $T_{p}f \in \mc{A}_{r}(N,\chi)$ and
      \begin{align*}
        (T_{p}f)(z) = (a_{0}(f)+\chi(p)a_{0}(f))y^{\frac{1}{2}+ir}&+(b_{0}(f)+\chi(p)b_{0}(f))y^{\frac{1}{2}-ir} \\
        &+ \sum_{n \neq 0}\left(a_{np}(f)+\chi(p)a_{\frac{n}{p}}(f)\right)\sqrt{y}K_{ir}(2\pi|n|y)e^{2\pi inx},
      \end{align*}
    ...
    \begin{proposition}\label{prop:Hecke_operators_commute_Maass}
      Let $p$ and $q$ be primes and $d,e \in (\Z/N\Z)^{\ast}$. Then the Hecke operators $T_{p}$ and $T_{q}$ and diamond operators $\<d\>$ and $\<e\>$ on $\mc{A}_{r}(\G_{1}(N))$ form a simultaneously commuting family:
      \[
    ...
    We use \cref{prop:Hecke_operators_commute_Maass} to construct diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$ exactly as for holomorphic forms. Explicitly, the \textbf{diamond operator}\index{diamond operator} $\<m\>:\mc{A}_{r}(\G_{1}(N)) \to \mc{A}_{r}(\G_{1}(N))$ is defined to be the linear operator given by
    \[
      \<m\> = \begin{cases} \<m\> \text{ with $m$ taken modulo $N$} & \text{if $(m,N) = 1$,} \\ 0 & \text{if $(m,N) > 1$.} \end{cases}
    \]
    For the Hecke operators, if $m = p_{1}^{r_{1}}p_{2}^{r_{2}} \cdots p_{k}^{r_{k}}$ is the prime decomposition of $m$, then the $m$-th \textbf{Hecke operator}\index{Hecke operator} $T_{m}:\mc{A}_{k,r}(\G,\chi) \to \mc{A}_{k,r}(\G,\chi)$ is the linear operator given by
    \[
    ...
    \begin{proposition}\label{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{A}_{r}(\G_{1}(N))$ with Fourier coefficients $a_{n}(f)$, $a_{0}(f)$, and $b_{0}(f)$. Then for $m \ge 1$ with $(m,N) = 1$,
      \begin{align*}
        (T_{m}f)(z) = \left(\sum_{d \mid m}a_{0}(\<d\>f)\right)y^{\frac{1}{2}+ir}&+\left(\sum_{d \mid m}b_{0}(\<d\>f)\right)y^{\frac{1}{2}-ir} \\
        &+ \sum_{n \neq 0}\left(\sum_{d \mid (n,m)}a_{\frac{nm}{d^{2}}}(\<d\>f)\right)\sqrt{y}K_{ir}(2\pi|n|y)e^{2\pi inx},
      \end{align*}
      is the Fourier series of $T_{m}f$. Moreover, if $f \in \mc{A}_{r}(N,\chi)$, then
      \begin{align*}
        (T_{m}f)(z) = \left(\sum_{d \mid m}\chi(d)a_{0}(f)\right)y^{\frac{1}{2}+ir}&+\left(\sum_{d \mid m}\chi(d)b_{0}(f)\right)y^{\frac{1}{2}-ir} \\
        &+ \sum_{n \neq 0}\left(\sum_{d \mid (n,m)}\chi(d)a_{\frac{nm}{d^{2}}}(f)\right)\sqrt{y}K_{ir}(2\pi|n|y)e^{2\pi inx}.
      \end{align*}
    \end{proposition}
    \begin{proof}
    ...
    \begin{proposition}\label{prop:Petersson_adjoint_Maass}
      ...
      \begin{enumerate}[label=(\roman*)]
        \item If $\a^{-1}\G\a \subseteq \PSL_{2}(\Z)$, then for all $f \in \mc{C}_{k,r}(\G,\chi)$ and $g \in \mc{C}_{r}(\a^{-1}\G\a)$, we have
        \[
          \<f[\a],g\>_{\a^{-1}\G\a} = \<f,g[\a^{-1}]\>_{\G}.
        \]
        \item For all $f,g \in \mc{C}_{k,r}(\G,\chi)$, we have
        \[
    ...
    \begin{proposition}\label{prop:Hecke_operators_normal_Maass}
      On $\mc{C}_{r}(\G_{1}(N))$, the diamond operators $\<m\>$ and Hecke operators $T_{m}$ are normal with respect to the Petersson inner product for all $m \ge 1$ with $(m,N) = 1$. Moreover, their adjoints are given by
      \[
    ...
    \begin{proposition}\label{prop:sign_operator_commutes_with_Hecke_and_diamond_and_is_normal}
      \phantom{ }
      \begin{enumerate}
          \item On $\mc{A}_{r}(\G_{1}(N))$, the operator $T_{-1}$ commutes with the diamond operators $\<m\>$ and Hecke operators $T_{n}$ for all $m,n \ge 1$.
          \item On $\mc{C}_{r}(\G_{1}(N))$ the operator $T_{-1}$ is normal with respect to the Petersson inner product and the adjoint is given by
          \[
            T_{-1}^{\ast} = -T_{-1}.
          \]
      \end{enumerate}
    ...
    \begin{theorem}\label{thm:eigenforms_forms_spectral_theory_Maass}
      $\mc{C}_{r}(\G_{1}(N))$ admits an orthonormal basis of eigenforms.
    \end{theorem}
    \begin{proof}
    ...
    \begin{proposition}[Hecke relations, Maass version]
      Let $f \in \mc{C}_{r}(N,\chi)$ be a Hecke-Maass eigenform with Fourier coefficients $\l_{f}(n)$. Then the Fourier coefficients are multiplicative and satisfy
      \[
  ...
  \section{Atkin–Lehner Theory}
    There is also an Atkin–Lehner theory for weight zero Maass form. As with holomorphic forms, we will only deal with congruence subgroups of the form $\G_{1}(N)$ and cusp forms on $\G_{1}(N)\backslash\H$. The trivial way to lift Maass forms from a smaller level to a larger level is via the natural inclusion $\mc{C}_{r}(\G_{1}(M)) \subseteq \mc{C}_{r}(\G_{1}(N))$ provided $M \mid N$ which follows from $\G_{1}(N) \le \G_{1}(M)$. Alternatively, for any $d \mid \frac{N}{M}$, let $\a_{d} = \begin{psmallmatrix} d & 0 \\ 0 & 1 \end{psmallmatrix}$. If $f \in \mc{C}_{r}(\G_{1}(M))$, then
    \[
      (f[\a_{d}])(z) = f(\a_{d} z) = f(dz).
    \]
    Similar to holomorphic forms, $[\a_{d}]$ maps $\mc{C}_{r}(\G_{1}(M))$ into $\mc{C}_{r}(\G_{1}(N))$:
    
    \begin{proposition}\label{equ:lifting_operator_Maass}
      Let $M$ and $N$ be positive integers such that $M \mid N$. For any $d \mid \frac{N}{M}$, $[\a_{d}]$ maps $\mc{C}_{r}(\G_{1}(M))$ into $\mc{C}_{r}(\G_{1}(N))$.
    \end{proposition}
    \begin{proof}
    ...
    \[
      i_{d}:\mc{C}_{r}\left(\G_{1}\left(\frac{N}{d}\right)\right)\x\mc{C}_{r}\left(\G_{1}\left(\frac{N}{d}\right)\right) \to \mc{C}_{r}(\G_{1}(N)) \qquad (f,g) \mapsto f+g[\a_{d}].
    \]
    ...
    \[
      \mc{C}_{r}(\G_{1}(N))^{\mathrm{old}} = \bigop_{p \mid N}\Im(i_{p}),
    \]
    and the subspace of \textbf{newforms of level $N$}\index{newforms of level $N$} is
    \[
      \mc{C}_{r}(\G_{1}(N))^{\mathrm{new}} = \left(\mc{C}_{r}(\G_{1}(N))^{\mathrm{old}} \right)^{\perp},
    \]
    ...
    \begin{proposition}\label{prop:old/new_subspaces_are_invariant_Maass}
      The spaces $\mc{C}_{r}(\G_{1}(N))^{\mathrm{old}}$ and $\mc{C}_{r}(\G_{1}(N))^{\mathrm{new}}$ are invariant under the diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$.
    \end{proposition}

    As a corollary, these subspaces admit orthogonal bases of eigenforms:

    \begin{corollary}\label{cor:old/new_eigenbasis_Maass}
      $\mc{C}_{r}(\G_{1}(N))^{\mathrm{old}}$ and $\mc{C}_{r}(\G_{1}(N))^{\mathrm{new}}$ admit orthonormal bases of eigenforms.
    \end{corollary}
    \begin{proof}
      This follows immediately from \cref{thm:eigenforms_forms_spectral_theory_Maass,prop:old/new_subspaces_are_invariant_Maass}
    \end{proof}

    We can remove the condition $(m,N) = 1$ for eigenforms in a basis of $\mc{C}_{r}(\G_{1}(N))^{\mathrm{new}}$ so that the eigenforms are eigenfunctions for all of the diamond and Hecke operators. As for holomorphic forms, we need a preliminary result (mimic the proof of \cref{lem:the_main_lemma_for_newforms_holomorphic} as given in \cite{diamond2005first}):

    \begin{lemma}\label{lem:the_main_lemma_for_newforms_Maass}
      If $f \in \mc{C}_{r}(\G_{1}(N))$ with Fourier coefficients $a_{n}(f)$ and such that $a_{n}(f) = 0$ whenever $(n,N) = 1$, then
      \[
        f = \sum_{p \mid N}f_{p}[\a_{p}],
      \]
      for some $f_{p} \in \mc{C}_{r}\left(\G_{1}\left(\frac{N}{p}\right)\right)$.
    \end{lemma}

    As was the case for holomorphic forms, we observe from \cref{lem:the_main_lemma_for_newforms_Maass} that if $f \in \mc{C}_{r}(\G_{1}(N))$ is such that its $n$-th Fourier coefficients vanish when $n$ is relatively prime to the level, then $f$ must be an oldform. The main theorem about $\mc{C}_{r}(\G_{1}(N))^{\mathrm{new}}$ can now be proved. We say that $f$ is a \textbf{primitive Hecke-Maass eigenform}\index{primitive Hecke-Maass eigenform} if it is a nonzero Hecke normalized Hecke-Maass eigenform in $\mc{C}_{r}(\G_{1}(N))^{\mathrm{new}}$. We can now prove the main result about newforms:

    \begin{theorem}\label{thm:newforms_characterization_Maass}
      Let $f \in \mc{C}_{r}(\G_{1}(N))^{\mathrm{new}}$ be an eigenform. Then the following hold:
      \begin{enumerate}[label=(\roman*)]
    ...
    Interestingly, unlike holomorphic forms it is unknown if the Fourier coefficients of Maass forms are real or even algebraic in general. We require one last piece of machinery. As for holomorphic forms, we have an involution on the space $\mc{C}_{r}(N,\chi)$. Recall the matrix
    \[
      W_{N} = \begin{pmatrix} 0 & -1 \\ N & 0 \end{pmatrix},
    \]
    with $\det(W_{N}) = N$. We define the \textbf{Atkin–Lehner involution}\index{Atkin–Lehner involution} $\w_{N}:\mc{C}_{r}(\G_{1}(N)) \to \mc{C}_{r}(\G_{1}(N))$ to be the linear operator given by
    \[
    ...
    \]
    This verifies automorphy for $\w_{N}f$. Also, it is clear that $\w_{N}f$ is a cusp form because $f$ is. Hence $\w_{N}f$ is well-defined. Moreover, it is an involution because $\w_{N}^{2}f = f$ and so its only possible eigenvalues are $\pm 1$. The crucial fact we need is how $\w_{N}$ acts on $\mc{C}_{r}(N,\chi)$. To state the result, for $f \in \mc{C}_{r}(N,\chi)$ define
    \[
      \conj{f}(z) = \conj{f(-z)}.
    \]
  ...
  \section{The Ramanujan-Petersson Conjecture}
    ...
    \begin{conjecture}[Ramanujan-Petersson conjecture, Maass version]
      Suppose $f \in \mc{C}_{r}(N,\chi)$ is a primitive Hecke-Maass eigenform with Hecke eigenvalues $\l_{f}(n)$. Let $\a_{1}(p)$ and $\a_{2}(p)$ be the roots of the $p$-th Hecke polynomial. Then for all primes $p$,
      \[
        |\l_{f}(p)| \le 2.
      \]
      Moreover, if $p \nmid N$, then
    ...
    \[
      \int_{Y}^{\infty}\int_{0}^{1}\sum_{n,m \neq 0}\l_{f}(n)\conj{\l_{f}(m)}K_{ir}(2\pi|n|y)\conj{K_{ir}(2\pi|m|y)}e^{2\pi i(n-m)x}e^{-2\pi(n+m)y}\,\frac{dx\,dy}{y}.
    \]
    ...
    \[
      \int_{Y}^{\infty}\sum_{n \neq 0}|\l_{f}(n)|^{2}|K_{ir}(2\pi|n|y)|^{2}e^{-4\pi ny}\,\frac{dx\,dy}{y}.
    \]
    As this expression is finite, we have
    \[
      |\l_{f}(n)|^{2}\int_{Y}^{\infty}K_{ir}(2\pi|n|y)|^{2}e^{-4\pi ny}\,\frac{dx\,dy}{y} \ll \int_{Y}^{\infty}\int_{0}^{1}|f(x+iy)|^{2}\,\frac{dx\,dy}{y^{2}},
    \]
    ...
    \[
      |\l_{f}(n)|^{2}\int_{Y}^{\infty}K_{ir}(2\pi|n|y)|^{2}e^{-4\pi ny}\,\frac{dx\,dy}{y} \ll \frac{1}{Y}.
    \]
  ...
  \section{Twists of Maass Forms}
    We can also twist of Maass forms by Dirichlet characters. Let $f \in \mc{A}_{r}(N,\chi)$ with Fourier series
    \[
      f(z) = a_{0}(f)y^{\frac{1}{2}+ir}+b_{0}(f)y^{\frac{1}{2}-ir}+\sum_{n \neq 0}a_{n}(f)\sqrt{y}K_{ir}(2\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \[
      (f \ox \psi)(z) = a_{0}(f)\psi(0)y^{\frac{1}{2}+ir}+b_{0}(f)\psi(0)y^{\frac{1}{2}-ir}+\sum_{n \neq 0}a_{n}(f)\psi(n)\sqrt{y}K_{ir}(2\pi|n|y)e^{2\pi inx}.
    \]
    ...
    \begin{proposition}\label{prop:twisted_Maass_forms_primitive}
      Suppose $f \in \mc{A}_{r}(N,\chi)$ and $\psi$ is a primitive Dirichlet character of conductor $q$. Then $f \ox \psi \in \mc{A}_{r}(Nq^{2},\chi\psi^{2})$. Moreover, if $f$ is a cusp form then so is $f \ox \psi$.
    \end{proposition}
    \begin{proof}
    ...
    The generalization of \cref{prop:twisted_Maass_forms_primitive} to all characters is slightly more involved. Define operators $U_{p}$ and $V_{p}$ on $\mc{A}_{r}(\G_{1}(N))$ to be the linear operators given by
    \[
      (U_{p}f)(z) = a_{0}(f)y^{\frac{1}{2}+ir}+b_{0}(f)y^{\frac{1}{2}-ir}+\sum_{n \neq 0}a_{np}(f)\sqrt{y}K_{ir}(2\pi|n|y)e^{2\pi inx},
    \]
    and
    \[
      (V_{p}f)(z) = a_{0}(f)(py)^{\frac{1}{2}+ir}+b_{0}(f)(py)^{\frac{1}{2}-ir}+\sum_{n \neq 0}a_{n}(f)\sqrt{py}K_{ir}(2\pi|n|py)e^{2\pi inpx},
    \]
    if $f$ has Fourier series
    \[
      f(z) = a_{0}(f)y^{\frac{1}{2}+ir}+b_{0}(f)y^{\frac{1}{2}-ir}+\sum_{n \neq 0}a_{n}(f)\sqrt{y}K_{ir}(2\pi|n|y)e^{2\pi inx}.
    \]
    We will show that both $U_{p}$ and $V_{p}$ map $\mc{A}_{r}(\G_{1}(N))$ into $\mc{A}_{r}(\G_{1}(Np))$ and more:

    \begin{lemma}\label{lem:twisted_Maass_lemma}
      For any prime $p$, $U_{p}$ and $V_{p}$ map $\mc{A}_{r}(\G_{1}(N))$ into $\mc{A}_{r}(\G_{1}(Np))$. In particular, $U_{p}$ and $V_{p}$ map $\mc{A}_{r}(N,\chi)$ into $\mc{A}_{r}(Np,\chi\chi_{p,0})$. Moreover, $U_{p}$ and $V_{p}$ preserve the subspace of cusp forms.
    \end{lemma}
    \begin{proof}
      Mimic the proof of \cref{thm:diamond_operator_decomposition_holomorphic}.
    \end{proof}
    ...
    \begin{proposition}\label{prop:twisted_Maass_forms}
      Suppose $f \in \mc{A}_{r}(N,\chi)$ and $\psi$ is a Dirichlet character modulo $M$. Then $f \ox \psi \in \mc{A}_{r}(NM^{2},\chi\psi^{2})$. Moreover, if $f$ is a cusp form then so is $f \ox \psi$.
    \end{proposition}
    \begin{proof}
      Mimicking the proof of \cref{prop:twisted_holomorphic_forms}, it remains to show that $f \ox \psi_{p,0}$ is an eigenfunction with eigenvalue $\l$ if $f$ is. As $U_{p} = T_{p}$ for the $p$-th Hecke operator on $\mc{A}_{r}(\G_{1}(Np))$, $U_{p}$ commutes with $\D$. It is also clear that $V_{p}$ commutes with $\D$. These facts together with
      \[
