\chapter{The Theory of Maass Forms}
  ...
  \section{Eisenstein \& Poincar\'e Series}
    ...
    \[
      E_{k,\chi,\mf{a}}(z,s) \ll \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{\Im(z)^{\Re(s)}}{|cz+d|^{2\Re(s)}}.
    \]
    ...
    \[
      E_{k,\chi,\mf{a}}(\s_{\mf{b}}z,s) \ll \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{\Im(\s_{\mf{b}}z)^{\Re(s)}}{|c\s_{\mf{b}}z+d|^{2\Re(s)}} = \frac{\Im(z)^{\Re(s)}}{|c'z+d'|^{2\Re(s)}}\sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{1}{|c\s_{\mf{b}}z+d|^{2\Re(s)}}.
    \]
    Now decompose this last sum as
    \[
      \sum_{(c,d) \in \Z^{2}-\{\mathbf{0}\}}\frac{1}{|c\s_{\mf{b}}z+d|^{2\Re(s)}} = \sum_{d \neq 0}\frac{1}{d^{2\Re(s)}}+\sum_{c \neq 0}\sum_{d \in \Z}\frac{1}{|c\s_{\mf{b}}z+d|^{2\Re(s)}} = 2\sum_{d \ge 1}\frac{1}{d^{2\Re(s)}}+2\sum_{c \ge 1}\sum_{d \in \Z}\frac{1}{|c\s_{\mf{b}}z+d|^{2\Re(s)}}.
    \]
    Notice that $\sum_{c \ge 1}\sum_{d \in \Z}\frac{1}{d^{2\Re(s)}}$ converges provided $\Re(s) > 1$. Moreover, the exact same argument as for holomorphic form Eisenstein series shows that $\sum_{c \ge 1}\sum_{d \in \Z}\frac{1}{|c\s_{\mf{b}}z+d|^{2s}}$ converges in this region as well. Now for all $\Im(z) \ge 1$ and $\Re(s) > 1$,
    \[
      \frac{\Im(z)^{\Re(s)}}{|c'z+d'|^{2\Re(s)}} \ll \frac{\Im(z)^{\Re(s)}}{|c'\Im(z)|^{2\Re(s)}} \ll \Im(z)^{\Re(s)}.
    \]
    So altogether,
    \[
      E_{\mf{a}}(\s_{\mf{b}}z,s) \ll \Im(z)^{\Re(s)} = o(e^{2\pi\Im(z)}),
    \]
  ...
  \section{Spectral Theory of the Laplace Operator}
    ...
    \]
    for any $f,g \in C_{0}^{\infty}(\R^{+})$. For each cups $\mf{a}$ of $\GH$ we associate the \textbf{Eisenstein transform}\index{Eisenstein transform} $E_{k,\chi,\mf{a}}:C_{0}^{\infty}(\R^{+}) \to \mc{A}_{k}(\G,\chi)$ defined by
    \[
