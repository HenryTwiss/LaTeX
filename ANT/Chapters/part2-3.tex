\chapter{The Theory of Maass Forms}
	Maass forms are the non-holomorphic analog to holomorphic forms. They are real-analytic, eigenfunctions for a differential operator, invariant with respect to a subgroup of $\PSL_{2}(\Z)$, and satisfy a growth condition. We introduce both Maass forms and their general theory. Throughout we assume that all of our congruence subgroups are reduced at infinity.
  \section{Maass Forms}
    Define $\e(\g,z)$ by
    \[
    ...
    \]
    If $\e$ and $k$ are clear from content we will suppress this dependencies accordingly. Any operator that commutes with the slash operators $|_{\e,k}\g$ for every $\g \in \GL_{2}^{+}(\Q)$ is said to be \textbf{invariant}\index{invariant}. We define differential operators $R_{k}:C^{\infty}(\H) \to C^{\infty}(\H)$ and $L_{k}:C^{\infty}(\H) \to C^{\infty}(\H)$ to be the linear operators given by
    \[
    ...
    \begin{enumerate}[label=(\roman*)]
      ...
      \]
    \end{enumerate}
    Property (ii) is called the \textbf{automorphy condition}\index{automorphy condition} and we say that $f$ is \textbf{automorphic}\index{automorphic}. The automorphy condition can equivalently be expressed as
    \[
    ...
    \]
    The coefficients $a_{\mf{a}}(n,s)$ and $a_{\mf{a}}^{\pm}(0,s)$ are the only part of the Fourier series that actually depend on the implicit congruence subgroup $\G$. Using these coefficients, $f$ admits \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
    ...
    \]
    where $\H_{Y}$ is the half-plane defined by $Y \le \Im(z) \le 2Y$. This integral exists since $\G_{\infty}\backslash\H_{Y}$ is compact. Substituting in the Fourier series of $f$ at the $\mf{a}$ cusp, this integral can be expressed as
    \[
    ...
    \[
      |a_{\mf{a}}(n,s)|^{2}\int_{Y}^{2Y}|W_{\sgn(n)\frac{k}{2},s-\frac{1}{2}}(4\pi|n|y)|^{2}\,\frac{dy}{y^{2}} \ll \int_{\G_{\infty}\backslash\H_{Y}}|(f|\s_{\mf{a}})(z)|^{2}\,d\mu.
    \]
    Moreover, $|(f|\s_{\mf{a}})(z)|^{2}$ is bounded on $\G_{\infty}\backslash\H_{Y}$ because this space is compact, so that
    \[
      \int_{\G_{\infty}\backslash\H_{Y}}|(f|\s_{\mf{a}})(z)|^{2}\,d\mu \ll \int_{Y}^{2Y}\int_{0}^{1}\frac{dx\,dy}{y^{2}} \ll \frac{1}{Y}.
    \]
    ...
    \]
    This implies $(f|\s_{\mf{a}})(z)$ exhibits decay although it is not necessarily rapid. Accordingly, we say that $f$ exhibits \textbf{moderate decay at the cusps}\index{moderate decay at the cusps}. Observe that $f|\s_{\mf{a}}$ is then bounded on $\H$ and, in particular, $f$ is bounded on $\H$. It is useful to have an alternative Fourier series in the case of weight zero Maass forms that specify the Whittaker function explicitly. When $k = 0$, \cref{thm:Whittaker_special_cases} implies that $f$ admits a \textbf{Fourier series at the $\mf{a}$ cusp}\index{Fourier series at the $\mf{a}$ cusp} given by
    \[
      (f|\s_{\mf{a}})(z) = a_{\mf{a}}^{+}(0,s)y^{s}+a_{\mf{a}}^{-}(0,s)y^{1-s}+\sum_{n \neq 0}a_{\mf{a}}(n,s)\sqrt{4|n|y}K_{s-\frac{1}{2}}(2\pi|n|y)e^{2\pi inx}.
    \]
  \section{Eisenstein Series}
    Let $\G$ be a congruence subgroup of level $N$. We will introduce Eisenstein series on $\GH$. Unlike holomorphic forms, there are not Poincar\'e series for Maass forms. The Eisenstein series however, are defined on a large space $\H \x \{s \in \C:\s > 1\}$ and hence are functions of two variables. Let $k \ge 0$, $\chi$ be a Dirichlet character with conductor $q \mid N$, and $\mf{a}$ be a cusp of $\GH$. Then \textbf{(Maass) Eisenstein series}\index{(Maass) Eisenstein series} $E_{k,\chi,\mf{a}}(z,s)$ of weight $k$ and character $\chi$ on $\GH$ at the $\mf{a}$ cusp is defined by
    \[
      E_{k,\chi,\mf{a}}(z,s) = \sum_{\g \in \G_{\mf{a}}\backslash\G}\cchi(\g)\e(\s_{\mf{a}}^{-1}\g,z)^{-k}\Im(\s_{\mf{a}}^{-1}\g z)^{s}.
    \]
    If $k = 0$, $\chi$ is the trivial character, or $\mf{a} = \infty$, we will drop these dependencies accordingly. We first show that $E_{k,\chi,\mf{a}}(z,s)$ is well-defined. It suffices to show that the summands are independent of the representatives $\g$ and $\s_{\mf{a}}$. This has already been accomplished when we introduced the holomorphic Poincar\'e series for $\cchi(\g)$. Now just as with the holomorphic Poincar\'e series, the set of representatives of $\s_{\mf{a}}^{-1}\g$ is $\G_{\infty}\s_{\mf{a}}^{-1}\g$ and it remains to verify independence from multiplication on the left by an element of $\G_{\infty}$ namely $\eta_{\infty}$. The cocycle relation implies
    \[
    ...
    \]
    because $\eta_{\infty}$ does not affect the imaginary part as it acts by translation. Therefore $\Im(\s_{\mf{a}}^{-1}\g z)$ is independent of the representatives $\g$ and $\s_{\mf{a}}$ as well. We conclude that $E_{k,\chi,\mf{a}}(z,s)$ is well-defined. We claim $E_{k,\chi,\mf{a}}(z,s)$ is also locally absolutely uniformly convergent for $z \in \H$ and $\s > 1$. To see this, the Bruhat decomposition for $\s_{\mf{a}}^{-1}\G$ yields
    \[
    ...
    \begin{align*}
      ...
      &= \sum_{\g' \in \G_{\mf{a}}\backslash\G}\cchi(\g')\left(\frac{\e(\s_{\mf{a}}^{-1}\g'\g,z)}{\e(\g,z)}\right)^{-k}\Im(\s_{\mf{a}}^{-1}\g'\g z)^{s} \\
    ...
    \[
      \e(\s_{\mf{b}},z)^{-k}E_{k,\chi,\mf{a}}(\s_{\mf{b}}z,s) = O(\Im(z)^{\s}),
    \]
    provided $\Im(z) \ge 1$ and $\s > 1$. This verifies the growth condition. The only thing left to verify is that $E_{k,\chi,\mf{a}}(z,s)$ is an eigenfunction for $\D_{k}$. To see this, first observe that
    \[
  ...
  \section{The Petersson Inner Product}
    Let $\mc{M}_{k}(\G,\chi)$ denote the space of all weight $k$ Maass forms with character $\chi$ on $\GH$ and let $\mc{C}_{k}(\G,\chi)$ denote the associated subspace of cusp forms. Moreover, let $\mc{M}_{k,\l}(\G,\chi)$ and $\mc{C}_{k,\l}(\G,\chi)$ denote the associated subspaces whose eigenvalue is $\l$. Moreover, if $k = 0$ or $\chi$ is the trivial character, we will suppress these dependencies. If $\G_{1}$ and $\G_{2}$ are two congruence subgroups such that $\G_{1} \le \G_{2}$, then we have the inclusion
    \[
      \mc{M}_{k,\l}(\G_{2},\chi) \subseteq \mc{M}_{k,\l}(\G_{1},\chi),
    \]
    and this respects the subspace of cusp forms. So in general, the smaller the congruence subgroup the more Maass forms there are. Our goal is to turn $\mc{C}_{k,\nu}(\G,\chi)$ into a Hilbert space to which we can apply a linear theory. To this end, for $f,g \in \mc{C}_{k,\l}(\G,\chi)$ define their \textbf{Petersson inner product}\index{Petersson inner product} by
    \[
      \<f,g\>_{\G} = \frac{1}{V_{\G}}\int_{\mc{F}_{\G}}f(z)\conj{g(z)}\,d\mu.
    \]
    If the congruence subgroup is clear from context we will suppress the dependence upon $\G$. Since $f$ and $g$ exhibit moderate decay at the cusps, the integral is locally absolutely uniformly convergent by \cref{prop:decay_finite_volume_integral}. The integrand is also $\G$-invariant so that the integral independent of the choice of fundamental domain. These two facts together imply that the Petersson inner product is well-defined. We will continue to use this notation even if $f$ and $g$ do not belong to $\mc{C}_{k,\l}(\G,\chi)$ provided the integral is locally absolutely uniformly convergent. A basic property of the Petersson inner product is that it is invariant with respect to the slash operator:

    \begin{proposition}\label{prop:Petersson_slash_invariance_Maass}
      For any $f,g \in \mc{C}_{k,\l}(\G,\chi)$ and $\a \in \PSL_{2}(\Z)$, we have
      \[
    ...
    It can be shown that the Laplace operator $\D_{k}$ is positive and symmetric on $\mc{C}_{k,\l}(\G,\chi)$ (see \cite{iwaniec2002spectral} for a proof in the weight zero case and \cite{duke2002subconvexity} for notes on the general case):

    \begin{theorem}\label{thm:Laplace_semi-definite_self-adjoint}
      On $\mc{C}_{k,\l}(\G,\chi)$, the Laplace operator $\D_{k}$ is positive semi-definite and self-adjoint.
    \end{theorem}

    So if $f \in \mc{C}_{k,\l}(\G,\chi)$, \cref{thm:Laplace_semi-definite_self-adjoint} implies $\l$ is real and positive. Since $\l = s(1-s)$ and $\l$ is real, $s$ and $1-s$ are either conjugates or real. In the former case, $s = 1-\conj{s}$ and we find that
    \[
    ...
    \[
      \l = s(1-s) = \frac{1}{4}-\nu^{2} = \frac{1}{4}+r^{2}.
    \]
    So to specify $\l$ it suffices to specify either $s$, the spectral parameter $r$, or the type $\nu$. Accordingly, we write $\mc{M}_{k,s}(\G,\chi)$, $\mc{M}_{k,r}(\G,\chi)$, or $\mc{M}_{k,\nu}(\G,\chi)$ for the space $\mc{M}_{k,\l}(\G,\chi)$ and $\mc{C}_{k,s}(\G,\chi)$, $\mc{C}_{k,r}(\G,\chi)$, or $\mc{C}_{k,\nu}(\G,\chi)$ for the space $\mc{C}_{k,\l}(\G,\chi)$. We will now need a dimensionality result regarding $\mc{C}_{k,\l}(\G,\chi)$ (see \cite{iwaniec2002spectral} for a proof in the case $\chi$ is trivial):

    \begin{theorem}\label{thm:Maass_forms_finite_dimensional}
      $\mc{C}_{k,\l}(\G,\chi)$ is finite dimensional.
    \end{theorem}

     Just as for holomorphic forms, the Petersson inner product turns $\mc{C}_{k,\l}(\G,\chi)$ into a Hilbert space:

    \begin{proposition}\label{prop:Petersson_inner_product_hermitian_Maass}
      $\mc{C}_{k,\l}(\G,\chi)$ is a Hilbert space with respect to Petersson inner product.
    \end{proposition}
    \begin{proof}
      The argument used in the proof of \cref{prop:Petersson_inner_product_hermitian_holomorphic} holds verbatim.
    \end{proof}
  \section{Double Coset Operators}
    We can extend the theory of double coset operators to Maass form just as we did for holomorphic forms. Indeed, for any $\a \in \GL_{2}^{+}(\Q)$, we define the operator $[\a]_{k}$ on $\mc{C}_{k,\nu}(\G)$ to be the linear operator given by
    \[
      (f[\a]_{k})(z) = \det(\a)^{-1}\e(\a,z)^{-k}f(\a z),
    \]
    Moreover, $[\a]_{k}$ is multiplicative. Indeed, if $\a,\a' \in \GL_{2}^{+}(\Q)$, then
    \begin{align*}
      ((f[\a']_{k})[\a]_{k})(z) &= \det(\a)^{-1}\e(\a,z)^{-k}(f[\a']_{k})(\a z) \\
      &= \det(\a')^{-1}\det(\a)^{-1}\e(\a',\a z)^{-k}\e(\a,z)^{-k}f(\a'\a z) \\
      &= \det(\a'\a)^{-1}\e(\a'\a,z)^{-k}f(\a'\a z) && \text{cocycle condition} \\
      &= (f[\a'\a]_{k})(z).
    \end{align*}
    Also, if $\g \in \G$ and we choose the representative with positive determinant, then the chain of equalities
    \[
      (f[\g]_{k})(z) = \e(\g,z)^{-k}f(\g z) = \chi(\g)f(z),
    \]
    is equivalent to the automorphy of $f$ on $\GH$ with character $\chi$. For any two congruence subgroups $\G_{1}$ and $\G_{2}$ (not necessarily of the same level) and any $\a \in \GL_{2}^{+}(\Q)$, we define the \textbf{double coset operator}\index{double coset operator} $[\G_{1}\a\G_{2}]_{k}$ to be the linear operator on $\mc{C}_{k,\nu}(\G_{1})$ given by
    \[
      (f[\G_{1}\a\G_{2}]_{k})(z) = \sum_{j}(f[\b_{j}]_{k})(z) = \sum_{j}\det(\b_{j})^{-1}\e(\b_{j},z)^{-k}f(\b_{j}z).
    \]
    As was the case for holomorphic forms, \cref{prop:double_congruence_subgroup_coset_decomposition_is_finite} implies that this sum is finite. It remains to check that $f[\G_{1}\a\G_{2}]_{k}$ is well-defined. Indeed, if $\b_{j}$ and $\b_{j}'$ belong to the same orbit, then $\b_{j}'\b_{j}^{-1} \in \G_{1}$. But then as $f \in \mc{C}_{k,\nu}(\G_{1})$, is it invariant under the $[\b_{j}'\b_{j}^{-1}]_{k}$ operator so that
    \[
    ...
    \begin{proof}
      ...
      \[
        \D(f[\G_{1}\a\G_{2}]_{k})(z) = \sum_{j}\D(f[\b_{j}]_{k})(z) = \l\sum_{j}\det(\b_{j})^{-1}\e(\b_{j},z)^{-k}f(\b_{j}z) = \l(f[\G_{1}\a\G_{2}]_{k})(z). 
      \]
  ...
  \section{Diamond \& Hecke Operators}
    Extending the theory of diamond operators and Hecke operators is also fairly straightforward. To see this, we have already shown that $\G_{1}(N)$ is normal in $\G_{0}(N)$ so
    \[
      \left(f\left[\G_{1}(N)\a\G_{1}(N)\right]_{k}\right)(z) = \sum_{j}(f[\b_{j}]_{k})(z) = (f[\a]_{k})(z),
    \]
    ...
    \[
      (\<d\>f)(z) = (f[\g]_{k})(z) = f(\g z) = \chi(d)f(z),
    \]
    is equivalent to the automorphy of $f$ with character $\chi$ on $\G_{0}(N)\backslash\H$. So $f$ is a Maass form with character $\chi$ on $\G_{0}(N)\backslash\H$ if and only if $f[\g]_{k} = \chi(\g)f$ for all $\g \in \G_{0}(N)$ where $\g$ is chosen to be the representative with positive determinant. It follows that the diamond operators sieve Maass forms on $\G_{1}(N)\backslash\H$ with trivial character in terms of Maass forms on $\G_{0}(N)\backslash\H$ with nontrivial characters. Precisely, $\mc{C}_{k,\nu}(N,\chi) = \mc{C}_{k,\nu}(\G_{0}(N),\chi)$ and $\mc{C}_{k,\nu}(N,\chi) = \mc{C}_{k,\nu}(\G_{0}(N),\chi)$. So by \cref{thm:diamond_operator_decomposition_Maass}, we have
    \[
      \mc{C}_{k,\nu}(\G_{1}(N)) = \bigop_{\chi \tmod{N}}\mc{C}_{k,\nu}(\G_{0}(N),\chi).
    \]
    ...
    \begin{proposition}\label{prop:diamond_Hecke_operators_commute_Maass}
      For every $d \in (\Z/N\Z)^{\ast}$ and prime $p$, the diamond operator $\<d\>$ and the Hecke operator $T_{p}$ on $\mc{C}_{k,\nu}(\G_{1}(N))$ commute:
      \[
        \<d\>T_{p} = T_{p}\<d\>
      \]
    \end{proposition}
    ...
    \begin{proposition}\label{prop:prime_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ Fourier coefficients $a_{n}(f)$ and $a_{0}^{\pm}(f)$. Then for all primes $p$,
      \[
    ...
    \begin{proposition}\label{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass}
      Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ with Fourier coefficients $a_{n}(f)$ and $a_{0}^{\pm}(f)$. Then for $m \ge 1$ with $(m,N) = 1$,
      \[
        (T_{m}f)(z) = \sum_{n \neq 0}\left(\sum_{d \mid (|n|,m)}d^{-1}a_{\frac{nm}{d^{2}}}(\<d\>f)\right)W_{\sgn(n)\frac{k}{2},\nu}(4\pi|n|y)e^{2\pi inx},
      \]
      is the Fourier series of $T_{m}f$. Moreover, if $f \in \mc{C}_{k,\nu}(N,\chi)$, then
      \[
    ...
    The diamond and Hecke operators turn out to be normal with respect to the Petersson inner product on the subspace of cusp forms. Just as with holomorphic forms, we can use \cref{lem:adjoint_lemma} to compute adjoints:
    ...
    \begin{proposition}\label{prop:Petersson_adjoint_Maass}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \[
          \<f[\a]_{k},g\>_{\a^{-1}\G\a} = \<f,g[\a^{-1}]_{k}\>_{\G}.
        \]
        \item For all $f,g \in \mc{C}_{k,\nu}(\G,\chi)$, we have
        \[
          \<f[\G\a\G]_{k},g\> = \<f,g[\G\a^{-1}\G]_{k}\>.
        \]
      \end{enumerate}
      In particular, if $\a^{-1}\G\a = \G$ then $[\a]_{k}^{\ast} = [\a^{-1}]_{k}$ and $[\G\a\G]_{k}^{\ast} = [\G\a^{-1}\G]_{k}$. 
    \end{proposition}
    \begin{proof}
    ...
    When we are considering Maass forms on the full modular group, \cref{prop:Hecke_operators_normal_Maass} says that all of the diamond and Hecke operators are normal. Before discussing the spectral theory, we need one last operator. Let $X:\mc{C}_{k,\nu}(\G_{1}(N)) \to \mc{C}_{k,\nu}(\G_{1}(N))$ be the linear operator defined by
    \[
      (Xf)(z) = f(-\conj{z}).
    \]
    As $-\conj{z} = -x+iy$, $X$ acts as reflection with respect to $x$. Then define the \textbf{parity operator}\index{parity operator} $T_{-1}$ to be the linear operator on $\mc{C}_{k,\nu}(\G_{1}(N))$ given by
    \[
      T_{-1} = R\prod_{\substack{-k < \ell < k \\ \ell \equiv k \tmod{2}}}L_{k}.
    \]
    We will also set
    \[
      \d(\nu,k) = \frac{\G\left(\nu+\frac{1-k}{2}\right)}{\G\left(\nu+\frac{1+k}{2}\right)}.
    \]
    Notice that $\d(\nu,0) = 1$. The parity operator acts as an involution on $\mc{C}_{k,\nu}(\G_{1}(N))$ and more as the following proposition shows (see \cite{duke2002subconvexity} for a proof):

    \begin{proposition}\label{prop:parity_operator_Fourier_series}
      $T_{-1}$ is an involution on $\mc{C}_{k,\nu}(\G_{1}(N))$. If $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ has Fourier coefficients $a_{n}(f)$ and $a_{0}^{\pm}(f)$, then
      \[
        (T_{-1}f)(z) = \sum_{n \ge 1}a_{-n}(f)\d(\nu,k)W_{\frac{k}{2},\nu}(4\pi |n|y)e^{2\pi inx}+a_{n}(f)\d(\nu,k)^{-1}W_{-\frac{k}{2},\nu}(4\pi |n|y)e^{-2\pi inx},
      \]
      is the Fourier series of $T_{-1}f$.
    \end{proposition}
    
    Moreover, we also have the following proposition encompassing important properties of the parity operator:

    \begin{proposition}\label{prop:sign_operator_commutes_with_Hecke_and_diamond_and_is_normal}
      \phantom{ }
      \begin{enumerate}
        \item On $\mc{C}_{k,\nu}(\G_{1}(N))$, the parity operator $T_{-1}$ commutes with the diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$.
        \item On $\mc{C}_{k,\nu}(\G_{1}(N))$ the parity operator $T_{-1}$ is self-adjoint with respect to the Petersson inner product and the adjoint is given by
        \[
          T_{-1}^{\ast} = -T_{-1}.
        \]
      \end{enumerate}
    \end{proposition}
    \begin{proof}
      To prove (i), by linearity it suffices to assume $f \in \mc{C}_{k,\nu}(N,\chi)$. Then commutativity with the diamond operators $\<m\>$ is immediate from the definition. As for the Hecke operators $T_{m}$, commutativity follows from \cref{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass,prop:parity_operator_Fourier_series}. Similarly (ii), follows from the Fourier series in \cref{prop:parity_operator_Fourier_series}
    \end{proof}

    Let $f \in \mc{C}_{k,\nu}(\G_{1}(N))$. As $T_{-1}$ is an involution, the only possible eigenvalues are $\pm 1$. Accordingly, we say that $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ is \textbf{even}\index{even} if $T_{-1}f = f$ and is \textbf{odd}\index{odd} if $T_{-1}f = -f$. Then by \cref{prop:parity_operator_Fourier_series},
    \[
      a_{-n}(f) = \pm a_{n}(f)\frac{\G\left(\nu+\frac{1+k}{2}\right)}{\G\left(\nu+\frac{1-k}{2}\right)},
    \]
    for all $n \ge 1$ and with $\pm$ according to if $f$ is even or odd. Moreover, $a_{0}^{\pm}(f) = 0$ if $f$ is odd. Thus the fourier series of $F$ takes the form
    \begin{align*}
      f(z) = a_{0}^{+}y^{\frac{1}{2}+\nu}&+a_{0}^{-}y^{\frac{1}{2}-\nu} \\
      &+\sum_{n \ge 1}a_{n}(f)\left(W_{\frac{k}{2},\nu}(4\pi |n|y)e^{2\pi inx}\pm\frac{\G\left(\nu+\frac{1+k}{2}\right)}{\G\left(\nu+\frac{1-k}{2}\right)}W_{-\frac{k}{2},\nu}(4\pi |n|y)e^{-2\pi inx}\right),
    \end{align*}
    with $\pm$ according to if $f$ is even or odd and for some potentially different Fourier coefficients $a_{0}^{\pm}(f)$ (they are zero if $f$ is odd). If the weight is zero, the Fourier series drastically simplifies via, \cref{thm:Whittaker_special_cases}, that $\d(\nu,0) = 1$, and the exponential identities for sine and cosine, so that we obtain
    \[
      f(z) = a_{0}^{+}y^{\frac{1}{2}+\nu}+a_{0}^{-}y^{\frac{1}{2}-\nu}+\sum_{n \ge 1}a_{n}(f)\sqrt{4|n|y}K_{\nu}(2\pi |n|y)\SC(2\pi nx),
    \]
    where $\SC(x) = \cos(x)$ if $f$ is even and $\SC(x) = i\sin(x)$ if $f$ is odd. The benefit of working with even and odd Maass forms is that it suffices to determine non-constant Fourier coefficients for $n \ge 1$ instead of $n \neq 0$. Now suppose $f$ is a non-constant Maass cusp form. Let the eigenvalue of $T_{m}$ for $f$ be $\l_{f}(m)$. We say that the $\l_{f}(m)$ are the \textbf{Hecke eigenvalues}\index{Hecke eigenvalues} of $f$. Similar to the case for holomorphic forms, if $f$ is a Maass form on $\G_{1}(N)\backslash\H$ that is a simultaneous eigenfunction for all diamond operators $\<m\>$ and Hecke operators $T_{m}$ with $(m,N) = 1$ and the parity operator $T_{-1}$, we call $f$ an \textbf{eigenform}\index{eigenform}. Necessarily, $f$ is even or odd because $T_{-1}^{2}f = f$ so that the only possible eigenvalues are $\pm 1$. If the condition $(m,N) = 1$ can be dropped, so that $f$ is a simultaneous eigenfunction for all diamond and Hecke operators, then we say $f$ is a \textbf{Hecke-Maass eigenform}\index{Hecke-Maass eigenform}. In particular, on $\G_{1}(1)\backslash\H$ all eigenforms are Hecke-Maass eigenforms. Now let $f$ have Fourier coefficient $a_{n}(f)$. As for holomorphic forms, if $f$ an eigenform \cref{prop:general_Hecke_operators_acting_on_Fourier_coefficients_Maass} immediately implies that the first Fourier coefficient of $T_{m}f$ is $a_{m}(f)$ and so
    \[
      a_{m}(f) = \l_{f}(m)a_{1}(f),
    \]
    for all $m \ge 1$ with $(m,N) = 1$. Therefore we cannot have $a_{1}(f) = 0$ for this would mean $f$ is constant. So we can normalize $f$ by dividing by $a_{1}(f)$ which guarantees that this Fourier coefficient is $1$. It follows that
    \[
      a_{m}(f) = \l_{f}(m),
    \]
    for all $m \ge 1$ with $(m,N) = 1$. This normalization is called the \textbf{Hecke normalization}\index{Hecke normalization} of $f$. The \textbf{Petersson normalization}\index{Petersson normalization} of $f$ is where we normalize so that $\<f,f\> = 1$. From the spectral theorem we have an analogous corollary as for holomorphic forms:

    \begin{theorem}\label{thm:eigenforms_forms_spectral_theory_Maass}
      $\mc{C}_{k,\nu}(\G_{1}(N))$ admits an orthonormal basis of eigenforms.
    \end{theorem}
    \begin{proof}
      By \cref{thm:Maass_forms_finite_dimensional}, $\mc{C}_{k,\nu}(\G_{1}(N))$ is finite dimensional. The claim then follows from the spectral theorem along with \cref{prop:Hecke_operators_commute_Maass,prop:Hecke_operators_normal_Maass,prop:sign_operator_commutes_with_Hecke_and_diamond_and_is_normal}.
    \end{proof}
    ...
    \begin{proposition}[Hecke relations, Maass version]
      Let $f \in \mc{C}_{k,\nu}(N,\chi)$ be a Hecke-Maass eigenform with Hecke eigenvalues $\l_{f}(n)$. Then the Hecke eigenvalues are multiplicative and satisfy
      \[
  ...
  \section{Atkin-Lehner Theory}
    ...
    \[
      (f[\a_{d}]_{k})(z) = d^{-1}\e(\a_{d},z)^{-k}f(\a_{d} z) = d^{-1}f(dz).
    \]
    Similar to holomorphic forms, $[\a_{d}]_{k}$ maps $\mc{C}_{k,\nu}(\G_{1}(M))$ into $\mc{C}_{k,\nu}(\G_{1}(N))$:
    
    \begin{proposition}\label{equ:lifting_operator_Maass}
      Let $M$ and $N$ be positive integers such that $M \mid N$. For any $d \mid \frac{N}{M}$, $[\a_{d}]_{k}$ maps $\mc{C}_{k,\nu}(\G_{1}(M))$ into $\mc{C}_{k,\nu}(\G_{1}(N))$.
    \end{proposition}
    \begin{proof}
      ...
      \[
        \D(f[\a_{d}]_{k})(z) = \l d^{-1}f(dz) = \l(f[\a_{d}]_{k})(z).
      \]
      Therefore $f[\a_{d}]_{k}$ is an eigenfunction for $\D$ with eigenvalue $\l$. This completes the proof.
    \end{proof}

    We can now define oldforms and newforms. For each divisor $d$ of $N$, set
    \[
      i_{d}:\mc{C}_{k,\nu}\left(\G_{1}\left(\frac{N}{d}\right)\right)\x\mc{C}_{k,\nu}\left(\G_{1}\left(\frac{N}{d}\right)\right) \to \mc{C}_{k,\nu}(\G_{1}(N)) \qquad (f,g) \mapsto f+g[\a_{d}]_{k}.
    \]
    ...
    \[
      \mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{new}} = \left(\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{old}} \right)^{\perp},
    \]
    where the orthogonal complement is taken with respect to the Petersson inner product. The elements of such subspaces are called \textbf{oldforms}\index{oldforms} and \textbf{newforms}\index{newforms} respectively. Both subspaces are invariant under the diamond and Hecke operators (argue as in the proof of \cref{prop:old/new_subspaces_are_invariant_holomorphic} given in \cite{diamond2005first}):

    \begin{proposition}\label{prop:old/new_subspaces_are_invariant_Maass}
      The spaces $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{old}}$ and $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{new}}$ are invariant under the diamond operators $\<m\>$ and Hecke operators $T_{m}$ for all $m \ge 1$ and the parity operator $T_{-1}$.
    \end{proposition}
    ...
    We can remove the condition $(m,N) = 1$ for eigenforms in a basis of $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{new}}$ so that the eigenforms are eigenfunctions for all of the diamond and Hecke operators. As for holomorphic forms, we need a preliminary result (argue as in the proof of \cref{lem:the_main_lemma_for_newforms_holomorphic} as given in \cite{diamond2005first}):
    ...
    \begin{lemma}\label{lem:the_main_lemma_for_newforms_Maass}
      ...
      \[
        f = \sum_{p \mid N}p^{-1}f_{p}[\a_{p}]_{k},
      \]
      for some $f_{p} \in \mc{C}_{k,\nu}\left(\G_{1}\left(\frac{N}{p}\right)\right)$.
    \end{lemma}

    As was the case for holomorphic forms, we observe from \cref{lem:the_main_lemma_for_newforms_Maass} that if $f \in \mc{C}_{k,\nu}(\G_{1}(N))$ is such that its $n$-th Fourier coefficients vanish when $n$ is relatively prime to the level, then $f$ must be an oldform. The main theorem about $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{new}}$ can now be proved. We say that $f$ is a \textbf{primitive Hecke-Maass eigenform}\index{primitive Hecke-Maass eigenform} if it is a nonzero Hecke normalized Hecke-Maass eigenform in $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{new}}$. We can now prove the main result about newforms:
    ...
    \begin{theorem}\label{thm:newforms_characterization_Maass}
      ...
      \end{enumerate}
      Moreover, the primitive Hecke-Maass eigenforms in $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{new}}$ form an orthogonal basis with respect to the Petersson inner product and each such primitive Hecke-Maass eigenform $f$ lies in an eigenspace $\mc{C}_{k,\nu}(N,\chi)$.
    \end{theorem}
    \begin{proof}
      The argument used in the proof of \cref{thm:newforms_characterization_holomorphic} holds verbatim.
    \end{proof}

    Statement (i) in \cref{thm:newforms_characterization_Maass} implies that primitive Hecke-Maass eigenforms satisfy the Hecke relations for all $n,m \ge 1$. Statement (ii) in \cref{thm:newforms_characterization_Maass} is referred to as the \textbf{multiplicity one}\index{multiplicity one} for Maass forms. So as is the case for holomorphic forms, $\mc{C}_{k,\nu}(\G_{1}(N))^{\mathrm{new}}$ contains one element per ``eigenvalue'' where we mean a set of eigenvalues one for each Hecke operator $T_{m}$. Since the Fourier coefficients are multiplicative, the Hecke relations imply that $f$ is actually determined by its Fourier coefficients at primes. 
    
    \begin{theorem}[Multiplicity one, Maass version]
      Let $f$ and $g$ be primitive Hecke-Maass eigenforms of the same weight, eigenvalue, and level. Denote the Hecke eigenvalues by $\l_{f}(n)$ and $\l_{g}(n)$ respectively. If $\l_{f}(p) = \l_{g}(p)$ for all primes $p$ not dividing the level, then $f = g$.
    \end{theorem}
    \begin{proof}
      The argument used in the proof of multiplicity one for holomorphic forms holds verbatim.
    \end{proof}

    We now require one last linear operator. Before introducing this operator, let us discuss conjugate Maass cusp forms. For any cusp form $f \in \mc{C}_{k,\nu}(N,\chi)$, we define the \textbf{conjugate}\index{conjugate} $\conj{f}$ of $f$ by
    \[
      \conj{f}(z) = \conj{f(-\conj{z})}.
    \]
    Note that if $f$ has Fourier coefficients $a_{n}(f)$, then $\conj{f}$ has Fourier coefficients $\conj{a_{n}(f)}$ by the conjugate symmetry of the Whittaker function (see \cref{append:Whittaker_Functions}). Moreover, it is generally not true that $\conj{f}(z)$ and $\conj{f(z)}$ are equal. However, it turns out that $\conj{f}$ is indeed a Maass cup form:
    ...
    Now we are ready to introduce the linear operator. Recall the matrix
    \[
    ...
    \[
      (\w_{N}f)(z) = N^{\frac{k}{2}}\e(W_{N},z)^{-k}f\left(W_{N}z\right) = \left(\frac{\sqrt{N}z}{|z|}\right)^{-k}f\left(-\frac{1}{Nz}\right).
    \]
    It is not too difficult to see that $\w_{N}$ acts on $\mc{C}_{k,\nu}(\G_{1}(N))$:
    ...
    \begin{proof}
      ...
      \[
        \D(\w_{N}f)(z) = \l \left(\frac{\sqrt{N}z}{|z|}\right)^{-k}f\left(W_{N}z\right) = \l(\w_{N}f)(z).
      \]
      Thus $\w_{N}f$ is an eigenfunction for $\D$ with eigenvalue $\l$. This completes the proof.
    \end{proof}

    The crucial fact we need is how $\w_{N}$ acts on primitive Hecke-Maass eigenforms (argue as in the proof of \cref{prop:Atkin_Lehner_conjugation_holomorphic} given in \cite{cohenmodular2017}):
  ...
  \section{The Ramanujan-Petersson Conjecture}
    As for the size of the Fourier coefficients of Maass form, much is currently unknown. But there is an analogous conjecture to the one for holomorphic forms. To state it, suppose $f \in \mc{C}_{k,\nu}(N,\chi)$ is a primitive Hecke-Maass eigenform with Fourier coefficients $\l_{f}(n)$. For each prime $p$, consider the polynomial
    \[
  ...
  \section{Twists of Maass Forms}
    We can also twist of Maass forms by Dirichlet characters. Let $f \in \mc{C}_{k,\nu}(N,\chi)$ with Fourier series
    \[
