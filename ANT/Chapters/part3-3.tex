\chapter{Classical Applications}
  We will discuss some classical applications of $L$-functions. Our first result is a crowning gem of analytic number theory: Dirichlet's theorem. This result is a consequence of a non-vanishing result for Dirichlet $L$-functions at $s = 1$. Then we discuss Siegel zeros in the case of Dirichlet $L$-functions and as a result obtain a lower bound for Dirichlet $L$-functions at $s = 1$. Lastly, we prove the prime number theorem and its variant for primes restricted to a certain residue class, the Siegel–Walfisz theorem, in the classical manner.
  \section{Dirichlet's Theorem}
    One of the more well-known arithmetic results proved using $L$-functions is \textbf{Dirichlet's theorem}\index{Dirichlet's theorem}:
