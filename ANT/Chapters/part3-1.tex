\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{A Zero-free Region}}
    ...
    \begin{theorem}
      Let $L(s,f)$ be an $L$-function such that the Rankin-Selberg convolutions $L(s,f \ox f)$ and $L(s,f \ox \conj{f})$ exist where the former is entire and the latter has a simple pole at $s = 1$ if $f$ is not self-dual. Moreover, suppose that $|\a_{j}(p)|^{2} \le \frac{p}{2}$ for ramified primes. Then there exists a constant $c > 0$ such that $L(s,f)$ has no zeros in the region
      \[
    ...
    \begin{proof}
      ...
      \]
      Clearly $d_{g} = (1+2d_{f})^{2}$ so and $q(g)$ satisfies
      \[
        q(g) \le q(it,f)^{4}q(2it,f \ox f)^{4}q(f \ox \conj{f})^{2} \le q(f)^{4+12d_{f}}(|t|+3)^{6d_{f}^{2}},
      \]
      We claim that $\Re(\L_{g}(n)) \ge 0$ for $(n,q(f)) = 1$. We will actually prove the stronger statement $\L_{g}(n) \ge 0$ for $(n,q(f)) = 1$ (so that these coefficients are necessarily real). To see this, let $p$ be an unramified prime. Then the local roots of $L(s,g)$ at $p$ are $1$ with multiplicity one, $\a_{j}(p)p^{-it}$ and $\conj{\a_{j}(p)}p^{it}$ both of multiplicity two, $\a_{j}(p)\a_{\ell}(p)p^{-2it}$ and $\conj{\a_{j}(p)\a_{\ell}(p)}p^{-2it}$ both of multiplicity two, and $\a_{j}(p)\conj{\a_{\ell}(p)}$ of multiplicity two. For any $k \ge 1$, the sum of $k$-th powers of these local roots is
      \[
        \left|1+\sum_{1 \le j \le d_{f}}\a_{j}(p)^{k}p^{-kit}+\sum_{1 \le \ell \le d_{f}}\conj{\a_{\ell}(p)}^{k}p^{kit}\right|^{2} \ge 0.
      \]
      From the definition of $\L_{g}(n)$ it follows that $\L_{g}(n) \ge 0$ for $(n,q(f)) = 1$. Thus the conditions of \cref{lem:non-vanshing_at_1_lemma} are satisfied for $L(s,g)$. Let $\rho = \b+i\g$ be a nontrivial zero of $L(s,f)$ with $\b \ge \frac{1}{2}$ and $\g \neq 0$. For $t = \g$, $L(s,g)$ has a pole at $s = 1$ of order at most $3$ and a zero at $s = \b$ of order at least $4$ (recall that if $\rho$ is a nontrivial zero of $L(s,f)$ then $\conj{\rho}$ is a nontrivial zero of $L(s,\conj{f})$). In particular, $r_{g} \ge 3$ and $\b$ is a nontrivial zero of $L(s,g)$. Denoting the constant in \cref{lem:non-vanshing_at_1_lemma} by $c'$, our estimate for $q(g)$, that $r_{g} \ge 3$, and the bound $d_{g} \le 9d_{f}^{2}$, together give
      \[
        \b < 1-\frac{c'}{d_{g}(r_{g}+1)\log{q(g)}} \le 1-\frac{c}{d_{f}^{4}\log(q(f)(|\g|+3))},
      \]
      where $c = \frac{c'}{36}$. Now let $\b$ be a real nontrivial zero of $L(s,f)$. For $t = 0$, $L(s,g)$ has \todo{xxx}
    \end{proof}
