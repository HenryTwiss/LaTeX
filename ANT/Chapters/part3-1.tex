\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Hypothesis \& Nontrivial Zeros}
    ...
    \end{equation}
    If $\s < 0$ then $L(1-s,\conj{f})$ is nonzero by our previous comments. Moreover, $\g(1-s,f)$ is holomorphic in this region because $\Re(\k_{i}) \ge -1$. We conclude that poles of $\g(s,f)$ are zeros of $L(s,f)$ for $\s < 0$. Such zeros are called \textbf{trivial zeros}\index{trivial zeros}. From the definition of $\g(s,f)$, they are all simple and of the form $s = -(\k_{i}+2n)$ for some local root at infinity $\k_{i}$ and some integer $n \ge 0$. Any other zero of $L(s,f)$ is called a \textbf{nontrivial zeros}\index{nontrivial zeros} and it lies inside of the critical strip. Now let $\rho$ be a nontrivial zero of $L(s,f)$. Note that $L(\conj{s},\conj{f}) = \conj{L(s,f)}$ by the identity theorem and that this equality holds for $\s > 1$ where $L(s,f)$ is defined by a Dirichlet series. It follows that $\conj{\rho}$ is a nontrivial zero of $L(s,\conj{f})$. In the interior of critical strip, the gamma factor is nonzero and holomorphic (again we have used $\Re(\k_{i}) \ge 1$). From the functional equation, it follows that $1-\conj{\rho}$ is also a nontrivial zero of $L(s,f)$. In short, the nontrivial zeros occur in pairs:
    \[
  ...
  \section{The Explicit Formula for the Logarithmic Derivative}
    ...
    \begin{equation}\label{equ:log_derivaive_direct_computation}
      -\frac{L'}{L}(s,f) = -\sum_{p}\sum_{1 \le i \le d_{j}}\frac{d}{ds}\log(1-\a_{i}(p)p^{-s}) = \sum_{p}\sum_{1 \le i \le d_{j}}\frac{\a_{i}(p)\log(p)}{(1-\a_{i}(p)p^{-s})p^{s}}.
    \end{equation}
    ...
    for $\s > 1$, where
    \[
      \L_{f}(n) = \begin{cases} \sum_{1 \le i \le d_{f}}\a_{i}(p)^{k}\log(p) & \text{if $n = p^{k}$ for some $k \ge 1$}, \\ 0 & \text{otherwise}. \end{cases}
    \]
  ...
  \section{Zero Density}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \]
        which gives (i).
        \item For (ii), we first bound two important quantities. For the first quantity, the definition of $\L_{f}(n)$ and that $|\a_{i}(p)| \le p$ together imply the weak bound $|\L_{f}(n)| \le d_{f}n\log(n)$. Then
  ...
  \section{\todo{A Zero-free Region}}
    ...
    \begin{lemma}
      Let $L(s,f)$ be an $L$-function such that $\Re(\l_{f}(n)) \ge 0$ provided $(n,q(f)) = 1$. Also suppose that $|\a_{i}(p)| \le \frac{p}{2}$ for ramified primes. Then $L(1,f) \neq 0$ and hence $r_{f} \ge 0$. Moreover, there exists a constant $c > 0$ such that $L(s,f)$ has at most $r_{f}$ real zeros in the interval
      \[
    ...
    \begin{proof}
      ...
      \[
        \Re\left(\frac{L'_{q(f)}}{L_{q(f)}}(\s,f)\right) \le \left|\frac{L'_{q(f)}}{L_{q(f)}}(\s,f)\right| = \left|\sum_{p \mid q(f)}\sum_{1 \le i \le d_{j}}\frac{\a_{i}(p)\log(p)}{(1-\a_{i}(p)p^{-\s})p^{\s}}\right| \le d_{f}\sum_{p \mid q(f)}\log(p) \le d_{f}\log(q(f)),
      \]
      where in the second inequality we have made use of the assumption $|\a_{i}(p)| \le \frac{p}{2}$ for ramified primes to conclude that $\left|\frac{\a_{i}(p)}{(1-\a_{i}(p)p^{-\s})p^{\s}}\right| \le 1$. These estimates together imply
      \[
        \sum_{1 \le j \le n}\frac{1}{\s-\b_{j}} < \frac{r_{f}}{\s-1}+O(\log{d_{f}q(f)}).
      \]
      From this inequality we see that $\b_{j} \neq 1$. For if $\b_{j} = 1$ for some $j$, $r_{f} < 0$ and the right hand side is negative for $\s$ sufficiently close to $1$ contradicting the positivity of the left-hand side. Thus $L(1,f) \neq 0$ and hence $r_{f} \ge 0$. As there are finitely many $\b_{j}$, there exists a $c > 0$ such that the $\b_{j}$ satisfy
      \[
