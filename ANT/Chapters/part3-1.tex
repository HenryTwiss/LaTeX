\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Perron Formulas}
    ...
    \begin{proposition}\label{prop:smoothing_function_Mellin_inverse_vertical_strips}
      Suppose $\psi:\R_{\ge 0} \to \R_{\ge 0}$ is a bump function and let $\Psi:\C_{\s > 0} \to \C$ denote its Mellin transform. Then for any $N \ge 1$,
      \[
        \Psi(s) \ll (|s|+1)^{-N},
      \]
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{theorem}[Approximate functional equation]
      ...
      \]
      where $V_{s}:\R_{> 0} \to \C$ is the inverse Mellin transform defined by
      \[
    ...
    \begin{proposition}\label{prop:V_function_decay}
      Let $L(s,f)$ be an $L$-function, set $\Phi(u) = \cos^{-4d_{f}M}\left(\frac{\pi u}{4M}\right)$ for some integer $M \ge 1$, and let $V_{s}:\R_{> 0} \to \C$ be the inverse Mellin transform defined by
      \[
    ...
    \begin{theorem}
      Let $\psi(s)$ be a bump function and let $\Psi:\C_{\s > 0} \to \C$ denote its Mellin transform. Then for any $L$-functions $L(s,f)$, we have
      \[
        \sum_{n \ge 1}a_{f}(n)\psi(n) = \frac{\e(f)}{\sqrt{q(f)}}\sum_{n \ge 1}a_{\conj{f}}(n)\phi(n)+R\Psi(1),
      \]
      where $\phi:\R_{> 0} \to \C$ is the inverse Mellin transform defined by
      \[
  ...
  \section{Estimating the Central Value}
    ...
    \begin{theorem}
      ...
      \[
        L\left(\frac{1}{2},f\right) \ll_{\e} \max_{X \ll \mathfrak{q}(f)^{\frac{1}{2}+\e}}\frac{A_{\psi}(X)}{q(f)^{\frac{1}{4}}}+\frac{S}{q(f)^{\frac{1}{4}}},
      \]
  ...
  \section{\todo{The Explicit Formula}}
    ...
    \begin{theorem}[Explict Formula]
      Let $\psi:\R_{> 0} \to \C$ be a bump function with compact support and let $\Psi_{s}:\C_{\s > 0} \to \C$ denote its Mellin transform. Set $\phi(y) = y^{-1}\phi(y^{-1})$ so that its Mellin transform satisfies $\Phi(s) = \Psi(1-s)$ and both functions have the same domain as $\psi$ and $\Psi$ respectively. Then for any $L$-function $L(s,f)$, we have
      \begin{align*}
