\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Analytic Data of \texorpdfstring{$L$}{L}-functions}
    ...
    \begin{remark}\label{rem:degree_0_L-function}
      Note that the only $L$-function of degree $0$ is $L(s,\mathbf{1})$ defined by setting $a_{\mathbf{1}}(1) = 1$ and $a_{\mathbf{1}}(n) = 0$ for all $n > 1$ so that $L(s,\mathbf{1}) \equiv 1$. Moreover, $\g(s,\mathbf{1}) \equiv 1$, $q(\mathbf{1}) = 1$, $\e(\mathbf{1}) = 1$, and $L(s,\mathbf{1})$ is self-dual.
    \end{remark}

    It is also clear that the product of $L$-functions is also an $L$-function. Accordingly, we say that an $L$-function is \textbf{primitive}\index{primitive} if is cannot be written as a product of two $L$-functions of smaller positive degree (thus we are excluding the $L$-function of degree $0$ introduced in \cref{rem:degree_0_L-function}). Clearly any $L$-function of degree $1$ is necessarily primitive.
    ...
    Lastly, we provide some very useful bounds about gamma functions, gamma factors, and their logarithmic derivatives that will be extremely useful in the study of $L$-functions. Suppose $\s$ is bounded and $|t| > 1$. Then $s$ is bounded away from zero and by \cref{equ:weaker_Stirling_formula} we have
    \[
