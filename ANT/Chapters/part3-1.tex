\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Analytic Data of \texorpdfstring{$L$}{L}-functions}
    ...
    \begin{enumerate}[label=(\roman*)]
      ...
      where we set
      \[
        q_{\infty}(s,f) = \prod_{1 \le j \le d_{f}}(|s+\k_{j}|+3).
      \]
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{proof}
      ...
      \]
      From the definition of $q(s,f)$, the above estimate implies
      \[
  ...
  \section{Zero Density}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \]
        Therefore from \cref{equ_sum_of_nontrivial_zeros_log_analytic_conductor_bound} (with $t = T$) and that $\log{q(it,f)} \ll \log{q(s,f)}$, the contribution of these terms is $O(\log{q(s,f)})$. It follows that
        \[
        ...
        \[
          \frac{\g'}{\g}(s,f) = -\frac{d_{f}}{2}\log(\pi)-\sum_{1 \le j \le d_{f}}\frac{1}{s+\k_{j}}+\sum_{1 \le j \le d_{f}}\frac{1}{2}\frac{\G'}{\G}\left(1+\frac{s+\k_{j}}{2}\right) = -\sum_{|s+\k_{j}| < 1}\frac{1}{s+\k_{j}}+O(\log{q_{\infty}(s,f)}),
        \]
    ...
    \begin{proof}
      ...
      \begin{equation}\label{equ:zero_counting_4}
        \D_{\eta_{L}}\arg\left(\prod_{1 \le j \le d_{f}}\G\left(\frac{s+\k_{j}}{2}\right)\right) = \frac{T}{2}\log\left(\frac{T^{d_{f}}}{(2e)^{d_{f}}}\right)+O(q(f)).
      \end{equation}
      ...
      and therefore
      \[
        N'(T,f) = \frac{T}{2\pi}\log\left(\frac{T^{d_{f}}}{(2\pi e)^{d_{f}}}\right)+O(\log{q(iT,f)}),
      \]
    ...
    \begin{proof}
      ...
      \[
        \frac{N(T,f)}{T} = \frac{1}{\pi}\log\left(\frac{T^{d_{f}}}{(2\pi e)^{d_{f}}}\right)\left(1+O\left(\frac{\log{q(iT,f)}}{\log\left(\frac{T^{d_{f}}}{(2\pi e)^{d_{f}}}\right)T}\right)\right) = \frac{1}{\pi}\log\left(\frac{T^{d_{f}}}{(2\pi e)^{d_{f}}}\right)\left(1+O\left(\frac{1}{T}\right)\right).
      \]
  ...
  \section{\todo{A Zero-free Region}}
    ...
    \begin{lemma}\label{lem:non-vanshing_at_1_lemma}
      Let $L(s,f)$ be an $L$-function such that $\Re(\L_{f}(n)) \ge 0$ provided $(n,q(f)) = 1$. Also suppose that $|\a_{j}(p)| \le \frac{p}{2}$ for ramified primes. Then $L(1,f) \neq 0$ and hence $r_{f} \ge 0$. Moreover, there exists a constant $c > 0$ such that $L(s,f)$ has at most $r_{f}$ real zeros in the interval
      \[
    ...
    \begin{proof}
      ...
      \[
        \sum_{1 \le j \le n}\frac{1}{\s-\b_{j}} < \frac{r_{f}}{\s-1}+\Re\left(\frac{L'}{L}(\s,f)\right)+O(\log{q(f)}).
      \]
      ...
      \]
      where in the second inequality we have made use of the assumption $|\a_{j}(p)| \le \frac{p}{2}$ for ramified primes to conclude that $\left|\frac{\a_{j}(p)}{(1-\a_{j}(p)p^{-\s})p^{\s}}\right| \le 1$. These estimates together imply
      \[
        \sum_{1 \le j \le n}\frac{1}{\s-\b_{j}} < \frac{r_{f}}{\s-1}+O(\log{d_{f}q(f)}).
      \]
      ...
      \[
        \b_{j} \ge 1-\frac{c}{d_{f}(r_{f}+1)\log{q(f)}}.
      \]
      Setting $\s = 1+\frac{2c}{d_{f}\log{q(f)}}$ and choosing $c$ smaller, if necessary, we guarantee $1 < \s < \frac{3}{2}$. Then the two inequalities above together imply
      \[
        \frac{nd_{f}\log{q(f)}}{2c+\frac{c}{(r_{f}+1)}} < \left(\frac{r_{f}}{2c}+O(1)\right)d_{f}\log{q(f)}.
      \]
      Isolating $n$, we see that
      \[
        n < r_{f}+\frac{r_{f}}{2(r_{f}+1)}+O(c),
      \]
      and taking $c$ smaller, if necessary, we have that $n \le r_{f}$. As $L(s,f)$ is nonzero for $\s > 1$, it follows that there are at most $r_{f}$ real zeros satisfying
      \[
        \s \ge 1-\frac{c}{d_{f}(r_{f}+1)\log{q(f)}}.
      \]
    ...
    \begin{theorem}
      Let $L(s,f)$ be an $L$-function that has a simple pole at $s = 1$ or is entire depending on if $f$ is self-dual or not. Also suppose that the Rankin-Selberg convolutions $L(s,f \ox f)$ and $L(s,f \ox \conj{f})$ exist and are such that the former is entire and the latter has a simple pole at $s = 1$ if $f$ is not self-dual. Moreover, suppose that $|\a_{j}(p)|^{2} \le \frac{p}{2}$ for ramified primes. Then there exists a constant $c > 0$ such that $L(s,f)$ has no zeros in the region
      \[
        \s \ge 1-\frac{c}{d_{f}^{4}\log(q(f)(|t|+3))},
      \]
      except for possibly one simple real zero $\b_{f} < 1$ in the case $f$ is self-dual.
    \end{theorem}
    \begin{proof}
      For $t \in \R$, let $L(s,g)$ be the $L$-function defined by
      \[
        L(s,g) = \z(s)L(s+it,f)^{2}L(s-it,\conj{f})^{2}L(s+2it,f \ox f)L(s-2it,\conj{f} \ox \conj{f})L(s,f \ox \conj{f})^{2}.
      \]
      Clearly $d_{g} = (1+2d_{f})^{2}$ and so $q(g)$ satisfies
      \[
        q(g) \le q(it,f)^{4}q(2it,f \ox f)^{4}q(f \ox \conj{f})^{2} \le q(f)^{4+12d_{f}}(|t|+3)^{6d_{f}^{2}},
      \]
      We claim that $\Re(\L_{g}(n)) \ge 0$ for $(n,q(f)) = 1$. We will actually prove the stronger statement $\L_{g}(n) \ge 0$ for $(n,q(f)) = 1$ (so that these coefficients are necessarily real). To see this, let $p$ be an unramified prime. Then the local roots of $L(s,g)$ at $p$ are $1$ of multiplicity one, $\a_{j}(p)p^{-it}$ and $\conj{\a_{j}(p)}p^{it}$ both of multiplicity two, $\a_{j}(p)\a_{\ell}(p)p^{-2it}$ and $\conj{\a_{j}(p)\a_{\ell}(p)}p^{-2it}$ both of multiplicity two, and $\a_{j}(p)\conj{\a_{\ell}(p)}$ of multiplicity two. So for any $k \ge 1$, the sum of $k$-th powers of these local roots is
      \[
        \left|1+\sum_{1 \le j \le d_{f}}\a_{j}(p)^{k}p^{-kit}+\sum_{1 \le \ell \le d_{f}}\conj{\a_{\ell}(p)}^{k}p^{kit}\right|^{2} \ge 0.
      \]
      From the definition of $\L_{g}(n)$ it follows that $\L_{g}(n) \ge 0$ for $(n,q(f)) = 1$. Thus the conditions of \cref{lem:non-vanshing_at_1_lemma} are satisfied for $L(s,g)$. Let $\rho = \b+i\g$ be a complex nontrivial zero of $L(s,f)$. Then $\g \neq 0$. For $t = \g$, $L(s,g)$ has a pole at $s = 1$ of order at most $3$ and a real nontrivial zero at $s = \b$ of order at least $4$ (recall that if $\rho$ is a nontrivial zero of $L(s,f)$ then $\conj{\rho}$ is a nontrivial zero of $L(s,\conj{f})$). But \cref{lem:non-vanshing_at_1_lemma} implies that $L(s,g)$ can have at most $3$ real zeros in the given region. So denoting the constant in \cref{lem:non-vanshing_at_1_lemma} by $c'$, our estimate for $q(g)$, that $r_{g} \ge 3$, and the bound $d_{g} \le 9d_{f}^{2}$, together give
      \[
        \b < 1-\frac{c'}{d_{g}(r_{g}+1)\log{q(g)}} \le 1-\frac{c}{d_{f}^{4}\log(q(f)(|\g|+3))},
      \]
      where $c = \frac{c'}{36}$. Now let $\b$ be a real nontrivial zero of $L(s,f)$. For $t = 0$, $L(s,g)$ has a pole at $s = 1$ of order $5$ or $3$, depending on if $f$ is self-dual or not, and has a real nontrivial zero at $s = \b$ of order at least $4$. In the case $f$ is not-self dual, \cref{lem:non-vanshing_at_1_lemma} implies that $L(s,g)$ can have at most $3$ real zeros in the given region. So just as in the case for complex nontrivial zeros, we have
       \[
        \b < 1-\frac{c'}{d_{g}(r_{g}+1)\log{q(g)}} \le 1-\frac{c}{d_{f}^{4}\log(3q(f))}.
      \]
      This proves the theorem in the case $f$ is not self-dual. If $f$ is self-dual, \cref{lem:non-vanshing_at_1_lemma} implies that $L(s,g)$ can have at most $5$ real zeros in the given region. As any real nontrivial zero of $L(s,g)$ is of order at least $4$, there is at most a single simple real nontrivial zero in the given region. \todo{xxx}
    \end{proof}
