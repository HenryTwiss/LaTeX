\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Approximate Functional Equation}
    ...
    \end{proposition}
    \begin{proof}
      Let $u = \tau+ir$. From \cref{equ:modified_gamma_estimates}, we deduce that
      \[
        \frac{\G(s+u)}{\G(s)} \ll \frac{(|t+r|+1)^{\s+\tau-\frac{1}{2}}}{(|t|+1)^{\s-\frac{1}{2}}}e^{\frac{\pi}{2}(|t|-|t+r|)} \ll (|r|+1)^{\s+\tau-\frac{1}{2}}e^{-\frac{\pi}{2}|r|},
      \]
      where the last estimate follows because $s$ is fixed and so $t+r \sim r$. From the definition of the analytic conductor, we have the estimate
      \[
        \frac{\g(s+u,f)}{\g(s)} \ll q_{\infty}(s,f)^{\frac{\tau}{2}}e^{-\frac{\pi}{2}d|r|},
      \]
      where we have made use of the fact that for $s$ in the critical strip, $\frac{\s-1}{2} \le 0$ so $(|r|+1)^{\frac{\s+\tau-1}{2}} \ll (|r|+1)^{\tau}$. The fact tht $\Phi(u)$ is bounded, $u$ is bounded away from zero, and the estimate for the ratio of gamma factors gives the first estimate in the following chain:
      \begin{align*}
        V_{s}(y) &= \frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{\g(s,f)}\Phi(u)y^{-u}\frac{du}{u} \\
  ...
  \section{Estimating the Central Value}
    ...
    \begin{proof}
      ...
      \]
      Using the polynomial decay of $V_{\frac{1}{2},k}(y)$ to crudely bound those terms for which $n \gg q\left(\frac{1}{2},f\right)^{\frac{1}{2}+\e} \gg q(f)^{\frac{1}{2}+\e}$ gives the estimate
      \[
