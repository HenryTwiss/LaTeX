  ...
  \section{\todo{Zero Density}}
    ...
    \begin{lemma}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \]
        \item For any $T \ge 2$, the number of zeros $\rho = \b+i\g$ with $\rho \neq 0,1$ and such that $|T-\g| \le 1$ is $O(\log{q(iT,f)})$.
        \item We have
        \[
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \]
        which gives (i).
      \item For (ii), we first bound a few important quantities when $\s > 2$. For the first quantity, the definition of $\L_{f}(n)$ implies the weak bound $|\L_{f}(n)| \le d_{f}n\log(n)$. Then
      \begin{equation}\label{equ:log_derivative_bound_conductor}
        \frac{L'}{L}(s,f) \ll d_{f}\z'(s-1) \ll \log{q(f)}.
      \end{equation}
      For the second quantity, \cref{equ:approximtion_for_digamma} provides the estimate $\frac{\G'}{\G}(s) \ll \log(|s|+1)$ and from the definition of $\g(s,f)$ it follows that $\frac{\g'}{\g}(s,f) \ll \log{q_{\infty}(s,f)}$. Thus
      \begin{equation}\label{equ:log_conductor_gamma_factor_bound_analytic_conductor}
        \frac{1}{2}\log{q(f)}+\frac{\g'}{\g}(s,f) \ll \log{q(s,f)}.
      \end{equation}
      Now fix $T > 0$ and let $s = 3+iT$. Then taking the real part of the explicit formula for the logarithmic derivative and combining \cref{equ:log_derivative_bound_conductor,equ:log_conductor_gamma_factor_bound_analytic_conductor} with (i), results in
      \[
      ...
      \item For (iii), let $s$ belong to the vertical strip $-\frac{1}{2} \le \s \le 2$. Using that $\log{q(f)} \ll \log{q(s,f)}$ and \cref{equ:log_derivative_bound_conductor}, we can write
      \[
        -\frac{L'}{L}(s,f) = -\frac{L'}{L}(s,f)+\frac{L'}{L}(3+it,f)+O(\log{q(s,f)}).
      \]
      Applying the explicit formula for the logarithmic derivative to the two terms on the right hand side and using again that $\frac{\g'}{\g}(s,f) \ll \log{q_{\infty}(s,f)}$, we get
      \[
