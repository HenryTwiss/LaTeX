\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Perron Formulas}
    ...
    \begin{proof}
      ...
      \]
      The claim will follow if we can show that the integral is bounded. Since $\psi(y)$ is compactly supported in $[0,M]$ so is $\psi^{(N)}(y)$. In particular, this implies $\psi^{(N)}(y)$ is bounded. Therefore
      \[
