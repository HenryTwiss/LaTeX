\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Perron Formulas}
    ...
    \]
    where $\psi:[0,\infty) \to [0,\infty)$ is smooth function such that $\psi(y) \to 0$ as $y \to \infty$. This is most useful in two cases. The first is when we choose $\psi(y)$ to be a bump function. In this setting, the bump function applies weight $1$ or $0$ to the Dirichlet coefficients $a(n)$ and we can estimate sums like
    \[
    ...
    \]
    In the second case, we want $\psi$ to dampen the $a(n)$ with a weight other than $1$ or $0$. Sums of this type are called \textbf{weighted}\index{weighted}. In any case, suppose the support of $\psi(y)$ is contained in $[0,C]$. These conditions will force the Mellin transform $\Psi(s)$ of $\psi(y)$ to exist and have nice properties. To see that $\Psi(s)$ exists, let $K$ be a compact set in the region $\s > 0$ and let $\b = \min_{s \in K}\{\s\}$. Note that $\psi(y)$ is bounded because it is compactly supported. Then for $s \in K$,
    \[
      \Psi(s) = \int_{0}^{\infty}\psi(y)y^{s}\,\frac{dy}{y} \ll \int_{0}^{C}y^{\s-1}\,dy = \frac{y^{\s}}{\s}\bigg|_{0}^{C} \le \frac{C^{\s}}{\b}.
    \]
    ...
    \end{proposition}
    \begin{proof}
      Fix $a$ and $b$ with $a < b$. Also, let the support of $\psi(y)$ is contained in $[0,C]$. Now consider
      \[
      ...
      \]
      As $s+k-1 \sim |s|+1$ for $1 \le k \le N$, we have
      \[
        \Psi(s) \ll (|s|+1)^{-N}\int_{0}^{\infty}\psi^{(N)}(y)y^{\s+N}\,\frac{dy}{y}.
      \]
      The claim will follow if we can show that the integral is bounded. Since $\psi(y)$ is compactly supported so is $\psi^{(N)}(y)$. In particular, this implies $\psi^{(N)}(y)$ is bounded. Therefore
      \[
        \int_{0}^{\infty}\psi^{(N)}(y)y^{\s+N}\,\frac{dy}{y} \ll \int_{0}^{C}y^{\s+N}\,\frac{dy}{y} = \frac{y^{\s+N}}{\s+N}\bigg|_{0}^{C} = \frac{C^{\s+N}}{\s+N} \ll \frac{C^{b+N}}{N} \ll 1,
      \]
  ...
  \section{The Approximate Functional Equation}
    If $L(s,f)$ is an $L$-function, then there is a formula which acts as a compromise between the functional equation for $L(s,f)$ and expressing $L(s,f)$ as a Dirichlet series. This formula is known as the approximate functional equation and it is important because it is valid inside of the critical strip and therefore can be used to obtain data about $L(s,f)$. First, we need to state an important asymptotic about the ratio of gamma factors that we will use later on. First suppose $|t| > 1$. This guarantees that $s$ is bounded away from zero so by \cref{equ:weaker_Stirling_formula}, we have
    \[
    ...
    \end{equation}
    It is not hard to obtain estimates that holds in vertical strips. Suppose $s$ is in the strip $a < \s < b$ and is distance $\e$ away from the poles of $\G(s)$. For $|t| > 1$, \cref{equ:simplified_gamma_estimates} is satisfied. Moreover $\G(s)$ is bounded on the compact region
    \[
      \{s \in \C:a \le \s \le b,|t| \le 1\},
    \]
    and distance $\e$ away from the poles. As the bounds in \cref{equ:simplified_gamma_estimates} are zero when $t = 0$, the estimates
    \begin{equation}\label{equ:modified_gamma_estimates}
      \G(s) \ll_{a,b} (|t|+1)^{\s-\frac{1}{2}}e^{-\frac{\pi}{2}|t|} \quad \text{and} \quad \frac{1}{\G(s)} \ll_{a,b} (|t|+1)^{\frac{1}{2}-\s}e^{\frac{\pi}{2}|t|},
    \end{equation}
    are valid in the strip $a < \s < b$ provided we are distance $\e$ away from the poles. We can use these estimates to show that $L(s,f)$ is has polynomial growth in the $t$-aspect in vertical strips:
    ...
    \begin{proposition}\label{prop:L_function_bounded_in_vertical_strips}
      ...
      \[
        \{s \in \C:a \le \s \le b, |t| > 1\}.
      \]
    \end{proposition}
    \begin{proof}
      For $\s > 1$ we have $L(\s,f) \ll_{\s} 1$. Now by the functional equation
      \[
        L(s,f) \ll \frac{\g(1-s,f)}{\g(s,f)}L(1-s,f).
      \]
      Using \cref{equ:simplified_gamma_estimates} and the definition of $\g(s,f)$, it follows that $\frac{\g(1-s,f)}{\g(s,f)}$ is polynomially bounded in $t$. Thus for $\s < 0$, we see that $L(s,f)$ is also polynomially bounded in $t$. As $L(s,f)$ is holomorphic for $|t| > 1$ and of order $1$, we can apply the Phragm\'en-Lindel\"of convexity principle (see \cref{append:The_Phragmen_Lindelof_Convexity_principle}) so that $L(s,f)$ is polynomially bounded in $t$ in any vertical strip with $|t| > 1$.
    \end{proof}
    ...
    \begin{theorem}[Approximate functional equation]
      ...
      \]
      where $V_{s}(y):(0,\infty) \to \C$ is the smooth function defined by
      \[
    ...
    \begin{proof}
      ...
      \]
      By our choice of $a$ and $s$, $L(s+u) \ll_{a} 1$. Moreover, from \cref{equ:modified_gamma_estimates} we see that $\g(s+u,f)$ has exponential decay in $\Im(u)$. Since $\Phi(u)$ is bounded, it follows that the integral is absolutely bounded by \cref{met:decay_compacta_integral}. By \cref{prop:L_function_bounded_in_vertical_strips} and \cref{equ:simplified_gamma_estimates}, the integrand also had exponential decay in a vertical strip containing $|\Re(u)| \le a$. Therefore we may shift the line of integration to $(-a)$. In doing so, we pass by a simple pole at $u = 0$ and possible poles at $u = 1-s$ and $u = -s$, giving
      \[
    ...
    \begin{remark}\label{rem:effect_of_V_in_approximate_functional_equation}
      The function $V_{s}(y)$ has the effect of smoothing out the two sums in the right-hand side of the approximate functional equation. For simplicity take $X = 1$. Indeed, this function restricts the sums to $n \ll q(f)^{\frac{1}{2}+\e}$, for any $\e > 0$, up to some bounded error. \todo{xxx}
    \end{remark}
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Arguments}
    ...
    \]
    and from \cref{equ:modified_gamma_estimates} we estimate the ratio of gamma factors as
    \[
      \frac{\g(1-s,f)}{\g(s,f)} \ll_{\e} q(s,f)^{\frac{1-2\s}{2}},
    \]
    for $s$ in any vertical strip and distance $\e$ away from the poles of $\g(1-s,f)$ and where we have used that $\left(\left|\frac{t+\Im(k_{i})}{2}\right|+1\right) \asymp (|s+k_{i}|+3)$. In particular, setting $\s = -\e$, our previous two bounds combine to give
    \[
      p_{r}(-\e+it)L(-\e+it,f) \ll_{\e} q(s,f)^{\frac{1}{2}+\e}.
    \]
    The Phragm\'en-Lindel\"of convexity principle and that $p_{r}(s) \sim 1$ together give
    \[
  ...
  \section{Estimating the Central Value}
    ...
    \begin{proof}
      ...
      \]
      for any $y \in (0,\infty)$. Then we can write
      \begin{align*}
        V_{s}(y) &= \frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{\g(s,f)}\Phi(u)y^{-u}\frac{du}{u} \\
      ...
      \]
      Note that $V_{s,k}(y)$ is compactly supported on $[2^{k-1},2^{k}]$ because $\psi_{k}(y)$ is compactly supported there. Moreover, \cref{rem:effect_of_V_in_approximate_functional_equation} implies that $V_{s,k}(y)$ also exhibits exponential decay for $n \gg q(f)^{\frac{1}{2}+\e}$ for any $\e > 0$. It follows that
      \[
      ...
      \]
      Using the exponential decay of $V_{\frac{1}{2},k}(y)$ to crudely bound those terms for which $n \gg q(f)^{\frac{1}{2}+\e}$ gives the estimate
      \[
