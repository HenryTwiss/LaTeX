\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet Series}
    ...
    \begin{proposition}\label{prop:Dirichlet_series_convergence_polynomial_bound}
      Suppose $D(s)$ is a Dirichlet series whose coefficients satisfy $a(n) \ll n^{\a}$ for some real $\a$. Then the abscissa of absolute convergence satisfies $\s_{a} \le 1+\a$.
    \end{proposition}
    \begin{proof}
      ...
      \[
        D(s) \ll \sum_{n \ge 1}\left|\frac{a(n)}{n^{s}}\right| \ll \sum_{n \ge 1}\frac{1}{n^{\s-\a}}.
      \]
    ...
    \begin{proposition}\label{prop:Dirichlet_series_coefficient_size_on_average}
      Suppose $D(s)$ is a Dirichlet series with finite and nonnegative abscissa of absolute convergence $\s_{a}$. Then for any $\e > 0$,
      \[
        A(X) \ll_{\e} X^{\s_{a}+\e}.
      \]
    \end{proposition}
    ...
    \begin{proposition}\label{prop:Dirichlet_series_convergence_polynomial_bound_average}
      Suppose $D(s)$ is a Dirichlet series such that $A^{\ast}(X) \ll X^{\a}$ for some real $\a \ge 0$. Then the abscissa of absolute convergence satisfies $\s_{a} \le \a$.
    \end{proposition}
    \begin{proof}
      ...
      \]
      By assumption $A^{\ast}(u) \ll u^{\a}$. Therefore there is some $N'$ such that $A^{\ast}(N) \ll N^{\a}$ for all $N \ge N'$. In particular $a(N) \ll N^{\a}$. So for such $N$, we estimate as follows:
      \begin{align*}
        \sum_{n \le N}\frac{|a(n)|}{n^{\s}} &= |a(N)|N^{-\s}-|a(1)|+\s\int_{1}^{N}A^{\ast}(u)u^{-(\s+1)}\,du \\
        &= |a(N)|N^{-\s}-|a(1)|+\s\int_{1}^{N'}A^{\ast}(u)u^{-(\s+1)}\,du+\s\int_{N'}^{N}A^{\ast}(u)u^{-(\s+1)}\,du \\
        &\ll |a(N)|N^{-\s}+|a(1)|+\s\int_{1}^{N'}|A^{\ast}(u)|u^{-(\s+1)}\,du+\s\int_{N'}^{N}u^{\a-(\s+1)}\,du.
      \end{align*}
    ...
    \end{proposition}
    \begin{proof}
        Since $a(n) \ll n^{\a}$, \cref{prop:Dirichlet_series_convergence_polynomial_bound} implies that $D(s)$ converges absolutely for $\s > 1+\a$. Let $s$ be such that $\s > 1+\a$. Since
        \[
        ...
        \]
        and the second sum is part of its tail. So the first sum must convergence hence defining an absolutely convergent Dirichlet series in $\s > 1+\a$, and the second sum must tend to zero. Lastly, if the $a(n)$ are completely multiplicative, then since $\s > 1+\a$, we have
        \[
          \frac{a(p)}{p^{s}} \ll \frac{1}{p^{\s-\a}} < \frac{1}{p} < 1.
        \]
  ...
  \section{Perron Formulas}
    ...
    \begin{proof}
      ...
      \]
      Now $A'(Y) \le A(Y)$ and $A(Y) \ll_{\e} Y^{\s_{a}+\e}$ for any $\e > 0$ by \cref{prop:Dirichlet_series_coefficient_size_on_average} so that $A'(Y)Y^{-s} \ll Y^{\s_{a}+\e-\s}$. Choosing $\e < \s-\s_{a}$, this latter term tends to zero as $Y \to \infty$, which implies that $A'(Y)Y^{-s} \to 0$ as $Y \to \infty$. Therefore we can write the equation above as
      \[
    ...
    \]
    for some $X$ and $Y$ with $Y < X$. Sums of this type are called \textbf{unweighted}\index{unweighted}. As an example of an unweighted sum, let $\psi:[0,\infty) \to [0,\infty)$ be a bump function that is identically $1$ on $[0,1]$ and has compact support within the interval $\left[1,\frac{X+1}{X}\right]$. Explicitly,
    \[
      \psi(x) = \begin{cases} 1 & \text{if $0 \le x \le 1$}, \\ e^{-\frac{1-x}{\frac{X+1}{X}-x}} & \text{if $1 \le x < \frac{X+1}{X}$}, \\ 0 & \text{if $x \ge \frac{X+1}{X}$}. \end{cases}
    \]
    Then 
    \[
      A_{\psi}(X) = \sum_{n \ge 1}a(n)\psi\left(\frac{n}{X}\right) = \sum_{n \le X}a(n).
    \]
    In the second case, we want $\psi$ to dampen the $a(n)$ with a weight other than $1$ or $0$. Sums of this type are called \textbf{weighted}\index{weighted}. In any case, suppose the support of $\psi(x)$ is contained in $[0,C]$. These conditions will force the Mellin transform $\Psi(s)$ of $\psi(x)$ to exist and have nice properties. To see that $\Psi(s)$ exists, let $K$ be a compact set in the region $\s > 0$ and let $\b = \min_{s \in K}\{\s\}$. Note that $\psi(x)$ is bounded because it is compactly supported. Then for $s \in K$,
    \[
      \Psi(s) = \int_{0}^{\infty}\psi(x)x^{s}\,\frac{dx}{x} \ll \int_{0}^{C}x^{\s-1}\,dx = \frac{x^{\s}}{\s}\bigg|_{0}^{C} \le \frac{C^{\s}}{\b}.
    \]
    Therefore $\Psi(s)$ is locally absolutely bounded for $\s > 0$. In particular, the Mellin inversion formula implies that $\psi(x)$ is the Mellin inverse of $\Psi(s)$. As for nice properties, $\Psi(s)$ does not grow too fast in vertical strips:
    ...
    \begin{proof}
      ...
      \]
      where the second to last estimate follows because $a < \s < b$ with $0 < a < b$. So the integral is bounded by a constant and the claim follows.
    \end{proof}
  ...
  \section{Analytic Data of \texorpdfstring{$L$}{L}-functions}
    ...
    \end{itemize}
    The adjustment for (i) forces $a_{f}(n) \ll n^{\e}$ for any $\e > 0$ while the adjustment for (ii) ensures that $\g(s,f)$ has at most a pole at $s = 0$ for $\s \ge 0$. The Selberg class constitutes a very special class of $L$-functions. Nevertheless, suppose we are given two $L$-functions $L(s,f)$ and $L(s,g)$ with degree $d$ and $e$ Euler products, local roots $\a_{i}(p)$ and $\b_{j}(p)$ at $p$, and local roots $\k_{i}$ and $\nu_{j}$ at infinity respectively. We say that an $L$-function $L(s,f \ox g)$ of degree $de$ is the \textbf{Rankin-Selberg convolution}\index{Rankin-Selberg convolution} of $L(s,f)$ and $L(s,g)$ (or \textbf{Rankin-Selberg square}\index{Rankin-Selberg square} if $f = g$) if it satisfies the following adjustments:
    \begin{itemize}
    ...
    \[
      \left\{s = \s+it \in \C:\left|\s-\frac{1}{2}\right| \le \frac{1}{2}\right\}.
    \]
  ...
  \section{The Approximate Functional Equation}
    ...
    \end{proposition}
    \begin{proof}
      If $|t| > 1$ we are away from the possible poles of $L(s,f)$, so by the functional equation
      \[
      ...
      \]
      where $\a(\s)$ is a real-valued continuous function. Letting $\s < 0$ and noting that $L(1-s) \ll_{\s} 1$, we have
      \[
        L(s,f) \ll_{\s} t^{\a(\s)},
      \]
      where $\a(\s)$ is real and depends upon $\s$. As $L(s,f)$ is holomorphic for $|t| > 1$ and of order $1$, we can apply the Phragm\'en-Lindel\"of convexity principle (see \cref{append:The_Phragmen_Lindelof_Convexity_principle}) so that the estimate
      \[
        L(s,f) \ll_{\s} t^{\a(\s)},
      \]
      holds for all $s$ with $|t| > 1$. Restricting $a < \s < b$, $\a(\s)$ takes a maximum value on the compact set $[a,b]$ and the implicit constant becomes dependent upon $a$ and $b$ which completes the proof. 
    \end{proof}
    ...
    \begin{theorem}[Approximate functional equation]
      ...
      \]
      where $V_{s}(x):(0,\infty) \to \C$ is the smooth function defined by
      \[
        V_{s}(x) = \frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{\g(s,f)}\Phi(u)x^{-u}\frac{du}{u},
      \]
      and
      \[
    ...
    \begin{proof}
      ...
      \]
      By our choice of $a$ and that $s$ is in the critical strip, $\Re(s+u) > 1$ so that $L(s+u) \ll_{a} 1$. Moreover, from \cref{equ:weaker_Stirling_formula} we see that $\g(s+u,f)$ has exponential decay in $\Im(u)$. Since $\Phi(u)$ is bounded, it follows that the integral is absolutely bounded by \cref{met:decay_compacta_integral}. By \cref{prop:L_function_bounded_in_vertical_strips} and \cref{equ:weaker_Stirling_formula}, the integrand also had exponential decay in a vertical strip containing $|\Re(u)| \le a$. Therefore we may shift the line of integration to $(-a)$. In doing so, we pass by a simple pole at $u = 0$ and possible poles at $u = 1-s$ and $u = -s$, giving
      \[
    ...
    \begin{remark}\label{rem:effect_of_V_in_approximate_functional_equation}
      The function $V_{s}(x)$ has the effect of smoothing out the two sums in the right-hand side of the approximate functional equation. For simplicity take $X = 1$. Indeed, this function restricts the sums to $n \ll \sqrt{q(f)}$ up to some bounded error. For if $n \gg q(f)^{\frac{1}{2}+\e}$ for some $\e > 0$ ($n \ll \sqrt{q(f)}$ includes those terms for which $n \asymp \sqrt{q(f)}$ and hence why we need $n \gg q(f)^{\frac{1}{2}+\e}$), the integrand of $V_{s}\left(\frac{n}{\sqrt{q(f)}}\right)$ has exponential decay in $\Im(u)$ coming from the $\left(\frac{n}{\sqrt{q(f)}}\right)^{-u}$ factor since the ratio of gamma factors has polynomial growth by \cref{equ:simplified_gamma_estimates} and $\Phi(u)$ is bounded by assumption. Alternatively, for $n \ll \sqrt{q(f)}$, we have
      \[
        V_{s}\left(\frac{n}{\sqrt{q(f)}}\right) \ll 1.
      \]
      As $s$ is in the critical strip, the exact same estimates hold for $V_{1-s}\left(\frac{n}{\sqrt{q(f)}}\right)$.
    \end{remark}
  \section{The Riemann Hypothesis \& Nontrivial Zeros}
    ...
    \begin{remark}\label{rem:poles_of_completed_L_function}
      Recall that for any $L$-function $L(s,f)$, the completed $L$-function $\L(s,f)$ is required to be meromorphic on $\C$ with at most poles at $s = 0$ and $s = 1$. We have shown that the trivial zeros of $L(s,f)$ are poles of $\g(s,f)$ for $\s < 0$ and they are all simple. As $L(s,f)$ is meromorphic on $\C$ with at most a pole at $s = 1$, and $\g(s,f)$ has at most a pole at $s = 0$ for $\s \ge 0$, $\L(s,f)$ is meromorphic on $\C$ with at most poles at $s = 0$ and $s = 1$.
    \end{remark}
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Arguments}
    ...
    \]
    is compact and $q\left(s,f\right) \sim q(f)t^{d}$ for $-\e \le \s \le 1+\e$, we have the estimate
  ...
  \section{Estimating the Central Value}
    ...
    \[
      \psi(x) = \begin{cases} e^{-\frac{1}{9-(4x-5)^{2}}} & \text{if $|4x-5| < 3$}, \\ 0 & \text{if $|4x-5| \ge 3$}. \end{cases}
    \]
    ...
    \begin{proof}
      ...
      \]
      Now consider the set of functions $\left\{\psi\left(\frac{x}{2^{k}}\right)\right\}_{k \in \Z}$. Since $\psi\left(\frac{x}{2^{k}}\right)$ has support in $[2^{k-1},2^{k+1}]$, the sum $\s(x) = \sum_{k \in \Z}\psi\left(\frac{x}{2^{k}}\right)$, defined for $x \ge 0$, is finite since at most finitely many terms are nonzero for every $x$. It is also bounded away from zero since for any $x > 0$ there is some $k \in \Z$ for which $2^{k} \le x \le 3 \cdot 2^{k-1}$ so that $\frac{x}{2^{k}}$ is at least distance $\frac{1}{2}$ from the endpoints of $\left[\frac{1}{2},2\right]$. Defining $\psi_{k}(x) = \psi\left(\frac{x}{2^{k}}\right)\s(x)^{-1}$, it follows that $\left\{\psi_{k}(x)\right\}_{k \in \Z}$ satisfies
      \[
        \sum_{k \in \Z}\psi_{k}(x) = 1,
      \]
      for any $x \in (0,\infty)$. Then we can write
      \begin{align*}
        V_{s}(x) &= \frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{\g(s,f)}\Phi(u)x^{-u}\frac{du}{u} \\
        &= \frac{1}{2\pi i}\int_{(a)}\sum_{k \in \Z}\frac{\g(s+u,f)}{\g(s,f)}\Phi(u)x^{-u}\psi_{k}(x)\frac{du}{u} \\
        &= \sum_{k \in \Z}\frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{\g(s,f)}\Phi(u)x^{-u}\psi_{k}(x)\frac{du}{u} && \text{DCT} \\
        &= \sum_{k \in \Z}V_{s,k}(x),
      \end{align*}
      where we have set
      \[
        V_{s,k}(x) = \frac{1}{2\pi i}\int_{(a)}\frac{\g(s+u,f)}{\g(s,f)}\Phi(u)x^{-u}\psi_{k}(x)\frac{du}{u}.
      \]
      Note that $V_{s,k}(x)$ is compactly supported on $[2^{k-1},2^{k}]$ because $\psi_{k}(x)$ is compactly supported there. Moreover, \cref{rem:effect_of_V_in_approximate_functional_equation} implies that $V_{s,k}(x)$ also exhibits exponential decay for $n \gg q(f)^{\frac{1}{2}+\e}$ for any $\e > 0$. It follows that
      \[
      ...
      \]
      Using the exponential decay of $V_{\frac{1}{2},k}(x)$ to crudely bound those terms for which $n \gg q(f)^{\frac{1}{2}+\e}$ gives the estimate
      \[
        \sum_{n \ge 1}\frac{a_{f}(n)}{\sqrt{n}}V_{\frac{1}{2}}\left(\frac{n}{\sqrt{q(f)}}\right) \ll \sum_{k \ll q(f)^{\e}}\sum_{\substack{2^{k-1} \le n \le 2^{k+1} \\ n \ll q^{\frac{1}{2}+\e}}}\frac{a_{f}(n)}{\sqrt{n}}V_{\frac{1}{2},k}\left(\frac{n}{\sqrt{q(f)}}\right),
      \]
      where in the first sum we have used that $\log(x) \ll_{\e} x^{\e}$. Now since $\s(x)$ is bounded away from zero, and $V_{\frac{1}{2},k}(x)$ and $\psi(x)$ are compactly supported on the same interval (so their nonzero values differ by a bounded amount), we have the weaker estimate
      \[
