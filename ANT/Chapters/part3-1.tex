\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet Series}
    ...
    \begin{proof}
      ...
      \]
      so that the integral exists for $\s > 0$. Thus $D(s)$ converges for $\s > 0$ and so $\s_{c} \le 0$.
    \end{proof}
  ...
  \section{Analytic Data of \texorpdfstring{$L$}{L}-functions}
    ...
    \end{enumerate}
    Property (ii) ensures that $\g(s,f)$ is holomorphic and nonzero for $\s \ge 1$. Then as $\g(1,f)$ is nonzero, $r_{f}$ is also the order of possible poles of $\L(s,f)$ at $s = 0$ and $s = 1$ which are equal by the functional equation. It follows that $r_{\conj{f}} = -r_{f}$. As $L(s,f)$ admits meromorphic continuation by property (v), we denote the continuation by $L(s,f)$ as well. It is also clear that the product of $L$-functions is also an $L$-function. Accordingly, we say that an $L$-function is \textbf{primitive}\index{primitive} if is cannot be written as a product of two $L$-functions.

    \begin{remark}
      In the literature it is common to refer to $L$-series satisfying a subset of the properties of $L$-functions as $L$-functions. The most common occurrence is when $L(s,f)$ is a linear combination of primitive $L$-functions. Even though it may cause slight confusion, we keep this convention.
    \end{remark}
    
    Moreover, we say that an $L$-function $L(s,f)$ belongs to the \textbf{Selberg class}\index{Selberg class} if the additional adjustments to (i), (ii), and (v) hold:
    \begin{itemize}
    ...
    \end{itemize}
    A few additional comments are in order for $L$-functions in general. The \textbf{critical strip}\index{critical strip} is the vertical strip left invariant by the transformation $s \to 1-s$, that is, the region defined by
    \[
  ...
  \section{The Approximate Functional Equation}
    If $L(s,f)$ is an $L$-function, then there is a formula which acts as a compromise between the functional equation for $L(s,f)$ and expressing $L(s,f)$ as a Dirichlet series. This formula is known as the approximate functional equation and it is important because it is valid inside of the critical strip and therefore can be used to obtain data about $L(s,f)$. First, we need to state an important asymptotic about the ratio of gamma factors that we will use later on. Suppose $\s$ is bounded and $|t| > 1$. Then $s$ is bounded away from zero and by \cref{equ:weaker_Stirling_formula} we have
    \[
    ...
    \end{equation}
    are valid in the vertical strip $a \le \s \le b$ provided $s$ is distance $\e$ away from the poles of $\g(s,f)$. We can use this \cref{equ:gamma_factor_analytic_conductor_estimate} to show that $L(s,f)$ is has polynomial growth in the $t$-aspect in vertical strips:
    ...
    \begin{proof}
      ...
      \end{align*}
      where in the third and fourth lines we have used that $cy \ll (1+cy)$ for all $y \ge 0$ and any $c$ and the last line holds by \cref{prop:decay_unbounded_inteval_integral} since the integrand exhibits rapid decay. This completes the proof.
    \end{proof}
    ...
    \begin{proof}
      ...
      \]
      By \cref{prop:smoothing_function_Mellin_inverse_vertical_strips,prop:L_function_bounded_in_vertical_strips}, the integrand has polynomial decay and therefore is locally absolutely uniformly convergent by \cref{prop:decay_unbounded_inteval_integral}. Shifting the line of integration to $(1-a)$, we pass by a potential pole at $s = 1$ from $L(s,f)$ and obtain
      \[
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Bounds}
    ...
    \]
    in the vertical strip $-\e \le \s \le 1+\e$. At the critical line we obtain the following \textbf{convexity bound}\index{convexity bound}:
    ...
    With a little more work, we can obtain a convexity bound for $L'(s,f)$. Indeed, since $L(s,f)$ is of order $1$ so is $L'(s,f)$ and thus we may apply the Lindel\"of convexity principle to $p_{r_{f}}(s)^{2}L'(s,f)$ (the order of the possible pole at $s = 1$ may increase by one). We now estimate the growth of $p_{r_{f}}(s)^{2}L(s,f)$ for $\s$ to the left of $0$ and to the right of $1$. For the right edge, set $\s = 1+\e$ so that
    \[
      p_{r_{f}}(1+\e+it)^{2}L'(1+\e+it,f) \ll_{\e} 1,
    \]
    which holds because $L'(s,f)$ is defined by a locally absolutely uniformly convergent Dirichlet series for $\s > 1$ (since we may differentiate $L(s,f)$ termwise). For the left edge, isolating $L(s,f)$ in the functional equation and differentiating gives
    \begin{align*}
      L'(s,f) &= -\e(f)\log(q(f))q(f)^{\frac{1}{2}-s}\frac{\g(1-s,f)}{\g(s,f)}L(1-s,f) \\
      &- \e(f)q(f)^{\frac{1}{2}-s}\frac{\g(1-s,f)}{\g(s,f)}\left(\frac{\g'}{\g}(1-s,f)-\frac{\g'}{\g}(s,f)\right)L(1-s,f) \\
      &+ \e(f)q(f)^{\frac{1}{2}-s}\frac{\g(1-s,f)}{\g(s,f)}L'(1-s,f).
    \end{align*}
    Applying \cref{equ:gamma_factor_analytic_conductor_estimate} and \cref{equ:digamma_gamma_factor_analytic_conductor_estimate}, we obtain
    \[
      L'(s,f) \ll_{\e} \mf{q}(s,f)^{\frac{1}{2}-\s}\left(\log\mf{q}(s,f)L(1-s,f)+L'(1-s,f)\right),
    \]
    for $s$ in any vertical strip with distance $\e$ away from the poles of both $\g(s,f)$ and $\g(1-s,f)$. Multiplying both sides by $p_{r_{f}}(s)^{2}$ and taking $\s = -\e$, it follows that
    \[
      p_{r_{f}}(-\e+it)^{2}L(-\e+it,f) \ll_{\e} \mf{q}(s,f)^{\frac{1}{2}+\e}\log\mf{q}(s,f),
    \]
    which holds since both $L(1-s,f)$ and $L'(1-s,f)$ are defined by locally absolutely uniformly convergent Dirichlet series for $\s < 0$. As $p_{r_{f}}(s)^{2}L'(s,f)$ is holomorphic in a region containing the vertical strip $-\e \le \s \le 1+\e$ and $p_{r_{f}}(s) \sim 1$, the Phragm\'en-Lindel\"of convexity principle gives
    \[
      L'(s,f) \ll_{\e} \mf{q}(s,f)^{\frac{1-\s}{2}+\e}\log\mf{q}(s,f),
    \]
    in the vertical strip $-\e \le \s \le 1+\e$. At the critical line we obtain the following \textbf{convexity bound}\index{convexity bound}:
    \[
      L'\left(\frac{1}{2}+it,f\right) \ll_{\e} \mf{q}\left(\frac{1}{2}+it,f\right)^{\frac{1}{4}+\e}\log\mf{q}\left(\frac{1}{2}+it,f\right).
    \]
    Note that the convexity bounds for $L(s,f)$ and $L'(s,f)$ only differ by a logarithm of the analytic conductor. This is typical of a large reality when dealing with derivatives of $L$-functions.
  ...
  \section{The Explicit Formula}
    ...
    \begin{proof}
      ...
      \]
      for any $c > 1$. Since the trivial zeros are isolated, let $\d > 0$ be such that there are no trivial zeros in the vertical strip $-2\d \le \s < 0$. By \cref{prop:smoothing_function_Mellin_inverse_vertical_strips,prop:L_function_bounded_in_vertical_strips}, the integrand has polynomial decay and therefore is locally absolutely uniformly convergent by \cref{prop:decay_unbounded_inteval_integral}. Shifting the line of integration to $(-\d)$, we pass by simple poles at $s = 1$ and $s = \rho$ for every nontrivial zero $\rho$ obtaining
      \[
