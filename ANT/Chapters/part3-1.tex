\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet Series}
    ...
    \end{proposition}
    \begin{proof}
      Fix $s$ such that $\Re(s) > 0$. Since $A(X) \ll 1$, $A(X)X^{-s} \to 0$ as $X \to \infty$. Abel's summation formula (see \cref{append:Summation_Formulas}) then implies
      \[
        D(s) = s\int_{1}^{\infty}A(u)u^{-(s+1)}\,du.
      \]
      But because $A(u) \ll 1$, we have
      \[
        s\int_{1}^{\infty}A(u)u^{-(s+1)}\,du \ll s\int_{1}^{\infty}u^{-(\Re(s)+1)}\,du = -\frac{s}{\Re(s)}u^{-\Re(s)}\big|_{1}^{\infty} = \frac{s}{\Re(s)}.
      \]
    ...
    \begin{proof}
      ...
      \[
        D(s) \ll \sum_{n \ge 1}\left|\frac{a(n)}{n^{s}}\right| \ll \sum_{n \ge 1}\frac{1}{n^{\Re(s)-\a}}.
      \]
    ...
    \end{proposition}
    \begin{proof}
      It suffices to show that $D(s)$ is absolutely convergent in the region $\Re(s) > \a+1$. Let $s$ be in this region and set $\Re(s) = \b$. Note that $\b > \a$. Then
      \[
    ...
    \begin{proof}
        ...
        \[
          \frac{a(p)}{p^{s}} \ll \frac{1}{p^{\Re(s)-\a}} < \frac{1}{p} < 1.
        \]
  ...
  \section{Perron Formulas}
    ...
    \begin{proof}
      ...
      \]
      Now $A'(Y) \le A(Y)$ and $A(Y) \ll_{\e} Y^{\s_{a}+\e}$ for any $\e > 0$ by \cref{prop:Dirichlet_series_coefficient_size_on_average} so that $A'(Y)Y^{-s} \ll Y^{\s_{a}+\e-\Re(s)}$. Choosing $\e < \Re(s)-\s_{a}$, this latter term tends to zero as $Y \to \infty$, which implies that $A'(Y)Y^{-s} \to 0$ as $Y \to \infty$. Therefore we can write the equation above as
      \[
    ...
    \]
    for some $X$ and $Y$ with $Y < X$. Sums of this type are called \textbf{unweighted}\index{unweighted}. As an example of an unweighted sum, let $\psi:[0,\infty) \to [0,\infty)$ be a bump function that is identically $1$ on $[0,1]$ and vanishes within the interval $\left(1,\frac{X+1}{X}\right]$. Explicitly,
    \[
