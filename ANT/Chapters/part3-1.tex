\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Bounds}
    ...
    \begin{remark}
      Some subconvexity bounds are deserving of names. The case $\d = \frac{3}{16}$ is referred to as the \textbf{Burgess bound}\index{Burgess bound} and the case $\d = \frac{1}{6}$ is referred to as the \textbf{Weyl bound}\index{Weyl bound}.
    \end{remark}
    ...
    \end{align*}
    Now \cref{equ:approximtion_for_digamma} gives the estimate $\frac{\G'}{\G}(s) \ll \log(|s|+1)$ provided $\s > 0$. It follows that $\frac{\g'}{\g}(1-s,f) \ll_{\e} \log{\mf{q}_{\infty}(1-s,f)}$ for $s$ with $\s < 0$ and distance $\e$ away from the poles of $\g(1-s,f)$. Using the formula $\frac{\G'}{\G}(s+1) = \frac{\G'}{\G}(s)+\frac{1}{s}$, we have that $\frac{\g'}{\g}(s,f) \ll_{\e} \log{\mf{q}_{\infty}(s,f)}$ for $s$ with $\s < 0$ and distance $\e$ away from the poles of $\g(s,f)$. Applying these bounds and \cref{equ:gamma_factor_analytic_conductor_estimate}, we obtain
    \[
    ...
    in the vertical strip $-\e \le \s \le 1+\e$. At the critical line we obtain the following \textbf{convexity bound}\index{convexity bound}:
    \[
      L'\left(\frac{1}{2}+it,f\right) \ll_{\e} \mf{q}\left(\frac{1}{2}+it,f\right)^{\frac{1}{4}+\e}\log\mf{q}(s,f).
    \]
  ...
  \section{Zero Density}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \end{equation}
        provided $\s > 2$. For the second quantity, \cref{equ:approximtion_for_digamma} gives the estimate $\frac{\G'}{\G}(s) \ll \log(|s|+1)$ provided $\s > 0$. It follows that $\frac{\g'}{\g}(s,f) \ll \log{\mf{q}_{\infty}(s,f)}$. Thus
        ...
        \]
        Applying the formula for the negative logarithmic derivative in \cref{prop:explicit_formula_log_derivative} to the two terms on the right-hand side and using that established bound $\frac{\g'}{\g}(s,f) \ll \log{\mf{q}_{\infty}(s,f)}$ provided $\s > 0$, we get
        \[
        ...
        \]
        Lastly, from the definition of $\g(s,f)$ and that $\frac{\G'}{\G}(s+1) = \frac{\G'}{\G}(s)+\frac{1}{s}$, we have
        \[
        ...
        \]
        where in the last line we have made use of the estimate $\frac{\G'}{\G}(s) \ll \log(|s|+1)$ from \cref{equ:approximtion_for_digamma} provided $\s > 0$. Then
        \[
          -\frac{L'}{L}(s,f) = \frac{r_{f}}{s}+\frac{r_{f}}{s-1}-\sum_{|s+\k_{j}| < 1}\frac{1}{s+\k_{j}}-\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\frac{1}{s-\rho}+O(\log{\mf{q}(s,f)}),
        \]
