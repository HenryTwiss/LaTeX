\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Approximate Functional Equation}
    ...
    \end{equation}
    It is not hard to obtain estimates that holds in vertical strips. Suppose $s$ is in the vertical strip $a < \s < b$ and is distance $\e$ away from the poles of $\G(s)$. For $|t| > 1$, \cref{equ:simplified_gamma_estimates} is satisfied. Moreover $\G(s)$ is bounded on the compact region $a \le \s \le b$ with $|t| \le 1$ provided $s$ is distance $\e$ away from the poles. As the bounds in \cref{equ:simplified_gamma_estimates} are zero when $t = 0$, the estimates
    ...
    \end{equation}
    are valid in the vertical strip $a < \s < b$ provided $s$ is distance $\e$ away from the poles. We can use these estimates to show that $L(s,f)$ is has polynomial growth in the $t$-aspect in vertical strips:

    \begin{proposition}\label{prop:L_function_bounded_in_vertical_strips}
      If $L(s,f)$ is an $L$-function, then $L(s,f)$ is of polynomial growth in the $t$-aspect in vertical strips $a \le \s \le b$ with $|t| \ge 1$ for any $a < b$.
    \end{proposition}
    \begin{proof}
      ...
      \]
      Using \cref{equ:simplified_gamma_estimates} and the definition of $\g(s,f)$, it follows that $\frac{\g(1-s,f)}{\g(s,f)}$ is of polynomial growth in the $t$-aspect. Thus for $\s < 0$, we see that $L(s,f)$ is also of polynomial growth in the $t$-aspect. As $L(s,f)$ is holomorphic for $|t| > 1$ and of order $1$, we can apply the Phragm\'en-Lindel\"of convexity principle (see \cref{append:The_Phragmen_Lindelof_Convexity_principle}) so that $L(s,f)$ is of polynomial growth in the $t$-aspect in any vertical strip with $|t| > 1$.
    \end{proof}
    ...
    \begin{theorem}[Approximate functional equation]
      Let $L(s,f)$ be an $L$-function and let $\Phi(u)$ be an even holomorphic function bounded in the vertical strip $|\Re(u)| < a+1$ for any $a > 1$, such that $\Phi(0) = 1$, and let $X > 0$. Then for $s$ in the critical strip,
      \[
    ...
    \begin{remark}\label{rem:effect_of_V_in_approximate_functional_equation}
      The function $V_{s}(y)$ has the effect of smoothing out the two sums in the right-hand side of the approximate functional equation. For simplicity take $X = 1$ and let $u = \tau+ir$. From \cref{equ:modified_gamma_estimates}, and that $s$ is fixed,
      \[
        \frac{\g(s+u,f)}{\g(s,f)} \ll \prod_{1 \le i \le d}\frac{(|s+u+\k_{i}|+3)^{\s+\tau-\frac{1}{2}}}{(|s+\k_{i}|+3)^{\s-\frac{1}{2}}}\frac{e^{-\frac{\pi}{2}|t+r|}}{e^{-\frac{\pi}{2}|t|}},
      \]
      where we have used that $\left(\left|\frac{t+r+\Im(\k_{i})}{2}\right|+1\right) \asymp (|s+u+\k_{i}|+3)$ and $\left(\left|\frac{t+\Im(\k_{i})}{2}\right|+1\right) \asymp (|s+\k_{i}|+3)$. \todo{xxx}
    \end{remark}
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Arguments}
    ...
    \]
    for $s$ in any vertical strip with distance $\e$ away from the poles of $\g(1-s,f)$ and where we have used that $\left(\left|\frac{t+\Im(\k_{i})}{2}\right|+1\right) \asymp (|s+\k_{i}|+3)$ and $1-s \sim s$. In particular, setting $\s = -\e$, our previous two bounds combine to give
    \[
  ...
  \section{Estimating the Central Value}
    ...
    \begin{theorem}
      Let $L(s,f)$ be an $L$-function and let $\psi:\R_{\ge 0} \to \R_{\ge 0}$ be a bump function with compact support in $\left[\frac{1}{2},2\right]$. Then for any $\e > 0$, we have
      \[
