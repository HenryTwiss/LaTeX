\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{remark}\label{rem:effect_of_V_in_approximate_functional_equation}
      The function $V_{s}(x)$ has the effect of smoothing out the two sums in the right-hand side of the approximate functional equation. For simplicity take $X = 1$. Indeed, this function restricts the sums to $n \ll q(f)^{\frac{1}{2}+\e}$, for any $\e > 0$, up to some bounded error. \todo{xxx}
    \end{remark}
