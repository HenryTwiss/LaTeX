\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet Series}
    ...
    \begin{proof}
      ...
      \]
      By assumption $A^{\ast}(u) \ll u^{\a}$. Therefore there is some $N'$ such that $A^{\ast}(N) \ll N^{\a}$ for all $N \ge N'$. In particular $a(N) \ll_{\a} N^{\a}$. So for such $N$, we estimate as follows:
      \begin{align*}
        \sum_{n \le N}\frac{|a(n)|}{n^{\s}} &= |a(N)|N^{-\s}-|a(1)|+\s\int_{1}^{N}A^{\ast}(u)u^{-(\s+1)}\,du \\
