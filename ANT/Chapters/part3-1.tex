\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Bounds}
    ...
    With a little more work, we can obtain a a similar bound for $L^{(n)}(s,f)$ for all $n \ge 1$. Indeed, Cauchy's integral formula implies
    \[
      L^{(n)}\left(\frac{1}{2}+it,f\right) = \frac{n!}{2\pi i}\oint_{C}\frac{L(s,f)}{(s-\frac{1}{2}-it)^{n+1}}\,ds \ll_{n,\e} \frac{1}{2\pi i}\oint_{C}\frac{L(s,f)}{s-\frac{1}{2}-it}\,ds = L\left(\frac{1}{2}+it,f\right),
    \]
    where $C$ is a circle about $\frac{1}{2}+it$ of radius $\e$. The convexity bound for $L(s,f)$ then gives
    \[
      L^{(n)}\left(\frac{1}{2}+it,f\right) \ll_{n,\e} \mf{q}\left(\frac{1}{2}+it,f\right)^{\frac{1}{4}+\e},
    \]
    for any $n \ge 1$, which is the \textbf{convexity bound}\index{convexity bound} for $L^{(n)}(s,f)$.
