  ...
  \section{\todo{A Zero-free Region}}
    ...
    \begin{proof}
      ...
      \[
        ...
        g)^{2}} > 0.
      \]
      So upon taking the real part of the bound in \cref{lem:powerful_L-function_approximation_lemma} (iii), we may disregard any of the nontrivial zeros in the second sum provided $1 < \s < 2$. Now let $\b_{j}$ be a real nontrivial zero with $\frac{1}{2} \le \b_{j} \le 1$. Letting $1 < \s < \frac{3}{2}$, and disregarding all but the nontrivial zeros $\b_{j}$ in the real part of of the bound in \cref{lem:powerful_L-function_approximation_lemma} (iii) (with $s = \s$), we obtain the inequality
      \[
        \sum_{j}\frac{1}{\s-\b_{j}} < \Re\left(\frac{L'}{L}(s,f)\right)+\frac{r_{f}}{s-1}+O(q(f)).
      \]
      ...
      \]
      This gives an estimate for the local factors of $L(s,f)$ corresponding to unramified primes. For the local factors corresponding to ramified primes, we use \cref{equ:log_derivaive_direct_computation} to compute
      \[
      ...
      \]
      where in the second inequality we have made use of the assumption $|\a_{i}(p)| \le \frac{p}{2}$ for ramified primes to conclude that $\left|\frac{\a_{i}(p)}{(1-\a_{i}(p)p^{-\s})p^{\s}}\right| \le 1$. \todo{xxx}
    \end{proof}
