  ...
  \section{Zero Density}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \end{equation}
        for $\s > 0$. Now fix $T > 0$ and let $s = 3+iT$. Then taking the real part of the formula for the logarithmic derivative in \cref{prop:explicit_formula_log_derivative} and combining \cref{equ:log_derivative_bound_conductor,equ:log_conductor_gamma_factor_bound_analytic_conductor} with (i), results in
        \[
        ...
        \]
        Applying the formula for the logarithmic derivative in \cref{prop:explicit_formula_log_derivative} to the two terms on the right hand side and using that $\frac{\g'}{\g}(s,f) \ll \log{\mathfrak{q}_{\infty}(s,f)}$, we get
        \[
