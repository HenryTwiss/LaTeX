\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Approximate Functional Equation}
    ...
    \end{equation}
    This last estimate will be used to show that $L(s,f)$ is has polynomial growth in the $t$-aspect in vertical strips:
  ...
  \section{The Riemann Hypothesis \& Nontrivial Zeros}
    ...
    \begin{theorem}[Riemann hypothesis, classical version]
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Arguments}
    ...
    \end{equation}
    Let $\e > 0$ and set $\s = -\e$. Then plugging \cref{equ:ratio_gamma_factor_explicit} into \cref{equ:convexity_bound_left_edge} and using the fact that $L((1+\e)+it,f) \ll_{\e} 1$ gives
    ...
    \begin{equation}\label{equ:convexity_bound_L_function}
      L\left(\frac{1}{2}+it,f\right) \ll_{\e} q(f)^{\frac{1}{4}+\e}t^{\frac{d}{4}+\e}.
    \end{equation}
    The \textbf{(classical) Lindel\"of hypothesis}\index{(classical) Lindel\"of hypothesis} says that both exponenets can be improved to be $\e$ for the Riemann zeta function $\z(s)$ (note that this is really just improvement in the $t$-aspect because $q(\z) = 1$):
    \begin{conjecture}[Lindel\"of hypothesis, classical version]
      For any $\e > 0$,
      \[
        \z\left(\frac{1}{2}+it\right) \ll_{\e} t^{\e}.
      \]
    \end{conjecture}
    In 1908 Lindel\"of made this conjecture (see \cite{lindelof1908quelques}). The classical Riemann hypothesis implies the classical Lindel\"of hypothesis although it is not an immediate consequence. Indeed, in 1919 Backlund showed the following equivalence for the classical Lindel\"of hypothesis (see \cite{backlund1919beziehung}):

    \begin{proposition}\label{prop:Lindelof_hypothesis_equivalence}
      The classical Lindel\"of hypothesis is equivalent to the following statement: Fix an $\e > 0$, a real $T$, and set
      \[
    ...
    If the Riemann hypothesis is true, then \cref{prop:Lindelof_hypothesis_equivalence} is immediate because $Z_{\e,T}$ would be empty for any $\e > 0$ and all $T$. Nevertheless, the classical Lindel\"of hypothesis still remains wide open today. The \textbf{(Selberg class) Lindel\"of hypothesis}\index{(Selberg class) Lindel\"of hypothesis} is the analog to the classical Lindel\"of hypothesis for any Selberg class $L$-function:
    ...
    For any $L$-function, any improvment upon either exponent in the convexity bound is called \textbf{breaking convexity}\index{breaking convexity} (or a \textbf{convexity breaking bound}\index{convexity breaking bound}). Any argument used to do so is called a \textbf{subconvexity argument}\index{subconvexity argument}. One method of breaking convexity is by appealing to the approximate functional equation. Setting $s = \frac{1}{2}+it$ and noting that $\e\left(\frac{1}{2}+it,f\right) \ll 1$ by \cref{equ:ratio_gamma_factor_explicit}, the approximate functional equation gives
    \[
      L\left(\frac{1}{2}+it,f\right) = \sum_{n \ge 1}\frac{a_{f}(n)}{n^{\frac{1}{2}+it}}V_{\frac{1}{2}+it}\left(\frac{n}{\sqrt{q(f)}X}\right)+\sum_{n \ge 1}\frac{\conj{a_{f}(n)}}{n^{\frac{1}{2}-it}}V_{\frac{1}{2}-it}\left(\frac{nX}{\sqrt{q(f)}}\right)-\frac{R}{q^{\frac{1}{4}+\frac{it}{2}}\g\left(\frac{1}{2}+it,f\right)}.
    \]
    Estimating the left-hand side can be acomplished using Perron-type formulas for the sums and the residue term can usually be computed outright or estimated directly.