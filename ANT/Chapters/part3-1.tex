\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{proof}
      ...
      \]
      $L(s,f)$ has polynomial growth in the $t$-aspect by \cref{prop:L_function_bounded_in_vertical_strips}. From \cref{equ:modified_gamma_estimates} we see that $\g(s+u,f)$ has exponential decay as $\Im(u) \to \infty$. Since $\Phi(u)$ is bounded, it follows that the integral has exponential decay in a vertical strip containing $|\Re(u)| \le a$. Therefore the integral is absolutely bounded by \cref{met:decay_compacta_integral}. Moreover, we may shift the line of integration to $(-a)$. In doing so, we pass by a simple pole at $u = 0$ and possible poles at $u = 1-s$ and $u = -s$, giving
      \[
  ...
  \section{Estimating the Central Value}
    ...
    \begin{proof}
      ...
      \]
      Note that $V_{s,k}(y)$ is compactly supported on $[2^{k-1},2^{k}]$ because $\psi_{k}(y)$ is compactly supported there. Moreover, \cref{rem:effect_of_V_in_approximate_functional_equation} implies that $V_{s,k}(y)$ also exhibits exponential decay for $n \gg q(f)^{\frac{1}{2}+\e}$. It follows that
      \[
