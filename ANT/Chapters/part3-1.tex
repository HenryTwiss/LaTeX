\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Perron Formulas}
    ...
    \[
      \d(y) = \frac{1}{2\pi i}\int_{\Re(s) = c}y^{s}\,\frac{ds}{s} = \begin{cases} 0 & \text{if $0 < y < 1$}, \\ \frac{1}{2} & \text{if $y = 1$}, \\ 1 & \text{if $y > 1$}. \end{cases}
    \]
  ...
  \section{The Approximate Functional Equation}
    ...
    \begin{theorem}[Approximate functional equation]
      ...
      \[
        V_{s}(y) = \frac{1}{2\pi i}\int_{\Re(u) = a}\frac{\g(s+u,f)}{\g(s,f)}G(u)y^{-u}\frac{du}{u},
      \]
      and
      \[
    ...
    \begin{proof}
      Let
      \[
        I(X,s,f) = \frac{1}{2\pi i}\int_{\Re(u) = a}\L(s+u,f)G(u)X^{u}\,\frac{du}{u}.
      \]
      ...
      \[
        I(X,s,f) = \frac{1}{2\pi i}\int_{\Re(u) = -a}\L(s+u,f)G(u)X^{u}\,\frac{du}{u}+\L(s,f)+R.
      \]
      ...
      \begin{align*}
        I(X,s,f) &= \frac{1}{2\pi i}\int_{\Re(u) = a}\L(s+u,f)G(u)X^{u}\,\frac{du}{u} \\
        &= \frac{1}{2\pi i}\int_{\Re(u) = a}q(f)^{\frac{s+u}{2}}\g(s+u,f)L(s+u,f)G(u)X^{u}\,\frac{du}{u} \\
        &= \frac{1}{2\pi i}\int_{\Re(u) = a}\sum_{n \ge 1}\frac{a_{f}(n)}{n^{s+u}}q(f)^{\frac{s+u}{2}}\g(s+u,f)G(u)X^{u}\,\frac{du}{u} \\
        &= \sum_{n \ge 1}\frac{1}{2\pi i}\int_{\Re(u) = a}\frac{a_{f}(n)}{n^{s+u}}q(f)^{\frac{s+u}{2}}\g(s+u,f)G(u)X^{u}\,\frac{du}{u} && \text{DCT} \\
        &= q(f)^{\frac{s}{2}}\g(s,f)\sum_{n \ge 1}\frac{a_{f}(n)}{n^{s}}\frac{1}{2\pi i}\int_{\Re(u) = a}\frac{\g(s+u,f)}{\g(s,f)}G(u)\left(\frac{\sqrt{q(f)}X}{n}\right)^{u}\,\frac{du}{u} \\
