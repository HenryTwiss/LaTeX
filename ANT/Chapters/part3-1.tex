\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Explicit Formula for the Logarithmic Derivative}
    ...
    \]
    Note that $\xi(s,f)$ is essentially just $\L(s,f)$ with the potential poles at $s = 0$ and $s = 1$ removed. We now state our desired result:
    ...
    \begin{proof}
      ...
      \]
      Equating these two expressions, we arrive at
      \[
    ...
    The formula for the logarithmic derivative in \cref{prop:explicit_formula_log_derivative} it called the \textbf{explicit formula for the logarithmic derivative}\index{explicit formula for the logarithmic derivative} of $L(s,f)$. The estimation of the constants $A(f)$ and $B(f)$ can be challenging even in specific cases and heavily depends upon the arithmetic object $f$. We will often need to estimate them on a case-by-case basis. In general, the negative logarithmic derivative of $L(s,f)$ will attract much attention. As $L(s,f)$ is holomorphic for $\s > 1$ and admits an Euler product there, we can take the logarithm of the Euler product (turning it into a sum) and differentiate termwise to obtain
    \[
  ...
  \section{\todo{Zero Density}}
    ...
    \begin{lemma}
      Let $L(s,f)$ be an $L$-function and let $\rho$ denote a nontrivial zero other than $0$ or $1$. The following statements hold:
      \begin{enumerate}
        \item The constant $B(f)$ satisfies
        \[
          \Re(B(f)) = -\sum_{\rho}\Re\left(\frac{1}{\rho}\right).
        \]
        \item For any $T > 0$, the number of zeros $\rho = \b+i\g$ such that $|\g-T| \le 1$ is $O(\log(q(iT,f)))$.
        \item We have
        \[
          -\frac{L'}{L}(s,f)+\frac{r_{f}}{s}+\frac{r_{f}}{s-1}-\sum_{|s+\k_{i}| < 1}\frac{1}{s+\k_{i}}-\sum_{|s-\rho| < 1}\frac{1}{s-\rho} \ll q(s,f),
        \]
