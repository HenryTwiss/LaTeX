\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Analytic Data of \texorpdfstring{$L$}{L}-functions}
    ...
    \begin{enumerate}[label=(\roman*)]
      ...
      where we set
      \[
        q_{\infty}(s) = \prod_{1 \le i \le d}(|s+\k_{i}|+1).
      \]
  ...
  \section{The Approximate Functional Equation}
    ...
    \end{equation}
    We then have an important estimate for the ratio of gamma factors which is
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Arguments}
    ...
    \end{equation}
    for $-\e \le \s \le 1+\e$ with $|t| > 1$. Since $\Re(s)$ is bounded, we have the estimate
    \begin{equation}\label{equ:ratio_gamma_factor_explicit}
      \frac{\g(1-s,f)}{\g(s,f)} \ll q(s,f)^{\frac{1-2\s}{2}},
    \end{equation}
    for $-\e \le \s \le 1+\e$. Now let $\e > 0$ and set $\s = -\e$. Then plugging \cref{equ:ratio_gamma_factor_explicit} into \cref{equ:convexity_bound_left_edge} and using \cref{equ:convexity_bound_1} gives
    \begin{equation}\label{equ:convexity_bound_2}
      p_{r}(-\e+it)L(-\e+it,f) \ll_{\e} q(s,f)^{\frac{1+\e-\s}{2}}.
    \end{equation}
    ...
    \[
      L(s,f) \ll_{\e} q(s,f)^{\frac{1-2\s}{2}},
    \]
    ...
    \end{equation}
    Note that $q\left(\frac{1}{2}+it,f\right) \sim q(f)t^{d}$. The \textbf{(Selberg class) Lindel\"of hypothesis}\index{(Selberg class) Lindel\"of hypothesis} says that the exponent can be reduced to $\e$:
