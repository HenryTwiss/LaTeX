\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Hypothesis \& Nontrivial Zeros}
    ...
    The \textbf{Riemann hypothesis}\index{Riemann hypothesis} says that this symmetry should be as simple as possible:

    \begin{conjecture}[Riemann hypothesis]
      All of the nontrivial zeros of the $L$-function $L(s,f)$ lie on the line $\s = \frac{1}{2}$.
    \end{conjecture}

    Somewhat confusingly, do not expect the Riemann hypothesis to hold for just any $L$-function $L(s,f)$ (but we expect it to hold for many $L$-functions). However, the \textbf{Grand Riemann hypothesis}\index{Grand Riemann hypothesis} says that this symmetry should hold for any $L$-function in the Selberg class and we expect this to hold:

    \begin{conjecture}[Grand Riemann hypothesis]
      All of the nontrivial zeros of any Selberg class $L$-function $L(s,f)$ lie on the line $\s = \frac{1}{2}$.
    \end{conjecture}
  \section{The Lindel\"of Hypothesis \& Convexity Arguments}
    ...
    Just as for the Riemann hypothesis, we do not expect the Lindel\"of hypothesis to hold for just any $L$-function $L(s,f)$. Accordingly, the \textbf{Grand Lindel\"of hypothesis}\index{Grand Lindel\"of hypothesis} says that the exponent can be reduced to $\e$ for any $L$-function $L(s,f)$ in the Selberg class and we expect this to hold:
  ...
  \section{Zero Density}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \[
          \sum_{|s+\k_{j}| < 1}\Re\left(\frac{1}{s+\k_{j}}\right)+\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\Re\left(\frac{1}{s-\rho}\right) \le \Re\left(-\frac{L'}{L}(s,f)\right)+\Re\left(\frac{r_{f}}{s}\right)+\Re\left(\frac{r_{f}}{s-1}\right)+O(\log{\mathfrak{q}(s,f)}).
        \]
