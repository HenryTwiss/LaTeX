\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Lindel\"of Hypothesis \& Convexity}
    Instead of asking about the zeros of an $L$-function $L(s,f)$ on the critical line, we can ask about the growth of $L(s,f)$ on the critical line. We will derive a bound by using the Phragm\'en-Lindel\"of convexity principle. The following argument is essentially a refinement of the proof of \cref{prop:L_function_bounded_in_vertical_strips}. Let
    \[
    ...
    \]
    which holds since $L(s,f)$ is defined by a locally absolutely uniformly convergent Dirichlet series for $\s > 1$. The left edge is only slightly more difficult. The functional equation and \cref{equ:gamma_factor_analytic_conductor_estimate} together imply
    \[
      p_{r_{f}}(s)L(s,f) \ll_{\e} p_{r_{f}}(s)\mf{q}(s,f)^{\frac{1}{2}-\s}L(1-s,f),
    \]
