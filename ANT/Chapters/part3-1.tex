  ...
  \section{\todo{Zero Density}}
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \]
        where we have made use of the fact that $\rho$ is a nontrivial zero of $L(s,f)$ if and only if $1-\conj{\rho}$ is also a nontrivial zero of $L(s,f)$ and hence a nontrivial zero of $L(1-s,\conj{f})$ too. Now fix $s$ such that it does not coincide with the ordinate of a nontrivial zero . Then $s$ is bounded away from all the $\rho$ and $1-\conj{\rho}$ so that $(s-\rho)^{-1}+(1-s-\rho)^{-1} \ll \rho^{-2}$ and $\rho^{-1}+\conj{\rho}^{-1} \ll \rho^{-2}$. It follows that the sums
        \[
        ...
        \]
        converge absolutely by \cref{prop:explicit_formula_log_derivative} and so we can sum them separately. The first sum vanishes by again using the fact that $\rho$ is a nontrivial zero if and only if $1-\conj{\rho}$ is. Thus
        \[
          2\Re(B(f)) = \sum_{\rho \neq 0,1}\left(\frac{1}{\rho}+\frac{1}{\conj{\rho}}\right) = -\sum_{\rho \neq 0,1}\Re\left(\frac{1}{\rho}\right),
        \]
        which is (i).
      \item For (ii), suppose $s$ is in the critical strip. As $\frac{\G'}{\G}(s)$ is bounded on this compact region with in the critical strip with $|t| < 1$ provided $s$ is distance $\e$ away from zero, \cref{equ:approximtion_for_digamma} provides the estimate $\frac{\G'}{\G}(s) \ll \log(|s|+3)$ which valid on this region. From the definition of $\g(s,f)$ it follows that $\frac{\g'}{\g}(s,f) \ll \log(q_{\infty}(s,f))$ provided $s$ is distance $\e$ away from $0$ and $1$. Thus
      \[
        \frac{1}{2}\log(q(f))+\frac{\g'}{\g}(s,f) \ll \log(q(s,f)).
      \]
      Now let $T > 0$ and suppose $\rho = \b+i\g$ is a zero with $|\g-T| \le 1$. Then
      \[
        \todo{\frac{2}{9+(\g-T)^{2}} \le \Re\left(\frac{1}{s-\rho}\right) \le \frac{3}{4+(\g-T)^{2}}}
      \]
      \end{enumerate}
    \end{proof}
