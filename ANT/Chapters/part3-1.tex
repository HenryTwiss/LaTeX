\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{A Zero-free Region}}
    Although the Riemann hypothesis remains out of reach, some progress has been made to understand regions inside of the critical strip for which $L$-functions do not have zeros. Such regions are known as \textbf{zero-free regions}\index{zero-free regions} and there is great interest in improving the breadth of such regions. We will derive a standard zero-free region for any $L$-function under some mild assumptions on certain Rankin-Selberg convolutions. First, we require a useful lemma:
    ...
    \begin{proof}
      ...
      \]
      To estimate $\Re\left(\frac{L'}{L}(s,f)\right)$, we first note that as $\Re(\l_{f}(n)) \ge 0$ provided $(n,q(f)) = 1$, the Dirichlet series of $\frac{L'^{(q(f))}}{L^{(q(f))}}(s,f)$ shows that
      \[
      ...
      \]
      where in the second inequality we have used \todo{xxx}.
