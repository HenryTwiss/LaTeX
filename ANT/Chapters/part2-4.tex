\chapter{Trace Formulas}
  ...
  \section{The Petersson Trace Formula}
    ...
    \begin{proposition}\label{prop:Fourier_coefficients_of_holomorphic_Poincare_series}
      The Fourier series of $P_{m,\chi}(z)$ on $\G_{0}(N)\backslash\H$ is given by
      \[
        P_{m,\chi}(z) = \sum_{t > 0}\left(\d_{m,t}+\left(\frac{\sqrt{t}}{\sqrt{m}}\right)^{k-1}\sum_{\substack{c \ge 1 \\ c \equiv 0 \tmod{N}}}\frac{2\pi i^{-k}}{c}J_{k-1}\left(\frac{4\pi\sqrt{mt}}{c}\right)S_{\chi}(m,t,c)\right)e^{2\pi itz}.
      \]
    \end{proposition}
    \begin{proof}
      From \cref{rem:Bruhat_bijection}, we have
      \[
        P_{m,\chi}(z) = e^{2\pi imz}+\sum_{\substack{c \ge 1, d \in \Z \\ c \equiv 0 \tmod{N} \\ (c,d) = 1}}\cchi(d)\frac{e^{2\pi im\left(\frac{a}{c}-\frac{1}{c^{2}z+cd}\right)}}{(cz+d)^{k}},
      \]
      ...
      \begin{equation}\label{equ:Mobius_transform_rewrite}
      ...
      \]
      We will apply the Poisson summation formula to $I_{c,r}(z)$. This is possible since the summands are absolutely integrable by \cref{prop:decay_unbounded_inteval_integral} because they exhibit polynomial decay of order $k$ and $I_{c,r}(z)$ is holomorphic because $P_{m,\chi}(z)$ is. By the identity theorem it suffices to apply the Poisson summation formula for $z = iy$ with $y > 0$. Accordingly, let $f(x)$ be given by
      \[
      ...
      \begin{align*}
        P_{n,\chi}(z) &= e^{2\pi imz}+\psum_{\substack{c \ge 1 \\ c \equiv 0 \tmod{N} \\ r \tmod{c}}}\cchi(r)\sum_{t \ge 1}\left(\frac{2\pi i^{-k}}{c}\left(\frac{\sqrt{t}}{\sqrt{m}}\right)^{k-1}J_{k-1}\left(\frac{4\pi\sqrt{mt}}{c}\right)e^{2\pi im\frac{a}{c}+2\pi it\frac{r}{c}}\right)e^{2\pi imz} \\
      ...
      \[
        P_{n,\chi}(z) = \sum_{t \ge 1}\left(\d_{m,t}+\left(\frac{\sqrt{t}}{\sqrt{m}}\right)^{k-1}\sum_{\substack{c \ge 1 \\ c \equiv 0 \tmod{N}}}\frac{2\pi i^{-k}}{c}J_{k-1}\left(\frac{4\pi\sqrt{mt}}{c}\right)S_{\chi}(n,t,c)\right)e^{2\pi itz}.
      \]
    \end{proof}
    We can now begin the derivation of the Petersson trace formula. Applying \cref{equ:Petersson_inner_product_with_Poincare_series} to the Fourier series in \cref{prop:Fourier_coefficients_of_holomorphic_Poincare_series} gives
    \[
    ...
    \end{align*}
    The last expression is an alternative Fourier series of $P_{n,\chi}(z)$. Applying \cref{equ:Petersson_inner_product_with_Poincare_series} to this Fourier series, we obtain
    \[
  ...
  \section{\todo{The Kuznetsov Trace Formula}}
    ...
    \]
    The Kuznetsov trace formula is an equation relating the Fourier coefficients $a_{j}(n)$ of the basis $\{u_{j}\}_{j \ge 1}$ to a sum of $J$-Bessel functions and Sali\'e sums. Similar to the Petersson trace formula, we will compute the inner product of two Poincar\'e series $P_{n,\chi}(z,s)$ and $P_{m,\chi}(z,s)$ in two different ways. The first will be geometric in nature while the second will be spectral. 
    
    
    \iffalse
    We first compute the Fourier series of $P_{n,\chi}(z,s)$. From \cref{rem:Bruhat_bijection}, we have
    \[
