\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    \subsection*{The Definition \& Euler Product}
      ...
      \[
        \z(s) = \prod_{p}(1-p^{-s})^{-1},
      \]
      in this region as well. The local factor at $p$ is $\z_{p}(s) = (1-p^{-s})^{-1}$ with local root $1$. As the degree of $\z(s)$ is $1$, it will necessarily be primitive once we show it is Selberg class.
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product}
      ...
      \]
      in this region as well. The last equality holds because if $p \mid m$ we have $\chi(p) = 0$. So for $p \mid m$, the local factor at $p$ is $L_{p}(s,\chi) = 1$ with local root $0$. For $p \nmid m$ the local factor at $p$ is $L_{p}(s,\chi) = (1-\chi(p)p^{-s})^{-1}$ with local root $\chi(p)$. As the degree of $L(s,\chi)$ is $1$, it will necessarily be primitive once we show it is Selberg class.
    \subsection*{The Integral Representation: Part I}
  ...
  \section{\todo{Dedekind Zeta Functions}}
    \subsection*{\todo{The Definition \& Euler Product}}
      ...
      \]
      We will show that $\z_{K}(s)$ is a Selberg class $L$-function. First we show that the coefficients are polynomially bounded. By \cref{equ:ideals_of_fixed_norm_bound}, $a_{K}(n) \le \s_{0}(n)^{d} \ll_{\e} n^{\e}$ (recall \cref{prop:sum_of_divisors_growth_rate}). Therefore $\z_{K}(s)$ is locally absolutely uniformly convergent for $\s > 1$.
