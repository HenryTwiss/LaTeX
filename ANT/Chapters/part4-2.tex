\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    ...
    \subsection*{The Jacobi Theta Function}
      ...
      \begin{proof}
        ...
        \]
        Making the change of variables $x \to \frac{x}{\sqrt{y}}$, the last integral becomes
        \[
          \frac{1}{\sqrt{y}}\int_{-\infty}^{\infty}e^{-2\pi\left(x^{2}+\frac{itx}{\sqrt{y}}\right)}.
        \]
        Complete the square in the exponent by noticing
        \[
          x^{2}+\frac{itx}{\sqrt{y}} = \left(x+\frac{it}{2\sqrt{y}}\right)^{2}+\frac{t^{2}}{4y},
        \]
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Dirichlet Theta Function}
      ...
      \begin{proof}
        ...
        \]
        By performing the change of variables $x \to \frac{x}{q\sqrt{y}}-\frac{a}{q}$, the last integral becomes
        \[
        ...
        \[
          x^{2}+\frac{itx}{q\sqrt{y}} = \left(x+\frac{it}{2q\sqrt{y}}\right)^{2}+\frac{t^{2}}{4q^{2}y},
        \]
        so that the previous integral is equal to
        \[
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{\todo{The Hecke Theta Function}}
      ...
      \]
      where the second equality follows since $\<\mathbf{N},\boldsymbol{\a}\> \ge (r_{K}+1)|\Norm(\a)|^{\frac{1}{r_{K}+1}} \ge |\Norm(\a)|^{\frac{2}{d}}$ provided $\a$ is nonzero by the arithmetic-geometric inequality, and $\Norm(\a\mf{f}^{-1}) = |\Norm(\a)|\Norm(\mf{f})$ is a positive integer for all $\a \in \mf{f}^{\ast}$ (as $\a\mf{f}^{-1}$ is an integral ideal by \cref{prop:explicit_inverse_ideal}), which combined show that each term is of smaller order than the single term where $\Norm(\a\mf{f}^{-1}) = 1$ (that is $\a\mf{f}^{-1} = \mc{O}_{K}$) corresponding to $\a = \Norm(\mf{f})$ so that the sum is bounded by a constant times this term. The relationship to $\w_{\mc{F}}(z)$ is given by
      \[
