  ...
  \section{The Explicit Formula for Logarithmic Derivatives}
    ...
    \subsection*{The Explicit Formula for the Logarithmic Derivative of \texorpdfstring{$\z(s)$}{\z(s)}}
      ...
      \[
        \xi(s) = \xi(1-s),
      \]
      which is the functional equation for $\xi(s)$. Moreover, $\xi(s)$ is of order $1$. To see this, by the functional equation for $\xi(s)$ it suffices to prove $\xi(s)$ is of order $1$ for $\Re(s) \ge \frac{1}{2}$. Now $\z(s)$ is of order $1$, being an $L$-function, and $\G\left(\frac{s}{2}\right)$ is of order $1$ for $\Re(s) > 0$. By the Hadamard factorization theorem (see \cref{append:Factorizations_and_Finite_Order}),
    ...
    \subsection*{The Explicit Formula for the Logarithmic Derivative of \texorpdfstring{$L(s,\chi)$}{L(s,\chi)}}
      ...
      \]
      which is the functional equation for $\xi(s,\chi)$. It is also not hard to show that $\xi(s,\chi)$ is of order $1$. Indeed, by the functional equation for $\xi(s,\chi)$ it suffices to prove $\xi(s,\chi)$ is of order $1$ for $\Re(s) \ge \frac{1}{2}$. Since $L(s,\chi)$ is an $L$-function it is of order $1$ and $\G\left(\frac{s+\mf{a}}{2}\right)$ is of order $1$ for $\Re(s) > 0$. By the Hadamard factorization theorem (see \cref{append:Factorizations_and_Finite_Order}),
  ...
  \section{Zero Counting \& Riemann-von Mangoldt Formulas}
    ...
    \subsection*{The Riemann–von Mangoldt formula for $L(s,\chi)$}
      ...
      \begin{proof}
        ...
        \[
          \frac{N(T)}{T} = \frac{1}{\pi}\log\left(\frac{qT}{2\pi}\right)+O\left(\frac{\log(qT)}{T}\right).
        \]
        Since $\log(qT) = o(T)$, the asymptotic follows.
      \end{proof}
