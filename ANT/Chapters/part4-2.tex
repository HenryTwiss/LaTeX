\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \[
        \mathbf{N} = (N_{\s})_{\s}, \quad \mathbf{n} = (n)_{\s}, \quad \text{and} \quad \boldsymbol{\a} = (|\s(\a)|^{N_{\s}})_{\s},
      \]
      for all $n \ge 1$ and $\a \in K$. With this notation,
      \[
        \boldsymbol{\a}^{\mathbf{N}} = |\Norm(\a)|^{2}.
      \]
      ...
      \]
      where we have interchange the two sums and integral by Fubini's theorem. Now perform the change of variables $\mathbf{y} \to \boldsymbol{\a}\mathbf{y}$ to obtain
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{\R_{+}^{r_{K}+1}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\<\mathbf{N},i\boldsymbol{\a}\mathbf{y}\>}(\boldsymbol{\a}\mathbf{y})^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}}.
      \]
      We will now alter the region of integration. Observe that any $\mathbf{y} \in \R_{+}^{r_{K}+1}$ can be written in the form
      \[
        \mathbf{y} = y^{\frac{1}{d}}\mathbf{w},
      \]
      where
      \[
        y = \Norm_{\R^{r_{K}+1}/\R}(\mathbf{y})^{\frac{d}{r_{K}+1}} \quad \text{and} \quad \mathbf{w} = \frac{\mathbf{y}}{\Norm_{\R^{r_{K}+1}/\R}(\mathbf{y})^{\frac{1}{r_{K}+1}}}.
      \]
      ...
      \[
        \frac{d\mathbf{y}}{\mathbf{y}} = \frac{d\mathbf{w}}{\mathbf{w}}\,\frac{dy}{y},
      \]
      where $\frac{d\mathbf{w}}{\mathbf{w}}$ is the restriction of the measure $\frac{d\mathbf{y}}{\mathbf{y}}$ to $S$. Therefore our expression takes the form
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{S}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},iy^{\frac{1}{d}}\boldsymbol{\a}\mathbf{w}\right\>}\boldsymbol{\a}^{\frac{s\mathbf{N}}{2}}y^{\frac{s}{2}}\,\frac{d\mathbf{w}}{\mathbf{w}}\,\frac{dy}{y}.
      \]
      Now consider the isomorphism
      \[
        \log:\R_{+}^{r_{K}+1} \to \R^{r_{K}+1} \qquad (x_{\s})_{\s} \to (\log(x_{\s}))_{\s}.
      \]
      In particular, it restricts to a bijection between the norm-one hypersurface $S$ and the trace-zero hyperplane $H$. By \cref{thm:unit_group_complete_lattice}, $\L$ is a complete lattice in $H$ and therefore has a fundamental domain $\mc{M}$. Then
      \[
      ...
      Then folding yields
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},iy^{\frac{1}{d}}\boldsymbol{\a}\log^{-1}\l(\e)\mathbf{w}\right\>}\boldsymbol{\a}^{\frac{s\mathbf{N}}{2}}y^{\frac{s}{2}}\,\frac{d\mathbf{w}}{\mathbf{w}}\,\frac{dy}{y},
      \]
      because $\log^{-1}\l(\e) = \mathbf{1}$ if $\e \in \mu(K)$ and we have interchanged the sum and integrals by Fubini's theorem. As $\boldsymbol{\e}^{\mathbf{N}} = |\Norm(\e)| = 1$ for all $\e \in \mc{O}_{K}^{\ast}$, we can use \cref{cor:unit_if_and_only_if_absolute_value_norm_1} to write our expression as
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\e\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},iy^{\frac{1}{d}}\boldsymbol{\a}\log^{-1}\l(\e)\mathbf{w}\right\>}(\boldsymbol{\a}\boldsymbol{\e})^{\frac{s\mathbf{N}}{2}}y^{\frac{s}{2}}\,\frac{d\mathbf{w}}{\mathbf{w}}\,\frac{dy}{y}.
      \]
      By the definition of $\l$, we have $\boldsymbol{\a}\log^{-1}\l(\e) = \boldsymbol{\a\e}$ and therefore we may combine the innermost two sums to obtain
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},iy^{\frac{1}{d}}\boldsymbol{\a}\mathbf{w}\right\>}\boldsymbol{\a}^{\frac{s\mathbf{N}}{2}}y^{\frac{s}{2}}\,\frac{d\mathbf{w}}{\mathbf{w}}\,\frac{dy}{y}.
      \]
      Performing the change of variables $y \to \frac{y}{\boldsymbol{\a}^{\mathbf{N}}}$ gives
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},iy^{\frac{1}{d}}\boldsymbol{\a}\mathbf{w}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{w}}{\mathbf{w}}\,\frac{dy}{y}.
      \]
      ...
      \[
        \w_{\mc{F}}(z) = \frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},z\boldsymbol{\a}\mathbf{w}\right\>}\frac{d\mathbf{w}}{\mathbf{w}},
      \]
      ...
      \[
        \w_{\mc{F}}(z) = O\left(\frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}e^{-\pi y\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}\mathbf{w}\right\>}\frac{d\mathbf{w}}{\mathbf{w}}\right) = O(e^{-\pi y}),
      \]
    ...
    \subsection*{\todo{The Hecke Theta Function}}
       The \textbf{Hecke theta function}\index{Hecke theta function} $\vt_{\mf{f}}(z\mathbf{w})$ attached to a fractional ideal $\mf{f}$ of $K$, is defined by
      \[
        \vt_{\mf{f}}(z\mathbf{w}) = \sum_{\a \in \mf{f}}e^{2\pi i\Norm(\mf{f}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},z\boldsymbol{\a}\mathbf{w}\right\>},
      \]
      for $z \in \H$ and $\mathbf{w} \in S$. It is locally absolutely uniformly convergent in this region by the Weierstrass $M$-test. Moreover,
      \[
        \vt_{\mf{f}}(z\mathbf{w})-1 = O\left(\sum_{\a \in \mf{f}^{\ast}}e^{-2\pi y||\mathbf{w}||_{\infty}\Norm(\mf{f}^{-1})^{\frac{2}{d}}\<\mathbf{N},\boldsymbol{\a}\>}\right) = O(e^{-2\pi y||\mathbf{w}||_{\infty}}),
      \]
      where the second equality follows since $\<\mathbf{N},\boldsymbol{\a}\> \ge (r_{K}+1)|2\Norm(\a)|^{\frac{1}{r_{K}+1}} \ge |\Norm(\a)|^{\frac{2}{d}}$ provided $\a$ is nonzero by the arithmetic-geometric inequality, and $\Norm(\a\mf{f}^{-1}) = |\Norm(\a)|\Norm(\mf{f})$ is a positive integer for all $\a \in \mf{f}^{\ast}$ (as $\a\mf{f}^{-1}$ is an integral ideal by \cref{prop:explicit_inverse_ideal}), which combined show that each term is of smaller order than the single term where $\Norm(\a\mf{f}^{-1}) = 1$ (that is $\a\mf{f}^{-1} = \mc{O}_{K}$) corresponding to $\a = \Norm(\mf{f})$ so that the sum is bounded by a constant times this term. The relationship to $\w_{\mc{F}}(z)$ is given by
      \[
        \w_{\mc{F}}(z) = \frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\left(\vt_{\mf{a}}\left(\frac{z\mathbf{w}}{2}\right)-1\right)\frac{d\mathbf{w}}{\mathbf{w}}.
      \]
      ...
      \begin{theorem}\label{thm:functional_equation_Hecke_theta}
        Let $K$ be a number field and let $\mf{f}$ be a fractional ideal. For any $z \in \H$ and $\mathbf{w} \in S$,
        \[
          \vt_{\mf{f}}(\mathbf{z}) = \frac{1}{\sqrt{-2iz}}\vt_{\mf{f}^{\vee}}\left(-\frac{1}{4\mathbf{z}}\right).
        \]
      \end{theorem}
      \begin{proof}
        We will apply the Poisson summation formula to
        \[
          \vt_{\mf{f}}(z\mathbf{w}) = \sum_{\a \in \mf{f}}e^{2\pi i\Norm(\mf{f}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},z\boldsymbol{\a}\mathbf{w}\right\>}.
        \]
        By the identity theorem it is enough to to verify this for $z = iy$ with $y > 0$ (recall that $\mathbf{w} \in S$ is real). Now set $f(\mathbf{x}) = e^{-2\pi y\Norm(\mf{f}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\mathbf{x}\mathbf{w}\right\>}$. We compute its Fourier transform:
        \[
          (\mc{F}f)(\mathbf{t}) = \int_{\R^{r_{K}+1}}f(\mathbf{x})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} = \todo{xxx}
        \]
        \iffalse
        Making the change of variables $x \to \frac{x}{\sqrt{y}}$, the last integral above becomes
        \[
