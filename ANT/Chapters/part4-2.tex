\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{\R_{+}^{r_{K}+1}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\<\mathbf{N},\boldsymbol{\a}^{2}i\mathbf{y}\>}(\boldsymbol{\a}^{2}\mathbf{y})^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}}.
      \]
      ...
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{S}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      ...
      Then folding yields
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}\log^{-1}\l(\e^{2})iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y},
      \]
      ...
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      Performing the change of variables $y \to \frac{y}{\boldsymbol{\a}^{2\mathbf{N}}}$ gives
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      Now interchange the the two integrals with the two sums by Fubini's theorem to obtain
      \[
        \int_{0}^{\infty}\int_{\mc{F}}\frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      We will now rewrite the inner sum. Since $\<\mathbf{N},\boldsymbol{\a}^{2}\> = \<\boldsymbol{j(\a)},\boldsymbol{j(\a)}\>$, our expression becomes
      \[
        \int_{0}^{\infty}\int_{\mc{F}}\frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{j(\a) \in j(\mf{a}^{\ast})}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{j(\a)},\boldsymbol{j(\a)}iy^{\frac{1}{d}}\mathbf{x}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      ...
      \[
        \w_{\mc{F}}(z) = \int_{\mc{F}}\frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{j(\a) \in j(\mf{a}^{\ast})}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{j(\a)},\boldsymbol{j(\a)}z\mathbf{x}\right\>}\,\frac{d\mathbf{x}}{\mathbf{x}}\,,
      \]
      for $z \in \H$. It is locally absolutely uniformly convergent in this region by the Weierstrass $M$-test and that $\mc{F}$ is compact (because $\mc{M}$ is and $\log(\mathbf{x})$ is continuous). \todo{Moreover, as this integral converges for $\s > 1$ we see that $\w_{\mc{F}}(z)$ exhibits exponential decay.} At last, this gives a final integral representation
    ...
    \subsection*{\todo{The Hecke Theta Function}}
      ...
      \[
        \vt_{\mf{f}}(z\mathbf{x}) = \sum_{\a \in \mf{f}}e^{2\pi i\Norm(\mf{f}^{-1})^{\frac{2}{d}}\<\mathbf{N},\boldsymbol{\a}^{2}z\mathbf{x}\>},
      \]
