\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      defined for $z \in \H$. It is locally absolutely uniformly convergent in this region by the ratio and Weierstrass $M$-tests. Moreover, we have
      \[
        \w(z) = O\left(\sum_{n \ge 1}e^{-\pi n^{2}y}\right) = O(e^{-\pi y}),
      \]
      where the second equality holds because each term is of smaller order than the next so that the series is bounded by a constant times its first term. It follows that $\w(z)$ exhibits rapid decay. Now consider the following Mellin transform:
      \[
    ...
    \subsection*{The Jacobi Theta Function}
      ...
      \]
      for $z \in \H$. Its relation to $\w(z)$ is given by the identity
      \begin{equation}\label{equ:omega_theta_relationship_for_zeta}
        \w(z) = \frac{\vt\left(\frac{z}{2}\right)-1}{2},
      \end{equation}
      so that $\vt(z)$ is locally absolutely uniformly convergent in the same region as $\w(z)$ and exhibits rapid decay. The essential fact we will need is a transformation law for the Jacobi theta function:

      \begin{theorem}\label{thm:transformation_law_Jacobi_theta}
        For $z \in \H$,
        \[
      ...
      We will use \cref{thm:transformation_law_Jacobi_theta} to analytically continue $\z(s)$.
    \subsection*{The Integral Representation: Part II}
      ...
      \]
      Now \cref{equ:omega_theta_relationship_for_zeta,thm:transformation_law_Jacobi_theta} together imply
      \begin{align*}
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      where the second equality holds because each term is of smaller order than the next so that the series is bounded by a constant times its first term. Hence $\w_{\chi}(z)$ exhibits rapid decay. Now consider the following Mellin transform:
      \[
    ...
    \subsection*{The Dirichlet Theta Function}
      ...
      \]
      for $z \in \H$. The relationship to $\w_{\chi}(z)$ is
      ...
      \end{equation}
      so that $\vt_{\chi}(z)$ is locally absolutely uniformly convergent in the same region as $\w_{\chi}(z)$ and exhibits rapid decay.
      ...
      The essential fact we will need is a transformation law for the Dirichlet theta function:

      \begin{theorem}\label{thm:transformation_law_Dirichlet_theta}
      ...
      \end{theorem}
      \begin{proof}
        By the identity theorem it suffices to verify this for $z = iy$ with $y > 0$. Since $\chi$ is $q$-periodic and $\vt_{\chi}(z)$ is absolutely convergent, we can write
        \[
          \vt_{\chi}(iy) = \sum_{a \tmod{q}}\chi(a)\sum_{m \in \Z}(mq+a)^{\d_{\chi}}e^{-2\pi(mq+a)^{2}y}.
        \]
        Set $f(x) = (xq+a)^{\d_{\chi}}e^{-2\pi(xq+a)^{2}y}$. Then $f(x)$ is of Schwarz class. We compute its Fourier transform:
        \[
        ...
        \begin{equation}\label{transformation_law_for_twisted_Jacobi_theta_function_1}
        ...
        \]
        and
        \begin{equation}\label{transformation_law_for_twisted_Jacobi_theta_function_2}
        ...
        \end{equation}
        where the last equality follows because the last integral is the Gaussian integral again. Since $\left(\frac{t}{2qi\sqrt{y}}\right)^{\d_{\chi}} = 1$ if $\d_{\chi} = 0$, \cref{transformation_law_for_twisted_Jacobi_theta_function_1,transformation_law_for_twisted_Jacobi_theta_function_2} together imply
        \[
          (\mc{F}f)(t) = \frac{e^{\frac{2\pi iat}{q}-\frac{\pi t^{2}}{2q^{2}y}}}{\sqrt{2}qy^{\frac{1+\d_{\chi}}{2}}}\left(\frac{t}{2qi\sqrt{y}}\right)^{\d_{\chi}}.
        \]
        By the Poisson summation formula, we have
        \begin{align*}
          \vt_{\chi}(iy) &= \sum_{a \tmod{q}}\chi(a)\sum_{t \in \Z}\frac{e^{\frac{2\pi iat}{q}-\frac{\pi t^{2}}{2q^{2}y}}}{\sqrt{2}qy^{\frac{1+\d_{\chi}}{2}}}\left(\frac{t}{2qi\sqrt{y}}\right)^{\d_{\chi}} \\
          &= \frac{1}{i^{\d_{\chi}}q^{1+\d_{\chi}}(2y)^{\frac{1}{2}+\d_{\chi}}}\sum_{a \tmod{q}}\chi(a)\sum_{t \in \Z}t^{\d_{\chi}}e^{\frac{2\pi iat}{q}-\frac{\pi t^{2}}{2q^{2}y}} \\
          &= \frac{1}{i^{\d_{\chi}}q^{1+\d_{\chi}}(2y)^{\frac{1}{2}+\d_{\chi}}}\sum_{t \in \Z}t^{\d_{\chi}}e^{-\frac{\pi t^{2}}{2q^{2}y}}\sum_{a \tmod{q}}\chi(a)e^{\frac{2\pi i at}{q}} \\
          &= \frac{1}{i^{\d_{\chi}}q^{1+\d_{\chi}}(2y)^{\frac{1}{2}+\d_{\chi}}}\sum_{t \in \Z}t^{\d_{\chi}}e^{-\frac{\pi t^{2}}{2q^{2}y}}\tau(t,\chi) && \text{definition of $\tau(t,\chi)$} \\
          &= \frac{\tau(\chi)}{i^{\d_{\chi}}q^{1+\d_{\chi}}(2y)^{\frac{1}{2}+\d_{\chi}}}\sum_{t \in \Z}\cchi(t)t^{\d_{\chi}}e^{-\frac{\pi t^{2}}{2q^{2}y}} && \text{\cref{cor:gauss_sum_primitive_formula}} \\
          &= \frac{\e_{\chi}}{i^{\d_{\chi}}(2qy)^{\frac{1}{2}+\d_{\chi}}}\sum_{t \in \Z}\cchi(t)t^{\d_{\chi}}e^{-\frac{\pi t^{2}}{2q^{2}y}} && \text{$\e_{\chi} = \frac{\tau(\chi)}{\sqrt{q}}$} \\
          &=  \frac{\e_{\chi}}{i^{\d_{\chi}}(2qy)^{\frac{1}{2}+\d_{\chi}}}\vt_{\cchi}\left(-\frac{1}{4q^{2}iy}\right),
        \end{align*}
        and the identity theorem finishes the proof.
      \end{proof}

      Notice that the functional equation relates $\vt_{\chi}(z)$ to $\vt_{\cchi}(z)$. Regardless, we will use \cref{thm:transformation_law_Dirichlet_theta} to analytically continue $L(s,\chi)$.
    \subsection*{The Integral Representation: Part II}
      ...
      \]
      Now \cref{equ:twisted_omega_theta_relationship_for_Dirichlet_L-functions,thm:transformation_law_Dirichlet_theta} together imply
      \begin{align*}
    ...
    \subsection*{Beyond Primitivity}
      ...
      \end{proof}
  \section{\todo{Dedekind Zeta Functions}}
    \subsection*{The Definition \& Euler Product}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \[
        \d = (\d_{\s})_{\s}, \quad \mathbf{N} = (N_{\s})_{\s}, \quad \text{and} \quad \mathbf{2}_{\d} = (2^{\d_{\s}})_{\s}.
      \]
      Consider the function
      \[
        \w_{K}(\mathbf{z}) = \sum_{n \ge 1}a_{K}(n)e^{\pi in^{\frac{2}{d}}\<\mathbf{z},\mathbf{2}_{\d}\>},
      \]
      defined for $\mathbf{z} \in \H^{r_{K}+1}$. It is locally absolutely uniformly convergent in this region by the ratio and Weierstrass $M$-tests. Taking $\e < 1$, the bound $a_{K}(n) \ll_{\e} n^{\e}$ gives
      \[
        \w_{K}(\mathbf{z}) = O\left(\sum_{n \ge 1}n^{r_{2}+1}e^{-\pi n^{\frac{2}{d}}||\mathbf{y}||_{\infty}}\right) = O(e^{-\pi||\mathbf{y}||_{\infty}}),
      \]
      where the second equality holds because each term is of smaller order than the next so that the series is bounded by a constant times its first term. Hence $\w_{K}(\mathbf{z})$ exhibits rapid decay. Now consider the following Mellin transform:
      \[
      ...
      \begin{align*}
        \int_{\R_{+}^{r_{K}+1}}\w_{K}(i\mathbf{y})\mathbf{y}^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} &= \int_{\R_{+}^{r_{K}+1}}\sum_{n \ge 1}a_{K}(n)e^{-\pi n^{\frac{2}{d}}\<\mathbf{y},\mathbf{2}_{\d}\>}\mathbf{y}^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} \\
        &= \sum_{n \ge 1}\int_{\R_{+}^{r_{K}+1}}a_{K}(n)e^{-\pi n^{\frac{2}{d}}\<\mathbf{y},\mathbf{2}_{\d}\>}\mathbf{y}^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} && \text{FT} \\
        &= \sum_{n \ge 1}\frac{a_{K}(n)}{(2\pi)^{r_{2}s}\pi^{\frac{r_{1}s}{2}}n^{s}}\int_{\R_{+}^{r_{K}+1}}e^{-\pi\<\mathbf{y},\mathbf{1}\>}\mathbf{y}^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} && \text{$\mathbf{y} \to \frac{\mathbf{y}}{\mathbf{2}_{\d}\pi\mathbf{n}^{\frac{2}{d}}}$} \\
      ...
      \end{equation}
      We cannot proceed until we obtain a functional equation for $\w_{K}(i\mathbf{y})$. So we will make a detour and come back to the integral representation after.
    \subsection*{\todo{The Hecke Theta Function}}
      Recall that the ideal class group $\Cl(K)$ is finite by \cref{equ:finitness_of_class_number}. Moreover, if $\mf{a} \in \Cl(K)$ is an integral ideal representative of a class then $\mc{O}_{K}^{\ast}$ acts on $\mf{a}^{\ast}$ by multiplication. The \textbf{Hecke theta function}\index{Hecke theta function} $\vt_{K}(z)$ attached to the number field $K$, is defined by
      \[
        \vt_{K}(\mathbf{z}) = \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}e^{2\pi i\Norm(a\mf{a}^{-1})^{\frac{2}{d}}\<\mathbf{z},\mathbf{2}_{\d}\>},
      \]
      for $\mathbf{z} \in \H^{r_{K}+1}$. We first need to verify that $\vt_{K}(\mathbf{z})$ is well-defined. By Minkowski's theorem, every ideal class contains an integral ideal so that the outer sum is defined. We now show that the sums are independent of the representatives $\mf{a}$ and $a$. Clearly the summand is independent of the representative $a$ since the set of representatives $a\mc{O}_{K}^{\ast}$ all have $|\Norm(a)|$ as the absolute value of their norm by \cref{cor:unit_if_and_only_if_absolute_value_norm_1}. To see that the inner sum is independent of the representative $\mf{a}$ requires a lemma:
      ...
      \end{lemma}
      \begin{proof}
        We first show that this map is well-defined. If $a \in \mf{a}^{\ast}$, then $a\mf{a}^{-1} = a\mc{O}_{K}\mf{a}^{-1} \subseteq \mc{O}_{K}$ by \cref{prop:explicit_inverse_ideal} so that $a\mf{a}^{-1}$ is an integral ideal belonging to $\mf{K}$ (because it differs from $\mf{a}^{-1}$ by a principal ideal). Therefore the map is well-defined. To show injectivity, suppose $a\mf{a}^{-1} = b\mf{a}^{-1}$ for some $a,b \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}$. Multiplying by $\mf{a}$, we obtain $a\mc{O}_{K} = b\mc{O}_{K}$ and hence $ab^{-1} \in \mc{O}_{K}^{\ast}$ from which injectivity follows. For surjectivity, the definition of $\mf{K}$ implies that any integral ideal $\mf{b} \in \mf{K}$ satisfies $\mf{b} = a\mf{a}^{-1}$ for some nonzero $a \in \mc{O}_{K}$ but then $a \in \mf{a}\mf{b} \subseteq \mf{a}$.
      \end{proof}
      
      By \cref{lem:Hecke_theta_function_lemma}, the inner sum is independent of the representative $\mf{a}$ because if $\mf{b}$ is another integral ideal belonging to the same ideal class as $\mf{a}$, then they differ by a principal ideal and hence $\mf{a}^{-1}$ and $\mf{b}^{-1}$ belong to the same ideal class. All of this work shows that $\vt_{K}(\mathbf{z})$ is well-defined. Moreover, we can use \cref{lem:Hecke_theta_function_lemma} to transition the double sum to a sum over integral ideals or positive integers. Indeed, as we have previously observed there is a complete set of representatives $\mf{a}_{1},\ldots,\mf{a}_{h_{K}}$ of $\Cl(K)$ consisting only of integral ideals (by Minkowski's theorem). As the ideal class group is a group, we see that $\mf{a}_{1}^{-1},\ldots,\mf{a}_{h_{K}}^{-1}$ is also a complete set of representatives. By \cref{lem:Hecke_theta_function_lemma}, this shows that every integral ideal is uniquely of the form $a\mf{a}_{i}^{-1}$ for some $1 \le i \le h_{K}$ and $a \in \mf{a}_{i}^{\ast}/\mc{O}_{K}^{\ast}$ and thus has norm $\Norm(a\mf{a}_{i}^{-1})$. It follows that the Hecke theta function takes the alternative forms
      \[
        \vt_{K}(\mathbf{z}) = \sum_{\text{$\mf{a}$ integral}}e^{2\pi i\Norm(a\mf{a}^{-1})^{\frac{2}{d}}\<\mathbf{z},\mathbf{2}_{\d}\>},
      \]
      and
      \[
        \vt_{K}(\mathbf{z}) = \sum_{n \ge 1}a_{K}(n)e^{2\pi in^{\frac{2}{d}}\<\mathbf{z},\mathbf{2}_{\d}\>},
      \]
      and hence the relationship to $\w_{K}(\mathbf{z})$ is given by
      \[
        \w_{K}(\mathbf{z}) = \vt_{K}\left(\frac{\mathbf{z}}{2}\right),
      \]
      so that $\vt_{K}(\mathbf{z})$ is locally absolutely uniformly convergent in the same region as $\w_{K}(\mathbf{z})$ and exhibits rapid decay. Now for any $\mathbf{y} \in \R_{+}^{r_{K}+1}$, write
      \[
        \mathbf{y} = t^{\frac{1}{r_{K}+1}}\mathbf{x}, \quad \mathbf{x} = \frac{\mathbf{y}}{\Norm_{\R^{r_{K}+1}/\R}(\mathbf{y})^{\frac{1}{r_{K}+1}}}, \quad \text{and} \quad t = \Norm_{\R^{r_{K}+1}/\R}(\mathbf{y}).
      \]
      Then we may decompose $\R_{+}^{r_{K}+1}$ as
      \[
        \R_{+}^{r_{K}+1} = S \x \R_{+},
      \]
      where $S$ is the norm-one surface of $\R^{r_{K}+1}$. As $S$ is a multiplicative subgroup of $\R_{+}^{r_{K}+1}$, the Haar measure $\frac{d\mathbf{y}}{\mathbf{y}}$ on $\R_{+}^{r_{K}+1}$ factors as
      \[
        \frac{d\mathbf{y}}{\mathbf{y}} = d\mathbf{x}\,\frac{dt}{t},
      \]
      where $d\mathbf{x}$ is the induced Haar measure $S$. We will decompose $S$ further. Recall that $\L$ is a complete lattice in $H \subset \R^{r_{K}+1}$. \todo{xxx}
    \subsection*{\todo{The Integral Representation: Part II}}
    \subsection*{\todo{The Functional Equation}}
      \todo{xxx} We identify the gamma factor of $\z_{K}(s)$ to be
      \[
      ...
      \]
      This shows $(s-1)\z_{K}(s)$ is of order $1$ and hence $\z_{K}(s)$ is too after removing the polar divisor. It now remains to compute the residue of $\z_{K}(s)$ at $s = 1$. This result is known as the infamous \textbf{analytic class number formula}\index{analytic class number formula}:
      ...
      We collect our work in the following theorem:
    ...
    \subsection*{Imprimitivity}
      ...
      \end{theorem}
      \begin{proof}
        By the identity theorem it suffices to prove this on a set containing a limit point. Therefore we may assume $\s > 1$. By \cref{prop:factorization_of_primes_quadratic}, the Euler product of $\z_{K}$ is given by
        \begin{align*}
  ...
  \section{The Rankin-Selberg Method}
    ...
    \subsection*{Explicit Fourier-Whittaker Series of Eisenstein Series}
      ...
      \begin{proof}
        ...
        \]
        We apply the Poisson summation formula to $I_{c,r}(z,s)$. This is allowed since the summands are absolutely integrable as they exhibit polynomial decay of order $\s > 1$ and $I_{c,r}(z,s)$ is holomorphic (because $E(z,s)$ is). By the identity theorem it suffices to apply the Poisson summation formula for $z = iy$ with $y > 0$. So let $f(x)$ be given by
        \[
      ...
      \end{equation}
      This latter series is absolutely uniformly convergent on $K$ by the ratio and Weierstrass $M$-tests. Therefore $E^{\ast}(z,s)$ is absolutely convergent on $K$ for any $z \in \H$ and the meromorphic continuation to $\C$ follows. It remains to investigate the poles and residues. We will accomplish this from direct inspection of the Fourier-Whittaker coefficients:
