\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      for all $n \ge 1$ and $\a \in K$. Consider the function
      \[
      ...
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{\R_{+}^{r_{K}+1}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\<i\mathbf{y},\mathbf{N}\>}\mathbf{y}^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}},
      \]
      ...
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{\R_{+}^{r_{K}+1}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\<i\boldsymbol{\a}\mathbf{y},\mathbf{N}\>}(\boldsymbol{\a}\mathbf{y})^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}}.
      \]
      ...
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{S}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<iy^{\frac{1}{d}}\boldsymbol{\a}\mathbf{x},\mathbf{N}\right\>}\boldsymbol{\a}^{\frac{s\mathbf{N}}{2}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      ...
      Then folding yields
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\e \in \mc{O}_{K}^{\ast}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<iy^{\frac{1}{d}}\log^{-1}\l(\e)\boldsymbol{\a}\mathbf{x},\mathbf{N}\right\>}\boldsymbol{\a}^{\frac{s\mathbf{N}}{2}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y},
      \]
      because $\log^{-1}\l(\e) = \mathbf{1}$ if $\e \in \mu(K)$ and we have interchanged sums and integrals by Fubini's theorem. As $\boldsymbol{\e}^{\mathbf{N}} = |\Norm(\e)| = 1$ for all $\e \in \mc{O}_{K}^{\ast}$, we can use \cref{cor:unit_if_and_only_if_absolute_value_norm_1} to write our expression as
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\e \in \mc{O}_{K}^{\ast}}\sum_{\a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\e\a\mf{a}^{-1})^{\frac{2}{d}}\left\<iy^{\frac{1}{d}}\log^{-1}\l(\e)\boldsymbol{\a}\mathbf{x},\mathbf{N}\right\>}(\boldsymbol{\e}\boldsymbol{\a})^{\frac{s\mathbf{N}}{2}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      By the definition of $\l$, we have $\log^{-1}\l(\e)\boldsymbol{\a} = \boldsymbol{\e\a}$ and therefore we may combine the innermost two sums to obtain
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<iy^{\frac{1}{d}}\boldsymbol{\a}\mathbf{x},\mathbf{N}\right\>}\boldsymbol{\a}^{\frac{s\mathbf{N}}{2}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      Performing the change of variables $y \to \frac{y}{\boldsymbol{\a}^{\mathbf{N}}}$ gives
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<iy^{\frac{1}{d}}\boldsymbol{\a}\mathbf{x},\mathbf{N}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      ...
      \[
        \w_{\mc{F}}(z) = \frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<z\boldsymbol{\a}\mathbf{x},\mathbf{N}\right\>}\frac{d\mathbf{x}}{\mathbf{x}},
      \]
      ...
      \[
        \w_{\mc{F}}(z) = O\left(\frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}^{\ast}}e^{-\pi y\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{\a}\mathbf{x},\mathbf{N}\right\>}\frac{d\mathbf{x}}{\mathbf{x}}\right) = O(e^{-\pi y}),
      \]
    ...
    \subsection*{\todo{The Hecke Theta Function}}
       The \textbf{Hecke theta function}\index{Hecke theta function} $\vt_{\mf{a}}(\mathbf{z})$ attached to a fractional ideal $\mf{f}$ of $K$, is defined by
      \[
        \vt_{\mf{f}}(\mathbf{z}) = \sum_{\a \in \mf{f}}e^{2\pi i\Norm(\a\mf{f}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{\a}\mathbf{z},\mathbf{N}\right\>},
      \]
      for $\mathbf{z} \in \H^{r_{K}+1}$. It is locally absolutely uniformly convergent in this region by the Weierstrass $M$-test. Moreover,
      \[
        \vt_{\mf{f}}(\mathbf{z})-1 = O\left(\sum_{\a \in \mf{f}^{\ast}}e^{-2\pi||\mathbf{y}||_{\infty}\Norm(\a\mf{f}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{\a},\mathbf{N}\right\>}\right) = O(e^{-2\pi ||\mathbf{y}||_{\infty}}),
      \]
      where the second equality follows since $\Norm(\a\mf{f}^{-1})$ is a positive integer for all $\a \in \mf{f}^{\ast}$ (as $\a\mf{f}^{-1}$ is an integral ideal by \cref{prop:explicit_inverse_ideal}) and so each term is of smaller order than the single term where $\Norm(\a\mf{f}^{-1}) = 1$ corresponding to $\a = \Norm(\mf{f}) \in \mf{f}$ (because every fractional ideal contains its norm as we have noted) so that the double sum is bounded by a constant times this term. The relationship to $\w_{\mc{F}}(z)$ is given by
      \[
      ...
      \begin{theorem}\label{thm:functional_equation_Hecke_theta}
        Let $K$ be a number field. For any fractional ideal $\mf{f}$ and $z \in \H$,
        \[
          \vt_{\mf{f}}(\mathbf{z}) = \todo{xxx}.
        \]
      \end{theorem}
      \begin{proof}
        \todo{xxx}.
