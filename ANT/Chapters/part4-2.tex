\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    ...
    \subsection*{The Jacobi Theta Function}
      ...
      \end{equation}
      and so $\vt(z)$ is locally absolutely uniformly convergent in this region and exhibits rapid decay. The essential fact we will need is a transformation law for the Jacobi theta function:
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Dirichlet Theta Function}
      ...
      \end{equation}
      and so $\vt_{\chi}(z)$ is locally absolutely uniformly convergent in this region and exhibits rapid decay.
  ...
  \section{\todo{Dedekind Zeta Functions}}
    \subsection*{The Definition \& Euler Product}
      ...
      \[
        \z_{K}(s) = \sum_{\mf{a}}\frac{1}{\Norm(\mf{a})^{s}},
      \]
      where the sum is over all integral ideals of $K$. Since the norm is completely multiplicative by \cref{prop:ideal_norm_is_multiplicative}, the unique factorization of integral ideals implies that we can express $\z_{K}(s)$ as the infinite product
      \[
        \z_{K}(s) = \prod_{\mf{p}}(1-\Norm(\mf{p})p^{-s})^{-1},
      \]
      for $\s > 1$, and where the product is over all prime integral ideals $\mf{p}$ of $K$. Since each such ideal is above a unique prime $p$ and this product is absolutely convergent for $\s > 1$, it can be expressed as
      \[
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \[
        \d = (\d_{\s})_{\s}, \quad \mathbf{N} = (N_{\s})_{\s}, \quad \mathbf{2}_{\d} = (2^{\d_{\s}})_{\s}, \quad \text{and} \quad \mathbf{n} = (n)_{\s},
      \]
      for $n \ge 1$. Consider the function
      \[
        \w_{K}(\mathbf{z}) = \sum_{n \ge 1}a_{K}(n)\mathbf{n}^{\d}e^{\pi in^{\frac{2}{d}}\<\mathbf{z},\mathbf{2}_{\d}\>},
      \]
      ...
      \[
        \int_{\R_{+}^{r_{1}+r_{2}}}\w_{K}(i\mathbf{y})\mathbf{y}^{\frac{s}{2}\mathbf{N}}\,\frac{d\mathbf{y}}{\mathbf{y}}.
      \]
      ...
      \begin{align*}
        \int_{\R_{+}^{r_{1}+r_{2}}}\w_{K}(i\mathbf{y})\mathbf{y}^{\frac{s}{2}\mathbf{N}}\,\frac{d\mathbf{y}}{\mathbf{y}} &= \int_{\R_{+}^{r_{1}+r_{2}}}\sum_{n \ge 1}a_{K}(n)\mathbf{n}^{\d}e^{-\pi n^{\frac{2}{d}}\<\mathbf{y},\mathbf{2}_{\d}\>}\mathbf{y}^{\frac{s}{2}\mathbf{N}}\,\frac{d\mathbf{y}}{\mathbf{y}} \\
        &= \sum_{n \ge 1}\int_{\R_{+}^{r_{1}+r_{2}}}a_{K}(n)\mathbf{n}^{\d}e^{-\pi n^{\frac{2}{d}}\<\mathbf{y},\mathbf{2}_{\d}\>}\mathbf{y}^{\frac{s}{2}\mathbf{N}}\,\frac{d\mathbf{y}}{\mathbf{y}} && \text{FT} \\
        &= \sum_{n \ge 1}\frac{a_{K}(n)}{(2\pi)^{r_{2}s}\pi^{\frac{r_{1}s}{2}}n^{s}}\int_{\R_{+}^{r_{1}+r_{2}}}e^{-\pi\<\mathbf{y},\mathbf{1}\>}\mathbf{y}^{\frac{s}{2}\mathbf{N}}\,\frac{d\mathbf{y}}{\mathbf{y}} && \text{$\mathbf{y} \to \frac{\mathbf{y}}{\mathbf{2}_{\d}\pi\mathbf{n}^{\frac{2}{d}}}$} \\
      ...
      \begin{equation}\label{equ:integral_representation_Dedekind_1}
        \z_{K}(s) = \frac{(2\pi)^{r_{2}s}\pi^{\frac{r_{1}s}{2}}}{\G\left(\frac{s}{2}\right)^{r_{1}}\G\left(s\right)^{r_{2}}}\int_{\R_{+}^{r_{1}+r_{2}}}\w_{K}(i\mathbf{y})\mathbf{y}^{\frac{s}{2}\mathbf{N}}\,\frac{d\mathbf{y}}{\mathbf{y}}.
      \end{equation}
    ...
    \subsection*{\todo{The Hecke Theta Function}}
      Recall that the ideal class group $\Cl(K)$ is finite by \cref{equ:finitness_of_class_number}. Moreover, if $\mf{a} \in \Cl(K)$ is an integral ideal representative of a class then $\mc{O}_{K}^{\ast}$ acts on $\mf{a}^{\ast}$ by multiplication. Therefore we have an orbit space $\mf{a}^{\ast}/\mc{O}_{K}^{\ast}$. It will be convenient to let
      \[
        \mathbf{a} = (2^{\d_{\s}}\s(a))_{\s},
      \]
      for $a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}$. The \textbf{Hecke theta function}\index{Hecke theta function} $\t_{K}(z)$ attached to the number field $K$, is defined by
      \[
        \t_{K}(\mathbf{z}) = \sum_{\mf{a} \in \Cl(K)}\sum_{a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}e^{2\pi i\<\mathbf{z},\mathbf{a}\>},
      \]
      for $\mathbf{z} \in \H^{r_{1}+r_{2}}$. We first need to show that $\t_{K}(\mathbf{z})$ is well-defined and thus independent of the representatives $mf{a}$ and $a$. To see this, we require a lemma:
      ...
      We can use \cref{lem:Hecke_theta_function_lemma}, to transition between summing over integral ideals and algebraic integers. Indeed, by Minkowski's theorem there is a complete set of representatives $\mf{a}_{1},\ldots,\mf{a}_{h_{K}}$ of $\Cl(K)$ consisting only of integral ideals. As the ideal class group is a group, $\mf{a}_{1}^{-1},\ldots,\mf{a}_{h_{K}}^{-1}$ is also a full set of representatives. By \cref{lem:Hecke_theta_function_lemma}, this shows that every integral ideal is uniquely of the form $a\mf{a}_{i}^{-1}$ for some $1 \le i \le h_{K}$ and $a \in \mf{a}_{i}^{\ast}/\mc{O}_{K}^{\ast}$. Moreover, the corresponding norms are $\frac{|\Norm(\a)|}{\Norm(\a_{i})}$. \todo{xxx}
