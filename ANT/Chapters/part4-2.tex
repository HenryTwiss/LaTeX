\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      for all $n \ge 1$ and $\a \in \mc{O}_{K}$. Consider the function
      \[
    ...
    \subsection*{\todo{The Hecke Theta Function}}
       The \textbf{Hecke theta function}\index{Hecke theta function} $\vt_{K}(\mathbf{z})$ attached to the number field $K$, is defined by
      \[
        \vt_{K}(\mathbf{z}) = \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}}e^{2\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{\a}\mathbf{z},\mathbf{N}\right\>},
      \]
      for $\mathbf{z} \in \H^{r_{K}+1}$. It is locally absolutely uniformly convergent in this region by the Weierstrass $M$-test. Moreover,
      \[
        \vt_{K}(\mathbf{z}) = O\left(\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}}e^{-2\pi||\mathbf{y}||_{\infty}\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{\a},\mathbf{N}\right\>}\right) = O(e^{-2\pi ||\mathbf{y}||_{\infty}}),
      \]
      where the second equality follows by noting that each term is of smaller order than the single term where $\Norm(\a\mf{a}^{-1}) = 1$ corresponding to the integral ideal $\mc{O}_{K}$ (by \cref{lem:Hecke_theta_function_lemma}) so that the double sum is bounded by a constant times this term. The relationship to $\w_{\mc{F}}(z)$ is given by
      \[
        \w_{\mc{F}}(z) = \frac{1}{w_{K}}\int_{\mc{F}}\left(\vt_{K}\left(\frac{z\mathbf{x}}{2}\right)-1\right)\frac{d\mathbf{x}}{\mathbf{x}}.
      \]
