\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \[
        \w_{K}(\mathbf{z}) = \sum_{\text{$\mf{a}$ integral}}e^{\pi i\Norm(\mf{a})^{\frac{2}{d}}\<\mathbf{z},\mathbf{2}_{\d}\>},
      \]
      ...
      \end{proof}
       ...
       \begin{equation}\label{equ:omega_K_function_special_form}
        \w_{K}(\mathbf{z}) = \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}e^{\pi i\Norm(a\mf{a}^{-1})^{\frac{2}{d}}\<\mathbf{z},\mathbf{2}_{\d}\>}.
      \end{equation}
    ...
    \subsection*{\todo{The Hecke Theta Function}}
       The \textbf{Hecke theta function}\index{Hecke theta function} $\vt_{K}(z)$ attached to the number field $K$, is defined by
      \[
        \vt_{K}(\mathbf{z}) = \todo{xxx},
      \]
      for $\mathbf{z} \in \H^{r_{K}+1}$. \todo{Absolute convergence and exponential decay properties}. Establishing the relationship to $\w_{K}(\mathbf{z})$ will require some work. Consider the isomorphism
      \[
      ...
      \]
      and setting $\mc{F} = \log^{-1}(\mc{M})$ gives
      \[
      ...
      \begin{align*}
        \w_{S}(t) &= \int_{S}\w_{K}(it^{\frac{1}{d}}\mathbf{x})\,\frac{d\mathbf{x}}{\mathbf{x}} \\
        &= \sum_{\e \in \mc{O}_{K}^{\ast}/\mu(K)}\int_{\log^{-1}\l(\e)\mc{F}}\w_{K}(it^{\frac{1}{d}}\mathbf{x})\,\frac{d\mathbf{x}}{\mathbf{x}} \\
        &= \frac{1}{w_{K}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{\mc{F}}\w_{K}(it^{\frac{1}{d}}\log^{-1}\l(\e)\mathbf{x})\,\frac{d\mathbf{x}}{\mathbf{x}} \\
        &=  \frac{1}{w_{K}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{a \in \mf{a}^{\ast}/\mc{O}_{K}^{\ast}}e^{\pi i\Norm(a\mf{a}^{-1})^{\frac{2}{d}}\left\<it^{\frac{1}{d}}\log^{-1}\l(\e)\mathbf{x},\mathbf{2}_{\d}\right\>}\,\frac{d\mathbf{x}}{\mathbf{x}} && \text{\cref{equ:omega_K_function_special_form}}
      \end{align*}
