\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      We cannot proceed until we obtain a functional equation for $\w_{S}(t)$. So we will make a detour and come back to the integral representation after.
    \subsection*{\todo{The Hecke Theta Function}}
       The \textbf{Hecke theta function}\index{Hecke theta function} $\vt_{K}(t)$ attached to the number field $K$, is defined by
      \[
      ...
      \]
      for $\mathbf{z} \in S$ where $S$ is the norm-one surface of $\R^{r_{K}+1}$. \todo{Absolute convergence and exponential decay properties}. Establishing the relationship to $\w_{S}(\mathbf{z})$ will require some work. Consider the isomorphism
      \[
      ...
      \]
      because $\log^{-1}\l(\e) = \mathbf{1}$ if $\e \in \mu(K)$. \todo{xxx}. In conclusion, the relationship to $\w_{K}(\mathbf{z})$ is given by
      \[
