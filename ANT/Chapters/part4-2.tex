  ...
  \section{Dedekind Zeta Functions}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \begin{align*}
        ...
        &= \sum_{n \ge 1}\frac{a_{K}(n)}{\pi^{\frac{r_{1}s}{2}}(2\pi)^{r_{2}s}n^{s}}\int_{\R_{+}^{r_{K}+1}}e^{-\<\mathbf{1},\mathbf{y}\>}\mathbf{y}^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} && \text{$\mathbf{y} \mapsto \frac{\mathbf{y}}{\mathbf{N}\pi n^{\frac{2}{d}}}$} \\
    ...
    \subsection*{The Integral Representation: Part II}
      ...
      \end{lemma}
      \begin{proof}
        Let $\e_{\tau}$ run over a system of fundamental units for $K$ as $\tau$ runs over a complete set of representatives for the real and pairs of complex $\Q$-embeddings from $K$ into $\conj{\Q}$. Then $\l(\e_{\tau})$ runs over a basis for $\L$ and hence $H$ (by \cref{thm:unit_group_complete_lattice}). As
        \[
          e:\R^{r_{K}+1} \to \R_{+}^{r_{K}+1},
        \]
        is an isomorphism that restricts to an isomorphism from $H$ to $S$, we obtain an isomorphism
        \[
          e:H \to S \qquad \sum_{\tau}x_{\tau}\l(\e_{\tau}) \mapsto \prod_{\tau}e^{x_{\tau}\l(e_\tau)}.
        \]
        \todo{xxx}
      \end{proof}
