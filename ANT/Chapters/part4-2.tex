  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \[
        \d_{\s} = N_{\s}-1.
      \]
      It will be useful to make the following conventions for any $\mathbf{z} \in \C^{r_{1}+r_{2}}$:
      \[
        \mathbf{z}^{\d} = (z_{\s}^{\d_{\s}})_{\s}, \quad ||\mathbf{z}||^{\d} = \prod_{\s}z_{\s}^{\d_{\s}}, \quad \text{and} \quad ||\mathbf{z}||^{N} = \prod_{\s}z_{\s}^{N_{\s}}.
      \]
      Consider the function
      \[
        \w_{K}(\mathbf{z}) = \sum_{n \ge 1}a_{K}(n)||\mathbf{n}||^{\d}e^{\pi in^{\frac{2}{d}}\<\mathbf{z},\mathbf{2}^{\d}\>},
      \]
      defined for $\mathbf{z} \in \H^{r_{1}+r_{2}}$ and where $\mathbf{n} = (n)_{\s}$ for $n \ge 1$. It is locally absolutely uniformly convergent in this region by the ratio and Weierstrass $M$-tests. Taking $\e < 1$, the bound $a_{K}(n) \ll_{\e} n^{\e}$ gives
      \[
        \w_{K}(\mathbf{z}) = O\left(\sum_{n \ge 1}n^{r_{2}+1}e^{-\pi n^{\frac{2}{d}}||\mathbf{y}||_{\infty}}\right) = O(e^{-\pi||\mathbf{y}||_{\infty}}),
      \]
      where the second equality holds because each term is of smaller order than the next so that the series is bounded by a constant times its first term. Hence $\w_{K}(\mathbf{z})$ exhibits rapid decay. Now consider the following $(r_{1}+r_{2})$-fold Mellin transform:
      \[
        \int_{0}^{\infty} \cdots \int_{0}^{\infty}\w_{K}(i\mathbf{y})||\mathbf{y}||^{\frac{Ns}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}}.
      \]
      ...
      \begin{align*}
        \int_{0}^{\infty} \cdots \int_{0}^{\infty}\w_{K}(i\mathbf{y})||\mathbf{y}||^{\frac{Ns}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} &= \int_{0}^{\infty} \cdots \int_{0}^{\infty}\sum_{n \ge 1}a_{K}(n)||\mathbf{n}||^{\d}e^{-\pi n^{\frac{2}{d}}\<\mathbf{y},\mathbf{2}^{\d}\>}||\mathbf{y}||^{\frac{Ns}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} \\
        &= \sum_{n \ge 1}\int_{0}^{\infty} \cdots \int_{0}^{\infty}a_{K}(n)||\mathbf{n}||^{\d}e^{-\pi n^{\frac{2}{d}}\<\mathbf{y},\mathbf{2}^{\d}\>}||\mathbf{y}||^{\frac{Ns}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} && \text{FT} \\
        &= \sum_{n \ge 1}\frac{a_{K}(n)}{(2\pi)^{r_{2}s}\pi^{\frac{r_{1}s}{2}}n^{s}}\int_{0}^{\infty} \cdots \int_{0}^{\infty}e^{-\pi\<\mathbf{y},\mathbf{1}\>}||\mathbf{y}||^{\frac{Ns}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} && \text{$\mathbf{y} \to \frac{\mathbf{y}}{\mathbf{2}^{\d}\pi\mathbf{n}^{\frac{2}{d}}}$} \\
      ...
      \begin{equation}\label{equ:integral_representation_Dedekind_1}
        \z_{K}(s) = \frac{(2\pi)^{r_{2}s}\pi^{\frac{r_{1}s}{2}}}{\G\left(\frac{s}{2}\right)^{r_{1}}\G\left(s\right)^{r_{2}}}\int_{0}^{\infty} \cdots \int_{0}^{\infty}\w_{K}(i\mathbf{y})||\mathbf{y}||^{\frac{Ns}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}}.
      \end{equation}
