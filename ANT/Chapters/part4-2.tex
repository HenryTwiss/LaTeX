\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dedekind Zeta Functions}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      where $d\mathbf{x} = \frac{d\mathbf{x}}{\mathbf{x}}$ (because $\mathbf{x} \in S$) is the restriction of the measure $\frac{d\mathbf{y}}{\mathbf{y}}$ to $S$. Therefore our expression takes the form
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in (\mf{a}-\{0\})/\mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{S}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}\,dy}{y}.
      \]
      Now consider the inverse isomorphisms
      \[
        \log:\R_{+}^{r_{K}+1} \to \R^{r_{K}+1} \qquad \text{and} \quad e:\R^{r_{K}+1} \to \R_{+}^{r_{K}+1}.
      \]
      In particular, they restricts to inverse bijections between the norm-one hypersurface $S$ and the trace-zero hyperplane $H$ ($\log(S) = H$ and $e^{H} = S$). By \cref{thm:unit_group_complete_lattice}, $2\L$ is a complete lattice in $H$ and therefore has fundamental domain $2\mc{M}$ where $\mc{M}$ is a fundamental domain for $\L$. Then
      \[
        H = \bigcup_{\l \in \L}(2\mc{M}+2\l).
      \]
      Setting $\mc{F} = e^{2\mc{M}}$, we see that $\mc{F}$ is a fundamental domain for the properly discontinuous action of $\L$ on $S$ by multiplication by $e^{2\l}$ for all $\l \in \L$. In particular,
      \[
        S = \bigcup_{\l \in \L}e^{2\l}\mc{F}.
      \]
      By \cref{prop:exact_sequence_for_Dirichlets_unit_theorem}, we may express this decomposition as
      \[
        S = \bigcup_{\e \in \mc{O}_{K}^{\ast}/\mu(K)}e^{\l(\e^{2})}\mc{F}.
      \]
      Then folding yields
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in (\mf{a}-\{0\})/\mc{O}_{K}^{\ast}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}e^{\l(\e^{2})}iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}\,dy}{y},
      \]
      because $e^{\l(\e^{2})} = \mathbf{1}$ if $\e \in \mu(K)$ and we have interchanged the sum and integrals by Fubini's theorem. As $\boldsymbol{\e}^{\mathbf{N}} = |\Norm(\e)| = 1$ for all $\e \in \mc{O}_{K}^{\ast}$, we can use \cref{cor:unit_if_and_only_if_absolute_value_norm_1} to write our expression as
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in (\mf{a}-\{0\})/\mc{O}_{K}^{\ast}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\e\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}e^{\l(\e^{2})}iy^{\frac{1}{d}}\mathbf{x}\right\>}(\boldsymbol{\a}\boldsymbol{\e})^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}\,dy}{y}.
      \]
      By the definition of $\l$, we have $\boldsymbol{\a}^{2}e^{\l(\e^{2})} = (\boldsymbol{\a\e})^{2}$ and therefore we may combine the innermost two sums to obtain
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}-\{0\}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}\,dy}{y}.
      \]
      Performing the change of variables $y \to \frac{y}{\boldsymbol{\a}^{2\mathbf{N}}}$ gives
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}-\{0\}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{x}\,dy}{y},
      \]
      ...
      \[
        \int_{0}^{\infty}\int_{\mc{F}}\frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{j(\a) \in j(\mf{a})-\{\mathbf{0}\}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{j(\a)},\boldsymbol{j(\a)}iy^{\frac{1}{d}}\mathbf{x}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{x}\,dy}{y}.
      \]
      ...
      \[
        \w_{\mc{F}}(z) = \int_{\mc{F}}\frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{j(\a) \in j(\mf{a})-\{\mathbf{0}\}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\<\boldsymbol{j(\a)},\boldsymbol{j(\a)}z\mathbf{x}\>}\,d\mathbf{x},
      \]
    ...
    \subsection*{The Integral Representation: Part II}
      ...
      \begin{align*}
        ...
        &= \frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\vt_{\mf{a}}\left(-\frac{\mathbf{x}}{2|\D_{K}|^{\frac{2}{d}}iy^{\frac{1}{d}}}\right)d\mathbf{x}-\frac{h_{K}}{w_{K}}\int_{\mc{F}}\,d\mathbf{x} && \text{\cref{equ:omega_theta_relationship_for_Dedekind_zeta_functions}} \\
        ...
        &= \sqrt{|\D_{K}|y}\w_{\mc{F}}(iy^{\frac{1}{d}})+\frac{\sqrt{|\D|_{K}y}h_{K}}{w_{K}}\int_{\mc{F}}\,d\mathbf{x}-\frac{h_{K}}{w_{K}}\int_{\mc{F}}\,d\mathbf{x} && \text{\cref{equ:omega_theta_relationship_for_Dedekind_zeta_functions}}.
      \end{align*}
      This chain implies that our first piece can be expressed as
      \[
      ...
      \begin{lemma}\label{lem:integral_of_fundamental_domain_for_norm-one_hypersurface}
        Let $K$ be a number field of signature $(r_{1},r_{2})$ and set $\mc{F} = e^{2\mc{M}}$ where $\mc{M}$ is a fundamental domain for $\L$ in the trace-zero hyperplane $H$. Then
        \[
          \int_{\mc{F}}\,d\mathbf{x} = 2^{r_{1}-1}R_{K}.
        \]
      \end{lemma}
      \begin{proof}
        Under the change of variables $\mathbf{x} \to e^{2\mathbf{x}}$, we have
        \[
          \int_{\mc{F}}\,d\mathbf{x} = 2^{r_{K}}\int_{\mc{M}}\,d\mathbf{x},
        \]
        because the absolute value of the determinant of the Jacobian matrix is $2^{r_{K}}e^{\<\mathbf{x},\mathbf{1}\>} = 2^{r_{K}}$ (as $\mathbf{x} \in S$). Let $e_{1},\ldots,e_{r_{K}}$ be the standard basis for $H$. Choosing a system of fundamental units $\e_{1},\ldots,\e_{r_{K}}$ for $K$, we have that $\l(\e_{1}),\ldots,\l(\e_{r_{K}})$ is a basis for $\L$ and hence $H$. The change of variables
        \[
          x_{1}e_{1}+\cdots+x_{r_{K}}e_{r_{K}} \to x_{1}\l(\e_{1})+\cdots+x_{r_{K}}\l(\e_{r_{K}})
        \]
        \todo{xxx}
      \end{proof}
    ...
    \subsection*{The Functional Equation}
      ...
      \]
      This shows $(s-1)\z_{K}(s)$ is of order $1$ and hence $\z_{K}(s)$ is too after removing the polar divisor. It now remains to compute the residue of $\z_{K}(s)$ at $s = 1$. This result is known as the infamous \textbf{analytic class number formula}\index{analytic class number formula} (and we include the case $d = 1$):
