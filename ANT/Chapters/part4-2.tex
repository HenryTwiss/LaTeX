\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{\todo{The Hecke Theta Function}}
       The \textbf{Hecke theta function}\index{Hecke theta function} $\vt_{\mf{a}}(z\mathbf{x})$ attached to a fractional ideal $\mf{f}$ of $K$, is defined by
      \[
        \vt_{\mf{f}}(z\mathbf{x}) = \sum_{\a \in \mf{f}}e^{2\pi i\Norm(\a\mf{f}^{-1})^{\frac{2}{d}}\left\<z\boldsymbol{\a}\mathbf{x},\mathbf{N}\right\>},
      \]
      ...
      \[
        \vt_{\mf{f}}(z\mathbf{x})-1 = O\left(\sum_{\a \in \mf{f}^{\ast}}e^{-2\pi y||\mathbf{x}||_{\infty}\Norm(\a\mf{f}^{-1})^{\frac{2}{d}}\<\boldsymbol{\a},\mathbf{N}\>}\right) = O(e^{-2\pi y||\mathbf{x}||_{\infty}}),
      \]
      where the second equality follows since $\<\boldsymbol{\a},\mathbf{N}\> \ge d|\Norm(\a)|^{\frac{1}{d}} \ge d$ by the arithmetic-geometric inequality and that $|\Norm(\a)| \ge 1$, and $\Norm(\a\mf{f}^{-1})$ is a positive integer for all $\a \in \mf{f}^{\ast}$ (as $\a\mf{f}^{-1}$ is an integral ideal by \cref{prop:explicit_inverse_ideal}), which combined show that each term is of smaller order than the single term where $\Norm(\a\mf{f}^{-1}) = 1$ corresponding to $\a = \Norm(\mf{f}) \in \mf{f}$ (because every fractional ideal contains its norm as we have noted) so that the double sum is bounded by a constant times this term. The relationship to $\w_{\mc{F}}(z)$ is given by
      \[
