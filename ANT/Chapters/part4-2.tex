\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    ...
    \subsection*{The Jacobi Theta Function}
      ...
      \end{equation}
      so that $\vt(z)$ is locally absolutely uniformly convergent in the same region as $\w(z)$ and exhibits rapid decay. The essential fact we will need is a functional equation for the Jacobi theta function:
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      We will need to manipulate this integral representation somewhat. For any $\mathbf{y} \in \R_{+}^{r_{K}+1}$, we may write
      \[
        \mathbf{y} = t^{\frac{1}{d}}\mathbf{x}, \quad \mathbf{x} = \frac{\mathbf{y}}{\Norm_{\R^{r_{K}+1}/\R}(\mathbf{y})^{\frac{1}{r_{K}+1}}}, \quad \text{and} \quad t = \Norm_{\R^{r_{K}+1}/\R}(\mathbf{y})^{\frac{d}{r_{K}+1}},
      \]
      so that $\R_{+}^{r_{K}+1}$ decomposes as
      \[
      ...
      \]
      where $d\mathbf{x}$ is the restriction of the measure $\frac{d\mathbf{y}}{\mathbf{y}}$ to $S$. Therefore we have an integral representation
      \begin{equation}\label{equ:integral_representation_Dedekind_1}
        \z_{K}(s) = \frac{(2\pi)^{r_{2}s}\pi^{\frac{r_{1}s}{2}}}{\G\left(\frac{s}{2}\right)^{r_{1}}\G\left(s\right)^{r_{2}}}\int_{0}^{\infty}\int_{S}\w_{K}(it^{\frac{1}{d}}\mathbf{x})t^{\frac{s}{2}}\,d\mathbf{x}\,\frac{dt}{t},
      \end{equation}
      which can be viewed as the Mellin transform of the function
      \[
        \w_{S}(t) = \int_{S}\w_{K}(it^{\frac{1}{d}}\mathbf{x})t^{\frac{s}{2}}\,d\mathbf{x}.
      \]
    ...
    \subsection*{\todo{The Hecke Theta Function}}
      We will require some further setup before introducing the Hecke theta function. Observe that the map
      \[
        \log:(\R_{+}^{r_{K}+1})^{\ast} \to \R^{r_{K}+1} \qquad (x_{\s})_{\s} \to (\log(x_{\s}))_{\s},
      \]
      takes the norm-one surface $S$ into the trace-zero hyperplane $H$. Since $\mc{O}_{K}^{\ast}$ is closed under conjugation, $|\e|^{2} = \e\conj{\e} \in \mc{O}_{K}^{\ast}$ for every $\e \in \mc{O}_{K}^{\ast}$. Accordingly, let $|\mc{O}_{K}^{\ast}|^{2}$ be the subgroup of $\mc{O}_{K}^{\ast}$ defined by
      \[
        |\mc{O}_{K}^{\ast}|^{2} = \{|\e|^{2} \in \mc{O}_{K}^{\ast}:\e \in \mc{O}_{K}^{\ast}\}.
      \]
      \todo{xxx}
      \iffalse
      Recall that the ideal class group $\Cl(K)$ is finite by \cref{equ:finitness_of_class_number}. Moreover, if $\mf{a} \in \Cl(K)$ is an integral ideal representative of a class then $\mc{O}_{K}^{\ast}$ acts on $\mf{a}^{\ast}$ by multiplication. Therefore we have an orbit space $\mf{a}^{\ast}/mc{O}_{K}^{\ast}$. The \textbf{Hecke theta function}\index{Hecke theta function} $\vt_{K}(z)$ attached to the number field $K$, is defined by
      \[
      ...
      \]
      so that $\vt_{K}(\mathbf{z})$ is locally absolutely uniformly convergent in the same region as $\w_{K}(\mathbf{z})$ and exhibits rapid decay.
      \fi
