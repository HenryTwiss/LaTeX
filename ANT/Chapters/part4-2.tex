\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    ...
    \subsection*{The Functional Equation}
      An immediate consequence of applying the symmetry $s \to 1-s$ to \cref{equ:integral_representation_zeta_final} is the following functional equation:
      \[
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Functional Equation}
      An immediate consequence of applying the symmetry $s \to 1-s$ to \cref{equ:integral_representation_Dirichlet_final} is the following functional equation:
      \[
  ...
  \section{Dedekind Zeta Functions}
    ...
    \subsection*{The Integral Representation: Part I}
      We will find an integral representation for $\z_{K}(s)$. To do this, let $\s$ run over the $n$ distinct $\Q$-embeddings of $K$ into $\conj{\Q}$ and let $\tau$ run over a complete set of representatives for the real and pairs of complex $\Q$-embeddings. Also recall that $N_{\s} = 1,2$ depending upon if $\s$ is real or complex. It will be useful to let
      \[
        \mathbf{N} = (N_{\tau})_{\tau}, \quad \mathbf{n} = (n)_{\tau}, \quad \boldsymbol{\a} = (|\tau(\a)|)_{\tau},  \quad \text{and} \quad \boldsymbol{j(\a)} = (|\s(\a)|)_{\s},
      \]
      ...
      \begin{align*}
        ...
        &= \sum_{n \ge 1}\frac{a_{K}(n)}{\pi^{\frac{r_{1}s}{2}}(2\pi)^{r_{2}s}n^{s}}\int_{\R_{+}^{r_{K}+1}}e^{-\pi\<\mathbf{1},\mathbf{y}\>}\mathbf{y}^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}} && \text{$\mathbf{y} \to \frac{\mathbf{y}}{\mathbf{N}\pi\mathbf{n}^{\frac{2}{d}}}$} \\
      ...
      \begin{lemma}\label{lem:Hecke_theta_function_lemma}
        ...
        \[
          (\mf{a}-\{0\})/\mc{O}_{K}^{\ast} \to \{\mf{b} \in \mf{K}:\text{$\mf{b}$ is an integral ideal}\} \qquad \a \to \a\mf{a}^{-1}.
        \]
      \end{lemma}
      \begin{proof}
      ...
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in (\mf{a}-\{0\})/\mc{O}_{K}^{\ast}}\int_{\R_{+}^{r_{K}+1}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\<\mathbf{N},i\mathbf{y}\>}\mathbf{y}^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}},
      \]
      ...
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in (\mf{a}-\{0\})/\mc{O}_{K}^{\ast}}\int_{\R_{+}^{r_{K}+1}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\<\mathbf{N},\boldsymbol{\a}^{2}i\mathbf{y}\>}(\boldsymbol{\a}^{2}\mathbf{y})^{\frac{s\mathbf{N}}{2}}\,\frac{d\mathbf{y}}{\mathbf{y}}.
      \]
      ...
      \[
        \sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in (\mf{a}-\{0\})/\mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{S}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      Now consider the isomorphism
      \[
        \log:\R_{+}^{r_{K}+1} \to \R^{r_{K}+1} \qquad (x_{\tau})_{\tau} \to (\log(x_{\tau}))_{\tau}.
      \]
      ...
      Then folding yields
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in (\mf{a}-\{0\})/\mc{O}_{K}^{\ast}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}\log^{-1}\l(\e^{2})iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y},
      \]
      ...
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in (\mf{a}-\{0\})/\mc{O}_{K}^{\ast}}\sum_{\e \in \mc{O}_{K}^{\ast}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\e\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}\log^{-1}\l(\e^{2})iy^{\frac{1}{d}}\mathbf{x}\right\>}(\boldsymbol{\a}\boldsymbol{\e})^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      ...
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}-\{0\}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\a\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}\boldsymbol{\a}^{s\mathbf{N}}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      Performing the change of variables $y \to \frac{y}{\boldsymbol{\a}^{2\mathbf{N}}}$ gives
      \[
        \frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{\a \in \mf{a}-\{0\}}\int_{0}^{\infty}\int_{\mc{F}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\mathbf{N},\boldsymbol{\a}^{2}iy^{\frac{1}{d}}\mathbf{x}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y},
      \]
      ...
      \[
        \int_{0}^{\infty}\int_{\mc{F}}\frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{j(\a) \in j(\mf{a})-\{\mathbf{0}\}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\left\<\boldsymbol{j(\a)},\boldsymbol{j(\a)}iy^{\frac{1}{d}}\mathbf{x}\right\>}y^{\frac{s}{2}}\,\frac{d\mathbf{x}}{\mathbf{x}}\,\frac{dy}{y}.
      \]
      ...
      \[
        \w_{\mc{F}}(z) = \int_{\mc{F}}\frac{1}{w_{K}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sum_{j(\a) \in j(\mf{a})-\{\mathbf{0}\}}e^{\pi i\Norm(\mf{a}^{-1})^{\frac{2}{d}}\<\boldsymbol{j(\a)},\boldsymbol{j(\a)}z\mathbf{x}\>}\,\frac{d\mathbf{x}}{\mathbf{x}}\,,
      \]
      for $z \in \H$. It is locally absolutely uniformly convergent in this region by the Weierstrass $M$-test and that $\mc{F}$ is compact (because $\mc{M}$ is and $\log(\mathbf{x})$ is continuous). At last, this gives a final integral representation
    ...
    \subsection*{The Hecke Theta Function}
      ...
      \[
        \vt_{\mf{f}}(z\mathbf{x}) = \sum_{j(\a) \in j(\mf{f})}e^{2\pi i\Norm(\mf{f}^{-1})^{\frac{2}{d}}\<\boldsymbol{j(\a)},\boldsymbol{j(\a)}z\mathbf{x}\>},
      \]
      ...
      \begin{equation}\label{equ:omega_theta_relationship_for_Dedekind_zeta_functions}
        \w_{\mc{F}}(z) = \frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\vt_{\mf{a}}\left(\frac{z\mathbf{x}}{2}\right)\,\frac{d\mathbf{x}}{\mathbf{x}}-\frac{h_{K}}{w_{K}}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}}.
      \end{equation}
      ...
      \begin{proof}
        ...
        \begin{align*}
          \vt_{\mf{f}}(z\mathbf{x}) &= \sum_{j(\a) \in j(\mf{f})}e^{2\pi i\Norm(\mf{f}^{-1})^{\frac{2}{d}}\<\boldsymbol{j(\a)},\boldsymbol{j(\a)}z\mathbf{x}\>} \\
          &= \frac{1}{V_{j(\mf{f})}}\sum_{\a^{\vee} \in \mf{f}^{\vee}}\Norm(\mf{f})\frac{e^{-\frac{\pi\Norm(\mf{f})^{\frac{2}{d}}\<\boldsymbol{j(\a^{\vee})},\boldsymbol{j(\a^{\vee})}\mathbf{x}\>}{-2iz}}}{(-2iz)^{\frac{d}{2}}} \\
          &= \frac{1}{\Norm(\mf{f})\sqrt{|\D_{K}|}}\sum_{\a^{\vee} \in \mf{f}^{\vee}}\Norm(\mf{f})\frac{e^{-\frac{\pi\Norm(\mf{f})^{\frac{2}{d}}\<\boldsymbol{j(\a^{\vee})},\boldsymbol{j(\a^{\vee})}\mathbf{x}\>}{-2iz}}}{(-2iz)^{\frac{d}{2}}} && \text{\cref{prop:covolume_of_fractional_ideal_under_j}} \\
          &= \frac{1}{(-2|\D_{K}|^{\frac{1}{d}}iz)^{\frac{d}{2}}}\sum_{\a^{\vee} \in \mf{f}^{\vee}}e^{-\frac{\pi\Norm(\mf{f})^{\frac{2}{d}}\<\boldsymbol{j(\a^{\vee})},\boldsymbol{j(\a^{\vee})}\mathbf{x}\>}{-2iz}} \\
          &= \frac{1}{(-2|\D_{K}|^{\frac{1}{d}}iz)^{\frac{d}{2}}}\sum_{\a^{\vee} \in \mf{f}^{\vee}}e^{-\frac{\pi\left(\Norm((\mf{f}^{\vee})^{-1})|\D|_{K}^{-1}\right)^{\frac{2}{d}}\<\boldsymbol{j(\a^{\vee})},\boldsymbol{j(\a^{\vee})}\mathbf{x}\>}{-2iz}} && \text{\cref{cor:norm_of_different}} \\
          &= \frac{1}{(-2|\D_{K}|^{\frac{1}{d}}iz)^{\frac{d}{2}}}\sum_{\a^{\vee} \in \mf{f}^{\vee}}e^{2\pi i\Norm((\mf{f}^{\vee})^{-1})^{\frac{2}{d}}\left\<\boldsymbol{j(\a^{\vee})},\boldsymbol{j(\a^{\vee})}\left(-\frac{\mathbf{x}}{4|\D_{K}|^{\frac{2}{d}}z}\right\>\right)} \\
    ...
    \subsection*{The Integral Representation: Part II}
      ...
      \end{equation}
      We will rewrite the first piece in the same form and symmetrize the result as much as possible. Perform the change of variables $y \to \frac{1}{|\D_{K}|^{2}y}$ to the first piece to obtain
      \[
      ...
      \begin{align*}
        ...
        &= \frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\vt_{\mf{a}}\left(-\frac{\mathbf{x}}{2|\D_{K}|^{\frac{2}{d}}iy^{\frac{1}{d}}}\right)\frac{d\mathbf{x}}{\mathbf{x}}-\frac{h_{K}}{w_{K}}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}} && \text{\cref{equ:omega_theta_relationship_for_Dedekind_zeta_functions}} \\
        &= \frac{1}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\sqrt{|\D_{K}|y}\vt_{\mf{a}}\left(\frac{iy^{\frac{1}{d}}\mathbf{x}}{2}\right)\frac{d\mathbf{x}}{\mathbf{x}}-\frac{h_{K}}{w_{K}}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}} && \text{\cref{thm:functional_equation_Hecke_theta}} \\
        &= \frac{\sqrt{|\D_{K}|y}}{w_{K}}\int_{\mc{F}}\sum_{\substack{\mf{a} \in \Cl(K) \\ \text{$\mf{a}$ integral}}}\vt_{\mf{a}}\left(\frac{iy^{\frac{1}{d}}\mathbf{x}}{2}\right)\frac{d\mathbf{x}}{\mathbf{x}}-\frac{h_{K}}{w_{K}}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}} \\
        &= \sqrt{|\D_{K}|y}\w_{\mc{F}}(iy^{\frac{1}{d}})+\frac{\sqrt{|\D|_{K}y}h_{K}}{w_{K}}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}}-\frac{h_{K}}{w_{K}}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}} && \text{\cref{equ:omega_theta_relationship_for_Dedekind_zeta_functions}}.
      \end{align*}
      This chain implies that our first piece can be expressed as
      \[
        \int_{\frac{1}{|\D_{K}|}}^{\infty}\left(\sqrt{|\D_{K}|y}\w_{\mc{F}}(iy^{\frac{1}{d}})+\frac{\sqrt{|\D|_{K}y}h_{K}}{w_{K}}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}}-\frac{h_{K}}{w_{K}}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}}\right)(|\D_{K}|^{2}y)^{-\frac{s}{2}}\,\frac{dy}{y},
      \]
      which is further equivalent to
      \[
        |\D_{K}|^{\frac{1}{2}-s}\int_{\frac{1}{|\D_{K}|}}^{\infty}\w_{\mc{F}}(iy^{\frac{1}{d}})y^{\frac{1-s}{2}}\,\frac{dy}{y}-\frac{2h_{K}}{\w_{K}\sqrt{|\D|_{K}}s(1-s)}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}},
      \]
      ...
      \begin{align*}
        \z_{K}(s) &= \frac{\pi^{\frac{r_{1}s}{2}}(2\pi)^{r_{2}s}}{\G\left(\frac{s}{2}\right)^{r_{1}}\G\left(s\right)^{r_{2}}}\Bigg[-\frac{2h_{K}}{\w_{K}\sqrt{|\D|_{K}}s(1-s)}\int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}}+|\D_{K}|^{\frac{1}{2}-s}\int_{\frac{1}{|\D_{K}|}}^{\infty}\w_{\mc{F}}(iy^{\frac{1}{d}})y^{\frac{1-s}{2}}\,\frac{dy}{y} \\
      ...
      \begin{lemma}\label{lem:integral_of_fundamental_domain_for_norm-one_hypersurface}
        Let $K$ be a number field of signature $(r_{1},r_{2})$ and set $\mc{F} = \log^{-1}(\mc{M}) \subset S$ where $\mc{M}$ is a fundamental domain for the unit lattice $\L$ in the trace-zero hyperplane $H$. Then
        \[
          \int_{\mc{F}}\,\frac{d\mathbf{x}}{\mathbf{x}} = 2^{r_{1}-1}R_{K}.
        \]
      \end{lemma}
      \begin{proof}
        \todo{xxx}
    ...
    \subsection*{Imprimitivity}
      ...
      Since the residue of $\z(s)$ at $s = 1$ is $1$, the analytic class number formula gives a relationship between the value of Dirichlet $L$-functions at $s = 1$ and the class number of quadratic number fields:
  ...
  \section{Hecke \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Functional Equation}
      An immediate consequence of applying the symmetry $s \to 1-s$ to \cref{equ:integral_representation_holomorphic_final} is the following functional equation:
      \[
  ...
  \section{Hecke-Maass \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Functional Equation}
      An immediate consequence of applying the symmetry $s \to 1-s$ to \cref{equ:integral_representation_Maass_final} is the following functional equation:
      \[
  ...
  \section{The Rankin-Selberg Method}
    ...
    \subsection*{The Functional Equation}
      An immediate consequence of applying the symmetry $s \to 1-s$ to \cref{equ:integral_representation_Rankin-Selberg_final} is the following functional equation:
      \[
