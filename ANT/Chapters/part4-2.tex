\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$\z(s)$}{\z(s)}}
      ...
      \]
      This is the prototypical example of a Dirichlet series as all the coefficients are $1$. We will see that $\z(s)$ is a primitive Selberg class $L$-function. As the coefficients are trivially polynomially bounded, $\z(s)$ is locally absolutely uniformly convergent for $\s > 1$. Also note that $\z(s)$ is necessarily nonzero in this region. Determining the Euler product is also an easy matter. As the coefficients are obviously completely multiplicative, we have the degree $1$ Euler product:
      \[
        \z(s) = \prod_{p}(1-p^{-s})^{-1},
      \]
  ...
  \section{\todo{Dedekind Zeta Functions}}
    \subsection*{\todo{The Definition \& Euler Product of \texorpdfstring{$\z_{K}(s)$}{\z_K(s)}}}
    \subsection*{\todo{The Integral Representation of \texorpdfstring{$\z_{K}(s)$}{\z_{K}(s)}: Part I}}
    \subsection*{\todo{The Number Field Zeta Function \texorpdfstring{$\vt_{K}(z)$}{\vt_{K}(z)}}}
    \subsection*{\todo{The Integral Representation of \texorpdfstring{$\z_{K}(s)$}{\z_{K}(s)}: Part II}}
    \subsection*{\todo{The Functional Equation \& Critical Strip of \texorpdfstring{$\z_{K}(s)$}{\z_K(s)}}}
