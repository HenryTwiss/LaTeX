\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    ...
    \subsection*{The Jacobi Theta Function}
      ...
      \begin{proof}
        ...
        \[
          \hat{f}(t) = \int_{\R}f(x)e^{-2\pi itx}\,dx = \int_{\R}e^{-2\pi x^{2}y}e^{-2\pi itx}\,dx = \int_{\R}e^{-2\pi(x^{2}y+itx)}\,dx.
        \]
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      defined for $z \in \H$. It is locally absolutely uniformly convergent in this region by the ratio and Weierstrass $M$-tests. Moreover, the Taylor series of $\frac{e^{y}}{1-e^{y}}$ and its first derivative together give
      \[
    ...
    \subsection*{The Dirichlet Theta Function}
      ...
      \begin{proof}
        ...
        \[
          \hat{f}(t) = \int_{\R}f(x)e^{-2\pi itx}\,dx = \int_{\R}(xq+a)^{\mf{a}}e^{-2\pi(xq+a)^{2}y}e^{-2\pi itx}\,dx = \int_{\R}(xq+a)^{\mf{a}}e^{-2\pi((xq+a)^{2}y+itx)}\,dx.
        \]
  ...
  \section{\todo{Dedekind Zeta Functions}}
    \subsection*{The Definition \& Euler Product}
      ...
      \]
      We will show that $\z_{K}(s)$ is a Selberg class $L$-function. First we show that the coefficients are polynomially bounded. By \cref{equ:ideals_of_fixed_norm_bound}, $a_{K}(n) \le \s_{0}(n)^{d} \ll_{\e} n^{\e}$ (recall \cref{prop:sum_of_divisors_growth_rate}). Therefore $\z_{K}(s)$ is locally absolutely uniformly convergent for $\s > 1$. To determine the Euler product, we need to express $\z_{K}(s)$ in another form. Since $\z_{K}(s)$ converges absolutely, we can write
      \[
      ...
      \[
        \z_{K}(s) = \prod_{p}\prod_{\text{$\mf{p}$ above $p$}}(1-\Norm(\mf{p})p^{-s})^{-1} = \prod_{p}\prod_{1 \le i \le r_{p}}(1-p^{-f_{\mf{p}_{i}}s})^{-1},
      \]
      ...
      \[
        d = \sum_{1 \le i \le r_{p}}f_{\mf{p}_{i}}.
      \]
      In particular, at least one prime does. Therefore, letting $\w_{p,i}$ be a primitive $f_{\mf{p}_{i}}$-th root of unity, we have the degree $d$ Euler product:
      \[
        \z_{K}(s) = \prod_{p}\prod_{\substack{1 \le i \le r_{p} \\ 1 \le k \le f_{\mf{p}_{i}}}}(1-\w_{p,i}^{k}p^{-s})^{-1},
      \]
      in this region as well. The local factor at $p$ is
      \[
        \z_{K,p}(s) = \prod_{\substack{1 \le i \le r_{p} \\ 1 \le k \le f_{\mf{p}_{i}}}}(1-\w_{p,i}^{k}p^{-s})^{-1},
      \]
      with local roots $\w_{p,i}^{k}$ if $p$ is unramified and \todo{xxx}.
    \subsection*{\todo{The Functional Equation}}
      In 1917, Hecke proved the meromorphic continuation of the Dedekind zeta function for any number field $K$ (see \cite{hecke1917ueber}). While the proof is not beyond the scope of this text, the argument is technical and requires a series of generalizations that we avoid. The gamma factor of $\z_{K}(s)$ was identified as
      \[
        \g(s,\z_{K}) = \pi^{-\frac{ds}{2}}\G\left(\frac{s}{2}\right)^{r_{1}+r_{2}}\Gamma\left(\frac{s+1}{2}\right)^{r_{2}},
      \]
      with $\k_{i} = 0$ for $1 \le i \le r_{1}+r_{2}$ and $\k_{i} = 1$ for $r_{1}+r_{2}+1 \le i \le d$ the local roots at infinity. Clearly these satisfy the required bounds. The conductor is $q(\z_{K}) = |\D_{K}|$ and $p$ is ramified if and only if it is ramified in $K$ by \cref{thm:ramification_division_equivalence}. Note that if $p$ is unramified then the local roots $\w_{p,i}^{k} \neq 0$ because \todo{xxx}. Nevertheless, the theorem is the following (see \cite{neukirch2013algebraic} for a proof):
  ...
  \section{The Rankin-Selberg Method}
    ...
    \subsection*{The Integral Representation: Part II}
      ...
      \begin{proof}
        ...
        \begin{align*}
          \hat{f}(t) = \int_{\R}f(x)e^{-2\pi itx}\,dx &= \int_{\R}\frac{y^{s}}{|cx+r+icy|^{2s}}e^{-2\pi itx}\,dx \\
