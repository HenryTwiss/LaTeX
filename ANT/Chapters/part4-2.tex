  ...
  \section{Zero Counting \& Riemann-von Mangoldt Formulas}
    ...
    \subsection*{The Riemann–von Mangoldt formula for $\z(s)$}
      ...
      \begin{proof}
        ...
        \end{equation}
        where in the last equality we have used the estimate $\arg(s) = \arctan\left(\frac{t}{\s}\right) = \frac{\pi}{2}+O_{\s}\left(\frac{1}{t}\right)$, with $s = \s+it$, which holds by the Laurent series of $\arctan(s)$ about $\infty$. For the second term, we perform an analogous computation:
