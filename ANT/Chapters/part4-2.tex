\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Integral Representation: Part I}
      ...
      \]
      defined for $z \in \H$. It is locally absolutely uniformly convergent in this region by the ratio and Weierstrass $M$-tests. Moreover, the Taylor series of the derivative of $\frac{e^{y}}{1-e^{y}}$ gives
      \[
  ...
  \section{\todo{Dedekind Zeta Functions}}
    ...
    \subsection*{\todo{The Integral Representation: Part I}}
      ...
      \]
      defined for $\mathbf{z} \in \H^{r_{1}+r_{2}}$ and where $\mathbf{n} = (n)_{\s}$. It is locally absolutely uniformly convergent in this region by the ratio and Weierstrass $M$-tests. Taking $\e < 1$, the bound $a_{K}(n) \ll_{\e} n^{\e}$ and the Taylor series of the $(r_{2}+1)$-st derivative of $\frac{e^{||\mathbf{y}||}}{1-e^{||\mathbf{y}||}}$ together give
      \[
        \w_{K}(\mathbf{z}) = O\left(\sum_{n \ge 1}n^{r_{2}+1}e^{-\pi n^{\frac{2}{d}}\<\mathbf{y},\mathbf{2}^{\d}\>}\right) = \todo{xxx},
      \]
      and so $\w_{K}(\mathbf{z})$ exhibits rapid decay. Now consider the following $(r_{1}+r_{2})$-fold Mellin transform:
      \[
