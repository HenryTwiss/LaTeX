\chapter{The Theory of Number Fields}
  ...
  \section{\todo{Quadratic Number Fields}}
    ...
    \end{proposition}
    \begin{proof}
      Suppose $K$ is a quadratic number field. In particular, $K/\Q$ is separable so by the primitive element theorem there exists $\t \in K$ such that $K = \Q(\t)$. The minimal polynomial $p_{\t}(x)$ of $\t$ is of the form
      \[
        p_{\t}(x) = x^{2}+ax+b,
      \]
    ...
    From \cref{prop:classification_of_quadratic_number_fields}, we see that $d$ for $\Q(\sqrt{d})$ satisfies $d \equiv 1,2,3 \tmod{4}$ (otherwise $d$ is square-free). Moreover, any element of a quadratic number field is of the form $a+b\sqrt{d}$ with $a,b \in \Q$ and for some square-free $d$ other than $0$ or $1$. We say that a quadratic number field $\Q(\sqrt{d})$ is \textbf{real}\index{real} if $d > 0$ and \textbf{imaginary}\index{imaginary} if $d < 0$. On the one hand, if $\Q(\sqrt{d})$ is real then the minimal polynomial has roots $\pm\sqrt{d}$ so that the $\Q$-embeddings of $\Q(\sqrt{d})$ into $\C$ are given by
    \[
      a+b\sqrt{d} \mapsto a+b\sqrt{d} \quad \text{and} \quad a+b\sqrt{d} \mapsto a-b\sqrt{d},
    \]
    so that the signature of $\Q(\sqrt{d})$ is $(2,0)$. On the other hand, if $\Q(\sqrt{d})$ is imaginary then the minimal polynomial has roots $\sqrt{d}$ and $\conj{\sqrt{d}}$ so that the $\Q$-embeddings of $\Q(\sqrt{d})$ into $\C$ are induced by
    \[
      a+b\sqrt{d} \mapsto a+b\sqrt{d} \quad \text{and} \quad a+b\sqrt{d} \mapsto a+b\conj{\sqrt{d}},
    \]
    so that the signature of $\Q(\sqrt{d})$ is $(0,1)$. In either case, \cref{prop:formulas_for_trace_and_norm} shows that the norm and trace of $\k = a+b\sqrt{d}$ for any algebraic number field $\Q(\sqrt{d})$ are
    \[
      \Trace(\k) = 2a \quad \text{and} \quad \Norm(\k) = a^{2}-b^{2}d.
    \]
    We now show that the ring of integers of a quadratic number field $\Q(\sqrt{d})$ has a particularly simple description depending upon the congruence condition of $d$ modulo $4$:
    ...
    \begin{proof}
      ...
      \[
        p_{\a}(x) = x^{2}+2ax+(a^{2}-b^{2}d) = (x-(a+b\sqrt{d}))(x-(a-b\sqrt{d})).
      \]
