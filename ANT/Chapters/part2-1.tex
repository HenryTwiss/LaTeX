\chapter{The Theory of Number Fields}
  ...
  \section{Traces \& Norms}
    ...
    In particular, for a separable extension $L/K$ \cref{prop:formulas_for_trace_and_norm} implies that
    \[
      \Trace_{R/K}(1) = n \quad \text{and} \quad \Norm_{R/K}(1) = 1.
    \]
  ...
  \section{Discriminants}
    ...
    It turns out that $\disc_{K}(\l_{1},\ldots,\l_{n})$ is nonzero if $L/K$ is separable. To see this, we require a lemma:
    ...
    Continue to assume that $L/K$ is separable. We can write $\disc_{K}(\l_{1},\ldots,\l_{n})$ in a much more useful form. To do this, we define an \textbf{embedding matrix}\index{embedding matrix} $M(\l_{1},\ldots,\l_{n})$ by
    \[
      M(\l_{1},\ldots,\l_{n}) = (\s_{i}(\l_{j}))_{i,j},
    \]
    for any basis $\{\l_{1},\ldots,\l_{n}\}$ of $L$. Then we have the following result:
