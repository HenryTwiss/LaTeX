\chapter{The Theory of Number Fields}
  ...
  \section{Minkowski Space}
    ...
    \[
      \log_{K}:K^{\ast} \to \R^{n} \qquad \k \to (\log|(\s_{K}(\k))_{1}|,\ldots,\log|(\s_{K}(\k))_{n}|).
    \]
    Then $\log_{K}$ is just the restriction of the canonical embedding $\s_{K}$ to $K$ composed with $\log|\cdot|$ at each component. Since $\log|\cdot|$ is a homomorphism and $\s_{K}$ is an embeddings, $\log_{K}$ is a homomorphism. All of these maps are related via the following commutative diagrams:
    ...
    \begin{center}
      ...
      \begin{tikzcd}
        ...
        \Q^{\ast} \arrow[hookrightarrow]{r} & \R^{\ast} \arrow{r}{\log|\cdot|} & \R
      \end{tikzcd}
    \end{center}
