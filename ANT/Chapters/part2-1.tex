\chapter{The Theory of Number Fields}
  ...
  \section{\todo{Quadratic Number Fields}}
    ...
    \begin{proof}
      ...
      \[
        p_{\t}(x) = x^{2}+q_{1}x+q_{0},
      \]
      for $q_{0},q_{1} \in \Q$. Then the quadratic formula gives
      \[
        \t = -\frac{q_{1}}{2}\pm\frac{\sqrt{q}}{2},
      \]
      where $q = q_{1}^{2}-4q_{0} \in \Q$. Clearly $q \neq 0$ and $q \neq 1$ for otherwise $\t \in \Q$. It follows that $K = \Q(\sqrt{q})$. Write $q = \frac{a}{b}$ for relatively prime $a,b \in \Z$ and set $d = b^{2}q = ab \in \Z$. Then $d$ is square-free, $d \neq 0$, and $d \neq 1$. Moreover, $\sqrt{d} = b\sqrt{q}$ so that $K = \Q(\sqrt{d})$. 
    \end{proof}

    In light of \cref{prop:classification_of_quadratic_number_fields}, we say that a quadratic number field $\Q(\sqrt{d})$ is \textbf{real}\index{real} if $d > 0$ and \textbf{imaginary}\index{imaginary} if $d < 0$. On the one hand, if $\Q(\sqrt{d})$ is real then the minimal polynomial has roots $\pm\sqrt{d}$ so that the $\Q$-embeddings of $\Q(\sqrt{d})$ into $\C$ are induced by
    \[
      \sqrt{d} \to \sqrt{d} \quad \text{and} \quad \sqrt{d} \to -\sqrt{d},
    \]
    ...
    \[
      \sqrt{d} \to \sqrt{d} \quad \text{and} \quad \sqrt{d} \to \conj{\sqrt{d}},
    \]
    so that the signature of $\Q(\sqrt{d})$ is $(0,1)$. We now show that the ring of integers of a quadratic number field $\Q(\sqrt{d})$ has a particularly simple description depending upon the congruence condition of $d$ modulo $4$:
