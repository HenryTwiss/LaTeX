\chapter{The Theory of Algebraic Number Fields}
  ...
  \section{Trace \& Norm}
    ...
    \]
    We will now derive alternative descriptions of the trace and norm. To do this, we need to work in the algebraic closure $\conj{K}$ of $K$. As $L/K$ is degree $n$ and $K$ is a finite extension of $\Q$, there are exactly $n$ distinct $K$-embeddings $\s_{1},\ldots,\s_{n}$ of $L$ into $\conj{K}$ (each given by letting $\t$ be a primitive element for $L$ so that $L = K[\t]$ and sending $\t$ to one of its conjugate roots in the minimal polynomial $p_{\t}(x)$ of $\t$). Clearly $\s_{1},\ldots,\s_{n}$ send $\mc{O}_{L}$ to itself and fix $\mc{O}_{K}$ pointwise. Moreover, we prove the following proposition:
  ...
  \section{Ramification}
    We now discuss the splitting of prime integral ideals in algebraic number fields. First, we need to introduce the concept of prime integral ideals above other prime integral ideals. Let $L/K$ be an extension of algebraic number fields and let $\mf{P}$ be a prime integral ideal of $L$. Then $\mf{P} \cap \mc{O}_{K}$ is a prime integral ideal of $K$. Indeed, it is clear that $\mf{P} \cap \mc{O}_{K}$ is an ideal of $\mc{O}_{K}$. It is proper because $1 \notin \mf{P} \cap \mc{O}_{K}$ as $\mf{P}$ does not contain units. It is nonzero because $\mc{O}_{L}/\mf{P}$ is a finite field by \cref{prop:prime_integral_ideals_are_maximal} and thus has characteristic dividing $\Norm(\mf{P})$ by definition of the norm so that $\Norm(\mf{P}) \in \mf{P}$. But as $\Norm(\mf{P})$ is also a positive integer, $\Norm(\mf{P}) \in \mf{P} \cap \Z$. To show that $\mf{P} \cap \mc{O}_{K}$ is prime, suppose $\a,\b \in \mc{O}_{K}$ are such that $\a\b \in \mf{P} \cap \mc{O}_{K}$. Then $\a\b \in \mf{P}$ and since $\mf{P}$ is prime either $\a \in \mf{P}$ or $\b \in \mf{P}$. But as $\a$ and $\b$ are both in $\mc{O}_{K}$, either $\a \in \mf{P} \cap \mc{O}_{K}$ or $\b \in \mf{P} \cap \mc{O}_{K}$ as desired. We have now shown that $\mf{P} \cap \mc{O}_{K}$ is a prime integral ideal of $K$. Hence
    \[
      \mf{P} \cap \mc{O}_{K} = \mf{p},
    \]
    for some prime integral ideal $\mf{p}$ of $K$. Accordingly, we say that $\mf{P}$ is \textbf{above}\index{above} $\mf{p}$, or equivalently, $\mf{p}$ is \textbf{below}\index{below} $\mf{P}$. Moreover, if $\mf{P}$ is above $\mf{p}$, then $\mf{P}$ must be a prime factor of $\mf{p}\mc{O}_{L}$. Indeed, $\mf{p} \subseteq \mf{P}$ so that $\mf{p}\mc{O}_{L} \subseteq \mf{P}$ and then the fact $\mf{P}$ is prime implies that some prime factor of $\mf{p}\mc{O}_{L}$ is contained in $\mf{P}$. Since prime integral ideals are maximal by \cref{prop:prime_integral_ideals_are_maximal}, this prime factor must be $\mf{P}$ itself. For the extension $K/\Q$, we see that every prime integral ideal $\mf{p}$ of $K$ lies above $p\Z$ of $\Z$ for a prime $p$ since these are exactly the prime integral ideals of $\Q$. That is,
    \[
      \mf{p} \cap \Z = p\Z.
    \]
    In this case, we say $\mf{p}$ is \textbf{above}\index{above} $p$, or equivalently, $p$ is \textbf{below}\index{below} $\mf{p}$. In particular, there is a unique prime $p$ for which $p \in \mf{p}$ and necessarily $\mf{p}$ is above $p$. Moreover, if we have a tower $L/K/\Q$ with $\mf{P}$ above $\mf{p}$ and $\mf{p}$ above $p$, then $\mf{P}$ is above $p$. This follows from the chain
    \[
      \mf{P} \cap \Z = \mf{P} \cap \mc{O}_{K} \cap \Z = \mf{p} \cap \Z = p\Z.
    \]
    Looking at the tower $L/K/\Q$, we illustrate these relations by the extension
    ...
    \]
    if and only if $p$ is unramified. Moreover, we say $p$ is \textbf{totally ramified}\index{totally ramified} if $r = 1$ and is \textbf{inert}\index{inert} if $p\mc{O}_{K}$ is prime. In particular, $p$ is totally ramified if and only if $p\mc{O}_{K}$ is a power of a prime integral ideal $\mf{p}$ and necessarily $\mf{p}$ is the only prime integral ideal above $p$. The degree of an algebraic number field is connected to the inertia degree and ramification index via the following proposition:

    \begin{proposition}
      Let $K$ be an algebraic number field of degree $n$ and let $p$ be a prime. Suppose $p\mc{O}_{K}$ has prime factorization
      \[
        p\mc{O}_{K} = \mf{p}_{1}^{e_{\mf{p}_{1}}} \cdots \mf{p}_{r}^{e_{\mf{p}_{r}}}.
      \]
      Then
    ...
    Ramification of primes is intimately connected to the discriminant of an algebraic number field. We will show that if $p$ is ramified then $p$ divides the discriminant $\D_{K}$:

    \begin{theorem}\label{thm:ramification_implies_division}
      Let $K$ be an algebraic number field. Then if $p$ is ramified, $p$ divides $\D_{K}$.
    \end{theorem}
    \begin{proof}
      Suppose $p$ is a ramified prime. Then there exists a prime integral ideal $\mf{p}$ such that
      \[
        p\mc{O}_{K} = \mf{p}\mf{a},
      \]
      where $\mf{p}$ is a prime factor of $\mf{a}$. In particular, the prime factors of $\mf{a}$ are the same as the prime factors of $p\mc{O}_{K}$. Now let $\{\a_{1},\ldots,\a_{n}\}$ be an integral basis for $K$. By uniqueness of the prime factorization of integral ideals, there exists a nonzero $\a \in \mf{a}-p\mc{O}_{K}$. Then
      \[
        \a = a_{1}\a_{1}+\cdots+a_{n}\a_{n},
      \]
      with $a_{i} \in \Z$ not all zero for $1 \le i \le n$. Moreover, as $\a \notin p\mc{O}_{K}$, there is some $a_{i}$ that is not divisible by $p$. Without loss of generality, we may assume that $a_{1}$ is not divisible by $p$. Recall that
      \[
        \D_{K} = \det\left(\begin{pmatrix} \s_{1}(\a_{1}) & \cdots & \s_{1}(\a_{n}) \\ \vdots & & \vdots \\ \s_{n}(\a_{1}) & \cdots & \s_{n}(\a_{n}) \end{pmatrix}\right)^{2},
      \]
      where $\s_{1},\ldots,\s_{n}$ are the $n$ distinct $\Q$-embeddings of $K$ into $\conj{K}$. Replacing $\a_{1}$ by $\a$, set
      \[
        D = \det\left(\begin{pmatrix} \s_{1}(\a) & \s_{1}(\a_{2}) & \cdots & \s_{1}(\a_{n}) \\ \vdots & \vdots & & \vdots \\ \s_{n}(\a) & \s_{n}(\a_{2}) & \cdots & \s_{n}(\a_{n}) \end{pmatrix}\right)^{2}.
      \]
      The the identity
      \[
        \begin{pmatrix} \s_{1}(\a) & \s_{1}(\a_{2}) & \cdots & \s_{1}(\a_{n}) \\ \vdots & \vdots & & \vdots \\ \s_{n}(\a) & \s_{n}(\a_{2}) & \cdots & \s_{n}(\a_{n}) \end{pmatrix} = \begin{pmatrix} \s_{1}(\a_{1}) & \cdots & \s_{1}(\a_{n}) \\ \vdots & & \vdots \\ \s_{n}(\a_{1}) & \cdots & \s_{n}(\a_{n}) \end{pmatrix}\begin{pmatrix} a_{1} & & & \\ a_{2} & 1 & & \\ \vdots & & \ddots & & \\ a_{n} & & & 1 \end{pmatrix},
      \]
      and that $\s_{1},\ldots,\s_{n}$ are embeddings together imply
      \[
        D = \D_{K}\a_{1}^{2}.
      \]
      In particular, $D$ is a positive integer. Since $p \nmid \a_{1}$, it suffices to show that $p \mid D$. To accomplish this, let $L$ be the Galois closure of $K/\Q$. Then $L/K$ is a finite extension so that $L$ is an algebraic number field and $\s_{1},\ldots,\s_{n} \in \Gal(L/\Q)$. Moreover, as $\a \in \mf{a}$ and the prime factors of $\mf{a}$ are the same as the prime factors of $p\mc{O}_{K}$, $\a \in \mf{p}$ for all prime integral ideals $\mf{p}$ above $p$ (recall $p\mc{O}_{K} \subseteq \mf{p}$ if $\mf{p}$ is above $p$). As $\a \in L$ (because $L$ is an extension of $K$), we claim that $\a$ belongs to all of the prime integral ideals $\mf{P}$ above such $\mf{p}$ as well. For if $\mf{P}$ is above $\mf{p}$, then $\mf{p} \subseteq \mf{P}$ and hence $\a \in \mf{P}$ because $\a \in \mf{p}$. In particular, $\a$ is an element of $\mf{P}$ for all $\mf{P}$ over $p$. Now let $\s \in \Gal(L/\Q)$. As $\s$ is an automorphism of $L$, $\s(\mf{P})$ is a prime integral ideal. Since $\s$ fixes $\Q$ pointwise and $\mf{P}$ is above $p$, we have that $\s(p) = p$ and $p \in \s(\mf{P})$. This means $\s(\mf{P})$ is above $p$ and so $\a \in \s(\mf{P})$. Since $\s$ was arbitrary, it follows that $\s(\a) \in \mf{P}$ for all $\s \in \Gal(L/\Q)$. In particular, $\s_{i}(\a) \in \mf{P}$ for all $i$ and so the first column of the matrix
      \[
        \begin{pmatrix} \s_{1}(\a) & \s_{1}(\a_{2}) & \cdots & \s_{1}(\a_{n}) \\ \vdots & \vdots & & \vdots \\ \s_{n}(\a) & \s_{n}(\a_{2}) & \cdots & \s_{n}(\a_{n}) \end{pmatrix},
      \]
      is an element of $\mf{P}$. Thus $D \in \mf{B}$. But as $D$ is an integer, $D \in \mf{P} \cap \Z = p\Z$ and hence $p \mid D$. This completes the proof.
    \end{proof}

    As an immediate corollary, we see that only finitely many primes can ramify:

    \begin{corollary}
      Let $K$ be an algebraic number field. Then finitely many primes ramify in $K$.
    \end{corollary}
    \begin{proof}
