\chapter{The Theory of Number Fields}
  ...
  \subsection*{Dirichlet's Unit Theorem}
    ...
    \]
    called the \textbf{norm-one surface}\index{norm-one surface} of $K_{\R}$ and the \textbf{trace-zero hyperplane}\index{trace-zero hyperplane} of $\R^{r_{1}+r_{2}}$ respectively. Note that $H$ is an $(r_{1}-r_{2}-1)$-dimensional subspace of $\R^{r_{1}+r_{2}}$. Let $\l$ denote the restriction of $\log_{K}$ to $\mc{O}_{K}^{\ast}$ and set
    \[
    ...
    \begin{center}
      \begin{tikzcd}
        ...
        K^{\ast} \arrow{d}{\Norm} \arrow{r}{j} & K_{\R} \arrow{d}{\Norm_{K_{\R}}}^{\ast} \arrow{r}{\ell} & \R^{r_{1}+r_{2}} \arrow{d}{\Trace_{\R^{r_{1}+r_{2}}/\Q}} \\
    ...
    Our aim now is to show that the unit lattice $\L$ is a a free abelian group of rank $r_{1}+r_{2}-1$. For this, we will require a lemma:
    ...
    By \cref{thm:unit_group_complete_lattice} and that $j$ is a $\Q$-embedding, there exist elements $\e_{1},\ldots,\e_{r_{1}+r_{2}-1}$ of $\mc{O}_{K}^{\ast}$ such that $\{\l(\e_{1}),\ldots,\l(\e_{r_{1}+r_{2}-1})\}$ is a basis for the unit lattice $\L$ (just take the preimage under $j$ of any basis for $\L$). We say that any choice $\e_{1},\ldots,\e_{r_{1}+r_{2}-1}$ are \textbf{fundamental units}\index{fundamental units} of $K$. The structure theorem for $\mc{O}_{K}^{\ast}$ known as \textbf{Dirichlet's unit theorem}\index{Dirichlet's unit theorem} says that any unit of $K$ is a product of a root of unity and powers of fundamental units:
