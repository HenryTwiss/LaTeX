\chapter{The Theory of Algebraic Number Fields}
  ...
  \section{Ramification}
    ...
    \]
    if and only if $p$ is unramified. We also say $p$ is \textbf{split}\index{split} if $p\mc{O}_{K}$ is not prime. The degree of an algebraic number field is connected to the inertia degree and ramification index via the following proposition:
    ...
    According to \cref{prop:inertia_ramification_relation}, if $r = 1$ then there is exactly one prime integral ideal $\mf{p}$ above $p$ and so
    \[
      p\mc{O}_{K} = \mf{p}^{e_{\mf{p}}}.
    \]
    If $e_{\mf{p}} = 1$, so that $p$ does not ramify, we say $p$ is \textbf{inert}\index{inert} and so $f_{\mf{p}} = n$ by \cref{prop:inertia_ramification_relation}. Thus $p$ is inert if and only if $p\mc{O}_{K}$ is prime. If $e_{\mf{p}} \ge 2$, then $p$ ramifies and we say $p$ is \textbf{totally ramified}\index{totally ramified} if $e_{\mf{p}} = n$ so that $f_{\mf{p}} = 1$ by \cref{prop:inertia_ramification_relation}. Equivalently, $p$ is totally ramified if and only if it is the power of a prime ideal with power equal to the degree of $K$. Ramification of primes is intimately connected to the discriminant of an algebraic number field as the following theorem shows:

    \begin{theorem}\label{thm:ramification_implies_division}
      Let $K$ be an algebraic number field. Then $p$ is ramified if and only if $p$ divides $\D_{K}$.
    \end{theorem}
    \begin{proof}
