\chapter{The Theory of Algebraic Number Fields}
  ...
  \section{\todo{Ramification}}
    ...
    \[
      \mf{p} \cap \Z = p\Z,
    \]
    for a unique prime $p$. Accordingly, we say that $\mf{p}$ lies \textbf{above}\index{above} $p\Z$, or equivalently, $p\Z$ lies \textbf{below}\index{below} $\mf{p}$. This is also illustrated by the extension
    \begin{center}
      \begin{tikzcd}
    ...
    \]
    Now let $p \in \Z$ be a prime. We wish to understand the prime integral ideals lying above $p\Z$. \todo{xxx}
