\chapter{The Theory of Number Fields}
  ...
  \section{Minkowski Space}
    ...
    \]
    for some choice of real $\Q$-embeddings $\rho_{1},\ldots,\rho_{r_{1}}$ and representatives of pairs of complex $\Q$-embeddings $\tau_{1},\ldots,\tau_{r_{2}}$, a \textbf{realization}\index{realization} of $K$. This is an embedding of $K$ into $\R^{n}$ since it is the composition of the embedding $j$ (whose image under $K$ is in $K_{\R}$ as we have noted) with the map
    \[
      K_{\R} \to \prod_{\s}\R \qquad z_{\s} \to x_{\s} = \begin{cases} z_{\s} & \text{if $\s = \rho$}, \\ \Re(z_{\s}) & \text{if $\s = \tau$}, \\ \Im(z_{\s}) & \text{if $\s = \conj{\tau}$}, \end{cases}
    \]
