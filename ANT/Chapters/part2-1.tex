\chapter{The Theory of Number Fields}
  ...
  \subsection*{Dirichlet's Unit Theorem}
    ...
    \]
    called the \textbf{norm-one surface}\index{norm-one surface} and \textbf{trace-zero hyperplane}\index{trace-zero hyperplane} respectively. Let $\l_{K}$ denote the restriction of $\log_{K}$ to $\mc{O}_{K}^{\ast}$ and set
    \[
