\chapter{The Theory of Number Fields}
  ...
  \section{Ramification}
    ...
    \begin{theorem}
      ...
      \[
        \conj{m_{\a}}(x) = \prod_{1 \le i \le r}\conj{m_{i}}(x)^{e_{i}},
      \]
      with $\conj{m_{i}}(x) \in \F_{p}[x]$ and $e_{i} \ge 0$, be the prime factorization of $\conj{m_{\a}}(x)$ in $\F_{p}[x]$. Let $m_{i}(x) \in \Z[x]$ be such that $m_{i}(x) \equiv \conj{m_{i}}(x) \tmod{p}$ and set
      \[
        \mf{p}_{i} = (p,m_{i}(\a)),
      \]
    ...
    \end{theorem}
    \begin{proof}
      Since $m_{\a}(x)$ is the minimal polynomial for $\a$, we have an isomorphism $\Z[\a] \cong \Z[x]/(m_{\a}(x))$. Then we have the chain of isomorphism
      \[
        \mc{O}_{K}/p\mc{O}_{K} \cong (\Z[x]/(m_{\a}(x)))/(p\Z[x]/(m_{\a}(x))) \cong \Z[x]/(p,m_{\a}(x)) \cong \F_{p}[x]/(\conj{m_{\a}(x)}),
      \]
      where the second and third isomorphisms follow by taking $\Z[x]/(p,m_{\a}(x))$ and first reducing $\Z[x]$ modulo $m_{\a}(x)$ or $p$ respectively. Now set $A = \F_{p}[x]/(\conj{m_{\a}(x)})$. Then the Chinese remainder theorem gives an isomorphism
      \[
        A \cong \bigop_{1 \le i \le r}\F_{p}/(\conj{m_{i}}(x)^{e_{i}}).
      \]
      \todo{xxx}
    \end{proof}
  \section{Lattices}
