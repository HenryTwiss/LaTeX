\chapter{The Theory of Number Fields}
  ...
  \section{Minkowski Space}
    ...
    \begin{center}
      \begin{tikzcd}
        ...
        \Q \arrow[hookrightarrow]{r} & \R \arrow[hookrightarrow]{r} & \R
      \end{tikzcd}
      \qquad
      \begin{tikzcd}
        K^{\ast} \arrow{d}{\Norm} \arrow{r}{j} & K_{\R} \arrow{d}{\Norm_{K_{\R}}}^{\ast} \arrow{r}{\log_{K}} & \R^{n} \arrow{d}{\Norm_{\R^{n}/\Q}} \\
        \Q^{\ast} \arrow[hookrightarrow]{r} & \C^{\ast} \arrow{r}{\d_{\s}\log|\cdot|} & \R
      \end{tikzcd}
    \end{center}
  ...
  \section{\todo{Dirichlet's Unit Theorem}}
    By abuse of language, we call any element $\mc{O}_{K}^{\ast}$ a \textbf{unit}\index{unit} of $K$. That is, the units are the elements of the ring of integers $\mc{O}_{K}$ which are themselves invertible in $\mc{O}_{K}$. By \cref{cor:unit_if_and_only_if_absolute_value_norm_1}, we have
    \[
