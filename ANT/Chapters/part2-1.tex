\chapter{The Theory of Number Fields}
  ...
  \section{Minkowski Space}
    ...
    \end{proof}
  \section{\todo{The Class Number \& Dirichlet's Unit Theorem}}
  ...
  \subsection*{Dirichlet's Unit Theorem}
    ...
    \]
    called the \textbf{norm-one surface}\index{norm-one surface} of $K_{\R}$ and the \textbf{trace-zero hyperplane}\index{trace-zero hyperplane} of $\R^{n}$ respectively. Let $\l_{K}$ denote the restriction of $\log_{K}$ to $\mc{O}_{K}^{\ast}$ and set
    \[
      \L = \log_{K}(\mc{O}_{K}^{\ast}),
    \]
    so that $\L$ is the image of $\l_{K}$. We call $\L$ the \textbf{logarithmic unit lattice}\index{logarithmic unit lattice} of $K$. Observe that $\L \subset H$ since the trace of $\l_{K}(\e) = \log_{K}(\e)$, for any $\e \in \mc{O}_{K}^{\ast}$, is just the logarithm of $\Norm(\e) = 1$. We first show that the logarithmic embedding fits into an exact sequence:
    ...
    \begin{proposition}
      ...
      \begin{center}
        \begin{tikzcd}
          1 \arrow{r} & \mu(K) \arrow{r} & \mc{O}_{K}^{\ast} \arrow{r}{\l_{K}} & \L \arrow{r} & 0,
        \end{tikzcd}
      \end{center}
    ...
    Our aim now is to show that the logarithmic unit lattice $\L$ is a a free abelian group of rank $r_{1}+r_{2}-1$. For this, we will require a lemma:
    ...
    Recall that $\L \subset H$. We now show that $\L$ is a complete lattice in $H$ and compute its rank as a free abelian group:

    \begin{theorem}
      Let $K$ be a number field of degree $n$ and signature $(r_{1},r_{2})$. Then the logarithmic unit lattice $\L$ is a complete lattice in the trace-zero hyperplane $H$ of $\R^{n}$. In particular, $\L$ is a free abelian group of rank $r_{1}+r_{2}-1$.
    \end{theorem}
    \begin{proof}
      \todo{xxx}
    \end{proof}
