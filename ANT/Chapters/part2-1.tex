\chapter{The Theory of Number Fields}
  ...
  \section{Integral \& Fractional Ideals}
    ...
    \begin{proposition}\label{prop:explicit_inverse_ideal}
      ...
      \]
      In particular, if $\mc{O}_{K} \subseteq \mf{f}$, then $\mf{f}^{-1}$ is an integral ideal.
    \end{proposition}
    \begin{proof}
      ...
      \]
      This proves the first statement. For the second statement suppose $\mc{O}_{K} \subseteq \mf{f}$. Then if $\a \in \mf{f}^{-1}$, we have $\a\mf{f} \subseteq \mc{O}_{K}$ and in particular $\a \in \mc{O}_{K}$. Therefore $\mf{f} \subseteq \mc{O}_{K}$. Moreover, $\a\mc{O}_{K} \subseteq \mc{O}_{K} \subseteq \mf{f}$. It follows that $\mf{f}$ is an integral ideal.
    \end{proof}
  ...
  \section{\todo{Lattices}}
    ...
    We define the \textbf{different ideal}\index{different ideal} $\mf{D}$ of $K$ by
    \[
    ...
    \]
    \todo{xxx}
  \section{\todo{Ramification \& The Different}}
