\chapter{The Theory of Number Fields}
  ...
  \section{Minkowski Space}
    ...
    \]
    where $\rho_{1},\ldots,\rho_{r_{1}}$ are the real $\Q$-embeddings and $\tau_{1},\ldots,\tau_{r_{2}}$ are representatives of pairs of complex $\Q$-embeddings. The canonical embedding $\s_{K}$ is then a $\Q$-embedding of $K$ into $\R^{n}$ since it is the composition of the $\Q$-embedding $j$ (whose image under $K$ is in $K_{\R}$ as we have noted), the isomorphism established by \cref{prop:Minkowski_space_isomorphism}, and the isomorphism $\prod_{\s}\R \cong \R^{n}$. It is also independent of the choice of representatives $\tau_{1},\ldots,\tau_{r_{2}}$ since the complex embeddings occur in conjugate pairs. There is an associated map on $K^{\ast}$. We define the \textbf{logarithmic embedding}\index{logarithmic embedding} $\log_{K}$ of $K$ by
    \[
      \log_{K}:K^{\ast} \to \C^{n} \qquad \k \to (\log|\rho_{1}(\k)|,\ldots,\log|\rho_{r_{1}}(\k)|,2\log|\tau_{1}(\k)|,\ldots,2\log|\tau_{r_{2}}(\k)|)
    \]
    Then $\log_{K}$ is just the restriction of $j$ to $K$ composed with $\d_{\s}\log|\cdot|$ at each component. Since $\d_{\s}\log|\cdot|$ is a homomorphism and $j$ is an embeddings, $\log_{K}$ is a homomorphism. All of these maps are related via the following commutative diagrams:

    \begin{center}
      \begin{tikzcd}
        K \arrow{d}{\Trace} \arrow{r}{j} & K_{\R} \arrow{d}{\Trace_{K_{\R}}} \arrow{r}{\s_{K}} & \R^{n} \arrow{d}{\Trace_{\R^{n}/\Q}} \\
        \Q \arrow{r} & \R \arrow{r} & \R
      \end{tikzcd}
      \qquad
      \begin{tikzcd}
        K^{\ast} \arrow{d}{\Norm} \arrow{r}{j} & K_{\R} \arrow{d}{\Norm_{K_{\R}}}^{\ast} \arrow{r}{\log_{K}} & \R^{n} \arrow{d}{\Norm_{\R^{n}/\Q}} \\
        \Q^{\ast} \arrow{r} & \C^{\ast} \arrow{r}{\d_{\s}\log|\cdot|} & \R
      \end{tikzcd}
    \end{center}
    
    Since $\s_{K}$ is an embedding and any fractional ideal $\mf{f}$ is a complete lattice, $\s_{K}(\mf{f})$ is a complete lattice in $\R^{n}$. We now determine the covolume of $\s_{K}(\mf{f})$:
    
    \begin{proposition}\label{prop:covolume_of_fractional_ideal_under_canonical_embedding}
  ...
  \section{\todo{The Ideal Class \& Unit Groups}}
    ...
    \[
      h_{K} = |\Cl(K)|.
    \]
    Now for the second group. The \textbf{unit group}\index{unit group} of $K$ is defined to be $\mc{O}_{K}^{\ast}$. That is, the unit group is the group of units in the ring of integers of $K$. By abuse of language, we call any element $\mc{O}_{K}^{\ast}$ a \textbf{unit}\index{unit} of $K$. By \cref{cor:unit_if_and_only_if_absolute_value_norm_1}, we have
    \[
      \mc{O}_{K}^{\ast} = \{\e \in \mc{O}_{K}:\Norm(\e) = \pm 1\}.
    \]
    The the ideal class and unit groups of $K$ are related via the following exact sequence:
    ...
    Since the middle map takes any $\k \in K^{\ast}$ to its associated principal ideal $\k\mc{O}_{K}$, we can think of this as passing from numbers to ideals. Then exactness means that unit group is measuring the contraction that takes place during this process while the class group is measuring the expansion.
  ...
  \subsection*{Dirichlet's Unit Theorem}
    Another very important piece of arithmetic data about a number field $K$ other than its class number is the structure of $\mc{O}_{K}^{\ast}$. Our goal will be to show that $\mc{O}_{K}^{\ast}$ is a direct product of a subgroup of the roots of unity and a certain finitely generated free group. Determining the exact rank of the free group will be the most difficult part of the proof. We will make use of two subsets
    \[
      S = \{\mathbf{x} \in K_{\R}:\Norm_{K_{\R}}(\mathbf{x}) = \pm 1\} \quad \text{and} \quad H = \{\mathbf{x} \in \R^{n}:\Trace_{\R^{n}/\Q}(\mathbf{x}) = 0\},
    \]
    called the \textbf{norm-one surface}\index{norm-one surface} and \textbf{trace-zero hyperplane}\index{trace-zero hyperplane} respectively. We set
    \[
      \L = \log_{K}(\mc{O}_{K}^{\ast}),
    \]
    so that $\L$ is the group of units of $K$ under the logarithmic embedding. Moreover, we have $\L \subset H$ because $\log|\tau(\k)| = \log|\conj{\tau}(\k)|$ and $\log|1| = 0$. \todo{xxx}
