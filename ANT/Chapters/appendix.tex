...
\chapter{Analysis}
  ...
  \section{Whittaker Functions}\label{append:Whittaker_Functions}
    ...
    \]
    Throughout we assume $-2\mu \notin \Z_{\ge 1}$. There are two linearly independent solutions to this equation. If we additionally assume that $w(z) = o(e^{2\pi\Im(z)})$ as $\Im(z) \to \infty$, then there is only one linearly independent solution. This solution is the \textbf{Whittaker function}\index{Whittaker function} $W_{\k,\mu}(z)$. It can be expressed in the form
    \[
    ...
    \begin{lemma}\label{lem:Whittaker_function_asymptotic}
      Let $\k,\mu \in \C$ with $-2\mu \notin \Z_{\ge 1}$ and $-2(\mu-\k) \notin \Z_{\ge 1}$. Then for $\Re(z) > 0$,
      \[
