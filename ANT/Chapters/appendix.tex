...
\chapter{Analysis}
  ...
  \section{The Phragm\'en-Lindel\"of Convexity Principle for a Strip}\label{append:The_Phragmen_Lindelof_Convexity_principle}
    ...
    \begin{theorem}[Phragm\'en-Lindel\"of Convexity principle for a strip]\label{thm:Phragmen-Lindelof_convexity_principle}
      Suppose $f$ is a holomorphic functions on an open neighborhood of the vertical strip $a < \s < b$ with $f(s) \ll e^{|s|^{A}}$ for some $A \ge 0$. Then the following hold:
      \begin{enumerate}[label=(\roman*)]
        \item If $|f(s)| \le M$ for $\s = a,b$, that is on the boundary edges of the strip, then $|f(s)| \le M$ for all $z$ in the strip.
        \item Assume that there is a continuous function $g(r):\R \to \R$ such that
        \[
          f(a+it) \ll_{a} g(r)^{\a} \quad \text{and} \quad f(b+it) \ll_{b} g(r)^{\b},
        \]
        for all $r \in \R$. Then
        \[
          f(s) \ll_{\s} g(r)^{\a\ell(\s)+\b(1-\ell(\s))},
        \]
