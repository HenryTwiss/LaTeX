\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Hecke \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      where $a_{f}(n) = \l_{f}(n)n^{-\frac{k-1}{2}}$ (here we are abusing notation for the Fourier coefficeints of holomorphic forms). We will see that $L(s,f)$ is a Selberg class $L$-function. By the Ramanujan-Petersson conjecture for holomorphic, $a_{f}(n) \ll n^{\e}$. So $L(s,f)$ is locally absolutely uniformly convergent for $\Re(s) > 1+\e$ and hence locally absolutely uniformly convergent for $\Re(s) > 1$. The $L$-function will also have an Euler product. Indeed, the Hecke relations imply that the coefficients $a_{f}(n)$ are multiplicative and satisfy
    ...
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      We now want to find an integral representation for $L(s,f)$. Consider the following Mellin tranform:
      \[
    ...
    \subsection*{The Functional Equation \& Critical Strip of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      with $\k_{1} = k-1$ and $\k_{2} = k+1$ the local roots at infinity. Clearly they satisfy the required bounds. The conductor is $q(f) = N$, so the primes dividing the level ramify, and by the Ramanujan-Petersson conjecture for holomorphic forms $\a_{i}(p) \neq 0$ for $i = 1,2$ and all primes $p \nmid N$. The completed $L$-function is
      \[
  ...
  \section{Hecke-Maass \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      where $a_{f}(n) = \l_{f}(n)$ (here we are abusing notation for the Fourier coefficeints of Maass forms). As the Ramanujan-Petersson conjecture for Maass forms is not known, $L(s,f)$ has not been proven to be a Selberg class $L$-function. Although, it is conjectured to be so throughout we will assume the Ramanujan-Petersson conjecture for Maass forms. By the Ramanujan-Petersson conjecture for Maass forms, $a_{f}(n) \ll n^{\e}$. So $L(s,f)$ is locally absolutely uniformly convergent for $\Re(s) > 1+\e$ and hence locally absolutely uniformly convergent for $\Re(s) > 1$. The $L$-function will have an Euler product. Indeed, the Hecke relations imply that the coefficients $a_{f}(n)$ are multiplicative and satisfy \cref{equ:primitive_Hecke_eigenform_recurrence_for_coefficients_of_modular_L-function}. Mimicing the argument exactly as for Hecke $L$-functions, 
      \[
        L(s,f) = \prod_{p \nmid N}(1-a_{f}(p)p^{-s}+\chi(p)p^{-2s})^{-1}\prod_{p \mid N}(1-a_{f}(p)p^{-s})^{-1}.
      \]
      If $p \nmid N$, let $\a_{1}(p)$ and $\a_{2}(p)$ be the roots of $1-a_{f}(p)p^{-s}+\chi(p)p^{-2s}$. That is,
    ...
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      Now consider the following Mellin tranform:
      \[
    ...
    \subsection*{The Functional Equation \& Critical Strip of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      with $\k_{1} = \mf{a}+it$ and $\k_{2} = \mf{a}-it$ the local roots at infinity. Clearly they satisfy the required bounds. The conductor is $q(f) = N$, so the primes dividing the level ramify, and by the Ramanujan-Petersson conjecture for Maass forms $\a_{i}(p) \neq 0$ for $i = 1,2$ and all primes $p \nmid N$. The completed $L$-function is
      \[
      ...
      \begin{theorem}
        For any primitive Hecke-Maass eigenform $f \in \mc{C}_{t}(N,\chi)$, $L(s,f)$ is a Selberg class $L$-function provided the Ramanujan-Petersson conjecture for Maass forms holds. For $\Re(s) > 1$, it has a locally absolutely uniformly convergent degree $2$ Euler product given by 
        \[
          L(s,f) = \prod_{p \nmid N}(1-a_{f}(p)p^{-s}+\chi(p)p^{-2s})^{-1}\prod_{p \mid N}(1-a_{f}(p)p^{-s})^{-1} = \prod_{p}(1-\a_{1}(p)p^{-s})^{-1}(1-\a_{2}(p)p^{-s})^{-1}.
        \]
        Moreover, it admits analytic continuation to $\C$ via the integral representation
  ...
  \section{The Rankin-Selberg Method}
    ...
    \subsection*{The Functional Equation, Critical Strip \& Residues of \texorpdfstring{$L(s,f \ox g)$}{L(s,f \ox g)}}
      ...
      \]
      with $\mu_{1,1} = k-1$, $\mu_{2,2} = k$, $\mu_{1,2} = 0$, and $\mu_{2,1} = 1$ the local roots at infinity. Clearly they satisfy the required bounds. The completed $L$-function is
      \[
    ...
    \subsection*{The Rankin-Selberg Method}
      The Rankin-Selberg method in general is much more comlicated, but the argument is essentially the same. Let $f$ and $g$ both be primitive Hecke or Heck-Maass eigenforms with Fourier coefficients $\l_{f}(n)$ and $\l_{g}(n)$ respectively. We suppose $f$ has weight $k$/eigenvalue $\l$, level $N$, and character $\chi$, and $g$ has weight $\ell$/eigenvalue $\nu$, level $M$, and character $\psi$. 
      The $L$-function $L(s,f \x g)$ of $f$ and $g$ is give by the $L$-series
      \[
  ...
  \section{Applications of the Rankin-Selberg Method}
    \subsection*{The Ramanujan-Petersson Conjecture on Average}
      Let $f$ be a Hecke or Hecke-Maass eigenform. As an application of the Rankin-Selberg method, it is possible to show the weaker result that $a_{f}(n) \ll n^{\e}$ holds on average, for any $\e > 0$, without assuming the corresponding Ramanujan-Petersson conjecture:
      ...
      The bound in \cref{prop:Ramanujan_Petersson_average} should be compared with the implication $\l_{f}(n) \ll n^{\frac{k-1}{2}+\e}$ that follows from the corresponding Ramanujan-Petersson conjecture. While \cref{prop:Ramanujan_Petersson_average} is not useful in the holomorphic form case, it is in the Maass form case. Indeed, recall that if $f$ is a primitive Hecke-Maass eigenform we needed to assume the Ramanujan-Petersson to ensure $a_{f}(n) \ll n^{\e}$ so that $L(s,f)$ was locally absolutely uniformly convergent for $\Re(s) > 1$. However, \cref{prop:Dirichlet_series_convergence_polynomial_bound,prop:Ramanujan_Petersson_average} together imply that $L(s,f)$ is locally absolutely uniformly convergent for $\Re(s) > 1+\e$ and hence for $\Re(s) > 1$. This method avoides assuming the Ramanujan-Petersson conjecture for Maass forms.
    \subsection*{Strong Multiplicity One}
      Let $f$ be a primitive Hecke or Hecke-Maass eigenform. Recall that the corresponding multiplicty one theorem states that $f$ is determined by its Fourier coefficients. Actually, since the Fourier coefficeints are multiplicative the corresponding Hecke relations imply that $f$ is determined by its Fourier coefficeints at primes. The \textbf{strong multiplicity one}\index{strong multiplicity one} theroem says that we can do better:

      \begin{theorem}[Strong multiplicity one, holomorphic and Maass versions]
        Let $f$ and $g$ both be primitive Hecke or Hecke-Maass eigenforms with character $\chi$ and Fourier coefficients $\l_{f}(n)$ and $\l_{g}(n)$. If $\l_{f}(p) = \l_{g}(p)$ for all but finitely many primes $p$, then $f = g$.
      \end{theorem}
      \begin{proof}
        Let $S$ be the set the primes for which $\l_{f}(p) \neq \l_{g}(p)$ and including the primes that ramify for $L(s,f)$ and $L(s,g)$. By assumption, $S$ is finite. As the local factors of $L(s,f \ox g)$ are holomorphic and nonzero at $s = 1$, the order of the pole of $L(s,f \ox g)$ is the same as the order of the pole of
        \[
          L(s,f \ox g)\prod_{p \in S}L_{p}(s,f \ox g)^{-1} = \prod_{p \notin S}L_{p}(s,f \ox g).
        \]
        But as $\l_{f}(p) = \l_{g}(p)$ and $\chi$ is the same character for both $f$ and $g$, we have
        \[
