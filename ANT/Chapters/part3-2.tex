\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Riemann Zeta Function}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$\z(s)$}{\z(s)}}
      ...
      \[
        \z(s) = \prod_{p}(1-p^{-s})^{-1},
      \]
      in this region as well. The local factor at $p$ is $\z_{p}(s) = (1-p^{-s})^{-1}$ with local root $1$. We package this into a theorem:

      \begin{theorem}
        For $\Re(s) > 1$, $\z(s)$ is locally absolutely uniformly convergent with degree $1$ Euler product:
        \[
          \z(s) = \prod_{p}(1-p^{-s})^{-1}.
        \]
      \end{theorem}
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,\chi)$}{L(s,\chi)}}
      ...
      \end{equation}
      From this relation, any Dirichlet $L$-function $L(s,\chi)$ will also posess any properties $L(s,\wtilde{\chi})$ does up to local factors at primes $p$ dividing $m$. Therefore we may assume $\chi$ is primitive. We may also assume $q > 1$ for otherwise $\chi$ is the trivial character and so $L(s,\chi) = \z(s)$. We collect this work in the following theroem:

      \begin{theorem}
        Let $\chi$ be a primitive Dirichlet character with conductor $q > 1$. For $\Re(s) > 1$, $L(s,\chi)$ is locally absolutely uniformly convergent with degree $1$ Euler product:
        \[
          L(s,\chi) = \prod_{p}(1-\chi(p)p^{-s})^{-1} = \prod_{p \nmid q}(1-\chi(p)p^{-s})^{-1}.
        \]
      \end{theorem}
  ...
  \section{Hecke \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      where $a_{f}(n) = \l_{f}(n)n^{-\frac{k-1}{2}}$ (here we are abusing notation for the Fourier coefficeints of holomorphic forms). We will see that $L(s,f)$ is a Selberg class $L$-function. By the Ramanujan conjecture, $a_{f}(n) \ll n^{\e}$. So $L(s,f)$ is locally absolutely uniformly convergent for $\Re(s) > 1+\e$ and hence locally absolutely uniformly convergent for $\Re(s) > 1$. The $L$-function will also have an Euler product. Indeed, the Hecke relations imply that the coefficients $a_{f}(n)$ are multiplicative and satisfy
      ...
      \end{equation}
      We package this work into a theorem:
      \begin{theorem}
        Let $f \in \mc{S}_{k}(N,\chi)$ be a primitive Hecke eigenform. For $\Re(s) > 1$, $L(s,f)$ is locally absolutely uniformly convergent with degree $2$ Euler product:
        \[
    ...
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \end{align*}
      Substituting this result back into \cref{equ:symmetric_integral_holomorphic_split} with \cref{equ:integral_representation_holomorphic_1} yields the following result:
      ...
      This integral will give analytic continuation. To see this, we know everything outside the brackets is entire. The two integrals are locally absolutely uniformly bounded on $\C$ by \cref{met:decay_compacta_integral}. Hence we have analytic continuation to all of $\C$. In particular, we have shown that $L(s,f)$ has no poles.
    \subsection*{The Functional Equation \& Critical Strip of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      with $\k_{1} = k-1$ and $\k_{2} = k+1$ the local roots at infinity. Clearly they satisfy the required bounds. The conductor is $q(f) = N$, so the primes dividing the level ramify, and by the Ramanujan conjecture $\a_{i}(p) \neq 0$ for $i = 1,2$ and all primes $p \nmid N$. The completed $L$-function is
      \[
  ...
  \section{Hecke-Maass \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      where $a_{f}(n) = \l_{f}(n)$ (here we are abusing notation for the Fourier coefficeints of Maass forms). As the Ramanujan-Petersson conjecture is not known, $L(s,f)$ has not been proven to be a Selberg class $L$-function. Although, it is conjectured to be so throughout we will assume the Ramanujan-Petersson conjecture is known. By the Ramanujan conjecture, $a_{f}(n) \ll n^{\e}$. So $L(s,f)$ is locally absolutely uniformly convergent for $\Re(s) > 1+\e$ and hence locally absolutely uniformly convergent for $\Re(s) > 1$. The $L$-function will have an Euler product. Indeed, the Hecke relations imply that the coefficients $a_{f}(n)$ are multiplicative and satisfy \cref{equ:primitive_Hecke_eigenform_recurrence_for_coefficients_of_modular_L-function}. Repeating the argument exactly as for Hecke $L$-functions, 
      \[
        L(s,f) = \prod_{p \nmid N}(1-a_{f}(p)p^{-s}+\chi(p)p^{-2s})^{-1}\prod_{p \mid N}(1-a_{f}(p)p^{-s})^{-1}.
      \]
      If $p \nmid N$, let $\a_{1}(p)$ and $\a_{2}(p)$ be the roots of $1-a_{f}(p)p^{-s}+\chi(p)p^{-2s}$. That is,
      ...
      \[
        L(s,f) = \prod_{p}(1-\a_{1}(p)p^{-s})^{-1}(1-\a_{2}(p)p^{-s})^{-1}.
      \]
      The local factor at $p$ is $L_{p}(s,f) = (1-\a_{1}(p)p^{-s})^{-1}(1-\a_{2}(p)p^{-s})^{-1}$ with local roots $\a_{1}(p)$ and $\a_{2}(p)$. Moreover, just as in the case for Hecke $L$-functions, the formula \cref{equ:Hecke_L_function_coefficient_formula} also holds. We package this work into a theorem:
      \begin{theorem}
        Let $f \in \mc{C}_{t}(N,\chi)$ be a primitive Hecke-Maass eigenform. For $\Re(s) > 1$, $L(s,f)$ is locally absolutely uniformly convergent with degree $2$ Euler product:
        \[
          L(s,f) = \prod_{p \nmid N}(1-a_{f}(p)p^{-s}+\chi(p)p^{-2s})^{-1}\prod_{p \mid N}(1-a_{f}(p)p^{-s})^{-1} = \prod_{p}(1-\a_{1}(p)p^{-s})^{-1}(1-\a_{2}(p)p^{-s})^{-1},
        \]
        provided the Ramanujan-Petersson conjecture holds.
      \end{theorem}
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \end{align*}
      Substituting this result back into \cref{equ:symmetric_integral_Maass_split} with \cref{equ:integral_representation_Maass_1} yields the following result:

      \begin{theorem}
        Let $f \in \mc{C}_{t}(N,\chi)$ be a primitive Hecke-Maass eigenform. For $\Re(s) > 1$,
        \begin{align*}
      ...
      This integral will give analytic continuation. Indeed, everything outside the brackets is entire and the two integrals are locally absolutely uniformly bounded on $\C$ by \cref{met:decay_compacta_integral}. Hence we have analytic continuation to all of $\C$. In particular, $L(s,f)$ has no poles.
    \subsection*{The Functional Equation \& Critical Strip of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      An immediate consequence of applying the symmetry $s \to 1-s$ to the integral representation is the following functional equation:
      \[
      ...
      \]
      with $\k_{1} = \mf{a}+it$ and $\k_{2} = \mf{a}-it$ the local roots at infinity. Clearly they satisfy the required bounds. The conductor is $q(f) = N$, so the primes dividing the level ramify, and by the Ramanujan-Petersson conjecture $\a_{i}(p) \neq 0$ for $i = 1,2$ and all primes $p \nmid N$. The completed $L$-function is
      \[
      ...
      \begin{theorem}
        For any primitive Hecke-Maass eigenform $f \in \mc{C}_{t}(N,\chi)$, $L(s,f)$ is a Selberg class $L$-function provided the Ramanujan-Petersson conjecture holds. It admits analytic continuation to $\C$ via the integral representation
        \begin{align*}
          L(s,f) &= \frac{2^{2-\mf{a}}\pi^{s}}{\G\left(\frac{s+\mf{a}+it}{2}\right)\G\left(\frac{s+\mf{a}-it}{2}\right)} \\
          &\cdot \left[w_{N}(f)(-1)^{\mf{a}}N^{\frac{1}{2}-s}\int_{\frac{1}{\sqrt{N}}}^{\infty}\left(\left(\frac{\del}{\del x}^{\mf{a}}\right)\conj{f}\right)(iy)y^{(1-s)-\frac{1}{2}+\mf{a}}\,\frac{dy}{y}+\int_{\frac{1}{\sqrt{N}}}^{\infty}\left(\frac{\del}{\del x}^{\mf{a}}f\right)(iy)y^{s-\frac{1}{2}+\mf{a}}\,\frac{dy}{y}\right].
        \end{align*}
  ...
  \section{The Rankin-Selberg Method}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,f \ox g)$}{L(s,f \ox g)}}
      The Rankin-Selberg method is a process by which we can construct new $L$-functions from old ones. We discuss the method only in the simplest case since many technical difficulties arise in the fully general setting. To somewhat combat not working in full generality, we make remarks where the theory needs to be adjusted in more general situations. Let $f,g \in \mc{S}_{k}(\PSL_{2}(\Z))$ be primitive Hecke eigenforms. Let the Fourier series for $f$ and $g$ be given by
      \[
      ...
      \]
      The local factor at $p$ is $L_{p}(s,f \ox g) = \prod_{1 \le i,j \le 2}\left(1-\a_{i}(p)\conj{\b_{j}(p)}p^{-s}\right)^{-1}$. We package this into a theorem:

      \begin{theorem}
        Let $f,g \in \mc{S}_{k}(\PSL_{2}(\Z))$ be primitive Hecke eigenforms. For $\Re(s) > 1$, $L(s,f \ox g)$ is locally absolutely uniformly convergent with degree $4$ Euler product:
        \[
          L(s,f \ox g) = \prod_{p}\prod_{1 \le i,j \le 2}\left(1-\a_{i}(p)\conj{\b_{j}(p)}p^{-s}\right)^{-1}.
        \]
      \end{theorem}
    ...
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f \ox g)$}{L(s,f \ox g)}: Part I}
      ...
      \]
      This will turn out to be a Mellin transform as we will soon see.
    ...
    \subsection*{The Fourier Series and Functional Equation of \texorpdfstring{$E_{\infty}(z,s)$}{E_{\infty}(z,s)}}
      ...
      Having computed the Fourier coefficients, we would like to meromorphically continue $E_{\infty}(z,s)$ in $s$ past the region $\Re(s) > 1$ (this follows from \cref{thm:meromorphic_continuation_of_Eisenstein_series} but we will demonstrate a full proof for the single Eisenstein series on $\PSL_{2}(\Z)\backslash\H$). From the computation of the Fourier coefficients we will have possible poles coming from their denominator. To remove this difficultly, we will multiply by a factor to clear the poles. In turn, this will give us a functional equation as $s \to 1-s$. This factor will be the completed zeta function $\L(2s,\z) = \pi^{-s}\G(s)\z(2s)$ scaled by $2$ in $s$. We define the the \textbf{completed real-analytic Eisenstein series}\index{completed real-analytic Eisenstein series} $E^{\ast}(z,s)$ by
      \[
      ...
      \]
      We already knew $E_{\infty}(z,s)$ possessed a functional equation as $s \to 1-s$ from \cref{thm:functional_equation_of_Eisenstein_series} but we needed an explicit version in order to understand $L(s,f \ox g)$.
    ...
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f \ox g)$}{L(s,f \ox g)}: Part II}
      We can now continue with the Rankin-Selberg convolution $L(s,f \ox g)$. Writing \cref{equ:Rankin-Selberg_integral-reresentation} in terms of $E^{\ast}(z,s)$ and $L(s,f \ox g)$ gives the following result:
