\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,\chi)$}{L(s,\chi)}}
      ...
      \]
      Since $\chi(n) = 0$ if $(n,m) > 1$, the above sum can be restricted to all positive integers relatively prime to $m$. We will see that $L(s,\chi)$ is a Selberg class $L$-function if $\chi$ is primitive and of conductor $q > 1$ (in the case $q = 1$, $L(s,\chi) = \z(s)$). From now we make this assumption about $\chi$. As $|\chi(n)| \ll 1$, $L(s,\chi)$ is locally absolutely uniformly convergent for $\Re(s) > 1$. Because $\chi$ is completely multiplicative we also have the degree $1$ Euler product,
      \[
    ...
    \subsection*{Dirichlet's Theta Function \texorpdfstring{$\vt_{\chi}(s)$}{\vt_{\chi}(s)}}
      ...
      \begin{theorem}[Functional equation for Dirichlet's theta function]
        Let $\chi$ be a primitive Dirichlet character of conductor $q > 1$. For $\Re(s) > 0$,
        \[
    ...
    \subsection*{The Functional Equation \& Critical Strip of \texorpdfstring{$L(s,\chi)$}{L(s,\chi)}}
      ...
      \begin{theorem}\label{thm:primitive_Dirichlet_Selberg}
        For any primitive Dirichlet character $\chi$ of conductor $q > 1$, $L(s,\chi)$ is a Selberg class $L$-function. For $\Re(s) > 1$, it has a locally absolutely uniformly convergent degree $1$ Euler product given by
        \[
          L(s,\chi) = \prod_{p}(1-\chi(p)p^{-s})^{-1} = \prod_{p \nmid q}(1-\chi(p)p^{-s})^{-1}.
        \]
  ...
  \section{Hecke \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \[
        f(z) = \sum_{n \ge 1}a_{f}(n)e^{2\pi inz}.
      \]
      The \textbf{Hecke $L$-function}\index{Hecke $L$-function} $L(s,f)$ of $f$ is defined as an $L$-series:
      \[
        L(s,f) = \sum_{n \ge 1}\frac{A_{f}(n)}{n^{s}} = \sum_{n \ge 1}\frac{a_{f}(n)}{n^{s+\frac{k-1}{2}}},
      \]
      where $A_{f}(n) = a_{f}(n)n^{-\frac{k-1}{2}}$. We will see that $L(s,f)$ is a Selberg class $L$-function if $f$ is a primitive Hecke eigenforms. From now on, we make this assumption about $f$ and denote its Fourier series by
      \[
        f(z) = \sum_{n \ge 1}\l_{f}(n)e^{2\pi inz},
      \]
      where $\l_{f}(n)$ are the Hecke eigenvalues of $f$. By the Ramanujan-Petersson conjecture, $A_{f}(n) \ll n^{\e}$. So $L(s,f)$ is locally absolutely uniformly convergent for $\Re(s) > 1+\e$ and hence locally absolutely uniformly convergent for $\Re(s) > 1$. The $L$-function will also have an Euler product. Indeed, the Hecke relations imply that the coefficients $A_{f}(n)$ are multiplicative and satisfy
    ...
    \subsection*{Beyond Primitivity of of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \begin{proof}
        ...
        \]
        for some $g,h \in \mc{S}_{k}\left(\frac{N}{d},\chi\right)$. Note that $V_{p}h \in \mc{S}_{k}\left(\frac{Np}{d},\chi\right)$ by \cref{lem:twisted_holomorphic_lemma}. The claim now follows by induction on the level of $f$ and that $V_{p}$ clearly preserves primitive Hecke eigenforms.
      \end{proof}
  \section{Hecke-Maass \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      We will investigate the $L$-functions of Maass cusp forms. Let $f \in \mc{C}_{t}(N,\chi)$ be even or odd and denote its Fourier series by
      \[
        f(z) = \sum_{n \ge 1}a_{f}(n)\sqrt{y}K_{it}(2\pi ny)\SC(2\pi nx).
      \]
      The \textbf{Hecke-Maass $L$-function}\index{Hecke-Maass $L$-function} $L(s,f)$ of $f$ is defined as an $L$-series:
      \[
        L(s,f) = \sum_{n \ge 1}\frac{A_{f}(n)}{n^{s}} = \sum_{n \ge 1}\frac{a_{f}(n)}{n^{s}},
      \]
      where $A_{f}(n) = a_{f}(n)$. We will see that $L(s,f)$ is a Selberg class $L$-function if $f$ is a primitive Hecke-Maass eigenforms under the Ramanujan-Petersson conjecture. From now on, we make this assumption about $f$ and denote its Fourier series by
      \[
        f(z) = \sum_{n \ge 1}\l_{f}(n)\sqrt{y}K_{it}(2\pi ny)\SC(2\pi nx).
      \]
      where $\l_{f}(n)$ are the Hecke eigenvalues of $f$. In this case, the Ramanujan-Petersson conjecture is not know so $L(s,f)$ has not been proven to be a Selberg class $L$-function. Although, it is conjectured to be, so throughout we will make this additional assumption. By the Ramanujan-Petersson conjecture, $A_{f}(n) \ll n^{\e}$. So $L(s,f)$ is locally absolutely uniformly convergent for $\Re(s) > 1+\e$ and hence locally absolutely uniformly convergent for $\Re(s) > 1$. The $L$-function will have an Euler product. Indeed, the Hecke relations imply that the coefficients $A_{f}(n)$ are multiplicative and satisfy \cref{equ:primitive_Hecke_eigenform_recurrence_for_coefficients_of_modular_L-function}. Mimicking the argument exactly as for Hecke $L$-functions, 
      \[
        L(s,f) = \prod_{p \nmid N}(1-A_{f}(p)p^{-s}+\chi(p)p^{-2s})^{-1}\prod_{p \mid N}(1-A_{f}(p)p^{-s})^{-1}.
      \]
      If $p \nmid N$, let $\a_{1}(p)$ and $\a_{2}(p)$ be the roots of $1-A_{f}(p)p^{-s}+\chi(p)p^{-2s}$. That is,
    ...
    \subsection*{The Functional Equation \& Critical Strip of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \begin{theorem}\label{equ:thm:primitive_Hecke-Maass_Selberg}
        For any even or odd primitive Hecke-Maass eigenform $f \in \mc{C}_{t}(N,\chi)$, $L(s,f)$ is a Selberg class $L$-function provided the Ramanujan-Petersson conjecture holds. For $\Re(s) > 1$, it has a locally absolutely uniformly convergent degree $2$ Euler product given by 
        \[
          L(s,f) = \prod_{p \nmid N}(1-A_{f}(p)p^{-s}+\chi(p)p^{-2s})^{-1}\prod_{p \mid N}(1-A_{f}(p)p^{-s})^{-1} = \prod_{p}(1-\a_{1}(p)p^{-s})^{-1}(1-\a_{2}(p)p^{-s})^{-1}.
        \]
        Moreover, it admits analytic continuation to $\C$ via the integral representation
    ...
    \subsection*{Beyond Primitivity of of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \begin{theorem}\label{thm:analytic_continuation_Hecke-Maass}
        For any even or odd $f \in \mc{C}_{t}(\G_{1}(N))$, $L(s,f)$ admits analytic continuation to $\C$.
      \end{theorem}
      \begin{proof}
  ...
  \section{The Rankin-Selberg Method}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,f \ox g)$}{L(s,f \ox g)}}
      ...
      \[
        L(s,f \x g) = \sum_{n \ge 1}\frac{A_{f \x g}(n)}{n^{s}} = \sum_{n \ge 1}\frac{A_{f}(n)\conj{A_{g}(n)}}{n^{s}} = \sum_{n \ge 1}\frac{\l_{f}(n)\conj{\l_{g}(n)}}{n^{s+k-1}},
      \]
      ...
      \[
        \sum_{n \ge 0}\frac{A_{f}(p^{n})\conj{A_{g}(p^{n})}}{p^{ns}} = \prod_{1 \le i,j \le 2}\left(1-\a_{i}(p)\conj{\b_{j}(p)}p^{-s}\right)^{-1}.
      \]
    ...
    \subsection*{The Rankin-Selberg Method}
      The Rankin-Selberg method is much more complicated in general, but the argument is essentially the same. Let $f$ and $g$ both be primitive Hecke or Heck-Maass eigenforms with Hecke eigenvalues $\l_{f}(n)$ and $\l_{g}(n)$ respectively. We suppose $f$ has weight $k$/eigenvalue $\l$, level $N$, and character $\chi$, and $g$ has weight $\ell$/eigenvalue $\nu$, level $M$, and character $\psi$. 
      The $L$-function $L(s,f \x g)$ of $f$ and $g$ is give by the $L$-series
      \[
      ...
      We make a few remarks about the Rankin-Selberg method. Local absolute uniform convergence for $\Re(s) > 1$ (we must assume the Ramanujan-Petersson conjecture in the Maass form case) are proved in the exactly the same way as we have described. The argument for the Euler product is also similar. However, if either $N > 1$ or $M > 1$, the computation becomes more difficult to compute since the local $p$ factors for $p \mid NM$ change. Moreover, the situation is increasingly complicated if $(N,M) > 1$ since conductor dropping can occur. The integral representation has a similar argument, but if the weights/eigenvalues are distinct the resulting Eisenstein series becomes more complicated. In particular, it is on $\G_{0}(NM)\backslash\H$ and if $NM > 1$, then there is more than just the cusp at $\infty$. Therefore the functional equation of the Eisenstein series at the $\infty$ cusp then reflects into a linear combination of Eisenstein series at the other cusps. This results in the requirement to compute the Fourier coefficients of all of these Eisenstein series. Moreover, this procedure can be generalized to remove the primitive Hecke and/or primitive Hecke-Maass eigenform conditions by taking linear combinations, but we won't attempt discussing this further. 
  \section{Applications of the Rankin-Selberg Method}
    \subsection*{The Ramanujan-Petersson Conjecture on Average}
      ...
      \begin{proposition}\label{prop:Ramanujan_Petersson_average}
        Let $f$ be a primitive Hecke or Hecke-Maass eigenform. Then for any $X > 1$, we have
        \[
