\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,\chi)$}{L(s,\chi)}}
      ...
      \end{equation}
      From this relation, any Dirichlet $L$-function $L(s,\chi)$ will also posess analytic continuation to $\C$ and a function equation as $s \to 1-s$, alongside its Euler product, if $L(s,\wtilde{\chi})$ does. Therefore we may assume $\chi$ is primitive. We may also assume $q > 1$ for otherwise $\chi$ is the trivial character and so $L(s,\chi) = \z(s)$. We collect this work in the following theroem:
