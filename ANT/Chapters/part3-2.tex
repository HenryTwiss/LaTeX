\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,\chi)$}{L(s,\chi)}}
      ...
      \end{equation}
      From this relation, any Dirichlet $L$-function $L(s,\chi)$ will also posess analytic continuation to $\C$ and a functional equation as $s \to 1-s$, alongside its Euler product, if $L(s,\wtilde{\chi})$ does. Therefore we may assume $\chi$ is primitive. We may also assume $q > 1$ for otherwise $\chi$ is the trivial character and so $L(s,\chi) = \z(s)$. We collect this work in the following theroem:
  ...
  \section{Hecke \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      We don't know \textit{a priori} that this integral defines an analytic function for $\Re(s) > 1$. In any case, we compute
      \begin{align*}
      ...
      \]
      Rewriting in terms of the Atkin–Lehner involution $\w_{N}f$ and recalling that $\w_{N}f = \w_{N}(f)\conj{f}$ by \cref{prop:Atkin_Lehner_conjugation_holomorphic}, we have
      \begin{align*}
  ...
  \section{Hecke-Maass \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \[
        \int_{0}^{\infty}f(iy)y^{s+\frac{1+\mf{a}}{2}}\,\frac{dy}{y}.
      \]
      \todo{xxx}
