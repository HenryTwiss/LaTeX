\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Hecke-Maass \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Integral Representation of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \begin{align*}
        ...
        &= \sum_{n \ge 1}\frac{a_{f}(n)}{(2\pi)^{s}n^{s}}\int_{0}^{\infty}K_{it}(y)y^{s+\mf{a}}\,\frac{dy}{y} &&\text{$y \to \frac{y}{2\pi n}$} \\
  ...
  \section{The Rankin-Selberg Method}
    ...
    \subsection*{The Fourier Series and Functional Equation of \texorpdfstring{$E_{\infty}(z,s)$}{E_{\infty}(z,s)}}
      ...
      \end{lemma}
      \begin{proof}
        If $\Re(s) > 1$ then the desired evaluation of the sum is locally absolutely uniformly convergent because the Riemann zeta function is in that region. Hence the sum will be too provided we prove the identity. Suppose $b = 0$. Then $r(0;m) = \vphi(m)$. Since $\vphi(m)$ is multiplicative we have
  ...
  \section{Applications of the Rankin-Selberg Method}
    ...
    \subsection*{Strong Multiplicity One}
      ...
      \begin{theorem}[Strong multiplicity one, holomorphic and Maass versions]
        Let $f$ and $g$ both be primitive Hecke or Hecke-Maass eigenforms of the same weight/eigenvalue, level, and character. Denote the Hecke eigenvalues by $\l_{f}(n)$ and $\l_{g}(n)$ respectively. If $a_{f}(p) = a_{g}(p)$ for all but finitely many primes $p$, then $f = g$.
      \end{theorem}
      \begin{proof}
