\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Applications of the Rankin-Selberg Method}
    \subsection*{The Ramanujan-Petersson Conjecture on Average}
      Let $f$ be a Hecke or Hecke-Maass eigenform. Using Rankin-Selberg convolutions, it is possible to show the weaker result that $a_{f}(n) \ll n^{\e}$ holds on average, for any $\e > 0$, without assuming the corresponding Ramanujan-Petersson conjecture:
    ...
    \subsection*{Strong Multiplicity One}
      Let $f$ be a primitive Hecke or Hecke-Maass eigenform. Recall that the corresponding multiplicty one theorem states that $f$ is determined by its Fourier coefficients. Actually, since the Fourier coefficeints are multiplicative the corresponding Hecke relations imply that $f$ is determined by its Fourier coefficeints at primes. The \textbf{strong multiplicity one}\index{strong multiplicity one} theroem for holomorphic or Maass forms says that we can do better:
