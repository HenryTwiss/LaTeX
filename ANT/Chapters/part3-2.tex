\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet \texorpdfstring{$L$}{L}-functions}
    \subsection*{The Definition \& Euler Product of \texorpdfstring{$L(s,\chi)$}{L(s,\chi)}}
      ...
      \end{equation}
      From this relation, any Dirichlet $L$-function $L(s,\chi)$ will also possess any properties $L(s,\wtilde{\chi})$ does up to local factors at primes $p$ dividing $m$. Therefore we may assume $\chi$ is primitive. We may also assume $q > 1$ for otherwise $\chi$ is the trivial character and so $L(s,\chi) = \z(s)$.
