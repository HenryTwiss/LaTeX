\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Hecke \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Functional Equation \& Critical Strip of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      with $\k_{1} = k-1$ and $\k_{2} = k+1$ the local roots at infinity. The conductor is $q(f) = N$, so the primes dividing the level ramify, and by the Ramanujan-Petersson conjecture for holomorphic forms $\a_{i}(p) \neq 0$ for $i = 1,2$ and all primes $p \nmid N$. The completed $L$-function is
      \[
  ...
  \section{Hecke-Maass \texorpdfstring{$L$}{L}-functions}
    ...
    \subsection*{The Functional Equation \& Critical Strip of \texorpdfstring{$L(s,f)$}{L(s,f)}}
      ...
      \]
      with $\k_{1} = \mf{a}+it$ and $\k_{2} = \mf{a}-it$ the local roots at infinity. The conductor is $q(f) = N$, so the primes dividing the level ramify, and by the Ramanujan-Petersson conjecture for Maass forms $\a_{i}(p) \neq 0$ for $i = 1,2$ and all primes $p \nmid N$. The completed $L$-function is
      \[
