\chapter{Types of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Rankin-Selberg Method}
    ...
    \subsection*{The Rankin-Selberg Method}
      The Rankin-Selberg method in general is much more comlicated, but the argument is essentially the same. Let $f$ and $g$ each be a primitive Hecke or Heck-Maass eigenform with Fourier coefficients $\l_{f}(n)$ and $\l_{g}(n)$ respectively. We suppose $f$ has weight $k$/eigenvalue $\l$, level $N$, and character $\chi$ and $g$ has weight $\ell$/eigenvalur $\nu$, level $M$, and character $\psi$. 
      The $L$-function $L(s,f \x g)$ of $f$ and $g$ is give by the $L$-series
      \[
      ...
      \begin{method}[Rankin-Selberg method]
        ...
        \begin{enumerate}[label=(\roman*)]
          \item $f$ is a primitive Hecke or Heck-Maass eigenform with weight $k$/eigenvalue $\l$, level $N$, and character $\chi$.
          \item $g$ is a primitive Hecke or Heck-Maass eigenform with weight $\ell$/eigenvalur $\nu$, level $M$, and character $\psi$.
        \end{enumerate}
      ...
      \begin{remark}
        If $f \in \mc{S}_{k}(N,\chi)$ and $g \in \mc{S}_{\ell}(M,\psi)$, the Euler product becomes more difficult to compute since the local $p$ factors for $p \mid NM$ change as either $\a_{2}(p) = 0$ or $\b_{2}(p) = 0$. Moreover, the situation is increasing complicated if $(N,M) > 1$ since conductor dropping can occur.
      \end{remark}
