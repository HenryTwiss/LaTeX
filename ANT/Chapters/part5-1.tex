\chapter{Moments of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Katz-Sarnak Philosophy}
    ...
    \subsection*{The Work of Katz \& Sarnak}
      ...
      Determining the symmetry type of a family is generally a difficult task. Below are some well-studied families and their symmetry types:
      \begin{center}
        \begin{stabular}[1.5]{|c|c|c|}
          \hline
          Symmetry Type & Family \\
          \hline
          \multirow{2}{*}{Unitary} & $\{L\left(\s+it,f\right):t \ge 0\}$ ordered by $t$ \\& $\{L(s,\chi):\text{primitive $\chi$ modulo $q$ with $q \ge 1$}\}$ ordered by $q$ \\
          \hline
          \multirow{4}{*}{Orthogonal} & $\{L(s,f):f \in \mc{S}_{k}(N,\chi), k \ge 1\}$ ordered by $k$ \\& $\{L(s,f):f \in \mc{S}_{k}(N,\chi), N \ge 1\}$ ordered by $N$ \\& $\{L(s,f):f \in \mc{C}_{\nu}(N,\chi), k \ge 1\}$ ordered by $k$ \\& $\{L(s,f):f \in \mc{C}_{\nu}(N,\chi), N \ge 1\}$ ordered by $N$ \\
          \hline
          Symplectic & $\{L(s,\chi_{d}):\text{$d$ a fundamental discriminant with $\chi_{d}(n) = \legendre{d}{n}$}\}$ ordered by $|d|$ \\
          \hline
        \end{stabular}
      \end{center}
