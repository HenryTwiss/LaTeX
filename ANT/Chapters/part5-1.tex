\chapter{Moments of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{The Katz-Sarnak Philosophy}}
    ...
    \subsection*{The Work of Montgomery \& Dyson}
      ...
      \]
      exists and what can be said about it. Montgomery conjectured the following (see \todo{cite} for a proof):
      ...
      Dyson noticed that \cref{conj:Montgomery_distribution_of_zeros_of_zeta} mimics a similar situation in random matrix theory. Consider an $N \x N$ unitary matrix $A \in U(N)$ with eigenvalue $e^{i\t}$ and corresponding eigenphase $\t$. Let $\Theta = (\t_{n})_{1 \le n \le N}$ denote the increasing sequence of eigenphases of $A$. Clearly the average density of the eigenphases in the interval $[0,2\pi)$ is $\frac{N}{2\pi}$. We consider the \textbf{two-point correlation function}\index{two-point correlation function} $F_{U}(\a,\b;A,N)$ for $A$ defined by
      \[
