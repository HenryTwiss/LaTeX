\chapter{Moments of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{\todo{Moments of \texorpdfstring{$L$}{L}-functions}}
    ...
    \begin{proof}
      ...
      \]
      for all $C > 0$. By \cref{lem:powerful_L-function_approximation_lemma}, we have
      \[
        L'(s,f) = -L(s,f)\left(\frac{r_{f}}{s}+\frac{r_{f}}{s-1}-\sum_{|s+\k_{j}| < 1}\frac{1}{s+\k_{j}}-\sum_{\substack{|s-\rho| < 1 \\ \rho \neq 0,1}}\frac{1}{s-\rho}+O(\log{\mf{q}(s,f)})\right),
      \]
      provided $-\frac{1}{2} \le \s \le 2$.
    \end{proof}
