  ...
  \section{\todo{Explicit Formulas for Chebyshev Functions}}
    \subsection*{The Explicit Formula for \texorpdfstring{$\psi(x)$}{$\psi(x)$}}
      ...
      \begin{proof}[Proof of the explicit formula for $\psi(x)$]
        ...
        \]
        where in the first estimate we have used the fact that $s \sim T$ and in the last estiamte we have used the choice of $c$. To estimate the remainder of the horizontal integrals, we need a bound for $\frac{\z'}{\z}(s)$ when $\s < -1$. To this end, write the functional equation for $\z(s)$ in the form
        \[
          \z(1-s) = \pi^{1-s}\frac{\G\left(\frac{s}{2}\right)}{\G\left(\frac{1-s}{2}\right)}\z(s),
        \]
        and take the logarithimic derivative to get
        \[
          \frac{\z'}{\z}(1-s) = -\log(\pi)+\frac{1}{2}\frac{\G'}{\G}\left(\frac{s}{2}\right)+\frac{1}{2}\frac{\G'}{\G}\left(\frac{1-s}{2}\right)+\frac{\z'}{\z}(s).
        \]
        Now let $s$ be such that $\s > 2$. The first term is contant and the four term is bounded by \cref{equ:Drichlet_series_log_derivative_zeta}. As for the digamma terms, \cref{equ:approximtion_for_digamma} implies $\frac{1}{2}\frac{\G'}{\G}\left(\frac{s}{2}\right) = O(\log|s|)$ and $\frac{1}{2}\frac{\G'}{\G}\left(\frac{1-s}{2}\right) = O(\log|2s|)$ \todo{??} where in the latter estimate we also make use of the fact that $1-s \sim s$. Putting these estimates together, we see that
        \[
          \frac{\z'}{\z}(s) \ll \log(1-s),
        \]
        for $\s < -1$.
      \end{proof}
