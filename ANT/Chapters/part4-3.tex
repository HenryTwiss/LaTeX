  ...
  \section{Prime Number Theorems}
    ...
    \subsection*{The Siegel–Walfisz Theorem}
      ...
      \begin{proof}
        ...
        \end{equation}
        When $\chi = \chi_{m,0}$, we have $\psi(x,\wtilde{\chi}) = \psi(x)$ and as $\psi(x,\chi) \sim \psi_{0}(x,\chi)$ for all $\chi$, substiuting \cref{equ:prime_number_theorem_zeta_1} into \cref{equ:Siegel-Walfisz_3} gives
        ...
        \end{equation}
        where we have used the simple estimates $\log(x) \ll \log(xm)$ and $\log(m) \ll \log(xm)$. Appealing to \cref{cor:explicit_formula_Dirichlet_corollary}...
      \end{proof}