  ...
  \section{Prime Number Theorems}
    ...
    \subsection*{The Siegel–Walfisz Theorem}
      ...
      \begin{proof}
        ...
        \end{equation}
        Now let $2 \le T \le x$. We wil estimate the expression for $\psi_{0}(x,\chi)$ given in \cref{cor:explicit_formula_Dirichlet_corollary}, and then as $\psi(x,\chi) \sim \psi_{0}(x,\chi)$ for all $\chi$, we can substitute the result into \cref{equ:Siegel-Walfisz_4}. Let $\rho = \b+i\g$ be a zero of $L(s,\chi)$ for some character $\chi$ modulo $m$ with $|\g| < T$ and not a Siegel zero. By the classical zero-free region for $L(s,\chi)$, all of the zeros $\rho$ satisfy
        \[
          \b < 1-\frac{c}{\log(qT)}, 
        \]
        for some possibly smaller constant $c$...
      \end{proof}