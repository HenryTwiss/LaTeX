\chapter{Explicit Formulas \& Prime Number Theorems}
  ...
  \section{Explicit Formulas for Chebyshev Functions}
    ...
    \subsection*{The Explicit Formula for \texorpdfstring{$\psi(x,\chi)$}{$\psi(x,\chi)$}}
      ...
      \begin{proof}
        ...
        \end{equation}
        because $x^{1-\b_{1}}-1 \sim x^{1-\b_{1}}$ and $1-\b_{1} \gg \frac{1}{\log(q)} \gg \frac{1}{\log(x)}$. Substiuting \cref{equ:explicit_formula_Dirichlet_corollary_8,equ:explicit_formula_Dirichlet_corollary_9} into \cref{equ:explicit_formula_Dirichlet_corollary_7} and noting that $\b_{1}$ is bounded, yields
  ...
  \section{Prime Number Theorems}
    ...
    \subsection*{The Siegel–Walfisz Theorem}
      ...
      We will prove the absolute error version of the \textbf{Siegel–Walfisz theorem}\index{Siegel–Walfisz theorem} which is slightly stronger as it bounds the absolute error between $\pi(x;a,m)$ and $\frac{\Li(x)}{\phi(m)}$:
