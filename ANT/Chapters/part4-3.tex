\chapter{Classical Applications}
  ...
  \section{Siegel's Theorem}
    ...
    The largest defect of Siegel's theorem is that the implicit constants $c_{1}(\e)$ and $c_{2}(\e)$ are ineffective (and not necessarily equal). To prove Siegel's theorem, we need a lemma about the size of $L^{(k)}(\s,\chi)$ for $\s$ close to $1$:
    ...
    \begin{proof}
      ...
      \]
      For the second sum, let $A(Y) = \sum_{n \le Y}\chi(n)$. Then $|A(Y)| \le m$ by the orthogonality relations (\cref{cor:Dirichlet_orthogonality_relations} (i)) and that $\chi$ is periodic. Hence $A(Y)\log(Y)Y^{-\s} \to 0$ as $Y \to \infty$ and Abel's summation formula (see \cref{cor:Abels_summation_formula_limit_version}) gives
  ...
  \section{The Class Number of Quadratic Number Fields}
    ...
    \]
    from \cref{prop:class_number_lower_bound} to reduce this to a finite computation because the constant $c(\e)$ is ineffective. In other words, we cannot obtain lower bound on $d$ as $d \to -\infty$ that tells us when all class numbers are larger than $n$.
