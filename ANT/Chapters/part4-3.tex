\chapter{Classical Applications}
  ...
  \section{Dirichlet's Theorem on Arithmetic Progressions}
    ...
    \begin{theorem}[Dirichlet's theorem on arithmetic progressions]\label{thm:Dirichlet's_theorem_on_primes_in_arithmetic_progressions}
      Let $a$ and $m$ be integers such that $m \ge 1$ and $(a,m) = 1$. Then the arithmetic progression $\{a+km \mid k \in \Z_{\ge 0}\}$ contain infinitely many primes.
    \end{theorem}
    ...
    \begin{proof}
      ...
      \]
      by taking the logarithm of the Euler product of $\z(s)$. The Taylor series of the logarithm gives
      \[
      ...
      \]
      The double sum restricted to $k \ge 2$ in this expression is uniformly bounded for $\s > 1$. To see this, first observe
      \[
  ...
  \section{Siegel's Theorem}
    ...
    \begin{proof}
      ...
      \]
      Then $d_{g} = 8$ and $\mathfrak{q}(g)$ satisfies
      \[
    ...
    Siegel zeros present an unfortunate obstruction to zero-free region results for Dirichlet $L$-functions when the primitive character $\chi$ is quadratic. However, if we no longer require the constant $c$ in the zero-free region to be effective, we can obtain a much better result for how close the Siegel zero can be to $1$. This result is known as \textbf{Siegel's theorem}\index{Siegel's theorem}:

    \begin{theorem}[Siegel's theorem]
      Let $\chi$ be a primitive quadratic Dirichlet character $\chi$ of conductor $q > 1$. Then for every $\e > 0$, there exists a positive constant $c_{1}(\e)$ such that
      \[
        L(1,\chi) \ge \frac{c_{1}(\e)}{q^{\e}}.
      \]
      In particular, there exists a positive constant $c_{2}(\e)$ such that $L(s,\chi)$ has no real zeros in the segment
      \[
        \s \ge 1-\frac{c_{2}(\e)}{q^{\e}}.
      \]
    \end{theorem}

    The largest defect of Siegel's theorem is that the implicit constants $c_{1}(\e)$ and $c_{2}(\e)$ are ineffective (and not necessarily equal). To prove Siegel's theorem, we need two small lemmas about the size of $L(\s,\chi)$ and $L'(\s,\chi)$ for $\s$ close to $1$:
    ...
    \end{lemma}
    \begin{proof}
      Recall that the Dirichlet $L$-series $L(s,\chi)$ converges for $\s > 0$ and thus for $0 \le 1-\s \le \frac{1}{\log(m)}$. Since
      \[
        L(\s,\chi) = \sum_{n \ge 1}\frac{\chi(n)}{n^{\s}} = \sum_{n < m}\frac{\chi(n)}{n^{\s}}+\sum_{n \ge m}\frac{\chi(n)}{n^{\s}},
      \]
      it suffices to show that the last two sums are both $O(\log^{2}(m))$. For the first sum, as $n < m$, we have
      \[
      ...
      \]
      As for the second sum, setting $A(Y) = \sum_{n \le Y}\chi(n)$ we have $A(Y) \ll 1$ by the orthogonality relations (\cref{cor:Dirichlet_orthogonality_relations} (i)) and that $\chi$ is periodic. Thus $A(Y)Y^{-\s} \to 0$ as $Y \to \infty$ and so Abel's summation formula (see \cref{cor:Abels_summation_formula_limit_version}) gives
    ...
    The second lemma is proved in much the same way:
    ...
    \end{lemma}
    \begin{proof}
      Recall that the Dirichlet $L$-series $L(s,\chi)$ converges for $\s > 0$ and thus for $0 \le 1-\s \le \frac{1}{\log(m)}$. As $L(s,\chi)$ admits analytic continuation to $\C$, it is holomorphic for $0 \le 1-\s \le \frac{1}{\log(m)}$ and so its derivative is given by
      \[
      ...
      \]
      For the second sum, let $A(Y) = \sum_{n \le Y}\chi(n)$. Then $A(Y) \ll 1$ by the orthogonality relations (\cref{cor:Dirichlet_orthogonality_relations} (i)) and that $\chi$ is periodic. Hence $A(Y)\log(Y)Y^{-\s} \to 0$ as $Y \to \infty$ and Abel's summation formula (see \cref{cor:Abels_summation_formula_limit_version}) gives
      ...
      \begin{align*}
        ...
        &\ll e\log(m),
      \end{align*}
      where in the fourth line we have used integration by parts and the last line holds because $1-\s \le \frac{1}{\log(m)}$. But $e\log(m) = O(\log^{2}(m))$ so the second sum is also $O(\log^{2}(m))$. Therefore we have shown $L'(\s,\chi) = O(\log^{2}(m))$ as desired. 
    \end{proof}

    Our path to proving Siegel's theorem will be to show that first statement implies the second and then prove the first statement. For the first statement, we will combine two Dirichlet $L$-series attached to distinct quadratic characters with distinct moduli and use this new $L$-series to derivative a lower bound for a single Dirichlet $L$-function attached to a quadratic character at $s = 1$. To this end, we will make use of the following lemma:
    ...
    We are now ready to prove Siegel's theorem. The key ingredient in the proof of the lower bound is an estimate for the $L$-series in \cref{lem:Siegel_zero_auxiliary_L-function_lemma} relative to the modulus $q_{1}q_{2}$ in a small interval on the real axis close to $1$:

    \begin{proof}[Proof of Siegel's theorem]
      We will being by showing that the first statement implies the second. Clearly the second statement holds for a single $q$ and hence for bounded $q$ by taking the maximum of all the constants. Therefore we may assume $q$ arbitrarily large. Now let $\e > 0$ and suppose that the first statement holds. We will prove the second statement by contradiction. For if there was a real zero $\b$ in this segment, then for large enough $q$ we have $0 \le 1-\b \le \frac{1}{\log(q)}$ so that $L'(\s,\chi) = O(\log^{2}(q))$ for $\b \le \s \le 1$ by \cref{lem:Siegels_theorem_second_version_lemma}. These two estimates and the mean value theorem together give
      \[
      ...
      \]
      which is a contradiction. Therefore the second statement holds provided the first does. We will now prove the first statement which will complete the proof. Let $\chi_{1}$ and $\chi_{2}$ be two distinct primitive quadratic and non-principal characters modulo $q_{1}$ and $q_{2}$ respectively. Let $L(s,g)$ be the $L$-series defined by
      \[
        L(s,g) = \z(s)L(s,\chi_{1})L(s,\chi_{2})L(s,\chi_{1}\chi_{2}).
      \]
     ...
     Now $L(s,g)$ is represented as an absolutely convergent series for $\s > 1$ so that it has a power series about $s = 2$ with radius $1$:
      ...
      \]
      where the first estimate follows because $\log(q) \ll_{\e} q^{\frac{\e}{2}}$. This is equivalent to the desired lower bound.
    \end{proof}

    The part of the proof in Siegel's theorem which makes $c_{1}(\e)$ (and hence $c_{2}(\e)$) ineffective is the value of $\b_{1}$. The choice of $\b_{1}$ depends upon the existence of a Siegel zero near $1$ relative to the given $\e$. Since we don't know if Siegel zeros exist, this makes estimating $\b_{1}$ relative to $\e$ ineffective. Many results in analytic number theory make use of Siegel's theorem and hence are also ineffective. Moreover, some important problems investigate methods to get around using Siegel's theorem in favor of a weaker result that is effective. So far, no Siegel zero has been shown to exist or not exist for Dirichlet $L$-functions. But some progress has been made to showing that they are rare:
    ...
    \end{proof}
  \section{Estimating the Class Number of Quadratic Number Fields}
    ...
    \begin{proof}
      ...
      \]
      As $a \mapsto -a$ is a bijection on $\Z/q\Z$, this sum vanishes if $\chi$ is even. But then $\chi(-1) = 1$ and the claim is is proved in the even case. For the second sum in \cref{equ:value_of_Dirichlet_L-functions_at_s=1_2}, we have an analogous relation of the form
      \[
        \sum_{a \tmod{q}}\chi(a)\log\sin\left(\frac{\pi a}{q}\right) = \chi(-1)\sum_{a \tmod{q}}\chi(-a)\log\sin\left(\frac{\pi a}{q}\right).
      \]
      As $a \mapsto -a$ is a bijection on $\Z/q\Z$, this sum vanishes if $\chi$ is odd. But then $\chi(-1) = -1$ and the claim is proved in the odd case. This completes the proof. 
    \end{proof}
    ...
    \end{proposition}
    \begin{proof}
      This follows from the Dirichlet class number formula and \cref{lem:Siegels_theorem_first_version_lemma} since $\D_{d} \asymp d$ (recall \cref{prop:discriminant_quadratic}).
    \end{proof}
    ...
    \begin{proposition}\label{prop:class_number_lower_bound}
      Let $\Q(\sqrt{d})$ be a quadratic number field. Then for every $\e > 0$, there exists a positive constant $c(\e)$ such that
      \[
    ...
    Of course, the constant in \cref{prop:class_number_lower_bound} is ineffective as we have made use of Siegel's theorem.
  \section{The Burgess Bound for Dirichlet \texorpdfstring{$L$}{L}-functions}
    ...
    \begin{theorem}\label{thm:Burgess_inequality}
      Let $\chi$ be a non-principal Dirichlet character modulo $m > 1$ and $M \ge 0$, $N \ge 1$, and $r \ge 1$ be integers. Then for any $\e > 0$, we have
      \[
    ...
    \begin{theorem}\label{thm:Burgess_bound_conductor_aspect_Dirichlet}
      Let $\chi$ be a primitive Dirichlet character of conductor $q > 1$. Then for $0 < \s < 1$ and any $\e > 0$, we have
      \[
  ...
  \section{The Siegel-Walfisz Theorem}
    ...
    \begin{proposition}
      Let $a$ and $m$ be positive integers with $m > 1$ and $(a,m) = 1$. For $x \ge 2$,
      \[
