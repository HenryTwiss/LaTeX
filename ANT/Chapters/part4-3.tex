  ...
  \section{Explicit Formulas for Chebyshev Functions}
    ...
    \subsection*{The Explicit Formula for \texorpdfstring{$\psi(x,\chi)$}{$\psi(x,\chi)$}}
      ...
      \begin{corollary}\label{cor:explicit_formula_Dirichlet_corollary}
        ...
        \[
          R(x,T,\chi) \ll \frac{x\log^{2}(xm)}{T}+x^{\frac{1}{4}}\log(x)+\log(x)\min\left(1,\frac{x}{T\<x\>}\right),
        \]
        where $\<x\>$ is the distance between $x$ and the nearest prime power other than $x$ if $x$ itself is a prime power. Moreover, if $x$ is an integer, we have the simplified estimate
        \[
      ...
      \end{equation}
      For the terms $\rho = \b+i\g$ with $|\g| > 1$, we estimate
      ...
      \end{equation}
      where the second to last estimate holds because $|\rho| \gg |2-\rho|$ (as $\b$ is bounded) and the last estimate holds by \cref{lem:Riemann_von_Mangoldt_Dirichlet_L-functions_lemma} (ii) since $|2-\rho| \ge |\g|$. For the terms $\rho = \b+i\g$, we have
      \begin{equation}\label{cor:explicit_formula_Dirichlet_corollary_4}
        \sum_{|\g| \le 1}\frac{1}{2-\rho} \le \sum_{|\g| \le 1}\frac{1}{|2-\rho|} \ll \sum_{|\g| \le 1}\frac{1}{|2-\rho|} \ll \log(q),
      \end{equation}
      ...
      \[
        b(\chi) = -\sum_{|\g| < 1}\frac{1}{\rho}+O(\log(q)).
      \]
      \todo{xxx}
      \end{proof}
