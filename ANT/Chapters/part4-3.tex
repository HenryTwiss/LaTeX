  ...
  \section{Prime Number Theorems}
    ...
    \subsection*{The Siegel–Walfisz Theorem}
      ...
      \begin{proof}
        ...
        \]
        where the first estiamte holds because $p^{v} \le x$ implies $v \le \frac{\log(x)}{\log(p)}$ so that there are $O(\log(x))$ many terms in the inner sum and the second estimate holds since $\prod_{p \mid m}p \ll m$. \todo{xxx}
      \end{proof}