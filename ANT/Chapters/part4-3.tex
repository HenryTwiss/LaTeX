  ...
  \section{Prime Number Theorems}
    ...
    \subsection*{The Siegel–Walfisz Theorem}
      ...
      \begin{proof}
        ...
        \end{equation}
        which holds by the orthogonality relations (\cref{prop:Dirichlet_orthogonality_relations} (ii)). When $\chi = \chi_{m,0}$, we estimate
        \[
        ...
        \]
        where the first estiamte holds because $p^{v} \le x$ implies $v \le \frac{\log(x)}{\log(p)}$ so that there are $O(\log(x))$ many terms in the inner sum and the second estimate holds since $\prod_{p \mid m}p \ll m$. This estimate is equivalent to
        \begin{equation}\label{equ:Siegel-Walfisz_2}
          \psi(x,\chi_{m,0}) = \psi(x)+O(\log(x)\log(m)).
        \end{equation}
        Substituting \cref{equ:prime_number_theorem_zeta_1} into \cref{equ:Siegel-Walfisz_2} gives
        ...
        \begin{equation}\label{equ:Siegel-Walfisz_4}
          \psi(x;a,m) = \frac{x}{\phi(m)}+\frac{1}{\phi(m)}\sum_{\substack{\chi \tmod{m} \\ \chi \neq \chi_{m,0}}}\cchi(a)\psi(x,\chi)+O\left(\frac{1}{\phi(m)}\left(xe^{-c\sqrt{\log(x)}}+\log(x)\log(m)\right)\right).
        \end{equation}
      \end{proof}