  ...
  \section{Prime Number Theorems}
    ...
    \subsection*{The Siegel–Walfisz Theorem}
      ...
      \begin{proof}
        ...
        \end{equation}
        Combining \cref{equ:Siegel-Walfisz_5,equ:Siegel-Walfisz_6,equ:Siegel-Walfisz_5,equ:Siegel-Walfisz_7} yields
        \begin{equation}\label{equ:Siegel-Walfisz_8}
          \psum_{|\g| < T}\frac{x^{\rho}}{\rho} \ll x\log^{2}(xm)e^{-c\frac{\log(x)}{\log(mT)}},
        \end{equation}
        where the $'$ in the sum indicates that we are excluding the terms corresponding to real zeros and the error in \cref{equ:Siegel-Walfisz_7} has been absorbed by that in \cref{equ:Siegel-Walfisz_6}. As $\psi(x,\chi) \sim \psi_{0}(x,\chi)$ and $x$ is an integer, inserting \cref{equ:Siegel-Walfisz_8} into \cref{cor:explicit_formula_Dirichlet_corollary} results in
        ...
        \[
          T = e^{\sqrt{\log(x)}},
        \]
        for $x \ge 3$. With this choice of $T$ (note that if $x \ge 2$ then $2 \le T \le x$) and that $m \ll \log^{N}(x)$, \cref{equ:Siegel-Walfisz_9} becomes
        \begin{align*}
          ...
          &\ll x\log^{2}(x)e^{-\min(1,c)\frac{\sqrt{\log(x)}}{\log(m)}}+x^{1-\b_{1}}\log(x).
        \end{align*}
        \todo{xxx}
      \end{proof}