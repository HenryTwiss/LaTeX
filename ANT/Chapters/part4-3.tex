  ...
  \section{Explicit Formulas for Chebyshev Functions}
    ...
    \subsection*{The Explicit Formula for \texorpdfstring{$\psi(x,\chi)$}{$\psi(x,\chi)$}}
      ...
      \begin{proof}[Proof of the explicit formula for $\psi(x,\chi)$]
        First suppose $\chi$ is primitive and odd.
        ...
        \end{equation}
        Now we estimate $J(x,T,\chi)$ by using the residue theorem. Let $U \ge 1$ be an even integer. Let $\W$ be the region enclosed by the contours $\eta_{1},\ldots,\eta_{4}$ in \cref{fig:explict_formula_Dirichlet_contour} and set $\eta = \sum_{1 \le i \le 4}\eta_{i}$ so that $\eta = \del \W$.
        ...
        We may write $J(x,T,\chi)$ as
        \[
          J(x,T,\chi) = \frac{1}{2\pi i}\int_{\eta_{1}}-\frac{L'}{L}(s,\chi)x^{s}\,\frac{ds}{s}.
        \]
        The residue theorem together with the explicit formula for $\frac{L'}{L}(s,\chi)$ and \cref{cor:logarithmic_derivative_of_gamma} give
        ...
        \end{equation}
        where $\rho = \b+i\g$ is a nontrivial zeros of $L(s,\chi)$. We now estimate the remaining integral. \todo{xxx}
      \end{proof}
