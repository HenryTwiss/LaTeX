\chapter{Explicit Formulas \& Prime Number Theorems}
  ...
  \section{Explicit Formulas for Chebyshev Functions}
    \subsection*{The Explicit Formula for \texorpdfstring{$\psi(x)$}{$\psi(x)$}}
      ...
      \end{corollary}
      \begin{proof}
        This follows from \cref{equ:explicit_formula_zeta_proof_14} by absorbing the constant $\frac{\z'}{\z}(0)$ into the error term. If $x$ is an integer, then $\<x\> \ge 1$ so that $\log(x)\min\left(1,\frac{x}{T\<x\>}\right) \le \frac{x\log(x)}{T}$ and this term can be absorbed by $\frac{x\log^{2}(xT)}{T}$.
      \end{proof}
    \subsection*{The Explicit Formula for \texorpdfstring{$\psi(x,\chi)$}{$\psi(x,\chi)$}}
      ...
      \begin{theorem}[Explicit formula for $\psi(x,\chi)$]
        ...
        \]
        if $\chi$ is even, and where $b(\chi)$ is the constant term in the Laurent series of $\frac{L'}{L}(s,\chi)$. In both expressions, $\rho$ runs over the nontrivial zeros of $L(s,\chi)$ counted with multiplicity and ordered with respect to the size of the ordinate.
      \end{theorem}
