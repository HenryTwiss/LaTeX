\chapter{Classical Applications}
  ...
  \section{Siegel's Theorem}
    ...
    \begin{proof}[Proof of Siegel's theorem]
      ...
      \]
      which is a contradiction. Therefore the second statement holds provided the first does. We will now prove the first statement which will complete the proof. Let $\e < frac{1}{2}$ and let $\chi_{1}$ and $\chi_{2}$ be distinct primitive quadratic characters of conductors $q_{1} > 1$ and $q_{2} > 1$ respectively. Consider the $L$-series $L(s,g)$ defined by
      \[
        L(s,g) = \z(s)L(s,\chi_{1})L(s,\chi_{2})L(s,\chi_{1}\chi_{2}).
      \]
      ...
      \]
      If there exists a Siegel zero $\b$ with $1-\e \le \b < 1$, let $\chi_{1}$ be the character corresponding to the Dirichlet $L$-function that admits this Siegel zero. Then $L(\b,g) = 0$ independent of the choice of $\chi_{2}$. If there is no such Siegel zero, choose $\chi_{1}$ to be any quadratic primitive character and $\b$ to be any number such that $1-\e \le \b < 1$. Then $L(\b,g) < 0$ independent of the choice of $\chi_{2}$. Indeed, $\z(s)$ is negative in this segment (actually $0 < s < 1$) while each of the Dirichlet $L$-series defining $L(s,g)$ is positive in this segment (the Dirichlet $L$-series converge for $\s > 0$, are positive for $\s > 1$ by their Euler product, are nonzero at $s = 1$ by \cref{thm:non-vanishing_of_Dirichlet_L-functions_at_s=1}, and do not admit a real zero in this segment by our choice of $\b$). So with our choice of $\chi_{1}$ (depending on the existence of a Siegel zero or not) we see that $L(\b,g) < 0$ for any choice of $\chi_{2}$. We now take $\chi_{2} = \chi$. Let $\psi(y)$ be a bump function that is identically $1$ on $[0,1]$ and compactly supported. Let $\Psi(s)$ be the Mellin transform of $\psi(y)$. On the one hand, our choice of $\psi(y)$ and \cref{lem:Siegel_zero_auxiliary_L-function_lemma} together imply
      ...
      \]
      By \cref{lem:log_growth_of_Dirichlet_L-series_near_1}, $\l \ll \log^{2}(q)L(1,\chi)$ and thus
      \[
      ...
      \]
      where we have used $\log(q) \ll_{\e} q^{\e}$. This is equivalent to the desired lower bound.
    \end{proof}
