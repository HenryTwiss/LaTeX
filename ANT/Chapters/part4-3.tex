  ...
  \section{Explicit Formulas for Chebyshev Functions}
    ...
    \subsection*{The Explicit Formula for \texorpdfstring{$\psi(x,\chi)$}{$\psi(x,\chi)$}}
      ...
      \begin{proof}[Proof of the explicit formula for $\psi(x,\chi)$]
        ...
        \]
        for all the nontrivial zeros of $L(s,\chi)$. \todo{xxx}
        \iffalse
        To estimate part of the horizontal integrals over $\eta_{2}$ and $\eta_{4}$, \cref{equ:Riemann_von_Mangoldt_Dirichlet_L-functions_13} with $s = \s+iT$ for $-1 \le \s \le 2$ gives
        \[
