\chapter{Explicit Formulas \& Prime Number Theorems}
  ...
  \section{Prime Number Theorems}
    ...
    \subsection*{The Siegel–Walfisz Theorem}
      ...
      \]
      This is just $\psi(x)$ restricted to only those terms equivalent to $a$ modulo $m$. The prime number theorem for $\pi(x;a,q)$ is known as the \textbf{Siegel–Walfisz theorem}\index{Siegel–Walfisz theorem}:
      ...
      \begin{proof}
        ...
        \end{equation}
        which holds by the orthogonality relations (\cref{prop:Dirichlet_orthogonality_relations} (ii)). Letting $\wtilde{\chi}$ be the primitive character inducing $\chi$, we estimate
        \begin{equation}\label{equ:Siegel-Walfisz_2}
          |\psi(x,\chi)-\psi(x,\wtilde{\chi})| \le \sum_{\substack{n \le x \\ (n,m) > 1}}\L(n) = \sum_{p \mid m}\sum_{\substack{v \ge 1 \\ p^{v} \le x}}\log(p) \ll \log(x)\sum_{p \mid m}\log(p) \ll \log(x)\log(m),
        \end{equation}
        where the first bound holds because $p^{v} \le x$ implies $v \le \frac{\log(x)}{\log(p)}$ so that there are $O(\log(x))$ many terms in the inner sum and the second bound holds since $\prod_{p \mid m}p \ll m$. In particular, \cref{equ:Siegel-Walfisz_2} implies 
        \begin{equation}\label{equ:Siegel-Walfisz_3}
          \psi(x,\chi) = \psi(x,\wtilde{\chi})+O(\log(x)\log(m)).
        \end{equation}
        When $\chi = \chi_{m,0}$, we have $\psi(x,\wtilde{\chi}) = \psi(x)$ so substiuting \cref{equ:prime_number_theorem_zeta_1} into \cref{equ:Siegel-Walfisz_3} gives
        \begin{equation}\label{equ:Siegel-Walfisz_4}
          \psi(x,\chi_{m,0}) = \psi(x)+O\left(xe^{-c\sqrt{\log(x)}}+\log(x)\log(m)\right),
        \end{equation}
        for some positive constant $c$. Upon combining \cref{equ:Siegel-Walfisz_1,equ:Siegel-Walfisz_4}, we obtain
        ...
        \end{equation}
        where we have used the simple estimates $\log(x) \ll \log(xm)$ and $\log(m) \ll \log(xm)$...
      \end{proof}