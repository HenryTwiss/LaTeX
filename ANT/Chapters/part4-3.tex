\chapter{Classical Applications}
  ...
  \section{Siegel's Theorem}
    ...
    \begin{theorem}[Siegel's theorem, lower bound version]
      Let $\chi$ be a primitive quadratic Dirichlet character modulo $q > 1$. Then for every $\e > 0$ there exists a positive constant $c_{1}(\e)$ such that
      \[
    ...
    \begin{theorem}[Siegel's theorem, zero-free region version]
      Let $\chi$ be a primitive quadratic Dirichlet character modulo $q > 1$. Then for every $\e > 0$ there exists a positive constant $c_{2}(\e)$ such that $L(s,\chi)$ has no real zeros in the segment
      \[
  ...
  \section{Estimating the Class Number of Quadratic Number Fields}
    ...
    \begin{corollary}\label{cor:formula_for_class_number_quadratic_number_field}
      ...
      \[
        h_{d} = -\frac{\tau(\chi)}{2\log|\e_{d}|\sqrt{|\D_{d}|}}\sum_{a \tmod{|\D_{d}|}}\cchi(a)\log\sin\left(\frac{\pi a}{|\D_{d}|}\right) \quad \text{or} \quad h_{d} = \frac{w_{d}\tau(\chi)i}{2|\D_{d}|^{\frac{3}{2}}}\sum_{a \tmod{|\D_{d}|}}\cchi(a)a,
      \]
