\chapter{Applications of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{The Siegel-Walfisz Theorem}
    ...
    We will prove the (absolute error) \textbf{Siegel–Walfisz theorem}\index{Siegel–Walfisz theorem} which is slightly stronger as it bounds the absolute error between $\pi(x;a,m)$ and $\frac{\Li(x)}{\vphi(m)}$:

    \begin{theorem}[Siegel–Walfisz theorem, absolute error]
