\chapter{Explicit Formulas \& Prime Number Theorems}
  ...
  \section{Prime Number Theorems}
    \subsection*{The Prime Number Theorem}
      ...
      \end{theorem}
      \begin{proof}
        It suffices to assume $x$ is an integer, because $\pi(x)$ can only change value at integers and the other functions in the statement are continuous (and hence bounded on compact sets). We will first prove
    ...
    \subsection*{The Siegel–Walfisz Theorem}
      ...
      \end{theorem}
      \begin{proof}
        It suffices to assume $x$ is an integer, because $\pi(x;a,m)$ can only change value at integers and the other functions in the statement are continuous (and hence bounded on compact sets). We being with the identity
        ...
        \end{equation}
        Now fix a non-principal $\chi$ modulo $m$ and let $\wtilde{\chi}$ be the primitive character modulo $q$ inducing $\chi$. Let $2 \le T \le x$ not coinciding with the ordinate of a nontrivial zero and let $\rho = \b+i\g$ be a nontrivial zero of $L(s,\wtilde{\chi})$ with $|\g| < T$ and not a real zero. By the classical zero-free region for $L(s,\wtilde{\chi})$, all of the zeros $\rho$ satisfy $\b < 1-\frac{c}{\log(qT)}$ for some possibly smaller constant $c$. It follows that
        \begin{equation}\label{equ:Siegel-Walfisz_5}
          |x^{\rho}| = x^{\b} < x^{1-\frac{c}{\log(qT)}} = xe^{-c\frac{\log(x)}{\log(qT)}} \ll xe^{-c\frac{\log(x)}{\log(xq)}},
        \end{equation}
        where in the last estimate we have used that $T \le x$. As $|\rho| > |\g|$, for those zeros with $|\g| > 1$ (unlike the Riemann zeta function we do not have a positive lower bound for the first ordinate $\g_{1}$ of a nontrivial zero that is not real since Siegel zeros may exist), applying integration by parts gives
        ...
        \end{equation}
        where in the second to last estimate we have used that $N(t,\wtilde{\chi}) \ll t\log(qt)$ which follows from \cref{cor:density_of_nontrivial_zeros_Dirichlet}. For the remaining sum of zeros with $|\g| \le 1$ (that are not real), we use \cref{cor:explicit_formula_Dirichlet_corollary_8}. Combining \cref{cor:explicit_formula_Dirichlet_corollary_8,equ:Siegel-Walfisz_5,equ:Siegel-Walfisz_6} yields
        \begin{equation}\label{equ:Siegel-Walfisz_8}
          \psum_{|\g| < T}\frac{x^{\rho}}{\rho} \ll x\log^{2}(xq)e^{-c\frac{\log(x)}{\log(xq)}},
        \end{equation}
        where the $'$ in the sum indicates that we are excluding the terms corresponding to real zeros and the error in \cref{cor:explicit_formula_Dirichlet_corollary_8} has been absorbed by that in \cref{equ:Siegel-Walfisz_6}. As $\psi(x,\chi) \sim \psi_{0}(x,\chi)$ and $x$ is an integer, inserting \cref{equ:Siegel-Walfisz_8} into \cref{cor:explicit_formula_Dirichlet_corollary} results in
        \begin{equation}\label{equ:Siegel-Walfisz_9}
          \psi(x,\chi)+\frac{x^{\b_{1}}}{\b_{1}} \ll x\log^{2}(xm)e^{-c\frac{\log(x)}{\log(xm)}}+\frac{x\log^{2}(xmT)}{T}+x^{1-\b_{1}}\log(x),
        \end{equation}
        where the terms corresponding to real zeros are present if and only if $L(s,\chi)$ admits a Siegel zero. \todo{xxx}
      \end{proof}