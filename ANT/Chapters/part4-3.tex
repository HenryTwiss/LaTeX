\chapter{Classical Applications}
  ...
  \section{The Burgess Bound for Dirichlet \texorpdfstring{$L$}{L}-functions}
    ...
    \begin{theorem}\label{thm:Burgess_bound_conductor_aspect_Dirichlet}
      ...
      \[
        L(s,\chi) \ll_{\e} \begin{cases} q^{\frac{4-5\s}{8}+\e} & \text{if $0 < \s \le \frac{1}{2}$}, \\ q^{\frac{3-3\s}{8}+\e} & \text{if $\frac{1}{2} \le \s < 1$}. \end{cases}
      \]
      In particular,
      \[
    ...
    \begin{proof}
      ...
      \]
      for any $N \ge 1$. As $S_{\chi}(N)N^{-s} \to 0$ as $N \to \infty$, taking the limit as $N \to \infty$ results in
      \[
        L(s,\chi) = \sum_{n \le N-1}S_{\chi}(n)(n^{-s}-(n+1)^{-s}) \ll \sum_{n \ge 1}\frac{S_{\chi}(n)}{n^{\s+1}},
      \]
      where the estimate follows by applying the mean value theorem to $f(x) = \frac{1}{x^{s}}$ on the interval $[n,n+1]$. Letting $M > 2$, $N \ge 1$, and decompose the right-hand side, we obtain
      \[
        L(s,\chi) \ll \sum_{1 \le M-1}\frac{S_{\chi}(n)}{n^{\s+1}}+\sum_{M \le n \le N}\frac{S_{\chi}(n)}{n^{\s+1}}+\sum_{n \ge N+1}\frac{S_{\chi}(n)}{n^{\s+1}}.
      \]
      ...
      \[
        L(s,\chi) \ll_{\e} \sum_{1 \le M-1}\frac{1}{n^{\s}}+q^{\frac{3}{16}+\e}\sum_{M \le n \le N}\frac{1}{n^{\s+\frac{1}{2}}}+q^{\frac{1}{2}+\e}\sum_{n \ge N+1}\frac{1}{n^{\s+1}},
      \]
      where we have used that $\log(q) \ll_{\e} q^{\e}$. Upper bounding the first and third sums with an integral and integrating, we get
      \[
        L(s,\chi) \ll_{\e} M^{1-\s}+q^{\frac{3}{16}+\e}\sum_{M \le n \le N}\frac{1}{n^{\s+\frac{1}{2}}}+q^{\frac{1}{2}+\e}N^{-\s},
      \]
      ...
      \[
        L(s,\chi) \ll_{\e} \begin{cases} q^{\frac{4-5\s}{8}+\e} & \text{if $0 < \s \le \frac{1}{2}$}, \\ q^{\frac{3-3\s}{8}+\e} & \text{if $\frac{1}{2} \le \s < \frac{1}{2}$}, \end{cases}
      \]
    ...
    In particular, \cref{thm:Burgess_bound_conductor_aspect_Dirichlet} shows that the Burgess bound in the conductor-aspect holds for Dirichlet $L$-functions. This is how the Burgess bound got it's name as it was the first subconvexity result for Dirichlet $L$-functions in the conductor-aspect.