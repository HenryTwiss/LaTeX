\chapter{The Theory of Sieves}
  ...
  \section{\todo{Estimating the Sifting Function}}
    ...
    \[
      h(p) = \frac{g(p)}{1-g(p)} \quad \text{and} \quad j(p) = 1+h(p).
    \]
    In particular, $j = h \ast \mathbf{1}$. Now as $\t = \l \ast \mathbf{1}$, the M\"obius inversion formula (see \cref{append:The_Mobius_Function}) implies $\l = \t \ast \mu$ so that \todo{xxx}
