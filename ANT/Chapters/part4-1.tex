\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet Series}
    ...
    Note that in \cref{prop:Dirichlet_series_Euler_product}, the requirement for a product to define an absolutely convergent Dirichlet series is that the series defining the factors in the product must be absolutely convergent. This is always the case for convergent geometric series and hence Dirichlet series whose coefficients are multiplicative and exhibit polynomial growth. Now suppose $D(s)$ is a Dirichlet series that has the product expression
    \[
