\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet Series}
    ...
    \begin{proof}
      ...
      \]
      Since the last integral is finite, $\sum_{M \le n \le N}\frac{a(n)}{n^{s}}$ can be made uniformly arbitrarily small for $s$ in the desired sector. The claim follows by the uniform version of Cauchy's criterion.
    \end{proof}
    ...
    \]
    for $\s > \s_{a}$ and some $\a_{i}(p) \in \C$ for $1 \le i \le d$ and all primes $p$. We call this product the \textbf{Euler product}\index{Euler product} of $D(s)$, and it is said to be of \textbf{degree}\index{degree} $d$. In \cref{prop:Dirichlet_series_Euler_product}, complete multiplicativity of the coefficients is enough to guarantee that $D(s)$ has an Euler product of degree $1$, but in general $D(s)$ will admit an Euler product of degree $d > 1$ if the coefficients are merely multiplicative but satisfy additional properties like a recurrence relation. If $D(s)$ has an Euler product, then for any $N \ge 1$ we let $D_{(N)}(s)$ denote the Dirichlet series with the factors $p \mid N$ in the Euler product removed. Then
    \[
  ...
  \section{Perron's Formula}
    ...
    There is also a truncated version of Perron's formula which is often more useful for more subtle estimates. To state it, we need to setup some notation and will require a lemma. For any $c > 0$, consider the discontinuous integral (see \cite{davenport1980multiplicative})
    \[
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Bounds}
    ...
    \begin{conjecture}[Lindel\"of hypothesis, $L(s,f)$ version]
      For the $L$-function $L(s,f)$ and any $\e > 0$, we have
      \[
    ...
    \begin{conjecture}[Grand Lindel\"of hypothesis]
      For any Selberg class $L$-function $L(s,f)$ and any $\e > 0$, we have
      \[
        L\left(\frac{1}{2}+it,f\right) \ll_{\e} \mf{q}\left(\frac{1}{2}+it,f\right)^{\e}.
      \]
    \end{conjecture}
  ...
  \section{Estimating the Central Value}
    ...
    \begin{theorem}
      Let $L(s,f)$ be an $L$-function and let $\psi(y)$ be a bump function with compact support in $\left[\frac{1}{2},2\right]$. Then for any $\e > 0$, we have
      \[
