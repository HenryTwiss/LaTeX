\chapter{The Theory of Sieves}
  ...
  \section{\todo{Composition of Sieves}}
    Composing sieves allows us to form new sieves from old ones. To explain the construction, let $\L' = (\l'_{d})_{d \ge 1}$ and $\L'' = (\l''_{d})_{d \ge 1}$ be two sieves of levels $D'$ and $D''$ respectively. Also let $\t' = \l' \ast \mathbf{1}$ and $\t'' = \l'' \ast \mathbf{1}$. Then we define the \textbf{composition sieve}\index{composition sieve} $\L = (\l_{d})_{d \ge 1}$ of $\L'$ and $\L''$ by
    \[
      \l_{d} = \sum_{[d_{1},d_{2}] = d}\l'_{d_{1}}\l''_{d_{2}},
    \]
    for $d \ge 1$. Clearly $\L$ is a sieve of level $D'D''$. \todo{xxx}
