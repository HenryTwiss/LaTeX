\chapter{Classical Applications}
  ...
  \section{The Prime Number Theorem}
    ...
    \begin{proof}[Proof of the explicit formula for $\psi(x)$]
      ...
      \[
        \g-T \gg \frac{1}{\log(T)},
      \]
      for all the nontrivial zeros of $\z(s)$. To estimate part of the horizontal integrals over $\eta_{2}$ and $\eta_{4}$, \cref{lem:powerful_L-function_approximation_lemma} (iii) applied to $\z(s)$ gives
      \[
  ...
  \section{The Siegel-Walfisz Theorem}
    ...
    \[
      \psi(x,\chi) = \sum_{n \le x}\L_{\chi}(n),
    \]
    ...
    \begin{proof}[Proof of the explicit formula for $\psi(x,\chi)$]
      ...
      \begin{equation}\label{equ:explicit_formula_Dirichlet_proof_1}
        \psi_{0}(x,\chi)-J(x,T,\chi) \ll x^{c}\sum_{\substack{n \ge 1 \\ n \neq x}}\frac{\L_{\chi}(n)}{n^{c}}\min\left(1,\frac{1}{T\left|\log\left(\frac{x}{n}\right)\right|}\right)+\d_{x}\chi(x)\L(x)\frac{c}{T},
      \end{equation}
      where
      \[
      ...
      \begin{equation}\label{equ:explicit_formula_Dirichlet_proof_2}
        \ll \frac{x^{c}}{T}\sum_{n \ge 1}\frac{\L_{\chi}(n)}{n^{c}} \ll \frac{x^{c}}{T}\left(-\frac{L'}{L}(c,\chi)\right) \ll \frac{x\log(x)}{T},
      \end{equation}
      where the last estimate follows from \cref{lem:powerful_L-function_approximation_lemma} (iii) applied to $L(s,\chi)$ and our choice of $c$ (in particular $\log(c) \ll \log(x)$). Now we estimate the terms $n$ close to $x$. So consider those $n$ for which $\frac{3}{4}x < n < x$ and let $x_{1}$ be the largest prime power less than $x$. We may also assume $\frac{3}{4}x < x_{1} < x$ since otherwise $\L(n) = 0$ and these terms do not contribute anything. Moreover, $\frac{x^{c}}{n^{c}} \ll 1$. For the term $n = x_{1}$, we have the estimate
      \[
        \log\left(\frac{x}{n}\right) = -\log\left(1-\frac{x-x_{1}}{x}\right) \ge \frac{x-x_{1}}{x},
      \]
      ...
      \begin{equation}\label{equ:explicit_formula_Dirichlet_proof_3}
        \ll \L_{\chi}(x_{1})\min\left(1,\frac{x}{T(x-x_{1})}\right) \ll \log(x)\min\left(1,\frac{x}{T(x-x_{1})}\right).
      \end{equation}
      ...
      \begin{equation}\label{equ:explicit_formula_Dirichlet_proof_4}
        \ll \sum_{0 < v < \frac{1}{4}x}\L_{\chi}(x_{1}-v)\frac{x_{1}}{Tv} \ll \frac{x}{T}\sum_{0 < v < \frac{1}{4}x}\frac{\L(x_{1}-v)}{v} \ll \frac{x\log(x)}{T}\sum_{0 < v < \frac{1}{4}x}\frac{1}{v} \ll \frac{x\log^{2}(x)}{T}.
      \end{equation}
      ...
      \]
      for all the nontrivial zeros of $L(s,\chi)$. To estimate part of the horizontal integrals over $\eta_{2}$ and $\eta_{4}$, \cref{lem:powerful_L-function_approximation_lemma} (iii) applied to $L(s,\chi)$ gives
      \[
    ...
    \begin{proof}
      ...
      \]
      and we have absorbed the error in \cref{equ:explicit_formula_Dirichlet_corollary_5} into $\frac{x\log^{2}(xqT)}{T}$ because $2 \le T \le x$. By \cref{thm:improved_zero-free_region_Dirichlet}, all of the zeros $\rho$ with $|\g| < T$ (we have assume $T \ge 2$ so that this includes the zeros with $|\g| \le 1$) satisfy $\b \ll 1-\frac{1}{\log(qT)}$ except for possibly a Siegel zero $\b_{\chi}$ if $\chi$ is quadratic. Since $1-\conj{\rho}$ is a zero if $\rho$ is, there will be another real zero $1-\b_{\chi}$. Extracting the terms corresponding to these two zeros in \cref{equ:explicit_formula_Dirichlet_corollary_6} gives
      ...
      \end{equation}
      where the $'$ on the sums indicates that we are excluding the terms corresponding to real zeros and the terms corresponding to $\b_{\chi}$ and $1-\b_{\chi}$ are present if and only if $L(s,\chi)$ admits a Siegel zero. We now estimate some of the terms in \cref{equ:explicit_formula_Dirichlet_corollary_7}. For the second sum, we have $\rho \gg \frac{1}{\log(q)}$ since $\g$ is bounded and these zeros satisfy $\b \ll 1-\frac{1}{\log(q)}$. Thus
      ...
      \end{equation}
      because $1-\b_{\chi} \gg \frac{1}{\log(q)} \gg \frac{1}{\log(x)}$. Substituting \cref{equ:explicit_formula_Dirichlet_corollary_8,equ:explicit_formula_Dirichlet_corollary_9} into \cref{equ:explicit_formula_Dirichlet_corollary_7} and noting that $\b_{\chi}$ is bounded, yields
