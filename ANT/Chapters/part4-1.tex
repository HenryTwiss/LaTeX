\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet Series}
    ...
    \end{theorem}
    \begin{proof}
      Set $R(u) = \sum_{n \ge u}\frac{a(n)}{n^{-s_{0}}}$ so that $a(n) = (R(n)-R(n+1))n^{s_{0}}$. Then for any two positive integers $N$ and $M$ with $1 \le M < N$, summation by parts (see \cref{append:Summation_Formulas}) implies
