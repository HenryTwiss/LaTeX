\chapter{The Theory of \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Dirichlet Series}
    ...
    \begin{proof}
      ...
      \]
      Since the last integral is finite, $\sum_{M \le n \le N}\frac{a(n)}{n^{s}}$ can be made uniformly arbitrarily small for $s$ in the desired sector. The claim now follows by the uniform version of Cauchy's criterion.
    \end{proof}
  ...
  \section{The Approximate Functional Equation}
    ...
    \end{equation}
    where in the estimate we have used the reverse triangle equality. It follows that $\Phi(u)$ exhibits exponential decay. With this choice of $ \Phi(u)$, we can prove a useful bound for $V_{s}(y)$:
    ...
    \begin{proof}
      ...
      \end{align*}
      where in the third and fourth lines we have used that $cy \ll (1+cy)$ for all $y \ge 0$ and any $c$ and the last line holds since the integrand exhibits exponential decay. This completes the proof.
    \end{proof}
  ...
  \section{The Lindel\"of Hypothesis \& Convexity Bounds}
    ...
    \[
      L^{(n)}\left(\frac{1}{2}+it,f\right) = \frac{n!}{2\pi i}\int_{C}\frac{L(s,f)}{(s-\frac{1}{2}-it)^{n+1}}\,ds \ll_{\e} \int_{C}|L(s,f)|ds,
    \]
  ...
  \section{A Zero-free Region}
    ...
    Some comments are in order. Since the zeros of $L(s,f)$ occur in pairs $\rho$ and $1-\conj{\rho}$, the zero-free region in \cref{thm:zero_free_region_generic} implies a symmetric zero-free region about the critical line with at most one real zero in each half as displayed in \cref{fig:symmetric_zero_free_region}.
