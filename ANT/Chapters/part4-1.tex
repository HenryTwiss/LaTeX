\chapter{The Theory of Sieves}
  ...
  \section{The Language of Sieves}
    ...
    \]
    The usefulness of sieves comes from replacing the M\"obius function with weighted values. In particular, we replace $\mu(d)$ with a weighting factor $\l_{d}$ coming from a real sequence $\L = (\l_{d})_{d \ge 1}$ satisfying $\l_{d} = 0$ unless $d \le D$ for some $D \ge 1$. We call $\L$ the \textbf{sieve}\index{sieve}, $\l_{d}$ a \textbf{sieve weight}\index{sieve weight}, and $D$ the \textbf{sieve level}\index{sieve level}. We also define the \textbf{sifting variable}\index{sifting variable} $s$ to be
    \[
      s = \frac{\log(D)}{\log(z)}.
    \]
    ...
    \]
    Note that $\t_{n}^{0}$ and $\t_{n}$ both depend on the sifting level $z$, but we suppress this dependence from the notation. In any case, we then have
    \[
  ...
  \section{\todo{Estimating the Sifting Function}}
    ...
    \end{equation}
    for some smooth functions $M(x)$ and $r_{d}(x)$ defined for $x > 0$. We call the function $g(d)$ a \textbf{density function}\index{density function} and the constant $\k$ as the \textbf{sieve dimension}\index{sieve dimension}. Using Legendre's identity, we can write
    \[
    ...
    \]
    Estimates for $V^{\L}(x)$ are more involved. \todo{xxx}
