\chapter{The Theory of Sieves}
  ...
  \section{The Language of Sieves}
    ...
    \]
    The usefulness of sieves comes from replacing the M\"obius function with weighted values. In particular, we replace $\mu(d)$ with a weighting factor $\l_{d}$ coming from a real sequence $\L = (\l_{d})_{d \ge 1}$ satisfying $\l_{d} = 0$ unless $d \le D$ for some $D \ge 1$. We will also let $\l$ represent the arithmetic function $\l(d) = \l_{d}$. We call $\L$ the \textbf{sieve}\index{sieve}, $\l_{d}$ a \textbf{sieve weight}\index{sieve weight}, and $D$ the \textbf{sieve level}\index{sieve level}. We also define the \textbf{sifting variable}\index{sifting variable} $s$ to be
    \[
      s = \frac{\log(D)}{\log(z)}.
    \]
