\chapter{The Theory of Sieves}
  ...
  \section{\todo{Estimating the Sifting Function}}
    ...
    \begin{align*}
      ...
      &= \todo{xxx}, \\
    \end{align*}
    where the second line holds because $g(de) = 0$ unless $(d,e) = 1$ and the third line holds by the multiplicativity of $g$ and the definition of the M\"obius function. Define the function $G^{\L}(z)$ by
    \[
      G^{\L}(z) = \sum_{d \mid P(z)}\t_{d}g(d),
    \]
    for $z \ge 2$. Then by \cref{equ:V_product_formula} we can write
    \[
      V^{\L}(z) = G^{\L}(z)V(z).
    \]
