  \chapter{Preliminaries}\label{ch:Preliminaries}
    ...
    \section{Notational Conventions}
      ...
      \begin{itemize}
        \item For the complex variable $s$, we will denote $s = \s+it$ so that $\s$ and $t$ are the real and imaginary parts of $s$ respectively.
        ...
        \item We will write $\int_{(a)}$ for the complex integral over the vertical line whose real part is $a$ and with positive orientation.
    ...
    \section{Asymptotic Notation}
      ...
      \begin{remark}
        Suppose $f,g:\Z_{\ge 0} \to \C$. Extending $f$ and $g$ by making them piecewise linear so that $f,g:[0,\infty) \to \C$ is piecewise continuous, we can consider estimates with $n$ in place of $z$. All of the following theory still holds. Moreover, if we further take $f$ or $g$ to be a constant function, the following theory will still hold.
      \end{remark}
    ...
    \section{Integral Methods \& Transforms}
      ...
      \subsection*{The Fourier Transform}
        ...
        \begin{proof}
          ...
          \]
          Since $f$ has exponential decay, $F$ is locally absolutely uniformly convergent by the Weierstrass $M$-test. Actually since $f$ is Schwarz, $F$ and all of its derivatives are locally absolutely uniformly convergent. By the uniform limit theorem $F$ is smooth. Since $F$ is also $1$-periodic it admits a Fourier series (see \cref{append:Fourier_Series}). The $t$-th Fourier coefficient of $F$ is
          \begin{align*}
      ...
      \subsection*{The Mellin Transform}
        Like the Fourier transform, the Mellin transform is another type of integral transform. If $f:(0,\infty) \to \R$ is some continuous function, then the \textbf{Mellin transform}\index{Mellin transform} $\{\mc{M}f\}(s)$ of $f(x)$ is given by
        \[
          \{\mc{M}f\}(s) = \int_{0}^{\infty}f(x)x^{s}\,\frac{dx}{x}.
        \]
        If $f(x)$ is a sufficiently nice function then the integral will be bounded in some half-plane in $s$. For example, this happens if $f(x)$ has exponential decay and remains bounded as $x \to 0$. In this case, the integral is absolutely uniformly bounded for $\s > 0$. The classical example is when $f(x) = e^{-x}$ so that $\{\mc{M}e^{-x}\}(s) = \G(s)$. If the Mellin transform $g(s)$ is sufficiently nice, then the initial function can be recovered via means of the \textbf{inverse Mellin transform}\index{inverse Mellin transform} $\{\mc{M}^{-1}g\}(x)$:
        \[
        ...
        \begin{theorem}[Mellin inversion formula]
          Let $a$ and $b$ be reals such that $a < b$. Suppose $g(s)$ is analytic in the strip $a < \s < b$, tends to zero uniformly as $\Im(s) \to \infty$ along any line $\s = c$ for $a < c < b$, and that the integral of $g(s)$ along this line is locally absolutely uniformly bounded. Then if
          \[
            f(x) = \frac{1}{2\pi i}\int_{(c)}g(s)x^{-s}\,ds,
          \]
          this integral is independent of $c$ and moreover $g(s) = \{\mc{M}f\}(s)$. Conversely, suppose $f(x):(0,\infty) \to \R$ is piecewise continuous and such that its value is halfway between the limit values at any jump discontinuity and
          \[
            g(s) = \int_{0}^{\infty}f(x)x^{s}\,\frac{dx}{x},
          \]
          is absolutely bounded for $a < \s < b$. Then $f(x) = \{\mc{M}^{-1}g\}(x)$.
        \end{theorem}
