    ...
    \section{Integration Techniques \& Transforms}
      ...
      \subsection*{The Fourier Transform}
        ...
        \begin{proof}
          ...
          \]
          Since $f(x)$ exhibits rapid decay, $F(x)$ is locally absolutely uniformly convergent by the Weierstrass $M$-test. Actually since $f(x)$ is Schwarz, $F(x)$ and all of its derivatives are locally absolutely uniformly convergent. By the uniform limit theorem $F(x)$ is smooth. Since $F(x)$ is also $1$-periodic it admits a Fourier series (see \cref{append:Fourier_Series}). The $t$-th Fourier coefficient of $F(x)$ is
          \begin{align*}
        ...
        Our primary application of the Poisson summation formula will be to derive very striking transformation laws which are the key ingredient in analytically continuing many important functions.
