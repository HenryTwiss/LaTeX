\chapter{Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Dirichlet Characters}
    ...
    \begin{proposition}[Orthogonality relations]\label{prop:Dirichlet_orthogonality_relations}
    \phantom{ }
      \begin{enumerate}[label=(\roman*)]
    ...
    \begin{proof}
      ...
      \begin{enumerate}[label=(\roman*)]
        ...
        \[
          \chi(b)\conj{\psi}(b)S = \frac{1}{\vphi(m)}\psum_{a \tmod{m}}\chi(ab)\conj{\psi}(ab) = S,
        \]
        upon making the change of variables $a \mapsto ab^{-1}$. Consequently $S = 0$ unless $\chi(b)\conj{\psi}(b) = 1$ for all $b$ such that $(b,m) = 1$. This happens if and only if $\psi = \chi$ in which case $S = 1$ proving (i).
        \item Denote the left-hand side by $S$. Let $\psi$ be any Dirichlet character modulo $m$. Then
        \[
          \psi(a)\conj{\psi}(b)S = \frac{1}{\vphi(m)}\sum_{\chi \tmod{m}}\chi\psi(a)\conj{\chi\psi}(b) = S,
        \]
        upon making the change of variables $\chi \mapsto \chi\conj{\psi}$. Thus $S = 0$ unless $\psi(a)\conj{\psi}(b) = \psi(a\conj{b}) = 1$ for all Dirichlet characters $\psi$ modulo $m$. If this happens, then $a\conj{b} = 1 \tmod{m}$. To see this, let $m = p_{1}^{r_{1}}p_{2}^{r_{2}} \cdots p_{k}^{r_{k}}$ be the prime factorization of $m$. By the structure theorem for finite abelian groups,
        \[
