  \chapter{Preliminaries}\label{ch:Preliminaries}
    ...
    \section{Notational Conventions}
      ...
      \begin{itemize}
        ...
        \item The nontrivial zeros of an $L$-function will be denoted by $\rho = \b+i\g$ unless specified otherwise. We will also explicitly state when certain zeros are to be excluded.
    ...
    \section{Asymptotic Notation}
      ...
      \begin{remark}
        Suppose $f,g:\Z_{\ge 0} \to \C$. Extending $f(x)$ and $g(x)$ by making them piecewise linear so that $f,g:\R_{\ge 0} \to \C$ is piecewise continuous, we can consider estimates with $n$ in place of $z$. All of the following theory still holds. Moreover, if we further take $f(x)$ or $g(x)$ to be a constant function, the following theory will still hold.
      \end{remark}
      ...
      \subsection*{Growth of Functions}
        We will also be interested in the growth rate of functions. We can compactly express these types of growth using asymptotic notation. There are many types of growth rates, but we will only recall the ones that will be of use to us. Let $f(z)$ be a complex function and let $z \to z_{0}$. If $f(z) = O(\log(z))$ we say that $f(z)$ is of \textbf{logarithmic growth}\index{logarithmic growth}. If $f(z) = O(z^{c})$, for some $c \in \R$, we say that $f(z)$ is of \textbf{polynomial growth}\index{polynomial growth}. If $f(z) = o(e^{z})$, we say that $f(z)$ is of \textbf{moderate growth}\index{moderate growth}. If $f(z) = O(e^{z})$, we say that $f(z)$ is of \textbf{exponential growth}\index{exponential growth}. Now suppose $z \to \infty$. If $f(z) = o(\log^{-1}(z))$ we say that $f(z)$ is of \textbf{logarithmic decay}\index{logarithmic decay}. If $f(z) = o(x^{-c})$ for some $c \ge 1$, we say that $f(z)$ has \textbf{polynomial decay}\index{polynomial decay}. If $f(z) = o(x^{-c})$ for all $c \ge 1$, we say that $f(z)$ has \textbf{moderate decay}\index{moderate decay}. If $f(z) = o(e^{-z})$, we say that $f(z)$ has \textbf{exponential decay}\index{exponential decay}.
    ...
    \section{The Gamma Function}
      The gamma function is ubiquitous in analytic number theory and the better one understands it the better one will be at seeing the forest for the trees in any problem involving analytic number theory. The \textbf{gamma function}\index{gamma function} $\G(z)$ is defined to be the Mellin transform of $e^{-x}$:
      \[
      ...
      \end{equation}
      By \cref{met:decay_compacta_integral}, the second integral in \cref{equ:Gamma_integral_bounded_1} is locally absolutely uniformly bounded in this region. As for the first integral, let $K$ be a compact set in the region $\Re(z) > 0$. Set $\b = \min_{z \in K}\{\Re(z)\}$. Then
      ...
      \end{equation}
      \cref{equ:Gamma_integral_bounded_2} implies that the first integral in \cref{equ:Gamma_integral_bounded_1} is locally absolutely uniformly bounded too. Altogether, this means $\G(z)$ is as well. Also note that for $z > 0$, $\G(z)$ is real. The most basic properties of $\G(z)$ are the following:
      ...
      \begin{proof}
        ...
        \]
        For (iii), since $\G(z)$ is real for $z > 0$ we have $\G(\conj{z}) = \conj{\G(z)}$ on this half-line and then the identity theorem implies that this holds on the entire domain of $\G(z)$.
      \end{proof}
      ...
      \begin{theorem}\label{thm:continuation_of_gamma_function}
        $\G(z)$ admits meromorphic continuation to $\C$ with poles at $z = -n$ for $n \ge 0$. All of these poles are simple and with residue $\frac{(-1)^{n}}{n!}$ at $z = -n$.
      \end{theorem}
      \begin{proof}
      ...
      \phantom{}
        \[
          \G(z) \sim_{\e,\d} \sqrt{2\pi}z^{z-\frac{1}{2}}e^{-z},
        \]
      ...
      Let $|\arg(z)| < \pi-\e$ and $|z| > \d$ for some $\e,\d > 0$. Then if $x$ is bounded, we have
        \[
          \G(z) \sim_{\e,\d} \sqrt{2\pi}y^{x-\frac{1}{2}}e^{-\frac{\pi}{2}|y|}.
        \]
      \end{corollary}
      \begin{proof}
        Stirling's formula can be equivalently expressed as
        \[
          \G(z) \sim_{\e,\d} \sqrt{2\pi}(x+iy)^{x-\frac{1}{2}+iy}e^{-x-iy}.
        \]
        Since $x$ is bounded, $e^{-x-iy} \ll 1$ and we obtain the the simplified asymptotic
        \[
          \G(z) \sim_{\e,\d} \sqrt{2\pi}(iy)^{x-\frac{1}{2}+iy}.
        \]
        Similarly, $x$ being bounded implies $i^{x-\frac{1}{2}} \ll 1$ and we compute
        \[
      ...
      \begin{equation}
          \G(z) = \sqrt{2\pi}z^{z-\frac{1}{2}}e^{-z}(1+O_{\e,\d}(1)).
      \end{equation}
      ...
      \begin{equation}\label{equ:log_gamma_estimate}
        \log\G(z) = \frac{1}{2}\log(2\pi)+\left(z-\frac{1}{2}\right)\log(z)-z+O_{\e,\d}\left(1\right),
      \end{equation}
      ...
      \begin{proof}
        ...
        \[
          \log\G(z) = \frac{1}{2}\log(2\pi)+z\log(z)-z+O_{\e,\d}(1).
        \]
        Set $g(z) = \frac{1}{2}\log(2\pi)+z\log(z)-z$ so that $\log\G(z) = g(z)+O_{\e,\d}(1)$. Then $\log\G(z)-g(z) = O_{\e,\d}(1)$, and by Cauchy's integral formula, we have
        \begin{align*}
          \frac{\G'}{\G}(z) &= \frac{d}{dz}\left(g(z)+O_{\e,\d}(1)\right) \\
          &= g'(z)+\frac{d}{dz}(\log\G(z)-g(z)) \\
          &= \log(z)+\frac{1}{2\pi i}\int_{|w-z| = r}\frac{\log\G(w)-g(w)}{(w-z)^{2}}\,dw,
        \end{align*}
        for some sufficiently small radius $r > 0$ depending upon $\e$ and $\d$. Therefore
        \[
          \left|\frac{\G'}{\G}(z)-\log(z)\right| \le \frac{1}{2\pi}\int_{|w-z| = r}\frac{|\log\G(w)-g(w)|}{r^{2}}\,|dw| \ll_{\e,d} 1,
        \]
        where the last estimate follows because $\log\G(z)-g(z) = O_{\e,d}(1)$.
      \end{proof}
