  \chapter{Preliminaries}\label{ch:Preliminaries}
    ...
    \section{Integral Methods \& Transforms}
      \subsection*{Integral Methods}
        Complex integrals are a core backbone of analytic number theory and all too often the domain we are integrating over is unbounded. Accordingly, the functions we would like to work with should be integrable over these domains. One way to ``have our cake and eat it too'' would be to require that our functions decay near the unbounded regions. If $f$ is such that $f(x) = o(x^{c})$ as $x \to x_{0}$ for all $c > 0$ then we say $f$ has \textbf{exponential decay}\index{exponential decay} (or \textbf{rapid decay}\index{rapid decay}) as $x \to x_{0}$. This means that $f$ decays faster than any polynomial in $x$. If $f(x) = o(x^{c})$ for some $c > 1$ as $x \to x_{0}$ then we say $f$ has \textbf{polynomial decay}\index{polynomial decay} (or \textbf{moderate decay}\index{moderate decay}) as as $x \to x_{0}$. This is weaker than exponential decay but it is enough to guarantee that many integrals we take will be locally absolutely uniformly bounded:
        ...
        \begin{method}\label{met:decay_compacta_integral}
          ...
          \begin{enumerate}[label=(\roman*)]
            ...
            \item For any fixed $z$, we can decompose $D$ as $D = D' \cup (D-D')$ where $D'$ is compact, $f$ is integrable on $D'$, and of moderate decay on $(D-D')$.
          \end{enumerate}
          Then the integral is holomorpic in $z$. By (i) and \cref{thm:analytic_integral} it suffices to show that the integral is locally absolutely uniformly bounded. We argue by splitting the region of integration into $D = D' \cup (D-D')$ where $D'$ is compact. Choosing some compact subset $K$ of $\W$, the moderate decay of $f$ implies that there is some $c > 1$ such that $f(z,x) = o\left(x^{c}\right)$. Then
          \[
