  \chapter{Preliminaries}\label{ch:Preliminaries}
    ...
    \section{Integration Techniques \& Transforms}
      ...
      \subsection*{The Mellin Transform}
        ...
        \]
        If $f(x)$ is a sufficiently nice function then the integral will be bounded in some half-plane in $s$. For example, this happens if $f(x)$ exhibits rapid decay and remains bounded as $x \to 0$. In this case, the integral is locally absolutely uniformly bounded for $\s > 0$. If the Mellin transform $F(s)$ is sufficiently nice, then the initial function can be recovered via means of the \textbf{inverse Mellin transform}\index{inverse Mellin transform} $(\mc{M}^{-1}F)(x)$:
        \[
