\chapter{Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Lattices}
    Lattices are ubiquitous in analytic number theory because they provide a way to study discrete points by geometric methods. Let $V$ be an $n$-dimensional nondegenerate inner product space over a characteristic zero field $F$ with inner product $\<\cdot,\cdot\>$. We say that a subset $\L$ of $V$ is a \textbf{lattice}\index{lattice} if $\L$ is a free abelian group of rank $n$. In particular, any lattice $\L$ is of the form
    \[
      \L = \Z v_{1}+\cdots+\Z v_{n},
    \]
    for some basis $\{v_{1},\ldots,v_{n}\}$ of $V$. The \textbf{covolume}\index{covolume} $V_{\L}$ of $\L$ is defined to be
    \[
      V_{\L} = |V/\L|,
    \]
    which is finite by \cref{prop:base_change_quotient_determinant}. Moreover, if $\{e_{1},\ldots,e_{n}\}$ is an orthonormal basis for $V$, write
    \[
    ...
    \]
    Then $P$ is the base change matrix from $\{e_{1},\ldots,e_{n}\}$ to $\{v_{1},\ldots,v_{n}\}$ and so \cref{prop:base_change_quotient_determinant} shows that
    \[
      V_{\L} = |\det(P)|,
    \]
    We can now introduce the notion of dual lattices. If $\L$ is a lattice in $V$, then the \textbf{dual lattice}\index{dual lattice} $\L^{\vee}$ of $\L$ is defined by
    \[
      \L^{\vee} = \{v \in V:\text{$\<\l,v\> \in \Z$ for all $\l \in \L$}\}.
    \]
    In other words, $\L^{\vee}$ consists of all of the vectors in $V$ whose inner product with elements of the lattice $\L$ are integers. The dual lattice is indeed a lattice as the following proposition shows:

    \begin{proposition}\label{prop:dual_lattice_exists}
      If $\<v_{1},\ldots,v_{n}\>$ is a basis for $\L$, then the dual basis $\<v_{1}^{\vee},\ldots,v_{n}^{\vee}\>$ is a basis for $\L^{\vee}$. In particular, $\L^{\vee}$ is a lattice.
    \end{proposition}
    \begin{proof}
      ...
      \]
      with $v_{j}^{\vee} \in \R$ for all $j$. Since $\<v_{i},v_{j}^{\vee}\> = \d_{i,j}$ for $1 \le i,j \le n$, it follows that $v \in \L^{\vee}$ if and only if $v_{j}^{\vee} \in \Z$ for all $j$. This means that $\<v_{1}^{\vee},\ldots,v_{n}^{\vee}\>$ is a free abelian group of rank $n$. Hence $\L^{\vee}$ is a lattice.
    \end{proof}

    Since the dual of the dual basis of a vector space is the original basis, $(\L^{\vee})^{\vee} = \L$. It also turns out that the covolume of the dual lattice is the inverse of the volume of the original lattice:

    \begin{proposition}
      Let $\L$ be a lattice in $V$ and let $\L^{\vee}$ be its dual. Then
      \[
        V_{\L^{\vee}} = \frac{1}{V_{\L}}.
      \]
    \end{proposition}
    \begin{proof}
      Let $\{e_{1},\ldots,e_{n}\}$ be an orthonormal basis of $V$ and let $\{v_{1},\ldots,v_{n}\}$ be a basis for $\L$. By \cref{prop:dual_lattice_exists}, $\{v_{1}^{\vee},\ldots,v_{n}^{\vee}\}$ is a basis for $\L^{\vee}$. If $P$ is the associated generator matrix for $\L$, then $P$ is the base change matrix from $\{e_{1},\ldots,e_{n}\}$ to $\{v_{1},\ldots,v_{n}\}$. Hence $(P^{-1})^{t}$ is the base change matrix from $\{e_{1},\ldots,e_{n}\}$ to $\{v_{1}^{\vee},\ldots,v_{n}^{\vee}\}$. The claim follows by \cref{prop:base_change_quotient_determinant}.
    \end{proof}
    
    We now turn to the case when $V$ is an $n$-dimensional vector space over $\R$ and so we have more structure. We let $d\l$ be the associated Lebesgue measure on $V$. If $\{v_{1},\ldots,v_{n}\}$ is a basis of $V$ so that any $v \in V$ has the form
    \[
    ...
    \begin{proposition}\label{prop:lattice_volume_generator_matrix}
      Let $\L$ be a lattice in $V$ and let $\mc{M}$ be a fundamental domain. Then
      \[
        V_{\L} = \Vol(\mc{M}).
      \]
    \end{proposition}
    \begin{proof}
      Let $\{e_{1},\ldots,e_{n}\}$ be an orthonormal basis of $V$ and let $\{v_{1},\ldots,v_{n}\}$ be a basis for $\L$. Making the change of variables $x_{i} \to t_{i}$ for $1 \le i \le n$ where
      \[
    ...
    The most important result we will require about lattices is \textbf{Minkowski's lattice point theorem}\index{Minkowski's lattice point theorem} which states that, under some mild conditions, a set of sufficiently large volume in $\R^{n}$ contains a nonzero point of a lattice:
  ...
  \section{Integration Techniques \& Transforms}
    ...
    \begin{proof}
      ...
      \begin{align*}
        ...
        &= \sum_{\mathbf{n} \in \Z^{n}}\int_{[0,1]^{n}}f(\mathbf{x}+\mathbf{n})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} && \text{FT} \\
        &= \sum_{\mathbf{n} \in \Z^{n}}\int_{\mathbf{n}+[0,1]^{n}}f(\mathbf{x})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} \\
        &= \int_{\R^{n}}f(\mathbf{x})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} && \text{FT} \\
        &= (\mc{F}f)(\mathbf{t}).
      \end{align*}
