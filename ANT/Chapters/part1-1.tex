  \chapter{Preliminaries}\label{ch:Preliminaries}
    ...
    \section{Notational Conventions}
      ...
      \begin{itemize}
        ...
        \item All continuous, meromorphic, holomorphic, and analytic functions are assumed to be real or complex valued if they are not already specified as such.
    ...
    \section{Asymptotics}
      ...
      \begin{center}
        ...
        \end{stabular}
      \end{center}
      Implicit in all of these asymptotics is some limiting process $z \to z_{0}$ where $z_{0}$ is finite or $\infty$. If $z_{0}$ is finite, then it is understood that the asymptotic is assumed to hold for all $z$ sufficiently close to $z_{0}$. If $z_{0}$ is infinite, then the asymptotic is assumed to hold for sufficiently large $z$. If the limiting process is not explicitly mentioned, it is assumed to be as $z \to \infty$. Often times, asymptotics will hold for all admissible values of $z$ and this will be clear from context although we still might suppress the specific limiting process.

      \begin{remark}
        Suppose $f,g:\Z_{\ge 0} \to \C$. Extending $f(n)$ and $g(n)$ by making them piecewise linear so that they are piecewise continuous, we can consider asymptotics with $n$ in place of $z$. All of the following theory still holds. Moreover, if we further take $f(z)$ or $g(z)$ to be a constant function, the following theory will still hold.
      \end{remark}
      ...
      \subsection*{\texorpdfstring{$O$}{O}-estimates \& Symbols}
        ...
        \[
          |f(z)| \le c|g(z)|,
        \]
        holds as $z \to z_{0}$. We call this an \textbf{$O$-estimate}\index{$O$-estimate} and say that $f(z)$ has \textbf{growth at most}\index{growth at most} $g(z)$. The $O$-estimate says that for $z$ close to $z_{0}$, the size of $f(z)$ grows like $g(z)$.
      ...
      \subsection*{\texorpdfstring{$o$}{o}-estimates \& Symbols}
        ...
        \]
        provided $g(z) \neq 0$ for all $z$ sufficiently close to $z_{0}$. We call this an \textbf{$o$-estimate}\index{$o$-estimate} and say that $f(z)$ has \textbf{growth less than}\index{growth less than} $g(z)$. The $o$-estimate says that for $z$ close to $z_{0}$, $g(z)$ dominates $f(z)$. If $f(z) = o(g(z))$ as $z \to z_{0}$, then $f(z) = O(g(z))$ as $z \to z_{0}$ where the implicit constant can be taken arbitrarily small by definition of the $o$-estimate. Therefore, $o$-estimates are stronger than $O$-estimates. As a symbol, let $o(g(z))$ stand for a function $f(z)$ that is $o(g(z))$. Then we may use the $o$-estimates in algebraic equations and inequalities. Note that this extends the definition of the symbol because $f(z) = o(g(z))$ means $f(z)$ is $o(g(z))$. We say $f(z)$ \textbf{is asymptotic to}\index{is asymptotic to} $g(z)$ as $z \to z_{0}$ and write $f(z) \sim g(z)$ if
        \[
          \lim_{z \to z_{0}}\left|\frac{f(z)}{g(z)}\right| = 1,
        \]
        provided $g(z) \neq 0$ for all $z$ sufficiently close to $z_{0}$. We call this an \textbf{asymptotic}\index{asymptotic} and say that $f(z)$ and $g(z)$ are \textbf{asymptotically equivalent}\index{asymptotically equivalent}. It is useful to think of asymptotic equivalence as $f(z)$ and $g(z)$ being the same size in the limit as $z \to z_{0}$. Immediately from the definition, we see that this is an equivalence relation on functions. In particular, if $f(z) \sim g(z)$ and $g(z) \sim h(z)$ then $f(z) \sim h(z)$. Also, if $f(z) \sim g(z)$ as $z \to z_{0}$, then $f(z) \asymp g(z)$ as $z \to z_{0}$ with $c_{1} \le 1 \le c_{2}$. So asymptotic equivalence is stronger than being of the same order of magnitude. Also note that $f(z) \sim g(z)$ is equivalent to $f(z) = g(z)(1+o(1))$ and hence implies $f(z) = g(z)(1+O(1))$. We say $f(z)$ \textbf{is of larger order than}\index{is of larger order than} $g(z)$ or $f(z)$ is $\W(g(z))$ as $z \to z_{0}$ and write $f(z) = \W(g(z))$ if
        \[
          \limsup_{z \to z_{0}}\left|\frac{f(z)}{g(z)}\right| > 0.
        \]
        provided $g(z) \neq 0$ for all $z$ sufficiently close to $z_{0}$. We call this an \textbf{$\W$-estimate}\index{$\W$-estimate} and say that $f(z)$ has \textbf{growth at least}\index{growth at least} $g(z)$. Observe that $f(z) = \W(g(z))$ is precisely the negation of $f(z) = o(g(z))$, so that $f(z) = \W(g(z))$ means $f(z) = o(g(z))$ is false. This is weaker than $f(z) \gg g(z)$ because $f(z) = \W(g(z))$ means $|f(z)| \ge c|g(z)|$ for some values of $z$ arbitrarily close to $z_{0}$ whereas $f(z) \gg g(z)$ means $|f(z)| \ge c|g(z)|$ for all values of $z$ sufficiently close to $z_{0}$.
      \subsection*{Algebraic Manipulation for \texorpdfstring{$O$}{O}-estimates and \texorpdfstring{$o$}{o}-estimates}
        ...
        \begin{proposition}
          Suppose $f(z) = O(g(z))$ as $z \to \infty$, $f(z)$ and $g(z)$ are integrable on a domain where this estimate holds, and let $z_{1}$ and $z_{2}$ belong to this region. Then
          \[
        ...
        \begin{proposition}
          Let $f(z)$ be a function such that $f(z) \to 0$ as $z \to z_{0}$. The following $O$-estimates hold as $z \to z_{0}$:
          \begin{enumerate}[label=(\roman*)]
            \item $\frac{1}{1+O(f(z))} = 1+O(f(z))$.
            \item $(1+O(f(z)))^{z} = 1+O(f(z))$ for any complex $z$.
            \item $\log(1+O(f(z))) = O(f(z))$.
            \item $e^{1+O(f(z))} = 1+O(f(z))$.
          \end{enumerate}
        \end{proposition}
        \begin{proof}
          Taking the Taylor series truncated after the first term and applying Taylor's theorem gives the following $O$-estimates as $x \to 0$:
          \begin{enumerate}[label=(\roman*)]
            \item $\frac{1}{1+x} = 1+O(x)$.
            \item $(1+x)^{z} = 1+O(x)$.
            \item $\log(1+x) = O(x)$.
            \item $e^{x} = 1+O(x)$.
          \end{enumerate}
      ...
      \subsection*{Growth \& Decay of Functions}
        We will also be interested in the growth rate of functions. There are many types of growth rates, but we will only recall the ones that are standard. Throughout let $c \ge 1$. First suppose $z \to z_{0}$. If $f(z) \asymp \log^{c}(z)$, we say that $f(z)$ is of \textbf{logarithmic growth}\index{logarithmic growth}. If $f(z) \asymp z^{c}$, we say that $f(z)$ is of \textbf{polynomial growth}\index{polynomial growth}. If $f(z) \asymp e^{cz}$, we say that $f(z)$ is of \textbf{exponential growth}\index{exponential growth}. Now suppose $z \to \infty$. If $f(z) \asymp \log^{-c}(z)$, we say that $f(z)$ is of \textbf{logarithmic decay}\index{logarithmic decay}. If $f(z) \asymp z^{-c}$ for some $c \ge 1$, we say that $f(z)$ is of \textbf{polynomial decay}\index{polynomial decay}. If $f(z) \asymp e^{-cz}$, we say that $f(z)$ is of \textbf{exponential decay}\index{exponential decay}. In all of these cases, we refer to the constant $c$ as the \textbf{order}\index{order} of growth or decay respectively. If $f(z) = \W(z^{n})$ for all $n \ge 0$, then we say $f(z)$ is of \textbf{rapid growth}\index{rapid growth}. Alternatively, if $f(z) = o(z^{-n})$ for all $n \ge 0$, then we say $f(z)$ is of \textbf{rapid decay}\index{rapid decay}.
    ...
    \section{Integration Techniques \& Transforms}
      \subsection*{Integration Techniques}
        ...
        \begin{proposition}\label{prop:decay_finite_volume_integral}
          ...
          \[
            \int_{D}f(z,w)\,d\mu,
          \]
          is locally absolutely uniformly convergent for any $z \in W$. In particular, it is holomorphic in $z$.
        \end{proposition}
        \begin{proof}
        ...
        There is also a useful analytic technique called \textbf{shifting the line of integration}\index{shifting the line of integration}:
      ...
      \subsection*{The Fourier Transform}
        ...
        There are two practical settings of interest. The first is when $f(x)$ is of \textbf{Schwarz class}\index{Schwarz class} which means that $f \in C^{\infty}(\R)$ and $f$ along with all of its derivative have rapid decay. For example $e^{-x^{2}}$ is of Schwarz class. If $f(x)$ is of Schwarz class, the rapid decay implies that $f(x)$ and all of its derivatives are absolutely integrable over $\R$ by \cref{prop:decay_unbounded_inteval_integral}. Moreover, this also implies that $F(x)$ and all of its derivatives are locally absolutely uniformly convergent by the Weierstrass $M$-test. The uniform limit theorem implies that $F(x)$ is smooth and thus the conditions of the Poisson summation formula are satisfied. The second application is when $f(z)$ is some holomorphic function that has at least polynomial decay of order $c > 1$ for any fixed $y$. Then $f(z)$ is absolutely integrable over $\R$ for any fixed $y$. Moreover, $F(z)$ is holomorphic. Then the conditions of the Poisson summation formula are satisfied for $z = iy$ with $y > 0$ and the identity theorem shows that this holds for all $z$. Our primary use of the Poisson summation formula will be to derive very striking transformation laws which are the key ingredient in analytically continuing many important functions.
