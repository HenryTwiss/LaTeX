\chapter{Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Lattices}
    Lattices are ubiquitous in analytic number theory because they provide a way to study discrete points by geometric methods. Let $V$ be an $n$-dimensional inner product space over a characteristic zero field $F$ with nondegenerate symmetric inner product $\<\cdot,\cdot\>$. We say that a subset $\L$ of $V$ is a \textbf{lattice}\index{lattice} if $\L$ is a free abelian group. In particular, any lattice $\L$ is of the form
    \[
      \L = \Z v_{1}+\cdots+\Z v_{m},
    \]
    for some linearly independent subset $\{v_{1},\ldots,v_{m}\}$ of $V$ with $m \le n$. We say $\L$ is \textbf{complete}\index{complete} if $n = m$. Necessarily, $\{v_{1},\ldots,v_{n}\}$ is a basis of $V$. The \textbf{covolume}\index{covolume} $V_{\L}$ of a complete lattice $\L$ is defined to be
    \[
      V_{\L} = |V/\L|,
    \]
    ...
    \[
      V_{\L} = |\det(P)|.
    \]
    We can now introduce the notion of dual lattices. If $\L$ is a complete lattice in $V$, then the \textbf{dual lattice}\index{dual lattice} $\L^{\vee}$ of $\L$ is defined by
    \[
    ...
    \begin{proposition}\label{prop:dual_lattice_exists}
      If $\<v_{1},\ldots,v_{n}\>$ is a basis for $\L$, then the dual basis $\<v_{1}^{\vee},\ldots,v_{n}^{\vee}\>$ is a basis for $\L^{\vee}$. In particular, $\L^{\vee}$ is a complete lattice.
    \end{proposition}
    \begin{proof}
    ...
    \begin{proposition}\label{prop:lattice_volume_generator_matrix}
      Let $\L$ be a complete lattice in an $n$-dimensional real inner product space $V$ and let $\mc{M}$ be a fundamental domain for $\L$. Then
      \[
        V_{\L} = \Vol(\mc{M}).
      \]
    \end{proposition}
    ...
    It turns out that for subgroups of $V$, being a lattice is equivalent to being discrete:

    \begin{proposition}
      Let $\L$ be a subgroup of an $n$-dimensional real inner product space $V$. Then $\L$ is a lattice if and only if it is discrete.
    \end{proposition}
    \begin{proof}
      First suppose $\L$ is a discrete subgroup. Then $\L$ is closed (since $V$ is a metric space). Let $V'$ be the subspace spanned by $\L$ and let its dimension be $m$. Choosing a basis $\{v'_{1},\ldots,v'_{m}\}$ of $V'$, set
      \[
        \L' = \Z v'_{1}+\cdots+\Z v'_{m}.
      \]
      Then $\L'$ is a complete lattice in $V'$ and $\L' \subseteq \L$. We claim that $[\L:\L']$ is finite. \todo{xxx}
    \end{proof}

    The most important result we will require about complete lattices is \textbf{Minkowski's lattice point theorem}\index{Minkowski's lattice point theorem} which states that, under some mild conditions, a set of sufficiently large volume in $\R^{n}$ contains a nonzero point of a complete lattice:
