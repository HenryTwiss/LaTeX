    ...
    \section{Asymptotics}
      ...
      \subsection*{\texorpdfstring{$O$}{O}-estimates \& Symbols}
        ...
        \[
          |f(z)| \le c|g(z)|,
        \]
        holds as $z \to z_{0}$. We call this a \textbf{$O$-estimate}\index{$O$-estimate}. We also say $f(z)$ has \textbf{growth}\index{growth} at most $g(z)$. The $O$-estimate says that for $z$ close to $z_{0}$, the size of $f(z)$ grows like $g(z)$.
      ...
      \subsection*{Growth \& Decay of Functions}
        We will also be interested in the growth rate of functions. We can compactly express these types of growth using asymptotics. There are many types of growth rates, but we will only recall the ones that will be of use to us. Throughout let $c \ge 1$. First suppose $z \to z_{0}$. If $f(z) = O(\log^{c}(z))$, we say that $f(z)$ is of at most \textbf{logarithmic growth}\index{logarithmic growth}. If $f(z) = O(z^{c})$, we say that $f(z)$ is of at most \textbf{polynomial growth}\index{polynomial growth}. If $f(z) = o(e^{cz})$, we say that $f(z)$ is of at most \textbf{moderate growth}\index{moderate growth}. If $f(z) = O(e^{cz})$, we say that $f(z)$ is of at most \textbf{exponential growth}\index{exponential growth}. Now suppose $z \to \infty$ (or $z \to \pm\infty$ in the real case). If $f(z) = O(\log^{-c}(z))$, for some $c \ge 1$, we say that $f(z)$ is of \textbf{logarithmic decay}\index{logarithmic decay}. If $f(z) = O(x^{-c})$ for some $c \ge 1$, we say that $f(z)$ is of \textbf{polynomial decay}\index{polynomial decay}. If $f(z) = O(e^{-cz})$, we say that $f(z)$ is of \textbf{moderate decay}\index{moderate decay}. If $f(z) = o(e^{-cz})$, we say that $f(z)$ is of \textbf{exponential decay}\index{exponential decay}. In all of these cases, we refer to the constant $c$ as the \textbf{order}\index{order} of growth or decay respectively.
