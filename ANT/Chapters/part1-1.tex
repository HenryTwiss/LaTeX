\chapter{Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Notational Conventions}
    ...
    \begin{itemize}
      ...
      \item On $\R^{n}$, the inner product and norm are denoted by $\<\cdot,\cdot\>$ and $||\cdot||$ respectively unless specified otherwise.
    ...
    \end{itemize}
  \section{Asymptotics}
    Throughout we let $\mathbf{z} \in \R^{n}$ or $\mathbf{z} \in \C^{n}$. Much of the language of analytic number theory is given in terms of asymptotics (or estimates or bounds) as they allows us to discuss approximate growth and dispense with superfluous constants. For this reason, asymptotics will be the first material that we will present. The asymptotics that we will cover are listed in the following table:
    \begin{center}
      \begin{stabular}[1.5]{|c|c|c|}
        \hline
        Asymptotics & Notation \\
        \hline
        Big O & $f(\mathbf{z}) = O(g(\mathbf{z}))$ \\
        \hline
        Vinogradov's symbol & $f(\mathbf{z}) \ll g(\mathbf{z})$ \\
        \hline
        Order of magnitude symbol & $f(\mathbf{z}) \asymp g(\mathbf{z})$ \\
        \hline
        Little o & $f(\mathbf{z}) = o(g(\mathbf{z}))$ \\
        \hline
        Asymptotic equivalence & $f(\mathbf{z}) \sim g(\mathbf{z})$ \\
        \hline
        Omega symbol & $f(\mathbf{z}) = \W(g(\mathbf{z}))$ \\
        \hline
      \end{stabular}
    \end{center}
    Implicit in all of these asymptotics is some limiting process $\mathbf{z} \to \mathbf{z}_{0}$ where $\mathbf{z}_{0}$ is finite or $\infty$. If $\mathbf{z}_{0}$ is finite, then it is understood that the asymptotic is assumed to hold for all $\mathbf{z}$ sufficiently close to $\mathbf{z}_{0}$ in norm. If $\mathbf{z}_{0}$ is infinite, then the asymptotic is assumed to hold for sufficiently large $\mathbf{z}$. If the limiting process is not explicitly mentioned, it is assumed to be as $\mathbf{z} \to \infty$. Often times, asymptotics will hold for all admissible values of $\mathbf{z}$ and this will be clear from context although we still might suppress the specific limiting process.

    \begin{remark}
      Suppose $f,g:\Z_{\ge 0}^{d} \to \C$. Extending $f(\mathbf{n})$ and $g(\mathbf{n})$ by making them piecewise linear so that they are piecewise continuous, we can consider asymptotics with $\mathbf{n}$ in place of $\mathbf{z}$. All of the following theory still holds. Moreover, if we further take $f(\mathbf{z})$ or $g(\mathbf{z})$ to be a constant function, the following theory will still hold.
    \end{remark}
    ...
    \subsection*{\texorpdfstring{$O$}{O}-estimates \& Symbols}
      We say $f(\mathbf{z})$ \textbf{is of order}\index{is of order} $g(\mathbf{z})$ or $f(\mathbf{z})$ is $O(g(\mathbf{z}))$ as $\mathbf{z} \to \mathbf{z}_{0}$ and write $f(\mathbf{z}) = O(g(\mathbf{z}))$
      if there is some positive constant $c$ such that
      \[
        |f(\mathbf{z})| \le c|g(\mathbf{z})|,
      \]
      holds as $\mathbf{z} \to \mathbf{z}_{0}$. We call this an \textbf{$O$-estimate}\index{$O$-estimate} and say that $f(\mathbf{z})$ has \textbf{growth at most}\index{growth at most} $g(\mathbf{z})$. The $O$-estimate says that for $\mathbf{z}$ close to $\mathbf{z}_{0}$, the size of $f(\mathbf{z})$ grows like $g(\mathbf{z})$.
      
      \begin{remark}
        Many authors assume that $g(\mathbf{z})$ is a nonnegative function so that the absolute values on $g(\mathbf{z})$ can be dropped. As we require asymptotics that will be used more generally, we do not make this assumption since one could very well replace $O(g(\mathbf{z}))$ with $O(|g(\mathbf{z})|)$. In practice this deviation causes no issue.
      \end{remark}
      
      As a symbol, let $O(g(\mathbf{z}))$ stand for a function $f(\mathbf{z})$ that is $O(g(\mathbf{z}))$. Then we may use the $O$-estimates in algebraic equations and inequalities. Note that this extends the definition of the symbol because $f(\mathbf{z}) = O(g(\mathbf{z}))$ means $f(\mathbf{z})$ is $O(g(\mathbf{z}))$. Moreover, in algebraic equations and inequalities involving $O$-estimates it is often customary to refer to such an $O$-estimate as an \textbf{error term}\index{error term}. The symbol $\ll$ is known as \textbf{Vinogradov's symbol}\index{Vinogradov's symbol} and it is an alternative way to express $O$-estimates. We write $f(\mathbf{z}) \ll g(\mathbf{z})$ as $\mathbf{z} \to \mathbf{z}_{0}$ if $f(\mathbf{z}) = O(g(\mathbf{z}))$ as $\mathbf{z} \to \mathbf{z}_{0}$. We also write $f(\mathbf{z}) \gg g(\mathbf{z})$ as $\mathbf{z} \to \mathbf{z}_{0}$ to mean $g(\mathbf{z}) \ll f(\mathbf{z})$ as $\mathbf{z} \to \mathbf{z}_{0}$. If there is a dependence of the implicit constant on parameters, we use subscripts to denote dependence on these parameters. If both $f(\mathbf{z}) \ll g(\mathbf{z})$ and $g(\mathbf{z}) \ll f(\mathbf{z})$ as $\mathbf{z} \to \mathbf{z}_{0}$, then we say $f(\mathbf{z})$ and $g(\mathbf{z})$ have the \textbf{same order of magnitude}\index{same order of magnitude} and write $f(\mathbf{z}) \asymp g(\mathbf{z})$ as $\mathbf{z} \to \mathbf{z}_{0}$. We also say $f(\mathbf{z})$ has \textbf{growth}\index{growth} $g(\mathbf{z})$. If there is a dependence of the implicit constant on parameters, we use subscripts to denote dependence on these parameters. From the definition of the $O$-estimate, this is equivalent to the existence of positive constants $c_{1}$ and $c_{2}$ such that
      \[
        c_{1}|g(\mathbf{z})| \le |f(\mathbf{z})| \le c_{2}|g(\mathbf{z})|.
      \]
      Equivalently, we can interchange $f(\mathbf{z})$ and $g(\mathbf{z})$ in the above equation.
    \subsection*{\texorpdfstring{$o$}{o}-estimates \& Symbols}
      We say $f(\mathbf{z})$ \textbf{is of smaller order than}\index{is of smaller order than} $g(\mathbf{z})$ or $f(\mathbf{z})$ is $o(g(\mathbf{z}))$ as $\mathbf{z} \to \mathbf{z}_{0}$ and write $f(\mathbf{z}) = o(g(\mathbf{z}))$ if
      \[
        \lim_{\mathbf{z} \to \mathbf{z}_{0}}\left|\frac{f(\mathbf{z})}{g(\mathbf{z})}\right| = 0,
      \]
      provided $g(\mathbf{z}) \neq 0$ for all $\mathbf{z}$ sufficiently close to $\mathbf{z}_{0}$ in norm. We call this an \textbf{$o$-estimate}\index{$o$-estimate} and say that $f(\mathbf{z})$ has \textbf{growth less than}\index{growth less than} $g(\mathbf{z})$. The $o$-estimate says that for $\mathbf{z}$ close to $\mathbf{z}_{0}$, $g(\mathbf{z})$ dominates $f(\mathbf{z})$. If $f(\mathbf{z}) = o(g(\mathbf{z}))$ as $\mathbf{z} \to \mathbf{z}_{0}$, then $f(\mathbf{z}) = O(g(\mathbf{z}))$ as $\mathbf{z} \to \mathbf{z}_{0}$ where the implicit constant can be taken arbitrarily small by definition of the $o$-estimate. Therefore, $o$-estimates are stronger than $O$-estimates. As a symbol, let $o(g(\mathbf{z}))$ stand for a function $f(\mathbf{z})$ that is $o(g(\mathbf{z}))$. Then we may use the $o$-estimates in algebraic equations and inequalities. Note that this extends the definition of the symbol because $f(\mathbf{z}) = o(g(\mathbf{z}))$ means $f(\mathbf{z})$ is $o(g(\mathbf{z}))$. We say $f(\mathbf{z})$ \textbf{is asymptotic to}\index{is asymptotic to} $g(\mathbf{z})$ as $\mathbf{z} \to \mathbf{z}_{0}$ and write $f(\mathbf{z}) \sim g(\mathbf{z})$ if
      \[
        \lim_{\mathbf{z} \to \mathbf{z}_{0}}\left|\frac{f(\mathbf{z})}{g(\mathbf{z})}\right| = 1,
      \]
      provided $g(\mathbf{z}) \neq 0$ for all $\mathbf{z}$ sufficiently close to $\mathbf{z}_{0}$ in norm. We call this an \textbf{asymptotic}\index{asymptotic} and say that $f(\mathbf{z})$ and $g(\mathbf{z})$ are \textbf{asymptotically equivalent}\index{asymptotically equivalent}. It is useful to think of asymptotic equivalence as $f(\mathbf{z})$ and $g(\mathbf{z})$ being the same size in the limit as $\mathbf{z} \to \mathbf{z}_{0}$. Immediately from the definition, we see that this is an equivalence relation on functions. In particular, if $f(\mathbf{z}) \sim g(\mathbf{z})$ and $g(\mathbf{z}) \sim h(\mathbf{z})$ then $f(\mathbf{z}) \sim h(\mathbf{z})$. Also, if $f(\mathbf{z}) \sim g(\mathbf{z})$ as $\mathbf{z} \to \mathbf{z}_{0}$, then $f(\mathbf{z}) \asymp g(\mathbf{z})$ as $\mathbf{z} \to \mathbf{z}_{0}$ with $c_{1} \le 1 \le c_{2}$. So asymptotic equivalence is stronger than being of the same order of magnitude. Also note that $f(\mathbf{z}) \sim g(\mathbf{z})$ is equivalent to $f(\mathbf{z}) = g(\mathbf{z})(1+o(1))$ and hence implies $f(\mathbf{z}) = g(\mathbf{z})(1+O(1))$. We say $f(\mathbf{z})$ \textbf{is of larger order than}\index{is of larger order than} $g(\mathbf{z})$ or $f(\mathbf{z})$ is $\W(g(\mathbf{z}))$ as $\mathbf{z} \to \mathbf{z}_{0}$ and write $f(\mathbf{z}) = \W(g(\mathbf{z}))$ if
      \[
        \limsup_{\mathbf{z} \to \mathbf{z}_{0}}\left|\frac{f(\mathbf{z})}{g(\mathbf{z})}\right| > 0.
      \]
      provided $g(\mathbf{z}) \neq 0$ for all $\mathbf{z}$ sufficiently close to $\mathbf{z}_{0}$ in norm. We call this an \textbf{$\W$-estimate}\index{$\W$-estimate} and say that $f(\mathbf{z})$ has \textbf{growth at least}\index{growth at least} $g(\mathbf{z})$. Observe that $f(\mathbf{z}) = \W(g(\mathbf{z}))$ is precisely the negation of $f(\mathbf{z}) = o(g(\mathbf{z}))$, so that $f(\mathbf{z}) = \W(g(\mathbf{z}))$ means $f(\mathbf{z}) = o(g(\mathbf{z}))$ is false. This is weaker than $f(\mathbf{z}) \gg g(\mathbf{z})$ because $f(\mathbf{z}) = \W(g(\mathbf{z}))$ means $|f(\mathbf{z})| \ge c|g(\mathbf{z})|$ for some values of $\mathbf{z}$ arbitrarily close to $\mathbf{z}_{0}$ whereas $f(\mathbf{z}) \gg g(\mathbf{z})$ means $|f(\mathbf{z})| \ge c|g(\mathbf{z})|$ for all values of $\mathbf{z}$ sufficiently close to $\mathbf{z}_{0}$ in norm.
    \subsection*{Algebraic Manipulation for \texorpdfstring{$O$}{O}-estimates and \texorpdfstring{$o$}{o}-estimates}
      ...
      \begin{proposition}\label{prop:Big_Oh_manipulations}
          The following $O$-estimates hold as $\mathbf{z} \to \mathbf{z}_{0}$:
          \begin{enumerate}[label=(\roman*)]
            \item If $f(\mathbf{z}) = O(g(\mathbf{z}))$ and $g(\mathbf{z}) = O(h(\mathbf{z}))$, then $f(\mathbf{z}) = O(h(\mathbf{z}))$. Equivalently, $O(O(h(\mathbf{z}))) = O(h(\mathbf{z}))$.
            \item If $f_{i}(\mathbf{z}) = O(g_{i}(\mathbf{z}))$ for $i = 1,2$, then $f_{1}(\mathbf{z})f_{2}(\mathbf{z}) = O(g_{1}(\mathbf{z})g_{2}(\mathbf{z}))$.
            \item If $f(\mathbf{z}) = O(g(\mathbf{z})h(\mathbf{z}))$, then $f(\mathbf{z}) = g(\mathbf{z})O(h(\mathbf{z}))$.
            \item If $f_{i}(\mathbf{z}) = O(g_{i}(\mathbf{z}))$ for $i = 1,2,\ldots,n$, then $\sum_{1 \le i \le n}f_{i}(\mathbf{z}) = O\left(\sum_{1 \le i \le n}|g_{i}(\mathbf{z})|\right)$.
            \item If $f_{n}(\mathbf{z}) = O(g_{n}(\mathbf{z}))$ for $n \ge 1$, then $\sum_{n \ge 1}f_{n}(\mathbf{z}) = O\left(\sum_{n \ge 1}|g_{n}(\mathbf{z})|\right)$ provided both $\sum_{n \ge 1}f_{n}(\mathbf{z})$ and $\sum_{n \ge 1}|g_{n}(\mathbf{z})|$ converge.
            \item If $f(\mathbf{z}) = O(g(\mathbf{z}))$ as $\mathbf{z} \to \mathbf{z}_{0}$ and $T(\mathbf{z})$ is such that $T(\mathbf{z}) \to \mathbf{z}_{0}$ as $\mathbf{z} \to \mathbf{z}_{0}$, then $(f \circ T)(\mathbf{z}) = O((g \circ T)(\mathbf{z}))$.
            \item If $f(\mathbf{z}) = O(g(\mathbf{z}))$, then $\Re(f(\mathbf{z})) = O(g(\mathbf{z}))$ and $\Im(f(\mathbf{z})) = O(g(\mathbf{z}))$.
          \end{enumerate}
      \end{proposition}
      \begin{proof}
      ...
      \begin{proposition}
        Suppose $f(\mathbf{z}) = O(g(\mathbf{z}))$ as $\mathbf{z} \to \infty$, $f(\mathbf{z})$ and $g(\mathbf{z})$ are integrable on a domain where this estimate holds, and let $\mathbf{z}_{1}$ and $\mathbf{z}_{2}$ belong to this region. Then
        \[
          \int_{\mathbf{z}_{1}}^{\mathbf{z}_{2}}f(\mathbf{z})\,d\mathbf{z} = O\left(\int_{\mathbf{z}_{1}}^{\mathbf{z}_{2}}|g(\mathbf{z})|\,d\mathbf{z}\right).
        \]
      \end{proposition}
      \begin{proof}
      ...
      \begin{proposition}
        Let $f(\mathbf{z})$ be a function such that $f(\mathbf{z}) \to 0$ as $\mathbf{z} \to \mathbf{z}_{0}$. The following $O$-estimates hold as $\mathbf{z} \to \mathbf{z}_{0}$:
        \begin{enumerate}[label=(\roman*)]
          \item $\frac{1}{1+O(f(\mathbf{z}))} = 1+O(f(\mathbf{z}))$.
          \item $(1+O(f(\mathbf{z})))^{w} = 1+O(f(\mathbf{z}))$ for any complex $w$.
          \item $\log(1+O(f(\mathbf{z}))) = O(f(\mathbf{z}))$.
          \item $e^{1+O(f(\mathbf{z}))} = 1+O(f(\mathbf{z}))$.
        \end{enumerate}
      \end{proposition}
      \begin{proof}
      ...
      \begin{proposition}\label{prop:Little_Oh_manipulations}
          ...
          \begin{enumerate}[label=(\roman*)]
            ...
            \item If $f(\mathbf{z}) = o(g(\mathbf{z})h(\mathbf{z}))$, then $f(\mathbf{z}) = g(\mathbf{z})o(h(\mathbf{z}))$.
            \item If $f_{i}(\mathbf{z}) = o(g_{i}(\mathbf{z}))$ for $i = 1,2,\ldots,n$, then $\sum_{1 \le i \le n}f_{i}(\mathbf{z}) = o\left(\sum_{1 \le i \le n}|g_{i}(\mathbf{z})|\right)$.
            \item If $f(\mathbf{z}) = o(g(\mathbf{z}))$ as $\mathbf{z} \to \mathbf{z}_{0}$ and $h(\mathbf{z})$ is such that $h(\mathbf{z}) \to \mathbf{z}_{0}$ as $\mathbf{z} \to \mathbf{z}_{0}$, then $(f \circ h)(\mathbf{z}) = o((g \circ h)(\mathbf{z}))$.
          \end{enumerate}
      \end{proposition}
      \begin{proof}
        Statements (i)-(iii) and (v) follow immediately from the definition of the $o$-estimate. Statement (iv) follows from the definition and that $\sum_{1 \le i \le n}|g_{i}(\mathbf{z})| \ge |g_{i}(\mathbf{z})|$.
      \end{proof}
    \subsection*{Growth \& Decay of Functions}
      We will also be interested in the growth rate of functions. There are many types of growth rates, but we will only recall the ones that are standard. Throughout let $c \ge 1$. First suppose $\mathbf{z} \to \mathbf{z}_{0}$. If $f(\mathbf{z}) \asymp \log^{c}||\mathbf{z}||$, we say that $f(\mathbf{z})$ is of \textbf{logarithmic growth}\index{logarithmic growth}. If $f(\mathbf{z}) \asymp ||\mathbf{z}||^{c}$, we say that $f(\mathbf{z})$ is of \textbf{polynomial growth}\index{polynomial growth}. If $f(\mathbf{z}) \asymp e^{c||\mathbf{z}||}$, we say that $f(\mathbf{z})$ is of \textbf{exponential growth}\index{exponential growth}. Now suppose $\mathbf{z} \to \infty$. If $f(\mathbf{z}) \asymp \log^{-c}||\mathbf{z}||$, we say that $f(\mathbf{z})$ is of \textbf{logarithmic decay}\index{logarithmic decay}. If $f(\mathbf{z}) \asymp ||\mathbf{z}||^{-c}$ for some $c \ge 1$, we say that $f(\mathbf{z})$ is of \textbf{polynomial decay}\index{polynomial decay}. If $f(\mathbf{z}) \asymp e^{-c||\mathbf{z}||}$, we say that $f(\mathbf{z})$ is of \textbf{exponential decay}\index{exponential decay}. In all of these cases, we refer to the constant $c$ as the \textbf{order}\index{order} of growth or decay respectively. If $f(\mathbf{z}) = \W(||\mathbf{z}||^{n})$ for all $n \ge 0$, then we say $f(\mathbf{z})$ is of \textbf{rapid growth}\index{rapid growth}. Alternatively, if $f(\mathbf{z}) = o(||\mathbf{z}||^{-n})$ for all $n \ge 0$, then we say $f(\mathbf{z})$ is of \textbf{rapid decay}\index{rapid decay}.
  ...
  \section{Lattices}
    Lattices are ubiquitous in analytic number theory because they provide a way to study discrete points by geometric methods. Let $V$ be an $n$-dimensional Euclidean vector space and let $\<\cdot,\cdot\>$ be the inner product on $V$ that induces the norm $||\cdot||$. We let $d\l$ be the associated Lebesgue measure on $V$. If $\{v_{1},\ldots,v_{n}\}$ is a basis of $V$ so that any $v \in V$ has the form
    \[
    ...
    \]
    is the volume of any measurable subset $X \subseteq V$. We say that $\L \subset V$ is a \textbf{lattice}\index{lattice} if $\L$ is a free abelian group of rank $n$. In particular, any lattice $\L$ is of the form
    \[
      \L = \Z v_{1}+\cdots+\Z v_{n},
    \]
    for some basis $\{v_{1},\ldots,v_{n}\}$ of $V$. Note that $\L$ acts on $V$ by automorphisms given by translation. That is, we have a group action
    \[
    ...
    \]
    Indeed, since $\L$ acts by translations, it is obvious that $\mc{M}$ is a fundamental domain for $V/\L$. Moreover, any translation of $\mc{M}$ by an element of $\L$ is also a fundamental domain. We define the \textbf{covolume}\index{covolume} $V_{\L}$ of $\L$ by
    \[
      V_{\L} = \mathrm{Vol}(\mc{M}).
    \]
    That is, the covolume of $\L$ is the covolume of the fundamental domain $\mc{M}$. Since $d\l$ is translation invariant (as it is the Lebesgue measure on $V$), $V_{\L}$ is independent of the choice of fundamental domain. If $\L$ is given by the basis $\{v_{1},\ldots,v_{n}\}$, we write
    \[
      v_{i} = \sum_{1 \le i \le n}v_{i,j}e_{j},
    \]
    where $\{e_{1},\ldots,e_{n}\}$ is an orthonormal basis for $V$, and define the \textbf{generator matrix}\index{generator matrix} $P$ of $\L$ by
    \[
      P = \begin{pmatrix} v_{1,1} & \cdots & v_{1,n} \\ \vdots & & \vdots \\ v_{n,1} & \cdots & v_{n,n} \end{pmatrix}.
    \]
    By two applications of \cref{prop:base_change_quotient_determinant}, one for a basis for $V$ and the other for a choice of orthonormal basis $\{e_{1},\ldots,e_{n}\}$, we see that $|\det(P)|$ is independent of both bases. The following proposition shows that the covolume of $\L$ is given by $|\det(P)|$:

    \begin{proposition}\label{prop:lattice_volume_generator_matrix}
      Let $\L$ be a lattice in $V$ and $P$ be its generator matrix. Then
      \[
        V_{\L} = |\det(P)|.
      \]
    \end{proposition}
    \begin{proof}
      Let $\{e_{1},\ldots,e{n}\}$ be an orthonormal basis of $V$. Making the change of variables $x_{i} \to t_{i}$ for $1 \le i \le n$ where
      \[
        x_{1}e_{1}+\cdots+x_{n}e_{n} \to t_{1}v_{1}+\cdots+t_{n}v_{n},
      \]
      the corresponding Jacobian matrix is $P$. Then
      \[
        V_{\L} = \int_{\mc{M}}\,dx_{1} \cdots dx_{n} = |\det(P)|\int_{[0,1]^{n}}\,dt_{1} \cdots dt_{n} = |\det(P)|,
      \]
      as claimed.
    \end{proof}
    ...
    \end{proof}
  \section{\todo{Integration Techniques \& Transforms}}
    ...
    \subsection*{The Fourier Transform}
      The first type of integral transform we will need is the Fourier transform. Suppose $f(x)$ is absolutely integrable on $\R$. The \textbf{Fourier transform}\index{Fourier transform} $\hat{f}(t)$ of $f(x)$ is defined by
      \[
        \hat{f}(t) = \int_{\R}f(x)e^{-2\pi itx}\,dx.
      \]
      This integral is absolutely convergent precisely because $f(x)$ is absolutely integrable on $\R$. Under some mild conditions on $f(x)$ the \textbf{Poisson summation formula}\index{Poisson summation formula} applies:
      ...
      There are two practical settings of interest. The first is when $f(x)$ is of \textbf{Schwarz class}\index{Schwarz class} which means that $f \in C^{\infty}(\R)$ and $f$ along with all of its derivative have rapid decay. For example $e^{-x^{2}}$ is of Schwarz class. If $f(x)$ is of Schwarz class, the rapid decay implies that $f(x)$ and all of its derivatives are absolutely integrable over $\R$. Moreover, this also implies that $F(x)$ and all of its derivatives are locally absolutely uniformly convergent by the Weierstrass $M$-test. The uniform limit theorem implies that $F(x)$ is smooth and thus the conditions of the Poisson summation formula are satisfied. The second application is when $f(z)$ is some holomorphic function that has at least polynomial decay of order $c > 1$ for any fixed $y$. Then $f(z)$ is absolutely integrable over $\R$ for any fixed $y$. Moreover, $F(z)$ is holomorphic. Then the conditions of the Poisson summation formula are satisfied for $z = iy$ with $y > 0$ and the identity theorem shows that this holds for all $z$. Our primary use of the Poisson summation formula will be to derive very striking transformation laws which are the key ingredient in analytically continuing many important functions.
    \subsection*{The Mellin Transform}
      Like the Fourier transform, the Mellin transform is another type of integral transform. If $f(x)$ is a continuous function and $f(\infty) = \lim_{x \to \infty}f(x)$ exists, then the \textbf{Mellin transform}\index{Mellin transform} $F(s)$ of $f(x)$ is given by
      \[
        F(s) = \int_{0}^{\infty}(f(x)-f(\infty))x^{s}\,\frac{dx}{x}.
      \]
      In most cases, we assume $f(x)$ has some type of decay so that $f(\infty) = 0$. Moreover, if $f(x)$ is a sufficiently nice function then the integral will be bounded in some half-plane in $s$. For example, this happens if $f(x)$ exhibits rapid decay and remains bounded as $x \to 0$. In this case, the integral is locally absolutely uniformly convergent for $\s > 0$. If the Mellin transform $F(s)$ is sufficiently nice, then the initial function can be recovered via means of the \textbf{inverse Mellin transform}\index{inverse Mellin transform} $(\mc{M}^{-1}F)(x)$:
      \[
