\chapter{Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Lattices}
    ...
    \]
    Then $P$ is the base change matrix from $\{e_{1},\ldots,e_{n}\}$ to $\{v_{1},\ldots,v_{n}\}$. Then the \textbf{covolume}\index{covolume} $V_{\L}$ of a complete lattice $\L$ is defined to be
    \[
      V_{\L} = |\det(P)|.
    \]
    By \cref{prop:base_change_quotient_determinant} and that the base change matrix between any two orthonormal basses is an orthogonal matrix (hence has determinant $\pm 1$), the covolume is independent of the choice of bases. We can now introduce the notion of dual lattices. If $\L$ is a complete lattice in $V$, then the \textbf{dual lattice}\index{dual lattice} $\L^{\vee}$ of $\L$ is defined by
    \[
