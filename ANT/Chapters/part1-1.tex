  \chapter{Preliminaries}\label{ch:Preliminaries}
    ...
    \section{Notational Conventions}
      ...
      \end{itemize}
    \section{Estimates}
      Much of the language of analytic number theory is given in terms of estimates as they allows us to discuss approximate growth and dispense with superfluous constants. For this reason, estimates will be the first material that we will present. The estimates that we will cover are listed in the following table:
      \begin{center}
      ...
      \subsection*{\texorpdfstring{$O$}{O}-estimates \& Symbols}
        ...
        \begin{remark}
          Many authors assume that $g(z)$ is a nonnegative function so that the absolute values on $g(z)$ can be dropped. As we require estimates that will be used more generally, we do not make this assumption since one could very well replace $O(g(z))$ with $O(|g(z)|)$. In practice this deviation causes no issue.
        \end{remark}
    ...
    \section{Special Sums}
      ...
      \subsection*{Ramanujan \& Gauss Sums}
        ...
        \]
        where the last equality follows because $\chi(a) = 0$ unless $(a,m) = 1$. If $b = 1$ we will write $\tau(\chi)$ instead. That is, $\tau(\chi) = \tau(1,\chi)$. We call $\tau(\chi)$ the \textbf{Gauss sum}\index{Gauss sum} associated to $chi$. Ramanujan and Gauss sums associated to Dirichlet characters are interesting because the Dirichlet character is a multiplicative character while the exponential is an additive one. So these sums are a convolution between a multiplicative and additive character. This is the fundamental reason that makes them difficult to study as one needs to separate the additive and multiplicative structures. Observe that if $m = 1$ then $\chi$ is the trivial character and $\tau(b,\chi) = 1$. So the interesting cases are when $m \ge 2$. There are some basic properties of these sums which are very useful:
    ...
    \section{Integration Techniques \& Transforms}
      ...
      \subsection*{The Fourier Transform}
        ...
        There are two practical settings of interest. The first is when $f(x)$ is of \textbf{Schwarz class}\index{Schwarz class} which means that $f \in C^{\infty}(\R)$ and $f$ along with all of its derivative have rapid decay. For example, $e^{-x^{2}}$ is of Schwarz class. For if $f(x)$ is of Schwarz class, the rapid decay implies that $f(x)$ is absolutely integrable over $\R$. Moreover, this also implies that $F(x)$ and all of its derivatives are locally absolutely uniformly convergent by the Weierstrass $M$-test. The uniform limit theorem then shows that $F(x)$ is smooth and thus the conditions of the Poisson summation formula are satisfied. The second application is when $f(z)$ is some holomorphic function that has at least polynomial decay of order $c > 1$ for any fixed $y$. Then $f(z)$ is absolutely integrable over $\R$ for any fixed $y$. Moreover, $F(z)$ is holomorphic. Then the conditions of the Poisson summation formula are satisfied for $z = iy$ with $y > 0$ and the identity theorem shows that this holds for all $z$. Our primary use of the Poisson summation formula will be to derive very striking transformation laws which are the key ingredient in analytically continuing many important functions.
    ...
    \section{The Gamma Function}
      ...
      If $\s$ is bounded, Stirling's formula gives a useful estimate showing that $\G(s)$ decays as $s \to \infty$:
      ...
      \end{proof}
      Strictly weaker than the asymptotic in Stirling's formula is the estimate
      \begin{equation}
      ...
      \end{equation}
      which will be useful. Using \cref{equ:log_gamma_estimate} we can obtain a useful estimate formula for the digamma function:
