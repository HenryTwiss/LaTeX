  \chapter{Preliminaries}\label{ch:Preliminaries}
    ...
    \section{Special Sums}
      ...
      \subsection*{Quadratic Gauss Sums}
        Another important sum is the quadratic Gauss sum. For a positive integer $m$ and any $b \in \Z$, the \textbf{quadratic Gauss sum}\index{quadratic Gauss sum} $g(b,m)$ is defined by
        \[
      ...
      \subsection*{Kloosterman \& Sali\'e Sums}
        Our last class of sums generalize both types of Ramanujan sums. For a positive integer $c$ and any integers $n$ and $m$, the \textbf{Kloosterman sum}\index{Kloosterman sum} $K(n,m,c)$ is defined by
        \[
    ...
    \section{Integration Techniques \& Transforms}
      \subsection*{Integration Techniques}
        ...
        \begin{proposition}\label{prop:decay_compacta_integral}
          Suppose $f(z,x)$ is analytic on $\W \x D$ for some regions $\W$ and $D$ with $D$ unbounded and of finite volume with respect to a measure $d\mu$. Also suppose that for any $z \in \W$, $f(z,x)$ is bounded on $D$. Then the integral
          \[
            \int_{D}f(z,x)\,d\mu,
          \]
        ...
        \end{proposition}
        \begin{proof}
          The second statement follows from \cref{thm:analytic_integral}. So it suffices to show that the integral is locally absolutely uniformly bounded. Let $K$ be a compact subset of $\W$. As $f(z,x)$ is bounded on $D$ for any $z$, $f(z,x)$ is locally absolutely uniformly bounded on $K \x D$. Then for $z \in K$, we have
          \[
        ...
        In particular, \cref{prop:decay_compacta_integral} holds if $f(z,x)$ admits any type of decay on $D$ for any $z \in \W$. There is also another useful analytic technique called \textbf{shifting the line of integration}\index{shifting the line of integration}:
        ...
        \begin{theorem}[Shifting the line of integration]
          ...
          \]
          and some real $b$ with $b < a$ in the first case and $b > a$ in the second case. Suppose $f(z)$ is meromorphic on a strip containing the lines $\Re(z) = a,b$ or $\Im(z) = a,b$ and is holomorphic about the lines $\Re(z) = a,b$ or $\Im(z) = a,b$ respectively. Moreover, suppose $f(z) \to 0$ as $y \to \infty$ or $x \to \infty$ respectively. Then
          \[
            \int_{(a)}f(z)\,dz = \int_{(b)}f(z)\,dz+2\pi i\sum_{\rho \in P}\Res_{z = \rho}f(z),
          \]
          where $P$ is the set of poles inside of the strip containing the lines $\Re(z) = a,b$ or $\Im(z) = a,b$ respectively.
        \end{theorem}
        \begin{proof}
        ...
        \begin{theorem}[Unfolding]
          ...
          \[
            D = \bigcup_{g \in G}gF,
          \]
          where the intersections $gF \cap hF$ are measure zero for all $g,h \in G$ with respect to some invariant measure $d\mu$ for $G$. Then 
          \[
    ...
    \section{The Gamma Function}
      ...
      \end{equation}
      By \cref{prop:decay_compacta_integral}, the second integral in \cref{equ:Gamma_integral_bounded_1} is locally absolutely uniformly bounded in this region. As for the first integral, let $\b = \min_{s \in K}\{\s\}$. Then
