\chapter{Preliminaries}\label{ch:Preliminaries}
  ...
  \section{Integration Techniques \& Transforms}
    ...
    \end{theorem}
    \begin{proof}
      By assumption $F(\mathbf{x})$ is smooth and it is clearly $1$-periodic in each component. Therefore it admits a Fourier series. We compute the $\mathbf{t}$-th Fourier coefficient of $F(\mathbf{x})$ as follows:
      \begin{align*}
        \hat{F}(\mathbf{t}) &= \int_{[0,1]^{n}}F(\mathbf{x})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} \\
        &= \int_{[0,1]^{n}}\sum_{\mathbf{n} \in \Z^{n}}f(\mathbf{x}+\mathbf{n})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} \\
        &= \sum_{\mathbf{n} \in \Z^{n}}\int_{[0,1]^{n}}f(\mathbf{x}+\mathbf{n})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} && \text{DCT} \\
        &= \sum_{\mathbf{n} \in \Z^{n}}\int_{\mathbf{n}+[0,1]^{n}}f(\mathbf{x})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} \\
        &= \int_{\R^{n}}f(\mathbf{x})e^{-2\pi i\<\mathbf{t},\mathbf{x}\>}\,d\mathbf{x} && \text{DCT} \\
        &= (\mc{F}f)(\mathbf{t}).
      \end{align*}
      Therefore the Fourier series of $F(\mathbf{x})$ is
      \[
        F(\mathbf{x}) = \sum_{\mathbf{t} \in \Z^{n}}(\mc{F}f)(\mathbf{t})e^{2\pi i\<\mathbf{t},\mathbf{x}\>}.
      \]
      and the first statement follows by the definition of $F(\mathbf{x})$. Setting $\mathbf{x} = \mathbf{0}$ proves the second statement.
    \end{proof}
