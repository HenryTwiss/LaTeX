\chapter{Zero-free Regions \& Zero Counting for Degree \texorpdfstring{$1$}{1} \texorpdfstring{$L$}{L}-functions}
  ...
  \section{Zero-free Regions, Siegel Zeros \& Siegel's Theorem}
    ...
    \subsection*{The Classical Zero-free Region for \texorpdfstring{$\z(s)$}{\z(s)}}
      ...
      \]
       Now onto the zero-free region. The \textbf{classical zero-free region for $\z(s)$}\index{classical zero-free region for $\z(s)$} describes a region slightly to the left of the line $\s = 1$ inside of the critical strip where $\z(s)$ cannot exhibit a zero:
